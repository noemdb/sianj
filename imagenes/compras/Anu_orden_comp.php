...
if(pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  $sSQL="Select * from comp008 WHERE nro_orden='$nro_orden' and tipo_compromiso='$tipo_compromiso'";    $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado); 
  if($filas==0){$error=1;?><script language="JavaScript">muestra('NUMERO ORDEN DE COMPRA NO EXISTE');</script><?}
     ...
     if(($error==0)and(substr($tipo_compromiso,0,1)=="A")){$error=1;?><script language="JavaScript">muestra('ORDEN DE COMPRA NO PUEDE SER ELIMINADA');</script><?}
     ...
     if($error==0){$sql="SELECT * FROM art_ord_compra where nro_orden='$nro_orden' and tipo_compromiso='$tipo_compromiso' order by nro_linea";  $res=pg_query($sql); $total=0;$desc_cod="";
       while($registro=pg_fetch_array($res)){$total=$total+$registro["total_articulo"]+$registro["total_iva"];
         $desc_cod=$desc_cod.", ARTICULO:".$registro["codigo_articulo"].' '.$registro["descripcion_articulo"]." CANTIDAD:".$registro["cantidad_ordenada"]." MONTO:".$registro["costo"]; }
     }
     if($error==0){ $sfecha=formato_aaaammdd($sfecha); $afecha=formato_aaaammdd($fecha_anu); $sql="SELECT ANULA_COMP008('$nro_orden','$tipo_compromiso','$afecha','$descrip_anu','$usuario_sia','$minf_usuario')";
       ...
       if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? } else{?><script language="JavaScript">muestra('ANULO EXITOSAMENTE');</script><?
           $desc_doc="ORDEN DE COMPRA:  NUMERO:".$nro_orden.", DESCRIPCION:".$adescripcion.", MONTO ORDEN:".$total;  $desc_doc=$desc_doc.$desc_cod;
