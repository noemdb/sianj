...
$codigo_mov=$_GET["txtcodigo_mov"]; $nro_requisicion=$_GET["nro_req"]; $fecha_hoy=asigna_fecha_hoy();
?>
<html>
<head>  <title>CARGAR REQUISICION EN LA ORDEN DE COMPRA</title>
...
echo "ESPERE POR FAVOR CARGANDO REQUISICION.... ","<br>";
...
$fecha_requisicion=""; $tiempo_entrega=""; $lugar_entrega=""; $direccion_entrega=""; $operacion="C"; $dias_credito="30"; $afecta_presupuesto=""; $fuente_financ=""; $anulado=""; $fecha_anulado=""; $cancelada="";
$fecha_cancelacion=""; $nro_ord_pago=""; $cant_articulo=""; $redondeo_total=""; $redondeo_impuesto=""; $aplica_impuesto="S"; $cod_presup_imp=""; $tasa_flete=""; $monto_flete=""; $cod_presup_flete=""; $des_unidad_sol="";
...
$sql="Select * from REQUISICIONES where nro_requisicion='$nro_requisicion'"; $res=pg_query($sql); $filas=pg_num_rows($res);
if($filas>0){$registro=pg_fetch_array($res);
  $nro_requisicion=$registro["nro_requisicion"];   $des_unidad_sol=$registro["denominacion_cat"]; $concepto=$registro["observacion"];
  $fecha_requisicion=$registro["fecha"]; $unidad_solicitante=$registro["unidad_solicitante"];  $nombre_departamento=$registro["nombre_departamento"]; 
  $cod_presup_imp=$unidad_solicitante."-".$cod_part_iva; $lugar_entrega=$nombre_departamento;
} echo $nro_requisicion,"<br>"; 
...
while($registro=pg_fetch_array($res)){ $cod_articulo=$registro["cod_articulo"]; $nro_linea=$registro["nro_linea"]; $cantidad=$registro["cant_requerida"]; $des_articulo=$registro["descripcion_articulo"];
 $cod_presup=$registro["cod_presup"]; $fuente_financ=$registro["fuente_financ"]; $total=$registro["cant_requerida"]*$registro["costo_actual"];
 $marca=""; $modelo=""; $unidad_medida=$registro["unidad_medida"]; $costo=$registro["costo_actual"]; $impuesto=$registro["tasa_impuesto"]; $total_iva=$registro["total_iva"]; $tasa=$registro["tasa_impuesto"]; $monto_iva=$registro["monto_iva"];
 $partida=$registro["cod_partida"]; $cod_cat=$registro["cod_categoria"];
 
 $sSQL="Select cod_articulo,marca,modelo from COMP002 WHERE cod_articulo='$cod_articulo'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
 if($filas==0){$error=1; echo "Codigo Articulo:".$cod_articulo; ?> <script language="JavaScript"> muestra('CODIGO DE ARTICULO NO EXISTE');</script><? }
    else{$reg=pg_fetch_array($resultado); $marca=$reg["marca"]; $modelo=$reg["modelo"]; }          
 $StrSQL="SELECT ACTUALIZA_COMP042(1,'$codigo_mov','$cod_articulo','00000000','0000000000','$nro_linea','','$sfecha','$marca','$modelo','$unidad_medida','$cod_presup','$fuente_financ',$costo,$tasa,0,$monto_iva,$total_iva,$cantidad,0,0,0,$total,0,0,'P','','$sfecha','','','$partida','$cod_cat',0,0,'','$sfecha','','','$des_articulo','')";
 $resultado=pg_exec($conn,$StrSQL);     $error=pg_errormessage($conn);
}


$fecha=$sfecha;
$sql="SELECT * FROM comp042 where codigo_mov='$codigo_mov' order by nro_linea";$res=pg_query($sql); $gsub_total=0; $gtotal_iva=0; $gtotal_ord=0;
while($registro=pg_fetch_array($res)){ $cod_articulo=$registro["codigo_articulo"]; $cod_presup=$registro["cod_presup"]; $fuente_financ=$registro["fuente_financ"]; $cant_articulo=$cant_articulo+1;
          ...
          if($error==0){ $sSQL="Select * from PRE026 WHERE codigo_mov='$codigo_mov' and cod_presup='$cod_presup' and fuente_financ='$fuente_financ'";
                  ...
                  if (verifica_disponibilidad($conn,$cod_presup,$fuente_financ,$fecha,$monto_cod)==0){$error=0;}else{$error=1; echo "Codigo Presupuestario:".$cod_presup,"<br>";}
                  if(($error==0)and($tipo_imput_presu=="C")){ $monto_credito=$monto_cod;
                        ...
                        $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
                        if ($filas==0){$error=1; echo "Codigo Articulo:".$cod_articulo; ?> <script language="JavaScript"> muestra('CÓDIGO NO EXISTE EN LA EJECUCIÓN DEL CREDITO ADICIONAL');</script><? }
          ...
          if($total_iva>0){ 
                ...
                if($error==0){ $sSQL="Select * from PRE026 WHERE codigo_mov='$codigo_mov' and cod_presup='$cod_pre_iva' and fuente_financ='$fuente_financ'";
                  ...
                  if (verifica_disponibilidad($conn,$cod_pre_iva,$fuente_financ,$fecha,$monto_cod_iva)==0){$error=0;}else{$error=1; echo "Codigo Presupuestario Iva:".$cod_pre_iva,"<br>";}
                  if(($error==0)and($tipo_imput_presu=="C")){ $monto_credito=$monto_cod;
...
<table width="10">
  <tr>
     ...
     <td width="5"><input name="txtnro_req" type="hidden" id="txtnro_req" value="<?echo $nro_requisicion?>"></td>   
     <td width="5"><input name="txtfecha_req" type="hidden" id="txtfecha_req" value="<?echo $fecha_requisicion?>"></td>
...
<?pg_close();
if ($error==0){?><script language="JavaScript">alert('Requisicion de Articulo Cargada'); Llamar_Inc_Orden();</script> <? }else {?>  <script language="JavaScript"> alert('Error en Carga'); history.back();</script> <? }
?>