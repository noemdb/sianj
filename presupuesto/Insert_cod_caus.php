...
$monto_credito=$_POST["txtmonto_credito"];$cod_contable=$_POST["txtcod_contable"];
$tipo_imput_presu=substr($tipo_imput_presu,0,1);$fecha=asigna_fecha_hoy(); $sfecha=formato_aaaammdd($fecha);
...
echo "ESPERE POR FAVOR INCLUYENDO....","<br>";$url="Det_inc_causados.php?codigo_mov=".$codigo_mov;
...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO YA EXISTE EN EL CAUSADO');</script><? }
   else{
    ...
    if(($error==0)and($tipo_imput_presu=="C")){
      ...
      $resultado=pg_query($sSQL);   $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO NO EXISTE EN LA EJECUCIÓN DEL CREDITO ADICIONAL');</script><? }
       else{
    ...
    if($error==0){ $sfecha=formato_aaaammdd($fecha);
      $resultado=pg_exec($conn,"SELECT INCLUYE_PRE026('$codigo_mov','$cod_presup','$fuente_financ','','0000','','0000','','0000','','0000','','','','$cod_contable','','','$sfecha','C','$tipo_imput_presu','$ref_imput_presu','$sfecha',$monto,0,$monto_credito,0)");
