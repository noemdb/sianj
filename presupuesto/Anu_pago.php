...
$tipo_pago=$_POST["txttipo_pago"];$referencia_pago=$_POST["txtreferencia_pago"];$cod_banco=$_POST["txtcod_banco"];$referencia_caus=$_POST["txtreferencia_caus"];
$tipo_causado=$_POST["txttipo_causado"];$referencia_comp = $_POST["txtreferencia_comp"];$tipo_compromiso = $_POST["txttipo_compromiso"];$fecha_anu = $_POST["txtfecha_anu"];
$descrip_anu = $_POST["txtdescrip_anu"];$unidad_sol="";$equipo = getenv("COMPUTERNAME");$minf_usuario = $usuario_sia." ".$equipo." ".date("d/m/y H:i a");
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE PAGO NO EXISTE');</script><?}
     ...
     $fecha_pago=$registro["fecha_pago"];     $adescripcion=$registro["descripcion_pago"];     $anulado=$registro["anulado"];
     if($anulado=='S'){$error=1;?><script language="JavaScript">muestra('PAGO YA ESTA ANULADO');</script><?}
     if($error==0){     $sfecha=$fecha_pago;
     ...
     if($error==0){ if ($afecha<$sfecha){$error=1; ?> <script language="JavaScript">muestra('FECHA ANULACION NO PUEDE SER MENOR A FECHA DEL PAGO');</script><? }    }	 
     ...
     if(($tipo_compromiso=='0000')and($error==0)){
       ...
       if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE PAGO DIRECTO NO EXISTE');</script><?}
        else{$registro=pg_fetch_array($resultado); $unidad_sol=$registro["unidad_sol"]; }
     }
     if($error==0){
       $resultado=pg_exec($conn,"SELECT ANULA_PRE008('$referencia_pago','$tipo_pago','$referencia_caus','$tipo_causado','$referencia_comp','$tipo_compromiso','$cod_banco','$afecha','$descrip_anu','$minf_usuario','$unidad_sol','S')");
       ...
       if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? }
           ...
           $desc_doc="MOV.PAGO: DOCUMENTO PAGO:".$tipo_pago.", REFERENCIA PAGO:".$referencia_pago.", DOCUMENTO CAUSADO:".$tipo_causado.", REFERENCIA CAUSADO:".$referencia_caus.", DOCUMENTO COMPROMISO:".$tipo_compromiso.", REFERENCIA COMPROMISO:".$referencia_comp.", DESCRIPCION:".$adescripcion;
...
if ($error==0){?><script language="JavaScript">cerrar_ventana();</script> <? }
