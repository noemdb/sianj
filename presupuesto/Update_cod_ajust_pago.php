...
$monto_c=$_POST["txtmonto"];$comprometido=$_POST["txtmonto_compromiso"];
$tipo_imput_presu=$_POST["txttipo_imput_presu"];$ref_imput_presu=$_POST["txtref_imput_presu"];
$monto_credito=$_POST["txtmonto_credito"];$credito=$_POST["txtcredito"];
$cod_contable=$_POST["txtcod_contable"];$tipo_imput_presu=substr($tipo_imput_presu,0,1);
...
$comprometido=formato_numero($comprometido);
...
$url="Det_inc_ajustes_pago.php?codigo_mov=".$codigo_mov;
...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO NO EXISTE EN EL AJUSTE');</script><? }
   else{$registro=pg_fetch_array($resultado);  $referencia_pago=$registro["referencia_pago"];
    $tipo_pago=$registro["tipo_pago"]; $referencia_caus=$registro["referencia_caus"];
    $tipo_causado=$registro["tipo_causado"]; $referencia_comp=$registro["referencia_comp"];   $tipo_compromiso=$registro["tipo_compromiso"];
    if($error==0){$sSQL="Select * from PRE095 WHERE cod_fuente_financ='$fuente_financ'";
      $resultado=pg_query($sSQL);$filas=pg_num_rows($resultado);
    ...
    if($error==0){$sSQL="Select * from PRE038 WHERE (referencia_pago='$referencia_pago') and (tipo_pago='$tipo_pago') and (referencia_caus='$referencia_caus') and (tipo_causado='$tipo_causado') and(referencia_comp='$referencia_comp') and (tipo_compromiso='$tipo_compromiso') and (cod_presup='$cod_presup') and (fuente_financ='$fuente_financ')";
      $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO NO EXISTE EN EL PAGO');</script><? }
       else{$registro=pg_fetch_array($resultado);
         $compromiso=$registro["monto"]-$registro["ajustado"]; $balance=$monto_c-$compromiso;
         if(($monto_c>$compromiso)and($balance>0.001)){$error=1; ?> <script language="JavaScript"> muestra('MONTO A AJUSTAR MAYOR AL SALDO DEL PAGO');</script><? }
       }
    }
    if($error==0){$sfecha=formato_aaaammdd($fecha);
      $resultado=pg_exec($conn,"SELECT MODIFICA_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$referencia_caus','$tipo_causado','$referencia_pago','$tipo_pago','','0000','','','','$cod_contable','','','$sfecha','C','$tipo_imput_presu','$ref_imput_presu','$sfecha',$monto,$comprometido,$monto_credito,$credito)");
