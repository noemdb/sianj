...
$minf_usuario = $usuario_sia." ".$equipo." ".date("d/m/y H:i a");  echo "ESPERE POR FAVOR LIBERANDO....","<br>";
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE MOVIMIENTO DIFERIDO NO EXISTE');</script><?}
   else{ $registro=pg_fetch_array($resultado); $fecha_diferido=$registro["fecha_diferido"]; $status_2=$registro["status_2"];  $adescripcion=$registro["descripcion_dife"]; 
       if($status_2=="L"){$error=1;?><script language="JavaScript">muestra('MOVIMIENTO DIFERIDO YA ESTA LIBERADO');</script><?}
   }   
...
*/  
  if($error==0){ 
     ...
     $total=0;$desc_cod="";   $sfecha=$fecha_diferido;
     ...
     $resultado=pg_exec($conn,"SELECT LIBERA_PRE023('$referencia_dife','$tipo_diferido','$sfecha')"); $error=pg_errormessage($conn); $error=substr($error, 0, 61);
     if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? }
      else{?><script language="JavaScript">muestra('LIBERO EXITOSAMENTE');</script><?
         ...
         $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('05','$usuario_sia','$usuario_sia','$equipo','Libero','$sfecha','$desc_doc')");
