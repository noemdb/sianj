...
$url="Act_fuentes.php?Gfuente=".$cod_fuente; echo "ESPERE POR FAVOR MODIFICANDO....","<br>"; $error=0;
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO DE FUENTE NO EXISTE');</script> <? }
   else{$error=1;$resultado=pg_exec($conn,"SELECT ACTUALIZA_PRE095(2,'$cod_fuente','$den_fuente','$tipo','$afecta')");$error=pg_errormessage($conn); $error=substr($error, 0, 61);
