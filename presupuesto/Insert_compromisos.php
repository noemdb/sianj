...
$descripcion_comp=cambiar_car_especiales($descripcion_comp);
...
if ($error==0){ if($fecha_aut=="S"){$fecha_compromiso=$fecha_hoy;}   $sfecha=formato_aaaammdd($fecha_compromiso);
  ...
  if ($registro=pg_fetch_array($resultado,0)){$tipo_doc_oc=$registro["campo504"]; $tipo_doc_os=$registro["campo505"];  } else {$tipo_doc_oc="0001"; $tipo_doc_os="0002";} 
...
*/  
  ...
  if($error==0){ $sql="Select * from CODIGOS_PRE026 where codigo_mov='$codigo_mov' order by cod_presup"; $res=pg_query($sql);  $total=0; $sfecha=formato_aaaammdd($fecha_compromiso);
    while(($registro=pg_fetch_array($res))and($error==0)){$total=$total+$registro["monto"];$cod_presup=$registro["cod_presup"];$fuente_financ=$registro["fuente_financ"];
      $monto_c=$registro["monto"];$ref_imput_presu=$registro["ref_imput_presu"];$monto_credito=$registro["monto_credito"]; $tipo_imput_presu=$registro["tipo_imput_presu"];
     
      if(strlen($cod_presup)==strlen($formato_presup)){
       if (verifica_disponibilidad($conn,$registro["cod_presup"],$registro["fuente_financ"],$fecha_compromiso,$monto_c)==0){$error=0;}
     ...
     else{$error=1; ?> <script language="JavaScript"> muestra('LONGITUD DE CODIGO PRESUPUESTARIO:<? echo $registro["cod_presup"]; ?> FUENTE:<? echo $registro["fuente_financ"]; ?> INVALIDA');</script><? }
  ...
  if($error==0){$sfecha=formato_aaaammdd($fecha_compromiso); $fechav=formato_aaaammdd($fecha_vencim);     //echo $g_comprobante;	 
