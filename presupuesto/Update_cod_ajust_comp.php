...
$cod_presup=$_POST["txtcod_presup"];$fuente_financ=$_POST["txtcod_fuente"];
$monto_c=formato_numero($_POST["txtmonto"]);$monto_c=$_POST["txtmonto"];
$comprometido=$_POST["txtmonto_compromiso"];$tipo_imput_presu=$_POST["txttipo_imput_presu"];
$ref_imput_presu=$_POST["txtref_imput_presu"];$monto_credito=$_POST["txtmonto_credito"];
$credito=$_POST["txtcredito"];$cod_contable=$_POST["txtcod_contable"];
...
echo "ESPERE POR FAVOR MODIFICANDO....","<br>";$url="Det_inc_ajustes_comp.php?codigo_mov=".$codigo_mov;
...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO NO EXISTE EN EL AJUSTE');</script><? }
   else{  $registro=pg_fetch_array($resultado);   $referencia_comp=$registro["referencia_comp"]; $tipo_compromiso=$registro["tipo_compromiso"];
    ...
    if($error==0){$sSQL="Select * from PRE036 WHERE (referencia_comp='$referencia_comp') and (tipo_compromiso='$tipo_compromiso') and (cod_presup='$cod_presup') and (fuente_financ='$fuente_financ')";
      ...
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO NO EXISTE EN EL COMPROMISO');</script><? }
         ...
         $compromiso=$registro["monto"]-$registro["causado"]-$registro["ajustado"]; $balance=$monto_c-$compromiso;
         if(($monto_c>$compromiso)and($balance>0.001)){$error=1; ?> <script language="JavaScript"> muestra('MONTO A AJUSTAR MAYOR AL SALDO DEL COMPROMISO');</script><? }
       }
    }
