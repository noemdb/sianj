...
$num_proyecto="0000000000";$refierea=$_POST["txtrefiereA"];$func_inv=$_POST["txtfunc_inv"];
if($func_inv==""){$func_inv="CORRIENTE";}$func_inv=substr($func_inv,0,1);
$genera_comprobante=$_POST["txtgenera_comprobante"];$genera_comprobante=substr($genera_comprobante,0,1);
$unidad_sol="";$cod_banco='0000';$equipo = getenv("COMPUTERNAME");
$minf_usuario = $usuario_sia." ".$equipo." ".date("d/m/y H:i a");
echo "ESPERE POR FAVOR INCLUYENDO....","<br>";
if (checkData($fecha_pago)=='1'){$error=0;}
...
if ($error==0){  $sfecha=formato_aaaammdd($fecha_pago);  $rfecha=$sfecha;
  ...
  if($error==0){
    ...
    $total=0;  $sfecha=formato_aaaammdd($fecha_pago);
    while(($registro=pg_fetch_array($res))and($error==0)){
      ...
      if($refierea=="NINGUNO"){
        if (verifica_disponibilidad($conn,$registro["cod_presup"],$registro["fuente_financ"],$fecha_pago,$monto_c)==0){$error=0;}
         else{$error=1;?><script language="JavaScript">muestra('ERROR EN EL CODIGO PRESUPUESTARIO:<? echo $registro["cod_presup"]; ?> FUENTE:<? echo $registro["fuente_financ"]; ?> REF.CREDITO:<? echo $registro["ref_imput_presu"]; ?>');</script><?}
      ...
      if($refierea=="COMPROMISO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<? echo $registro["cod_presup"]; ?> FUENTE:<? echo $registro["fuente_financ"]; ?> REF.CREDITO:<? echo $registro["ref_imput_presu"]; ?>');</script><?}
           ...
           $compromiso=$registro["monto"]-$registro["pagado"]-$registro["ajustado"];  $balance=$monto_c-$compromiso;
           if(($monto_c>$compromiso)and($balance>0.001)){$error=1; echo $monto_c.' '.$compromiso.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CODIGO DEL COMPROMISO POR PAGAR');</script><? }
         }
      }
      if($refierea=="CAUSADO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL CAUSADO EL CODIGO PRESUPUESTARIO:<? echo $registro["cod_presup"]; ?> FUENTE:<? echo $registro["fuente_financ"]; ?> REF.CREDITO:<? echo $registro["ref_imput_presu"]; ?>');</script><?}
           ...
           $compromiso=$registro["monto"]-$registro["pagado"]-$registro["ajustado"]; $balance=$monto_c-$compromiso;
           if(($monto_c>$compromiso)and($balance>0.001)){$error=1; echo $monto_c.' '.$compromiso.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CODIGO DEL CAUSADO POR PAGAR');</script><? }
         }
      }
    }
  ...
  if($error==0){
     ...
     if (!$resultado){?><script language="JavaScript"> muestra('<? echo $error; ?>');</script><? $error=1;}
       ...
       $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE');</script><? $resultado=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");
