...
$monto_c=$_POST["txtmonto"];$operacion="+";$grupo="01";
...
echo "ESPERE POR FAVOR INCLUYENDO....","<br>";$url="Det_inc_adiciones.php?codigo_mov=".$codigo_mov;
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
 else{ $sql="Select * from SIA005 where campo501='05'";  $resultado=pg_query($sql); if ($registro=pg_fetch_array($resultado,0)){$formato_presup=$registro["campo504"];}  $error=0;
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO YA EXISTE EN EL MOVIMIENTO');</script><? }
    ...
    if($error==0){ if(strlen($cod_presup)==strlen($formato_presup)){
