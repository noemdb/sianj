...
$url="Act_parroquias.php"; echo "ESPERE POR FAVOR MODIFICANDO....","<br>"; $error=0;
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO DE PARROQUIA NO EXISTE');</script> <? }
   else{$error=1;$resultado=pg_exec($conn,"SELECT ACTUALIZA_PRE096(2,'$codigo','$nombre')");$error=pg_errormessage($conn); $error=substr($error, 0, 61);
