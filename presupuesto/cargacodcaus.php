...
while($registro=pg_fetch_array($res)){ $monto=$registro["monto"]-$registro["pagado"]-$registro["ajustado"];
  if($monto>0){    $cod_presup=$registro["cod_presup"];     $fuente=$registro["fuente_financ"]; $pagado=$monto;
    $cod_contable=$registro["cod_contable"];    $fecha=$registro["fecha_causado"];     $tipo_imput_presu=$registro["tipo_imput_presu"];
    ...
    $ssql="SELECT INCLUYE_PRE026('$codigo_mov','$cod_presup','$fuente','$referencia_comp','$tipo_compromiso','','0000','','0000','','0000','C','','','$cod_contable','','','$fecha','C','$tipo_imput_presu','$ref_imput_presu','$fecha',$pagado,$monto,$causc,$montoc)";
