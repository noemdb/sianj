...
if ($tipo_modif=='TODOS'){$nombre="";} $criterio_est="";
  ...
  if($tipo_modif=='SALDO FINAL DE CAJA'){$nombre='SALDO FINAL DE CAJA'; $criterio_est="(pre009.tipo_modif='6') And ";}
  if($tipo_modif=='INCREMENTO DE INGRESOS'){$nombre='INCREMENTO DE INGRESOS'; $criterio_est="(pre009.tipo_modif='7') And ";}
  
  if($tipo_regis=='TO'){$nombre2='TODOS';}
  if($tipo_regis=='ANU'){$nombre=$nombre.' (ANULADOS) '; $criterio_est=$criterio_est."(pre009.anulado='S') And ";}  
  if($tipo_regis=='NOANU'){$nombre2='NO ANULADOS'; $criterio_est=$criterio_est."(pre009.anulado='N') And ";}  
  if($tipo_regis=='APRO'){$nombre=$nombre.' (APROBADOS)';  $criterio_est=$criterio_est."(pre009.modif_aprob='S') And ";}
  if($tipo_regis=='NAPRO'){$nombre=$nombre.' (NO APROBADOS)'; $criterio_est=$criterio_est."(pre009.anulado='N') And (pre009.modif_aprob='N') And ";}
  
  
  if($nombre==""){$nombre=$nombre2; }
  
  $criteriop=$criterio_est."(pre009.Referencia_Modif>='$referencia_d' and pre009.Referencia_Modif<='$referencia_h') And (pre009.Fecha_Modif>='$fecha_desde' and pre009.Fecha_Modif<='$fecha_hasta') And (pre039.cod_presup>='$cod_presup_d' and pre039.cod_presup<='$cod_presup_h') And (pre039.fuente_financ>='$cod_fuente_d' and pre039.fuente_financ<='$cod_fuente_h')";
  $criteriop=$criterio_est."(pre009.Referencia_Modif>='$referencia_d' and pre009.Referencia_Modif<='$referencia_h') And (pre009.Fecha_Modif>='$fecha_desde' and pre009.Fecha_Modif<='$fecha_hasta') ";
  
  $criterio1="Fecha Desde : ".$fecha_d."   "."Hasta : ".$fecha_h;   $criterio2=$nombre;  $criterio3="Fuente de Financiamiento : ".$cod_fuente_d."    ".$cod_fuente_h;
  if($error==0){ $sql="Select * from SIA005 where campo501='05'"; $resultado=pg_query($sql); $formato_presup="XX-XX-XX-XXX-XX-XX-XX";
			...
			if(($long_d=$long_u)and ($long_h=$long_u)){ $criterio=""; 
			   for ($i=0; $i<10; $i++) { $m=$mcontrol[$i]; if($i==0){$a=0;}else{$a=$mcontrol[$i-1];} 
				 if ($m<>0){if($i==0){ $len_nivel=$m; $criterio=""; }else{ $mpos=1+$a;  $len_nivel=($m-$a-1); $criterio=$criterio." and "; }
					...
					$criterio=$criterio."substring(pre039.cod_presup,".$mpp.",".$len_nivel.")>='".$cod_d."' and "; $criterio=$criterio."substring(pre039.cod_presup,".$mpp.",".$len_nivel.")<='".$cod_h."' ";  }
			   } $criterio=$criterio."and  pre039.fuente_financ>='".$cod_fuente_d."' and pre039.fuente_financ<='".$cod_fuente_h."'";
			}else{$criterio="pre039.cod_presup>='".$codigo_d."' and pre039.cod_presup<='".$codigo_h."' and  pre039.fuente_financ>='".$cod_fuente_d."' and pre039.fuente_financ<='".$cod_fuente_h."'";}
		  }else{$criterio="pre039.cod_presup>='".$cod_presup_d."' and pre039.cod_presup<='".$cod_presup_h."' and  pre039.fuente_financ>='".$cod_fuente_d."' and pre039.fuente_financ<='".$cod_fuente_h."'";}
...
pre009.Fecha_Modif as Fecha_Doc,'' as Ref_AEP,pre039.Grupo, pre009.Fecha_Modif as Fecha_AEP,'','',pre009.Anulado, pre009.Fecha_Anu AS Fecha_Anulado, substr(pre039.cod_presup,1,".$t.") as ced_rif,
...
if($agrupa_par=="SI"){ $ordenado="ORDER BY pre021.ced_rif,pre021.cod_presup,pre021.fuente_financ,pre021.Fecha_Doc, pre021.Referencia_Comp, pre021.Tipo_Compromiso";
  ...
  $sSQL = "SELECT nombre_usuario, tipo_registro, referencia_comp, tipo_compromiso, nombre_abrev_comp, nombre_tipo_comp, referencia_caus, tipo_causado, nombre_tipo_caus, nombre_abrev_caus, referencia_pago, tipo_pago, nombre_tipo_pago, nombre_abrev_pago, cod_presup, fuente_financ, denominacion,
       substr(denominacion,1,60) as den_corta, fecha_doc, ref_aep, num_proyecto, fecha_aep, tipo_documento,nro_documento, anulado, fecha_anulado, ced_rif, nombre_benef, func_inv, substr(cod_presup,".$ini.",".$p.") as cod_partida,
    ...
    if($tipo_rep=="PDF"){  $res=pg_query($sSQL); $ced_rif=""; $fecha_doc_grupo=""; $tipo_compromiso_grupo="0000"; $referencia_comp_grupo="00000000"; 
       ...
       if($agrupa_cat=="SI"){
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $criterio2; global $tam_logo; global $registro;
				$this->Image('../../imagenes/Logo_emp.png',7,7,$tam_logo);
				$this->SetFont('Arial','B',15);
				$this->Cell(50);
				$this->Cell(100,10,'MODIFICACIONES',1,0,'C');
				$this->Ln(15);
        ...
        // FIN NMDB 30-04-2018
		 ...
		 //$pdf->MultiCell(200,3,$sSQL,0,1);   
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1;  $fecha_doc=$registro["fecha_doc"]; $tipo_compromiso=$registro["tipo_compromiso"]; $referencia_comp=$registro["referencia_comp"]; 				
			   ...
			   $pdf->SetFont('Arial','',7);
			   $pdf->Cell(45,3,$cod_presup."   ".$fuente_financ,0,0,'L'); 			   
          ...
          // FIN NMDB 18-05-2018
			...
			$pdf->AddPage();
			$pdf->SetFont('Arial','',7);
			$i=0;  $total_ajustado=0; $sub_total_ajustado=0; $sub_total_dism=0; $sub_total_aum=0; $cantidad_ordenes=0; $prev_fecha_doc=""; $prev_tipo_compromiso=""; $prev_referencia_comp=""; 
			while($registro=pg_fetch_array($res)){ $i=$i+1;  $fecha_doc=$registro["fecha_doc"]; $tipo_compromiso=$registro["tipo_compromiso"]; $referencia_comp=$registro["referencia_comp"]; 				$ced_rif=$registro["ced_rif"]; $nombre_benef=$registro["nombre_benef"];$descripcion_doc=$registro["descripcion_doc"]; 
			    ...
			    if(($prev_fecha_doc<>$fecha_doc_grupo)or($prev_tipo_compromiso<>$tipo_compromiso_grupo)or($prev_referencia_comp<>$referencia_comp_grupo)){			      
				   ...
				   //if(($sub_total_ajustado==0)and($sub_total_dism<>0)){ $sub_total_ajustado=$sub_total_dism*-1;  }
				   if($i>1){ $total_ajustado=$total_ajustado+$sub_total_ajustado; $sub_total_ajustado=formato_monto($sub_total_ajustado); 
					  $pdf->Cell(180,2,'',0,0);
				  	  $pdf->Cell(20,2,'--------------------',0,1,'R');
				   ...
				   $prev_fecha_doc=$fecha_doc_grupo; $prev_tipo_compromiso=$tipo_compromiso_grupo; $prev_referencia_comp=$referencia_comp_grupo; $sub_total_ajustado=0; $sub_total_dism=0; 
				}
		       $fecha_doc=$registro["fecha_doc"]; $tipo_compromiso=$registro["tipo_compromiso"]; $referencia_comp=$registro["referencia_comp"]; 
			   $descripcion_doc=$registro["descripcion_doc"]; $cod_presup=$registro["cod_presup"]; $fuente_financ=$registro["fuente_financ"]; $denominacion=$registro["denominacion"];  				   $ajustado=$registro["ajustado"]; $func_inv=$registro["func_inv"]; $monto=$registro["monto"]; $comprometido=$registro["comprometido"];
			   $sub_total_dism=$sub_total_dism+$comprometido; $sub_total_aum=$sub_total_aum+$ajustado;				
			   if($func_inv=="+"){$resultado=$monto; $sub_total_ajustado=$sub_total_ajustado+$monto; }else{$resultado=$comprometido; $sub_total_ajustado=$sub_total_ajustado+$comprometido; }
			   $ajustado=formato_monto($ajustado); $monto=formato_monto($monto); $comprometido=formato_monto($comprometido); $fecha_doc=formato_ddmmaaaa($fecha_doc);	
			   if($php_os=="WINNT"){$descripcion_doc=$registro["descripcion_doc"]; }else{$descripcion_doc=utf8_decode($descripcion_doc);$denominacion=utf8_decode($denominacion);}
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1; $fecha_doc=$registro["fecha_doc"]; $tipo_compromiso=$registro["tipo_compromiso"]; $referencia_comp=$registro["referencia_comp"]; 				$ced_rif=$registro["ced_rif"]; $nombre_benef=$registro["nombre_benef"];$descripcion_doc=$registro["descripcion_doc"]; 
			...
			$fecha_doc_grupo=$fecha_doc; $tipo_compromiso_grupo=$tipo_compromiso; $referencia_comp_grupo=$referencia_comp; $ced_rif_grupo=$ced_rif; 
			 $descripcion_doc_grupo=$descripcion_doc; $nombre_benef_grupo=$nombre_benef;
			   ...
			   if(($prev_fecha_doc<>$fecha_doc_grupo)or($prev_tipo_compromiso<>$tipo_compromiso_grupo)or($prev_referencia_comp<>$referencia_comp_grupo)){
			    if($sub_total_ajustado>0){ $total_ajustado=$total_ajustado+$sub_total_ajustado; $sub_total_ajustado=formato_monto($sub_total_ajustado); 		
			     ?>	 				 
				<tr>
			    	   <td width="100" align="left"></td>
			    	   <td width="100" align="left"></td>
