...
if ($_GET) {
    $cod_presup_d = $_GET["cod_presupd"];
    $cod_presup_h = $_GET["cod_presuph"];
    $cod_fuente_d = $_GET["cod_fuented"];
    $cod_fuente_h = $_GET["cod_fuenteh"];
    $mes_desde    = $_GET["mes_desde"];
    $mes_hasta    = $_GET["mes_hasta"];
    $asig_global  = $_GET["asig_global"];
    $c_cat        = $_GET["csubtotal"];
    $cod_esp      = $_GET["cod_esp"];
    $cod_contab   = $_GET["cod_contab"];
    $tipo_rep     = $_GET["tipo_rep"];
    $det_modif    = $_GET["det_modif"];
    $most_gen     = $_GET["most_gen"];
} else {
    $codigod   = "";
    $codigoh   = "";
    $fuented   = "";
...
$cod_mov  = "pre020" . $usuario_sia;
$php_os   = PHP_OS; //$asig_global="N"; 
$mdes_cat = array(
    "NINGUNA",
    "",
    "",
...
function buscar_control($clave, $formato)
{
    global $mcontrol;
    $j = 0;
    for ($i = 0; $i < strlen($formato); $i++) {
        if (substr($formato, +$i, 1) == "-") {
            $j++;
        } else {
            $mcontrol[$j]++;
        }
...
function Rellenarcerosizq($str, $n)
{
    $numeroarellenar = $n - strlen($str);
    $texto           = "";
...
if ($cod_fuente_h == $cod_fuente_d) {
    $sSQL      = "Select * from PRE095 WHERE cod_fuente_financ='$cod_fuente_d'";
    $resultado = pg_query($sSQL);
    if ($registro = pg_fetch_array($resultado, 0)) {
        $criterio1 = $criterio1 . "  FUENTE : " . $registro["des_fuente_financ"];
    }
} else {
    if ($cod_fuente_d == "01") {
        $criterio1 = $criterio1 . " ";
...
$a        = buscar_control($cod_presup_d, $formato_presup);
$criterio = "";
$en_d     = 0;
$en_h     = 0;
$mpos     = 0;
if ($c_cat == 0) {
    ...
    $criterio_s = $mdes_cat[$c_cat];
    $ls         = $mcontrol[$c_cat - 1];
}
$pos = strrpos($cod_presup_d, "?");
if ($pos === false) {
    $en_d = 0;
} else {
    $en_d = 1;
}
$pos = strrpos($cod_presup_h, "?");
if ($pos === false) {
    $en_h = 0;
} else {
    $en_h = 1;
...
if (($en_d == 1) or ($en_h == 1)) {
    $codigo_d = str_replace("?", "0", $cod_presup_d);
    $long_d   = strlen($codigo_d);
    $codigo_h = str_replace("?", "9", $cod_presup_h);
    $long_h   = strlen($codigo_h);
    if (($long_d = $long_u) and ($long_h = $long_u)) {
        $criterio = "";
        for ($i = 0; $i < 10; $i++) {
            $m = $mcontrol[$i];
            if ($i == 0) {
            ...
            if ($m <> 0) {
                ...
                $cod_d    = substr($codigo_d, $mpos, $len_nivel);
                $cod_h    = substr($codigo_h, $mpos, $len_nivel);
                $mpp      = $mpos + 1;
                $criterio = $criterio . "substring(cod_presup," . $mpp . "," . $len_nivel . ")>='" . $cod_d . "' and ";
                $criterio = $criterio . "substring(cod_presup," . $mpp . "," . $len_nivel . ")<='" . $cod_h . "' ";
            }
        }
        $criterio = $criterio . "and  cod_fuente>='" . $cod_fuente_d . "' and cod_fuente<='" . $cod_fuente_h . "'";
    } else {
        $criterio = "cod_presup>='" . $codigo_d . "' and cod_presup<='" . $codigo_h . "' and  cod_fuente>='" . $cod_fuente_d . "' and cod_fuente<='" . $cod_fuente_h . "'";
    }
} else {
...
if ($cod_esp == "SI") {
    $criterioc = " and (pre001.cod_contable='$cod_contab') ";
} else {
    $criterioc = "";
}
$per_hasta       = $mes_hasta;
...
$sql_Diferido    = "";
If ($per_hasta == 0) {
    $sql_Traslados   = "0 as Traslados,";
    $sql_Trasladon   = "0 as Trasladon,";
    $sql_Adicion     = "0 as Adicion,";
    $sql_Disminucion = "0 as Disminucion,";
    $sql_Compromiso  = "0 as Compromiso,";
    $sql_Causado     = "0 as Causado,";
    $sql_Pagado      = "0 as Pagado,";
    $sql_Asignacion  = "0 as asignado,";
    $sql_Asignacion  = "asignado,";
    $sql_Diferido    = "0 as Diferido";
} else {
    for ($i = 1; $i <= $per_hasta; $i++) {
        $pos = $i;
        $pos = Rellenarcerosizq($pos, 2);
        If ($i == 1) {
            $scampo  = "(Traslados" . $pos;
            $scampo1 = "(Trasladon" . $pos;
            $scampo2 = "(Adicion" . $pos;
            $scampo3 = "(Disminucion" . $pos;
            $scampo7 = "(asignado" . $pos;
        } else {
            $scampo  = "+Traslados" . $pos;
            $scampo1 = "+Trasladon" . $pos;
            $scampo2 = "+Adicion" . $pos;
        ...
        $sql_Traslados   = $sql_Traslados . $scampo;
        $sql_Trasladon   = $sql_Trasladon . $scampo1;
        $sql_Adicion     = $sql_Adicion . $scampo2;
        $sql_Disminucion = $sql_Disminucion . $scampo3;
        $sql_Asignacion  = $sql_Asignacion . $scampo7;
    }
    for ($i = $mes_desde; $i <= $per_hasta; $i++) {
        $pos = $i;
        $pos = Rellenarcerosizq($pos, 2);
        If ($i == $mes_desde) {
            $scampo4 = "(Compromiso" . $pos;
            $scampo5 = "(Causado" . $pos;
            $scampo6 = "(Pagado" . $pos;
            $scampo8 = "(Diferido" . $pos;
        } else {
            $scampo4 = "+Compromiso" . $pos;
            $scampo5 = "+Causado" . $pos;
            $scampo6 = "+Pagado" . $pos;
        ...
        $sql_Compromiso = $sql_Compromiso . $scampo4;
        $sql_Causado    = $sql_Causado . $scampo5;
        $sql_Pagado     = $sql_Pagado . $scampo6;
        $sql_Diferido   = $sql_Diferido . $scampo8;
    }
    $sql_Traslados   = $sql_Traslados . ") as Traslados,";
    $sql_Trasladon   = $sql_Trasladon . ") as Trasladon,";
    $sql_Adicion     = $sql_Adicion . ") as Adicion,";
    $sql_Disminucion = $sql_Disminucion . ") as Disminucion,";
    $sql_Compromiso  = $sql_Compromiso . ") as Compromiso,";
    $sql_Causado     = $sql_Causado . ") as Causado,";
    $sql_Pagado      = $sql_Pagado . ") as Pagado,";
    $sql_Asignacion  = $sql_Asignacion . ") as asignado,";
    $sql_Asignacion  = "asignado,";
...
// $excluir_par =  " and cod_presup <> '01-02-00-00-00-401-07-08-00' ";
...
$StrSQL = $StrSQL . " FROM PRE001 WHERE length(cod_presup)=" . $l_c . " and " . $criterio . $criterioc. $excluir_par;
$res    = pg_exec($conn, $StrSQL);
$error  = pg_errormessage($conn);
$error  = substr($error, 0, 91);
if (!$res) {
...
$ordenar = " ORDER BY pre020.cod_partida ASC";
if (($c_cat > 0) or ($tipo_rep == "EXCEL2")) {
    $ordenar = " ORDER BY pre020.cod_categoria,pre020.cod_partida,pre020.cod_fuente";
    $sSQL    = "Select cod_presup,denominacion from pre001 WHERE cod_presup in (select distinct cod_categoria from pre020 where (tipo_registro='2') and (nombre_usuario='$cod_mov'))";
    $res     = pg_query($sSQL);
    while ($registro = pg_fetch_array($res)) {
        $cod_presup   = $registro["cod_presup"];
        $denominacion = $registro["denominacion"];
        $sql          = "update pre020 set denomina_cat='$denominacion' where tipo_registro='2' and nombre_usuario='$cod_mov' and cod_categoria='$cod_presup'";
...
$sSQL = "SELECT pre020.cod_presup,pre020.cod_fuente, pre020.denominacion,pre020.cod_categoria,pre020.denomina_cat,pre020.cod_partida, substring(pre020.cod_partida,1,3) as partida, substring(pre020.cod_partida,1,6) as generica, pre020.Asignado, pre020.Traslados, pre020.Trasladon, pre020.Adicion, 
            ...
            (pre020.Asignado+pre020.Traslados-pre020.Trasladon+pre020.Adicion-pre020.Disminucion) AS Asig_Actualizada, (pre020.Asignado+pre020.Traslados-pre020.Trasladon+pre020.Adicion-pre020.Disminucion-pre020.Compromiso) AS Disponibilidad
             FROM pre020 WHERE tipo_registro='2' and nombre_usuario='$cod_mov' and " . $criterio. $excluir_par . $ordenar;


$query = "SELECT pre020.cod_presup,pre020.cod_fuente, pre020.denominacion,pre020.cod_categoria,pre020.denomina_cat,pre020.cod_partida, substring(pre020.cod_partida,1,3) as partida, substring(pre020.cod_partida,1,6) as generica, pre020.Asignado, pre020.Traslados, pre020.Trasladon, pre020.Adicion, 
             pre020.disminucion, pre020.compromiso, pre020.causado, pre020.pagado, pre020.disponible, 
            (pre020.Traslados-pre020.Trasladon+pre020.Adicion-pre020.Disminucion) AS Modificaciones, (pre020.Traslados+pre020.Adicion) AS Aumentos, (pre020.Trasladon+pre020.Disminucion) AS Disminuciones,
            (pre020.Asignado+pre020.Traslados-pre020.Trasladon+pre020.Adicion-pre020.Disminucion) AS Asig_Actualizada, (pre020.Asignado+pre020.Traslados-pre020.Trasladon+pre020.Adicion-pre020.Disminucion-pre020.Compromiso) AS Disponibilidad
             FROM pre020 WHERE tipo_registro='2' and nombre_usuario='$cod_mov' and " . $criterio. $excluir_par . $ordenar;
...
// echo '<pre>' . print_r($datos_pre, true) . '</pre>';
...
if ($tipo_rep == "HTML") {
    ...
    if ($c_cat == 0) {
        $nomb_rpt = "Rpt_Ejec_Pre_Par.xml";
        if ($det_modif == "SI") {
            $nomb_rpt = "Rpt_Ejec_Pre_Par_Modif.xml";
        }
    } else {
        $nomb_rpt = "Rpt_Ejec_Pre_Par_Subt.xml";
        if ($det_modif == "SI") {
            $nomb_rpt = "Rpt_Ejec_Pre_Par_Subt_Modif.xml";
        }
    }
    ...
    $oRpt->setParameters(array(
        "criterio1" => $criterio1,
        "criterio2" => $criterio2,
        "criterios" => $criterio_s
    ));
    $oRpt->run();
}
if (($tipo_rep == "PDF") and ($det_modif <> "SI")) {
    $res   = pg_query($sSQL);
    ...
    class PDF extends FPDF
    {
        function Header()
        {
            ...
            global $php_os;
            global $tam_logo;
            $cod_presup_cat   = $registro["cod_categoria"];
            $denominacion_cat = $registro["denomina_cat"];
            if ($php_os == "WINNT") {
                $denominacion = $registro["denominacion"];
            } else {
                $denominacion_cat = utf8_decode($denominacion_cat);
            }
    ...
    while ($registro = pg_fetch_array($res)) {
        $i                = $i + 1;
        $cod_presup       = $registro["cod_presup"];
        $fuente_financ    = $registro["cod_fuente"];
        $denominacion     = $registro["denominacion"];
        $cod_categoria    = $registro["cod_categoria"];
        $denominacion_cat = $registro["denomina_cat"];
        $cod_partida      = $registro["cod_partida"];
        $clave            = $registro["partida"];
        ...
        $modificaciones   = $registro["modificaciones"];
        $comprometido     = $registro["compromiso"];
        $causado          = $registro["causado"];
        $pagado           = $registro["pagado"];
        $clave_g          = $registro["generica"];
        $disponible       = $registro["disponibilidad"];
        $asignado         = $registro["asignado"];
        $asig_actualizada = $registro["asig_actualizada"];
        ...
        if (($prev_clave <> $clave)) {
            ...
            if ($prev_par <> "") {
                $par_totalg = formato_monto($par_totalg);
                $par_totalf = formato_monto($par_totalf);
                $par_totald = formato_monto($par_totald);
                $par_totale = formato_monto($par_totale);
                $par_totalc = formato_monto($par_totalc);
                $par_totala = formato_monto($par_totala);
                $par_totalp = formato_monto($par_totalp);
                $par_totalm = formato_monto($par_totalm);
                ...
                $par_totald = 0;
                $par_totale = 0;
                $prev_par   = $cod_partida;
                $prev_den   = $denominacion;
            }
            if ($prev_par == "") {
                $prev_par = $cod_partida;
                $prev_den = $denominacion;
            ...
            if (($prev_gen <> $clave_g) and ($prev_gen <> "") and ($most_gen == "SI")) {
                $gen_totalg = formato_monto($gen_totalg);
                $gen_totalf = formato_monto($gen_totalf);
                $gen_totald = formato_monto($gen_totald);
                $gen_totale = formato_monto($gen_totale);
                $gen_totalc = formato_monto($gen_totalc);
                $gen_totala = formato_monto($gen_totala);
                $gen_totalp = formato_monto($gen_totalp);
                $gen_totalm = formato_monto($gen_totalm);
                $pdf->Cell(100, 1, '', 0, 0);
            ...
            if (($prev_clave <> $clave) and ($prev_clave <> "")) {
                $sub_totalg = formato_monto($sub_totalg);
                $sub_totalf = formato_monto($sub_totalf);
                $sub_totald = formato_monto($sub_totald);
                $sub_totale = formato_monto($sub_totale);
                $sub_totalc = formato_monto($sub_totalc);
                $sub_totala = formato_monto($sub_totala);
                $sub_totalp = formato_monto($sub_totalp);
            ...
            if (($categoria <> $prev_cat) and ($c_cat > 0) and ($prev_cat <> "")) {
                $cat_totalg = formato_monto($cat_totalg);
                $cat_totalf = formato_monto($cat_totalf);
                $cat_totald = formato_monto($cat_totald);
                $cat_totale = formato_monto($cat_totale);
                $cat_totalc = formato_monto($cat_totalc);
                $cat_totala = formato_monto($cat_totala);
                $cat_totalp = formato_monto($cat_totalp);
                $cat_totalm = formato_monto($cat_totalm);
                $pdf->Cell(100, 1, '', 0, 0);
                $pdf->Cell(20, 1, '__________________', 0, 0, 'R');
                $pdf->Cell(20, 1, '__________________', 0, 0, 'R');
                $pdf->Cell(20, 1, '__________________', 0, 0, 'R');
        ...
        $totalg     = $totalg + $asignado;
        $totalf     = $totalf + $asig_actualizada;
        $sub_totalg = $sub_totalg + $asignado;
        $sub_totalf = $sub_totalf + $asig_actualizada;
        $cat_totalg = $cat_totalg + $asignado;
        $cat_totalf = $cat_totalf + $asig_actualizada;
        $par_totalg = $par_totalg + $asignado;
        $par_totalf = $par_totalf + $asig_actualizada;
        $totald     = $totald + $disponible;
        $totale     = $totale + $deuda;
        $sub_totald = $sub_totald + $disponible;
        $sub_totale = $sub_totale + $deuda;
        $cat_totald = $cat_totald + $disponible;
        $cat_totale = $cat_totale + $deuda;
        $par_totald = $par_totald + $disponible;
        $par_totale = $par_totale + $deuda;
        $totalm     = $totalm + $modificaciones;
        $totalc     = $totalc + $comprometido;
        $sub_totalm = $sub_totalm + $modificaciones;
        $sub_totalc = $sub_totalc + $comprometido;
        $cat_totalm = $cat_totalm + $modificaciones;
        $cat_totalc = $cat_totalc + $comprometido;
        $par_totalm = $par_totalm + $modificaciones;
        $par_totalc = $par_totalc + $comprometido;
        $totala     = $totala + $causado;
        $totalp     = $totalp + $pagado;
        $sub_totala = $sub_totala + $causado;
        $sub_totalp = $sub_totalp + $pagado;
        $cat_totala = $cat_totala + $causado;
        $cat_totalp = $cat_totalp + $pagado;
        $par_totala = $par_totala + $causado;
        $par_totalp = $par_totalp + $pagado;
        $gen_totalg = $gen_totalg + $asignado;
        $gen_totalf = $gen_totalf + $asig_actualizada;
        $gen_totald = $gen_totald + $disponible;
        $gen_totale = $gen_totale + $deuda;
        $gen_totalm = $gen_totalm + $modificaciones;
        $gen_totalc = $gen_totalc + $comprometido;
        $gen_totala = $gen_totala + $causado;
        $gen_totalp = $gen_totalp + $pagado;
        if ($c_cat == 100) {
            $modificaciones   = formato_monto($modificaciones);
            $comprometido     = formato_monto($comprometido);
            $causado          = formato_monto($causado);
            $pagado           = formato_monto($pagado);
            $disponible       = formato_monto($disponible);
            $asignado         = formato_monto($asignado);
            $asig_actualizada = formato_monto($asig_actualizada);
            $deuda            = formato_monto($deuda);
    ...
    $totalf = formato_monto($totalf);
    $totald = formato_monto($totald);
    $totale = formato_monto($totale);
    $totalc = formato_monto($totalc);
    $totala = formato_monto($totala);
    $totalp = formato_monto($totalp);
...
if (($tipo_rep == "PDF") and ($det_modif == "SI")) {
    ...
    while ($registro = pg_fetch_array($res)) {
        $i                = $i + 1;
        $cod_presup       = $registro["cod_presup"];
        $fuente_financ    = $registro["fuente_financ"];
        $denominacion     = $registro["denominacion"];
        $cod_categoria    = $registro["cod_categoria"];
        $denominacion_cat = $registro["denomina_cat"];
        $cod_partida      = $registro["cod_partida"];
        ...
        $causado          = $registro["causado"];
        $pagado           = $registro["pagado"];
        $aumentos         = $registro["aumentos"];
        $disminuciones    = $registro["disminuciones"];
        $disponible       = $registro["disponibilidad"];
        $asignado         = $registro["asignado"];
        $asig_actualizada = $registro["asig_actualizada"];
        $deuda            = $registro["causado"] - $registro["pagado"];
        ...
        if (($prev_clave <> $clave) or ($prev_gen <> $clave_g) or (($categoria <> $prev_cat) and ($c_cat > 0))) {
            if ($prev_par <> "") {
                ...
                $par_totalp = formato_monto($par_totalp);
                $par_totalm = formato_monto($par_totalm);
                $par_totaln = formato_monto($par_totaln);
                $par_totalo = formato_monto($par_totalo);
                $pdf->Cell(15, 3, $prev_par, 0, 0);
                $x = $pdf->GetX();
                $y = $pdf->GetY();
                $n = 80;
            ...
            if (($prev_gen <> $clave_g) and ($prev_gen <> "") and ($most_gen == "SI")) {
                ...
                $gen_totalp = formato_monto($gen_totalp);
                $gen_totalm = formato_monto($gen_totalm);
                $gen_totalo = formato_monto($gen_totalo);
                $gen_totaln = formato_monto($gen_totaln);
                $pdf->Cell(95, 1, '', 0, 0);
                $pdf->Cell(20, 1, '__________________', 0, 0, 'R');
            ...
            if (($prev_clave <> $clave) and ($prev_clave <> "")) {
                ...
                $sub_totalp = formato_monto($sub_totalp);
                $sub_totalm = formato_monto($sub_totalm);
                $sub_totaln = formato_monto($sub_totaln);
                $sub_totalo = formato_monto($sub_totalo);
                $pdf->Cell(95, 1, '', 0, 0);
                $pdf->Cell(20, 1, '__________________', 0, 0, 'R');
                $pdf->Cell(15, 1, '__________________', 0, 0, 'R');
                $pdf->Cell(15, 1, '__________________', 0, 0, 'R');
            ...
            if (($categoria <> $prev_cat) and ($c_cat > 0) and ($prev_cat <> "")) {
                ...
                $cat_totalp = formato_monto($cat_totalp);
                $cat_totalm = formato_monto($cat_totalm);
                $cat_totaln = formato_monto($cat_totaln);
                $cat_totalo = formato_monto($cat_totalo);
                $pdf->Cell(95, 1, '', 0, 0);
                $pdf->Cell(20, 1, '__________________', 0, 0, 'R');
                $pdf->Cell(15, 1, '__________________', 0, 0, 'R');
                $pdf->Cell(15, 1, '__________________', 0, 0, 'R');
        ...
        $par_totalm = $par_totalm + $modificaciones;
        $par_totalc = $par_totalc + $comprometido;
        $par_totalo = $par_totalo + $aumentos;
        $par_totaln = $par_totaln + $disminuciones;
        $totalo     = $totalo + $aumentos;
        $totaln     = $totaln + $disminuciones;
        $sub_totalo = $sub_totalo + $aumentos;
        $sub_totaln = $sub_totaln + $disminuciones;
        $cat_totalo = $cat_totalo + $aumentos;
        $cat_totaln = $cat_totaln + $disminuciones;
        $totala     = $totala + $causado;
        $totalp     = $totalp + $pagado;
        $sub_totala = $sub_totala + $causado;
        $sub_totalp = $sub_totalp + $pagado;
        ...
        $gen_totala = $gen_totala + $causado;
        $gen_totalp = $gen_totalp + $pagado;
        $gen_totalo = $gen_totalo + $aumentos;
        $gen_totaln = $gen_totaln + $disminuciones;
        ...
        if ($c_cat == 100) {
            ...
            $asig_actualizada = formato_monto($asig_actualizada);
            $deuda            = formato_monto($deuda);
            $aumentos         = formato_monto($aumentos);
            $disminuciones    = formato_monto($disminuciones);
            $pdf->Cell(15, 3, $cod_partida, 0, 0);
            $x = $pdf->GetX();
            $y = $pdf->GetY();
            $n = 80;
...
?>
      <table border="0" cellspacing='0' cellpadding='0' align="left">
         ...
         <tr height="20">
            <td width="140" align="left" ><strong></strong></td>
            <td width="400" align="center" > <font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong><?php
    echo $criterio1;
?></strong></font></td>
         </tr>
         <tr height="20">
...
?><td width="120" align="right" bgcolor="#99CCFF" ><strong>Modificaciones</strong></td><?php
    }
?>
    ...
    while ($registro = pg_fetch_array($res)) {
        ...
        $denominacion_cat = conv_cadenas($denominacion_cat, 0);
        $modificaciones   = $registro["modificaciones"];
        $comprometido     = $registro["compromiso"];
        $causado          = $registro["causado"];
        $pagado           = $registro["pagado"];
...
?>      
                    <tr>
                       <td width="140" align="left"><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><?php
                echo $prev_par;
?></td>
...
?><td width="120" align="right">-----------------</td><?php
                }
?>
                     <td width="120" align="right">-----------------</td>
                      <td width="120" align="right">-----------------</td>
...
?>
                    ...
                    <tr>
                      <td width="140" align="left"></td>
                      <td width="400" align="right"><?php
                echo "Total " . $criterio_s . " " . $prev_cat . " : ";
?></td>
                      <td width="120" align="right"><?php
                echo $cat_totalg;
?></td>
...
?></td>
                    ...
                    <tr>
                     <td width="140" align="left"><strong>'<?php
                echo $categoria;
?> </strong></td>                    
                     <td width="400" align="left"><strong><?php
                echo $denominacion_cat;
?> </strong></td>
...
?>
    ...
    while ($registro = pg_fetch_array($res)) {
        ...
        if (($prev_clave <> $clavep)) {
            if ($prev_clave <> "") {
                $mcat       = elimina_guion($prev_cat);
                $mpartida   = elimina_guion($prev_par);
                $mcat       = "11" . substr($mcat, 0, 4) . "00" . substr($mcat, 4, 2);
                $par_totalg = formato_monto($par_totalg);
                $par_totalf = formato_monto($par_totalf);
                $par_totald = formato_monto($par_totald);
                $par_totale = formato_monto($par_totale);
...
?></td>
                ...
                $prev_par     = $cod_partida;
                $prev_den     = $denominacion;
                $prev_cat     = $cod_categoria;
                $prev_den_cat = $denominacion_cat;
                $prev_clave   = $clavep;
            }
            if ($prev_clave == "") {
                $prev_clave   = $clavep;
