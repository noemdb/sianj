...
if ($_GET){$cod_presup_d=$_GET["cod_presupd"];$cod_presup_h=$_GET["cod_presuph"];$cod_fuente_d=$_GET["cod_fuented"];$cod_fuente_h=$_GET["cod_fuente_h"];$mes=$_GET["mes"];$tipo=$_GET["tipo"];}
...
if ($tipo=='C')
{
	...
	if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
			...
			(PRE020.Asignado+PRE020.Traslados-PRE020.Trasladon+PRE020.Adicion-PRE020.Disminucion-PRE020.Causado) AS Absoluto
			FROM PRE020 
			ORDER BY PRE020.Cod_Presup";
			$oRpt = new PHPReportMaker();
			$oRpt->setXML("Rpt_resumen_ejecucion_comp_gasto.xml");
			$oRpt->setUser("$user");
...
else
{
	...
	if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
			...
			(PRE020.Traslados-PRE020.Trasladon+PRE020.Adicion-PRE020.Disminucion) AS Modificaciones,
			(PRE020.Asignado+PRE020.Traslados-PRE020.Trasladon+PRE020.Adicion-PRE020.Disminucion) AS Asig_Actualizada,
			(PRE020.Asignado+PRE020.Traslados-PRE020.Trasladon+PRE020.Adicion-PRE020.Disminucion-PRE020.Pagado) AS Absoluto
			FROM PRE020 
			ORDER BY PRE020.Cod_Presup";
			$oRpt = new PHPReportMaker();
			$oRpt->setXML("Rpt_resumen_ejecucion_comp_gasto.xml");
