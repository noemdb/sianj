...
if ($_GET){$cod_presup_d=$_GET["cod_presupd"];$cod_presup_h=$_GET["cod_presuph"];$cod_fuente_d=$_GET["cod_fuented"];$cod_fuente_h=$_GET["cod_fuenteh"];$fecha=$_GET["fecha"];$tipo_rep=$_GET["tipo_rep"]; $det_modif=$_GET["det_modif"]; $disp_dif=$_GET["disp_dif"]; $total_part=$_GET["total_part"];  }
else{$codigod="";$codigoh="";$fuented="";$fuenteh="";$fecha="";$tipo_rep="HTML"; $det_modif="NO"; $total_part="N"; }  $equipo=getenv("COMPUTERNAME"); $cod_mov="pre020".$usuario_sia; $fecha_hoy=asigna_fecha_hoy(); $sfecha=formato_aaaammdd($fecha);
 ...
 if($tipo_rep=="HTML"){ include ("../../class/phpreports/PHPReportMaker.php"); $nomb_rpt="Rpt_Disponibilidad_Diaria.xml"; 
  ...
  if(($tipo_rep=="PDF")and($Cod_Emp<>"71")){	$res=pg_query($sSQL); $filas=pg_num_rows($res); if($filas>=1){ $registro=pg_fetch_array($res,0); }
      ...
      class PDF extends FPDF{ 
		function Header(){global $criterio1; global $registro; global $tam_logo; global $php_os;
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1;  
			...
			$cod_presup_cat=$registro["cod_presup_cat"]; $denominacion_cat=$registro["denominacion_cat"];   $cod_partida=$registro["cod_partida"];  $clave_partida=substr($cod_partida,0,3); $clave=$cod_presup_cat;			
			if($php_os=="WINNT"){$denominacion=$registro["denominacion"]; }   else{$denominacion_cat=utf8_decode($denominacion_cat); $denominacion=utf8_decode($denominacion);}
		    if($prev_clave<>$clave){ 
			    if(($prev_par<>"")and($total_part<>"N")){ 
					...
					$pdf->Cell(20,2,'-----------------------',0,1,'R');
					$pdf->Cell(85,3,'',0,0);
                    $pdf->Cell(15,3,'Total '.$prev_par,0,0);					
					...
					$pdf->Cell(100,2,'',0,1);
					$par_totalg=0; $par_totalf=0; $par_totalm=0; $par_totalc=0; $par_totala=0; $par_totalp=0; $par_totald=0; $par_totale=0; $prev_par=$clave_partida; $prev_den=$denominacion;
				}
				if ($prev_par==""){ $prev_par=$clave_partida; }
			...
			if(($prev_par<>$clave_partida)and($total_part<>"N")){ 
				    $par_totalg=formato_monto($par_totalg);$par_totalf=formato_monto($par_totalf);  $par_totald=formato_monto($par_totald);  $par_totale=formato_monto($par_totale); 
					$par_totalc=formato_monto($par_totalc);$par_totala=formato_monto($par_totala);  $par_totalp=formato_monto($par_totalp);  $par_totalm=formato_monto($par_totalm); 
					$pdf->Cell(100,2,'',0,0);
					$pdf->Cell(20,2,'-----------------------',0,0,'R');
			...
			if(($prev_par<>$clave_partida)and($total_part=="N")){  $pdf->Cell(100,3,'',0,1); $prev_par=$clave_partida; $prev_den=$denominacion; }
            ...
            $totalg=$totalg+$asignado; $totalf=$totalf+$asig_actualizada; $sub_totalg=$sub_totalg+$asignado; $sub_totalf=$sub_totalf+$asig_actualizada;	$par_totalg=$par_totalg+$asignado; $par_totalf=$par_totalf+$asig_actualizada;		
			$totald=$totald+$disponible; $totale=$totale+$deuda; $sub_totald=$sub_totald+$disponible; $sub_totale=$sub_totale+$deuda; $par_totald=$par_totald+$disponible; $par_totale=$par_totale+$deuda;
			$totalm=$totalm+$modificaciones; $totalc=$totalc+$comprometido; $sub_totalm=$sub_totalm+$modificaciones; $sub_totalc=$sub_totalc+$comprometido; $par_totalm=$par_totalm+$modificaciones; $par_totalc=$par_totalc+$comprometido;
		    $totala=$totala+$causado; $totalp=$totalp+$pagado; $sub_totala=$sub_totala+$causado; $sub_totalp=$sub_totalp+$pagado; $par_totala=$par_totala+$causado; $par_totalp=$par_totalp+$pagado;
	...
	if(($tipo_rep=="PDF")and($Cod_Emp=="71")){	$res=pg_query($sSQL); $filas=pg_num_rows($res); if($filas>=1){ $registro=pg_fetch_array($res,0); 
      ...
      require('../../class/fpdf/fpdf.php');
      class PDF extends FPDF{ 
		function Header(){global $criterio1; global $cod_presup_cat; global $denominacion_cat; global $tam_logo;
			$this->Image('../../imagenes/Logo_emp.png',7,7,$tam_logo);
			$this->SetFont('Arial','B',10);
			$this->Cell(50);
	  ...
	  $i=0;  $totalg=0; $totalf=0; $totalm=0; $totalc=0; $totala=0; $totalp=0; $totald=0; $totale=0;
      $sub_totalg=0; $sub_totalf=0; $sub_totalm=0; $sub_totalc=0; $sub_totala=0; $sub_totalp=0; $sub_totald=0; $sub_totale=0; $prev_clave="";  
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $clave=$registro["cod_presup_cat"]; $cod_partida=$registro["cod_partida"];  $clave_partida=substr($cod_partida,0,3);
		    if($prev_clave<>$clave){ 
			    $pdf->SetFont('Arial','B',6); 
			    if(($sub_totalg>0)or($sub_totalf>0)or($sub_totalc>0)or($sub_totalm>0)or($sub_totala>0)or($sub_totalp>0)or($sub_totald>0)or($i>1)){ 
				   $sub_totalg=formato_monto($sub_totalg);$sub_totalf=formato_monto($sub_totalf);  $sub_totald=formato_monto($sub_totald);  $sub_totale=formato_monto($sub_totale); 
				   $sub_totalc=formato_monto($sub_totalc);$sub_totala=formato_monto($sub_totala);  $sub_totalp=formato_monto($sub_totalp);  $sub_totalm=formato_monto($sub_totalm); 
				    $pdf->Cell(100,2,'',0,0);
					$pdf->Cell(20,2,'-----------------------',0,0,'R');
	...
	if(($tipo_rep=="PDF3")){	$res=pg_query($sSQL); $filas=pg_num_rows($res); if($filas>=1){ $registro=pg_fetch_array($res,0); 
	  ...
	  $par_totalg=0; $par_totalf=0; $par_totalm=0; $par_totalc=0; $par_totala=0; $par_totalp=0; $par_totald=0; $par_totale=0; $par_totali=0; $prev_par="";	$prev_den="";	
	  while($registro=pg_fetch_array($res)){ $i=$i+1;  $clave=$registro["cod_presup_cat"]; $cod_partida=$registro["cod_partida"];  $clave_partida=substr($cod_partida,0,3);
		    if($prev_clave<>$clave){ 
			    if(($prev_par<>"")and($total_part<>"N")){ 
				    $par_totalg=formato_monto($par_totalg);$par_totalf=formato_monto($par_totalf);  $par_totald=formato_monto($par_totald);  $par_totale=formato_monto($par_totale); 
					$par_totalc=formato_monto($par_totalc);$par_totala=formato_monto($par_totala);  $par_totalp=formato_monto($par_totalp);  $par_totalm=formato_monto($par_totalm); $par_totali=formato_monto($par_totali);
					$pdf->Cell(100,2,'',0,0);
					$pdf->Cell(20,2,'-----------------------',0,0,'R');
					$pdf->Cell(20,2,'-----------------------',0,0,'R');
					$pdf->Cell(20,2,'-----------------------',0,0,'R');
					...
					$pdf->Cell(20,3,$par_totalf,0,0,'R'); 	
					$pdf->Cell(20,3,$par_totalc,0,0,'R');
                    $pdf->Cell(20,3,$par_totali,0,0,'R');
                    $pdf->Cell(20,3,$par_totald,0,0,'R');					
					$pdf->Cell(20,3,$par_totala,0,0,'R'); 
					$pdf->Cell(20,3,$par_totalp,0,1,'R');
					$pdf->Cell(100,2,'',0,1);
					$par_totalg=0; $par_totalf=0; $par_totalm=0; $par_totalc=0; $par_totala=0; $par_totalp=0; $par_totald=0; $par_totale=0; $par_totali=0; $prev_par=$clave_partida; $prev_den=$denominacion;
				}
				if ($prev_par==""){ $prev_par=$clave_partida; }
			    $pdf->SetFont('Arial','B',6); 
			    if(($sub_totalg>0)or($sub_totalf>0)or($sub_totalc>0)or($sub_totalm>0)or($sub_totala>0)or($sub_totalp>0)or($sub_totald>0)or($i>1)){ 
			...
			$totalg=$totalg+$asignado; $totalf=$totalf+$asig_actualizada; $sub_totalg=$sub_totalg+$asignado; $sub_totalf=$sub_totalf+$asig_actualizada;		$par_totalg=$par_totalg+$asignado; $par_totalf=$par_totalf+$asig_actualizada;		
			$totald=$totald+$disponible; $totale=$totale+$deuda; $sub_totald=$sub_totald+$disponible; $sub_totale=$sub_totale+$deuda; $totali=$totali+$diferido; $sub_totali=$sub_totali+$diferido; $par_totali=$par_totali+$diferido;  $par_totald=$par_totald+$disponible; $par_totale=$par_totale+$deuda;
			$totalm=$totalm+$modificaciones; $totalc=$totalc+$comprometido; $sub_totalm=$sub_totalm+$modificaciones; $sub_totalc=$sub_totalc+$comprometido; $par_totalm=$par_totalm+$modificaciones; $par_totalc=$par_totalc+$comprometido;
		    $totala=$totala+$causado; $totalp=$totalp+$pagado; $sub_totala=$sub_totala+$causado; $sub_totalp=$sub_totalp+$pagado; $par_totala=$par_totala+$causado; $par_totalp=$par_totalp+$pagado;
		    $modificaciones=formato_monto($modificaciones); 	$comprometido=formato_monto($comprometido);  $causado=formato_monto($causado); 	$pagado=formato_monto($pagado); 
	...
	if(($tipo_rep=="PDF2")){	$res=pg_query($sSQL); $filas=pg_num_rows($res); if($filas>=1){ $registro=pg_fetch_array($res,0); 
	  ...
	  $par_totalg=0; $par_totalf=0; $par_totalm=0; $par_totalc=0; $par_totala=0; $par_totalp=0; $par_totald=0; $par_totale=0; $par_totalts=0; $par_totaltn=0; $par_totalad=0; $prev_par="";	$prev_den="";	
	  while($registro=pg_fetch_array($res)){ $i=$i+1;  $clave=$registro["cod_presup_cat"]; $cod_partida=$registro["cod_partida"];  $clave_partida=substr($cod_partida,0,3);
		    if($prev_clave<>$clave){
                if(($prev_par<>"")and($total_part<>"N")){  
				    $par_totalg=formato_monto($par_totalg);$par_totalf=formato_monto($par_totalf);  $par_totald=formato_monto($par_totald);  $par_totale=formato_monto($par_totale); 
					$par_totalc=formato_monto($par_totalc);$par_totala=formato_monto($par_totala);  $par_totalp=formato_monto($par_totalp);  $par_totalm=formato_monto($par_totalm); 
					$par_totalts=formato_monto($par_totalts); $par_totalad=formato_monto($par_totalad); $par_totaltn=formato_monto($par_totaltn);
					$pdf->Cell(138,2,'',0,0);
					$pdf->Cell(20,2,'-----------------------',0,0,'R');
					$pdf->Cell(19,2,'-----------------------',0,0,'R');
					$pdf->Cell(19,2,'-----------------------',0,0,'R');
					...
					$pdf->Cell(20,2,'-----------------------',0,1,'R');
					$pdf->Cell(120,3,'',0,0);
                    $pdf->Cell(18,3,'Total '.$prev_par,0,0);					
					$pdf->Cell(20,3,$par_totalg,0,0,'R'); 
					$pdf->Cell(19,3,$par_totalts,0,0,'R'); 
					$pdf->Cell(19,3,$par_totaltn,0,0,'R'); 
					$pdf->Cell(22,3,$par_totalad,0,0,'R'); 
					$pdf->Cell(22,3,$par_totalf,0,0,'R'); 	
					$pdf->Cell(20,3,$par_totalc,0,0,'R');
                    $pdf->Cell(20,3,$par_totala,0,0,'R'); 
					$pdf->Cell(20,3,$par_totalp,0,0,'R');
					$pdf->Cell(20,3,$par_totald,0,0,'R');
					$pdf->Cell(20,4,$par_totale,0,1,'R');
					$pdf->Cell(100,2,'',0,1);
					$par_totalg=0; $par_totalf=0; $par_totalm=0; $par_totalc=0; $par_totala=0; $par_totalp=0; $par_totald=0; $par_totale=0; $par_totalts=0; $par_totaltn=0; $par_totalad=0;  $prev_par=$clave_partida; $prev_den=$denominacion;
				}
				if ($prev_par==""){ $prev_par=$clave_partida; }						
			    $pdf->SetFont('Arial','B',6); 
			    if(($sub_totalg>0)or($sub_totalf>0)or($sub_totalc>0)or($sub_totalm>0)or($sub_totala>0)or($sub_totalp>0)or($sub_totald>0)or($i>1)){ 
			...
			$cod_presup=$registro["cod_presup"]; $cod_fuente=$registro["cod_fuente"]; $denominacion=$registro["denominacion"]; $cod_presup_cat=$registro["cod_presup_cat"]; $denominacion_cat=$registro["denominacion_cat"]; $cod_unidad_ejec=$registro["cod_unidad_ejec"]; $nombre_unidad_ejec=$registro["nombre_unidad_ejec"];  $cod_partida=$registro["cod_partida"];  
			if($php_os=="WINNT"){$denominacion=$registro["denominacion"]; }   else{$denominacion_cat=utf8_decode($denominacion_cat); $denominacion=utf8_decode($denominacion); $nombre_unidad_ejec=utf8_decode($nombre_unidad_ejec); }
		    $modificaciones=$registro["modificaciones"]; $comprometido=$registro["compromiso"];   $causado=$registro["causado"]; $pagado=$registro["pagado"]; 
			$disponible=$registro["saldo"];  $asignado=$registro["asignado"];  $asig_actualizada=$registro["asig_actualizada"]; $deuda=$registro["deuda"];
			...
			$par_totalts=$par_totalts+$traslados; $par_totaltn=$par_totaltn+$trasladon; $par_totalad=$par_totalad+$adic_dism;
			$totalm=$totalm+$modificaciones; $totalc=$totalc+$comprometido; $sub_totalm=$sub_totalm+$modificaciones; $sub_totalc=$sub_totalc+$comprometido; $par_totalm=$par_totalm+$modificaciones; $par_totalc=$par_totalc+$comprometido;
		    $totala=$totala+$causado; $totalp=$totalp+$pagado; $sub_totala=$sub_totala+$causado; $sub_totalp=$sub_totalp+$pagado; $par_totala=$par_totala+$causado; $par_totalp=$par_totalp+$pagado;
		    $modificaciones=formato_monto($modificaciones); 	$comprometido=formato_monto($comprometido);  $causado=formato_monto($causado); 	$pagado=formato_monto($pagado); 
		    $disponible=formato_monto($disponible); $asignado=formato_monto($asignado);  $asig_actualizada=formato_monto($asig_actualizada);   $deuda=formato_monto($deuda); 
