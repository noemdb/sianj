...
if (pg_ErrorMessage($conn)){$error=1;?> <script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script> <? } else { $Nom_Emp=busca_conf(); if($utf_rpt=="SI"){if($php_os=="WINNT"){$php_os="LINUX";}else{$php_os="WINNT";}}}
  ...
  $criteriod4="(pre006.tipo_compromiso>='$doc_comp_d' and pre006.tipo_compromiso<='$doc_comp_h')";
  $criteriod="(tipo_compromiso>='$doc_comp_d' and tipo_compromiso<='$doc_comp_h')";
  if(($doc_comp_d==$doc_comp_h)and($tipo_regis=='TO')and($most_anu=='SI')){ $doc_comp_h="A".substr($doc_comp_h,1,3);
    $criteriod="(tipo_compromiso='$doc_comp_d' or tipo_compromiso='$doc_comp_h')";
	$criteriod4="(pre006.tipo_compromiso='$doc_comp_d' or pre006.tipo_compromiso='$doc_comp_h')";
  }
  
  $criterio="(cod_presup>='$cod_presupd' and cod_presup<='$cod_presuph') and ".$criteriod." and (referencia_comp>='$referencia_d' and referencia_comp<='$referencia_h') And (fecha_doc>='$cfecha_d' and fecha_doc<='$cfecha_h') And (nro_documento>='$nro_doc_d' and nro_documento<='$nro_doc_h') And (fuente_financ>='$cod_fuente_d' and fuente_financ<='$cod_fuente_h') And (ced_rif>='$cedula_d' and ced_rif<='$cedula_h')";
  $criteriop="(referencia_comp>='$referencia_d' and referencia_comp<='$referencia_h') and ".$criteriod." and (fecha_doc>='$cfecha_d' and fecha_doc<='$cfecha_h') And (nro_documento>='$nro_doc_d' and nro_documento<='$nro_doc_h') And (fuente_financ>='$cod_fuente_d' and fuente_financ<='$cod_fuente_h') And (ced_rif>='$cedula_d' and ced_rif<='$cedula_h')";
  
  $criterio4=$criterio_est."(pre036.cod_presup>='$cod_presupd' and pre036.cod_presup<='$cod_presuph') and ".$criteriod4." (pre006.referencia_comp>='$referencia_d' and pre006.referencia_comp<='$referencia_h') And (pre006.nro_documento>='$nro_doc_d' and pre006.nro_documento<='$nro_doc_h') And (pre006.Fecha_Compromiso>='$cfecha_d' and pre006.Fecha_Compromiso<='$cfecha_h') And (pre036.fuente_financ>='$cod_fuente_d' and pre036.fuente_financ<='$cod_fuente_h') And (pre006.ced_rif>='$cedula_d' and pre006.ced_rif<='$cedula_h')";
  $criterio4=$criterio_est." (pre006.cod_tipo_comp>='$tipo_comp_d' and pre006.cod_tipo_comp<='$tipo_comp_h') and ".$criteriod4." And (pre006.referencia_comp>='$referencia_d' and pre006.referencia_comp<='$referencia_h') And (pre006.nro_documento>='$nro_doc_d' and pre006.nro_documento<='$nro_doc_h') And (pre006.Fecha_Compromiso>='$cfecha_d' and pre006.Fecha_Compromiso<='$cfecha_h') And (pre006.ced_rif>='$cedula_d' and pre006.ced_rif<='$cedula_h')";
...
$cant_orden=0;
   ...
   $res=pg_query($sql); if($registro=pg_fetch_array($res,0)){$cant_orden=$registro["cant_orden"];}   	
	...
	if($tipo_rep=="HTML"){	include ("../../class/phpreports/PHPReportMaker.php");
        ...
        $oRpt->setXML("Rpt_compromisos_general.xml");
        $oRpt->setUser("$user");
    ...
    if($tipo_rep=="PDF"){  $res=pg_query($sSQL); $fecha_doc_grupo=""; $referencia_comp_grupo="00000000"; $tipo_compromiso_grupo="0000"; 
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $criterio2; global $fecha_doc_grupo; global $referencia_comp_grupo; global $tipo_compromiso_grupo; global $registro; global $tam_logo;
				...
				$this->Cell(50);
				$this->Cell(150,10,'COMPROMISOS',1,0,'C');
				$this->Ln(15);
		  ...
		  $pdf->AddPage();
		  $pdf->SetFont('Arial','',7);
		  //$pdf->MultiCell(260,3," A ".$tempsql,0,1);
	...
	if($tipo_rep=="EXCEL"){	
		...
		header("Content-Disposition: attachment; filename=Rpt_compromisos_general.xls"); 
		
        ?>
		   <table border="0" cellspacing="0" cellpadding="0" align="left">
			 <tr height="20">
				<td width="100" align="left" ><strong></strong></td>
			    <td width="400" align="center" > <font size="4" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong>COMPROMISOS</strong></font></td>
			 </tr>
			 <tr height="20">
				<td width="100" align="left" ><strong></strong></td>
          ...
          $total_monto_ajustado=formato_monto($total_monto_ajustado);
		  if(($sub_total_monto<>0)or($sub_total_ajuste>0)or($sub_total_monto_ajustado>0)or($sub_total_causado>0)or($sub_total_pagado>0)){ $sub_total_monto=formato_monto($sub_total_monto); $sub_total_ajuste=formato_monto($sub_total_ajuste); $sub_total_ajustado=formato_monto($sub_total_ajustado); $sub_total_monto_ajustado=formato_monto($sub_total_monto_ajustado); $sub_total_causado=formato_monto($sub_total_causado);$sub_total_pagado=formato_monto($sub_total_pagado);	
			     ?>	 				 
				<tr>
			    	   <td width="100" align="left"></td>
