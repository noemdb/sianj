<? error_reporting(E_ALL ^ E_NOTICE);include ("../../class/conect.php"); require ("../../class/fun_fechas.php"); require ("../../class/fun_numeros.php");
$ref_credito=$_GET["ref_credito"]; $cod_presup_d=$_GET["cod_presup_d"]; $cod_presup_h=$_GET["cod_presup_h"]; $fecha_d=$_GET["fecha_d"];  $fecha_h=$_GET["fecha_h"];$tipo_rep=$_GET["tipo_rep"]; $subt_cod=$_GET["subt_cod"];
$Sql="";$date = date("d-m-Y");$hora = date("H:i:s a"); $cod_fuented="00"; $cod_fuenteh="99";
$criterio1=""; $criterio2="";  $php_os=PHP_OS; 
...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
  ...
  $sql="select * from pre009 where (referencia_modif='$ref_credito') and (tipo_modif = '1')";  $resultado=pg_query($sql); 
  if ($registro=pg_fetch_array($resultado,0)){$fecha_d=$registro["fecha_modif"];  $fecha_d=formato_ddmmaaaa($fecha_d);}
  ...
  $criterio1="Rendicion del Credito Adicional Decreto Numero ".$ref_credito; 
  $criterio2="Monto Bs. ".$monto_cred;
  ...
  $criterio="(cod_presup>='$cod_presupd' and cod_presup<='$cod_presuph') And (cod_fuente>='$cod_fuented' and cod_fuente<='$cod_fuenteh')";
   ...
   if(($en_d==1)or($en_h==1)){$codigo_d=str_replace("?","0",$cod_presupd); $long_d=strlen($codigo_d); $codigo_h=str_replace("?","9",$cod_presuph); $long_h=strlen($codigo_h);
    ...
    $res=pg_exec($conn,"SELECT ACTUALIZA_REND_CREDITO('E','$cod_mov','A','$ref_credito','$sfechad','$sfechah')");  $error=pg_errormessage($conn); $error=substr($error, 0, 61);if (!$res){ ?> <script language="JavaScript">  muestra('<? echo $error; ?>'); </script> <? }
    ...
    $res=pg_exec($conn,"SELECT ACTUALIZA_REND_CREDITO('C','$cod_mov','A','$ref_credito','$sfechad','$sfechah')");  $error=pg_errormessage($conn); $error=substr($error, 0, 61);if (!$res){ ?> <script language="JavaScript">  muestra('<? echo $error; ?>'); </script> <? }
	...
	if($tipo_rep=="HTML"){	include ("../../class/phpreports/PHPReportMaker.php");
    	$nomb_rpt="Rpt_rendicion_caus_credito_adicional.xml"; if($subt_cod=="S"){ $nomb_rpt="Rpt_rendicion_caus_credito_adic_codigo.xml"; }
   ...
   if($tipo_rep=="PDF"){ $res=pg_query($sSQL); $cod_presup_grupo=""; $prev_y=0; $fin=0;
      ...
      class PDF extends FPDF{
		function Header(){global $criterio1; global $criterio2; global $criterio3;  global $registro; global $prev_y;
			...
			$this->Cell(73,4,'','LT',0,'C',true);
			$this->Cell(90,4,'','LT',0,'C',true);
			$this->Cell(20,4,'','RLT',1,'C',true);
			
						
			$this->Cell(17,4,'DE PAGO','LB',0,'C',true);
			$this->Cell(73,4,'BENEFICIARIO','LB',0,'C',true);
			$this->Cell(90,4,'CONCEPTO','LB',0,'C',true);
		...
		function Footer(){global $fin;   global $prev_y; $ffechar=date("d-m-Y");$fhorar=date("H:i:s a"); 		    
			...
			$this->Line(27,$prev_y,27,$y);
			$this->Line(100,$prev_y,100,$y);
   	  ...
   	  $pdf->AddPage();
		  ...
		  while($registro=pg_fetch_array($res)){  $cod_presup=$registro["cod_presup"]; $denominacion=$registro["denominacion"]; 
		    ...
		    if(($prev_cod_presup<>$cod_presup_grupo)and($subt_cod=="S")){ $pdf->SetFont('Arial','B',7);
			   ...
			   }	$temp_den=substr($denominacion_grupo,0,57);
               $pdf->Cell(17,3,"",0,0,'C'); 			   
			   $pdf->Cell(73,5,$cod_presup_grupo,0,0,'L'); 
			   $pdf->Cell(90,5,$temp_den,0,1,'L'); 
			   $i=$i+1; 
		   ...
		   if($y>=253){ $nombre=substr($nombre,0,45);}
		   $pdf->SetFont('Arial','',7); 
		   $pdf->Cell(17,3,$referencia_caus,0,0,'C'); 
		   $x=$pdf->GetX();   $y=$pdf->GetY(); $n=73; $w=90;
		  ...
		  $pdf->Line(27,$prev_y,27,$y);
		  $pdf->Line(100,$prev_y,100,$y);
	...
	?>
       <table border="1" cellspacing='0' cellpadding='0' align="left">
		...
		<tr height="20">
         ...
         <tr height="20">
           <td width="100" align="left" bgcolor="#99CCFF"><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong>Nro Orden Pago</strong></td>
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_presup=$registro["cod_presup"]; $denominacion=$registro["denominacion"];  $cod_presup_grupo=$cod_presup; $denominacion_grupo=$denominacion;
			  if(($prev_cod_presup<>$cod_presup_grupo)and($subt_cod=="S")){
			   ...
			   $monto=formato_monto($monto);	  $nombre=conv_cadenas($nombre,0);  $descripcion_doc=conv_cadenas($descripcion_doc,0); 
			    ?>	   
        		<tr>
           			<td width="100" align="left"><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033">'<? echo $referencia_caus; ?></td>
