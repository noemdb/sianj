...
if ($_GET){$cod_presup_d=$_GET["cod_presupd"];$cod_presup_h=$_GET["cod_presuph"];$cod_fuente_d=$_GET["cod_fuented"];$cod_fuente_h=$_GET["cod_fuenteh"];$mostrar_diferido=$_GET["mostrar_diferido"];}
...
if (pg_ErrorMessage($conn)){$error=1;?> <script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script> <? } else { $Nom_Emp=busca_conf(); }
  ...
  $criterio1="(cod_presup>='$cod_presup_d' and cod_presup<='$cod_presup_h') and (cod_fuente>='$cod_fuente_d' and cod_fuente<='$cod_fuente_h') and  PRE020.Disp_Diferida <>'0'";
  $criterio2="(cod_presup>='$cod_presup_d' and cod_presup<='$cod_presup_h') and (cod_fuente>='$cod_fuente_d' and cod_fuente<='$cod_fuente_h') and  PRE020.Disp_Diferida = '0'";
  $per_hasta=0;
...
if ($mostrar_diferido=='N')
{
        $sSQL = "SELECT PRE020.Cod_Presup, PRE020.Denominacion, PRE020.Disponible, PRE020.Diferido, PRE020.Disp_Diferida 
                 FROM PRE020 WHERE ".$criterio1."
                 ORDER BY PRE020.Cod_Presup";
        $oRpt = new PHPReportMaker();
        $oRpt->setXML("Rpt_Disponibilidad_Diferida.xml");
        $oRpt->setUser("$user");
