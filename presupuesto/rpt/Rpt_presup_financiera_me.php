...
if ($_GET){$cod_presup_d=$_GET["cod_presupd"];$cod_presup_h=$_GET["cod_presuph"];$cod_fuente_d=$_GET["cod_fuented"];$cod_fuente_h=$_GET["cod_fuenteh"];$mes=$_GET["mes"];$asig_global=$_GET["asig_global"];$tipo_deuda=$_GET["tipo_deuda"];}
...
if (pg_ErrorMessage($conn)){$error=1;?> <script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script> <? } else { $Nom_Emp=busca_conf(); }
  ...
  $criterio1="Desde el Mes de :".$mesd."        "."Hasta : ".$mesh; $criterio2=""; 
...
if ($tipo_deuda=='CA')
{  		
    $sSQL = "SELECT PRE020.Cod_Presup, PRE020.Denominacion, PRE020.Asignado, PRE020.Traslados, PRE020.Trasladon, PRE020.Adicion, 
             ...
             (PRE020.Asignado-PRE020.Trasladon-PRE020.Traslados+PRE020.Adicion-PRE020.Disminucion-PRE020.Compromiso) AS Disponibilidad,
	     (PRE020.Causado-PRE020.Pagado) AS Deuda
             FROM PRE020 WHERE ".$criterio."
             ORDER BY PRE020.Cod_Presup";

        $oRpt = new PHPReportMaker();
        $oRpt->setXML("Rpt_presup_financiera_mes.xml");
        $oRpt->setUser("$user");
...
{ 
        $sSQL = "SELECT PRE020.Cod_Presup, PRE020.Denominacion, PRE020.Asignado, PRE020.Traslados, PRE020.Trasladon, PRE020.Adicion, 
             ...
             (PRE020.Asignado+PRE020.Trasladon-PRE020.Traslados+PRE020.Adicion-PRE020.Disminucion) AS Asig_Actualizada,
             (PRE020.Asignado-PRE020.Trasladon-PRE020.Traslados+PRE020.Adicion-PRE020.Disminucion-PRE020.Compromiso) AS Disponibilidad,
	     (PRE020.Compromiso-PRE020.Pagado) AS Deuda
             FROM PRE020 WHERE ".$criterio." 
             ORDER BY PRE020.Cod_Presup";
