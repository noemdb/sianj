...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if($error==0){ $sSQL="Select * from pre016 WHERE tipo_comp='$tipo_comp'";  $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
    if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO TIPO DE COMPROMISO YA EXISTE'); </script> <? }
     else{ $error=1;  $sSQL="SELECT ACTUALIZA_PRE016(1,'$tipo_comp','$des_tipo_comp','$cod_contable','$cod_part_iva','$func_inv_tpcomp','$c_imp_unico')";
...
pg_close();if ($error==0){?><script language="JavaScript">document.location ='Act_tipo_compromiso.php';</script> <? }else {?>  <script language="JavaScript">history.back();</script> <? }?>