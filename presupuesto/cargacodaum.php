...
if($aprueba_comp=="S"){$sql="SELECT * FROM codigos_compromisos where (tipo_compromiso<>'0000') and (tipo_compromiso<>'A000') and (referencia_comp='$referencia_comp' and tipo_compromiso='$tipo_compromiso') and (text(referencia_comp)||text(tipo_compromiso) not in ( select text(referencia_comp)||text(tipo_compromiso) from pre006 where ((anulado='S') or (aprobado='N')) )) order by cod_presup"; }
$res=pg_query($sql); 
while($registro=pg_fetch_array($res)){ $monto=$registro["monto"]-$registro["causado"]-$registro["ajustado"]; $monto=round($monto,2);
