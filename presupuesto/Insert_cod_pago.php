...
$fuente_financ=$_POST["txtcod_fuente"]; $monto_c=$_POST["txtmonto"]; $tipo_imput_presu=$_POST["txttipo_imput_presu"];
$ref_imput_presu=$_POST["txtref_imput_presu"];$monto_credito=$_POST["txtmonto_credito"];  $cod_contable=$_POST["txtcod_contable"];
$tipo_imput_presu=substr($tipo_imput_presu,0,1);$fecha=asigna_fecha_hoy(); $monto_c=formato_numero($monto_c); $sfecha=formato_aaaammdd($fecha);
...
echo "ESPERE POR FAVOR INCLUYENDO....","<br>"; $url="Det_inc_pagos.php?codigo_mov=".$codigo_mov;
...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO PRESUPUESTARIO YA EXISTE EN EL PAGO');</script><? }
   else{
