...
$refierea=$_POST["txtrefiereA"];$equipo = getenv("COMPUTERNAME");$minf_usuario = $usuario_sia." ".$equipo." ".date("d/m/y H:i a");
...
if ($error==0){ $sfecha=formato_aaaammdd($fecha_ajuste);  $rfecha=$sfecha;
  ...
  if($error==0){
    $sSQL="Select * from pre005 WHERE tipo_ajuste='$tipo_ajuste'";   $resultado=pg_exec($conn,$sSQL);   $filas=pg_numrows($resultado);
    if ($filas==0){$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE AJUSTE NO EXISTE');</script><?}
        ...
        if($refierea==$registro["refierea"]){$error;} else{$error=1;?><script language="JavaScript">muestra('DOCUMENTO DE AJUSTE NO VALIDO');</script><?}
      }}
  ...
  if($error==0){
    ...
    if($refierea=="COMPROMISO"){
      ...
      if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE COMPROMISO NO EXISTE');</script><?}
         ...
         $rfecha=$registro["fecha_compromiso"];    $anulado=$registro["anulado"];
         if($anulado=='S'){$error=1;?><script language="JavaScript">muestra('COMPROMISO ESTA ANULADO');</script><?}
       }
    }
    if($refierea=="CAUSADO"){
      $sSQL="Select * from pre007 WHERE referencia_caus='$referencia_caus' and tipo_causado='$tipo_causado' and tipo_compromiso='$tipo_compromiso' and referencia_comp='$referencia_comp' and tipo_causado<>'0000'";
      $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
      if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE CAUSADO NO EXISTE');</script><?}
         else{
           $registro=pg_fetch_array($resultado);
           $rfecha=$registro["fecha_causado"];   $anulado=$registro["anulado"];
           if($anulado=='S'){$error=1;?><script language="JavaScript">muestra('CAUSADO ESTA ANULADO');</script><?}
         }
    }
    if($refierea=="PAGO"){
      $sSQL="Select * from pre008 WHERE tipo_pago='$tipo_pago' and referencia_pago='$referencia_pago' and referencia_caus='$referencia_caus' and tipo_causado='$tipo_causado' and tipo_compromiso='$tipo_compromiso' and referencia_comp='$referencia_comp' and tipo_pago<>'0000'";
      $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
      if ($filas==0){$error=1;?><script language="JavaScript">muestra('REFERENCIA DE PAGO NO EXISTE');</script><?}
         else{
           $registro=pg_fetch_array($resultado);
           $rfecha=$registro["fecha_pago"];    $anulado=$registro["anulado"];
  ...
  if($error==0){
    ...
    $total=0;
    $sfecha=formato_aaaammdd($fecha_ajuste);
    while(($registro=pg_fetch_array($res))and($error==0)){
      ...
      if($refierea=="COMPROMISO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<? echo $registro["cod_presup"]; ?> FUENTE:<? echo $registro["fuente_financ"]; ?> REF.CREDITO:<? echo $registro["ref_imput_presu"]; ?>');</script><?}
           ...
           if (verifica_disponibilidad($conn,$cod_presup,$fuente_financ,$fecha_ajuste,$monto_c)==0){$error=0;}else{$error=1;}
         }
      }
      if($refierea=="CAUSADO"){
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL CAUSADO EL CODIGO PRESUPUESTARIO:<? echo $registro["cod_presup"]; ?> FUENTE:<? echo $registro["fuente_financ"]; ?> REF.CREDITO:<? echo $registro["ref_imput_presu"]; ?>');</script><?}
           ...
           $registro=pg_fetch_array($resultado);
           $compromiso=$registro["monto"]-$registro["pagado"]-$registro["ajustado"];
           if($monto_c>$compromiso){$error=1; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CODIGO DEL CAUSADO POR AJUSTAR');</script><? }
         }
      }
      if($refierea=="PAGO"){
        ...
        $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL CAUSADO EL CODIGO PRESUPUESTARIO:<? echo $registro["cod_presup"]; ?> FUENTE:<? echo $registro["fuente_financ"]; ?> REF.CREDITO:<? echo $registro["ref_imput_presu"]; ?>');</script><?}
         else{  $registro=pg_fetch_array($resultado);    $compromiso=$registro["monto"]-$registro["ajustado"];
           if($monto_c>$compromiso){$error=1; ?> <script language="JavaScript"> muestra('MONTO A PAGAR MAYOR AL MONTO DEL CODIGO DEL PAGO POR AJUSTAR');</script><? }
         }
      }
    }
  ...
  if($error==0){
     $sfecha=formato_aaaammdd($fecha_ajuste);
     $sSQL="SELECT INCLUYE_PRE011('$codigo_mov','$referencia_ajuste','$tipo_ajuste','$referencia_pago','$tipo_pago','$referencia_caus','$tipo_causado','$referencia_comp','$tipo_compromiso','$sfecha','A','P','N','','','$minf_usuario','$descripcion_ajuste','N','$tipo_ajuste_p','$tipo_ajuste_a','$tipo_ajuste_c','S')";
