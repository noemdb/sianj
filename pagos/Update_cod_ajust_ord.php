...
$monto_c=$_POST["txtmonto"];$comprometido=$_POST["txtsaldo_causado"];
...
$cod_contable=$_POST["txtcod_contable"];$referencia_comp=$_POST["txtreferencia_comp"];    $tipo_compromiso=$_POST["txttipo_compromiso"];
$tipo_imput_presu=substr($tipo_imput_presu,0,1);$fecha=asigna_fecha_hoy();
...
$comprometido=formato_numero($comprometido); if(is_numeric($comprometido)){$comprometido=$comprometido;} else{$comprometido=0;}
$equipo = getenv("COMPUTERNAME");$MInf_Usuario = $equipo." ".date("d/m/y H:i a");echo "ESPERE POR FAVOR MODIFICANDO....","<br>";
$url="Det_inc_ajustes_orden.php?codigo_mov=".$codigo_mov;
...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO PRESUPUESTARIO NO EXISTE EN EL AJUSTE');</script><? }
    ...
    $referencia_caus=$registro["referencia_caus"]; $tipo_causado=$registro["tipo_causado"];  $referencia_comp=$registro["referencia_comp"]; $tipo_compromiso=$registro["tipo_compromiso"];
	$cod_contable=$registro["cod_con_g_pagar"]; $operacion=$registro["operacion"];
    ...
    if($error==0){
      ...
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO NO EXISTE EN EL CAUSADO');</script><? }
         ...
         if($monto_c>$compromiso){$error=1; ?> <script language="JavaScript"> muestra('MONTO A AJUSTAR MAYOR AL SALDO DEL CAUSADO');</script><? }
       }
    }
    if($error==0){
      $sfecha=formato_aaaammdd($fecha);
      //$resultado=pg_exec($conn,"SELECT MODIFICA_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$referencia_caus','$tipo_causado','','0000','','0000','','','','$cod_contable','','','$sfecha','C','$tipo_imput_presu','$ref_imput_presu','$sfecha',$monto,$compromiso,$monto_credito,$credito)");
      $resultado=pg_exec($conn,"SELECT MOD_COD_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$referencia_caus','$tipo_causado','','0000','','0000','$operacion','','','$cod_contable','','','$sfecha','C','$tipo_imput_presu','$ref_imput_presu','$sfecha',$monto,$compromiso,$monto_credito,$credito)");
