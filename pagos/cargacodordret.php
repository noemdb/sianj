...
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname.""); $mcod_part_iva="403-18-01";  $l=strlen($mcod_part_iva);
...
$StrSQL="select * from pre026 where (codigo_mov='$codigo_mov') and (monto>0) and (substring(cod_presup,1,$l)<>'$mcod_part_iva') order by tipo_compromiso,referencia_comp,cod_presup,fuente_financ";  $res=pg_query($StrSQL);
