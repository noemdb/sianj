...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
   ...
   if($tipo_rpt=="PDF"){	
	  ...
	  $pdf->AliasNbPages();
   	  $pdf->AddPage();
	  $pdf->SetFont('Arial','',7);
	  $i=0;  $total=0; $totaln=0; $totalr=0; $cant_ret=0;
	  $res=pg_query($sSQL);
	  while($registro=pg_fetch_array($res)){ $i=$i+1;
		...
		$pdf->Cell(16,2,'==========',0,0,'R');
		$pdf->Cell(15,2,'==========',0,1,'R');		
		$pdf->Cell(60,5,'Cantidad : '.$cant_ret,0,0,'L');		
		$pdf->Cell(51,5,'Total : ',0,0,'R');
		$pdf->Cell(20,5,$totald,0,0,'R'); 
		$pdf->Cell(18,5,$totale,0,0,'R'); 
		$pdf->Cell(20,5,$totalb,0,0,'R'); 
		$pdf->Cell(16,5,$totali,0,0,'R'); 
	...
	?>
     ...
     <?	  
	  $i=0;  $total=0; $totaln=0; $totalr=0; $cant_ret=0; $res=pg_query($sSQL);
	  while($registro=pg_fetch_array($res)){ $i=$i+1;
           ...
           $monto_iva=formato_monto ($monto_iva);$monto_iva_retenido=formato_monto($monto_iva_retenido); 
		   $totald=$totald+$registro["monto_documento"]; $totale=$totale+$registro["monto_exento_iva"]; $totalb=$totalb+$registro["base_imponible"];  $totali=$totali+$registro["monto_iva"];
            $totalir=$totalir+$registro["monto_iva_retenido"]; $nombre=conv_cadenas($nombre,0);  $cant_ret=$cant_ret+1;
	?>	   
        <tr>
