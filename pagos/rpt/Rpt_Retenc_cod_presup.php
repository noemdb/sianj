...
$sformula="";
   ...
   if ($status_orden=='N'){$criterio2="PENDIENTE"; 
	  ...
	  $sformula=" and  (substring(pag001.tipo_causado,1,1)<>'A') and ";
	  $sformula=$sformula."( ((pag004.status_r='N') and (pag001.anulado='N') ) or ((pag004.status_r='I') And (pag004.fecha_cheque_r>'".$fecha_hasta."')))";
	}
...
function buscar_control($clave, $formato){  global $mcontrol;  $j=0;
  ...
  return $actual;
   ...
   if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
	 ...
	 /*
     $criterio = $criterio . " And  ( ((pag004.tipo_pago_r='000') and (pag004.status_r<>'L')) OR ((pag004.status_r='L') and (pag004.fecha_Cheque_r>'".$fecha_hasta."')) OR (pag004.tipo_pago_r='O/P' AND pag004.nro_cheque_r IN (SELECT a.nro_orden FROM pag001 a WHERE ( (a.fecha_Cheque>'".$fecha_hasta."') or (a.status='N')    )) ) OR ((pag004.tipo_pago_r='CHQ') and (pag004.fecha_cheque_r>'".$fecha_hasta."') )   )";
     ...
     $criterio = $criterio . " And ((pag001.anulado='N') or ((pag001.anulado='S') and (pag001.fecha_anulado>'".$fecha_hasta."')) )";
	*/
   ...
   if($tipo_rpt=="PDF"){ $res=pg_query($sSQL); $ced_rif_r_grupo=""; $nombre_grupo="";	
      ...
      class PDF extends FPDF{ 	
		function Header(){ global $criterio1; global $ced_rif_r_grupo; global $nombre_grupo; global $registro; global $tam_logo;
			...
			$this->Cell(50);
			$this->Cell(150,10,'ORDENES RETENCIONES/CODIGOS PRESUPUESTARIOS',1,0,'C');
			$this->Ln(10);
			$this->SetFont('Arial','B',8);
			$this->Ln(3);
			$this->Cell(50);
			$this->Cell(150,5,$criterio1,0,1,'C');				
			$this->Ln(3);
			$this->SetFont('Arial','B',8);
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; 
		   ...
		   $st_orden=muestra_st_orden($status_r,$anulado,$fecha_anulado,$fecha_cheque); 
		   ...
		   $pdf->Cell(13,3,$aux_orden,0,0); 	
           $pdf->Cell(15,3,$fecha,0,0);	 	  
		   $x=$pdf->GetX();   $y=$pdf->GetY();  $w=98;		   
		   $pdf->SetXY($x+$w,$y);	
		   $pdf->Cell(2,3,'',0,0);
           $pdf->Cell(40,3,$cod_presup_ret."  ".$fuente_fin_ret,0,0,'C');		   
		   $pdf->Cell(18,3,$monto_retencion,0,0,'R');
		   $pdf->Cell(14,3,$st_orden,0,1);
		   $pdf->SetXY($x,$y);	
		...
		$pdf->Cell(126,3,'TOTAL GENERAL : ',0,0,'R');
		$pdf->Cell(20,3,$total,0,1,'R'); 
		$pdf->Output();   
    }
    if($tipo_rpt=="EXCEL"){	
	 header("Content-type: application/vnd.ms-excel");
     header("Content-Disposition: attachment; filename=Retenciones_cod_presupuestario.xls");	
	...
	?>
       <table border="0" cellspacing='0' cellpadding='0' align="left">
	     <tr height="20">
            ...
            <td width="400" align="center" > <font size="4" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong>ORDENES RETENCIONES/CODIGOS PRESUPUESTARIOS</strong></font></td>
	     </tr>
	     <tr height="20">
			<td width="80" align="left" ><strong></strong></td>
			<td width="100" align="left" ><strong></strong></td>
         ...
         <tr height="20">
           ...
           <td width="250" align="center" bgcolor="#99CCFF" ><strong>CODIGO PRESUPUESTARIO</strong></td>
