...
if ($registro=pg_fetch_array($res)){  
  $orden=$registro["nro_orden"];  $aux_orden=$registro["aux_orden"]; $tipo_retencion=$registro["tipo_retencion"];  $planilla=$registro["tipo_planilla"]; $nro_planilla=$registro["nro_planilla"]; $descripcion=$registro["descripcion"];
  $total_r=formato_monto($registro["monto_retencion"]); $monto_o=formato_monto($registro["monto_objeto"]); $tasa=$registro["tasa"]; $temp_totalr=$registro["monto_retencion"];
...
$busca_comp=0;$total_d=0;  $total_s=0;  $total_b=0;  $total_iva=0;  $total_ret=0; $total_ret_fact=0; $aplica_sust=1; $cant_fact=0;
if($tipo_planilla=="01"){$sql="SELECT * FROM PAG016  where nro_orden='$orden' order by fecha_factura,nro_factura";}	else {$sql="SELECT * FROM PAG016  where nro_orden='$orden'  and status_2='N' order by fecha_factura,nro_factura";}	$res=pg_query($sql);
while(($registro=pg_fetch_array($res))and($error==0)) { $monto=$registro["monto_factura"]; $pmonto=$monto;      $montos=$registro["monto_sin_iva"]; $pmontos=$montos; $tasa_iva=$registro["tasa_iva1"];  	$montoo=$registro["monto_iva1_so"];  $montob=$registro["monto_iva4_so"]; 
	...
	$montor=($montob*$tasa)/100; $pmontob=$montob; 
	if($aplica_sust==0){$montor=$montor-$sustraendo; $aplica_sust=1;} 	$pmontor=$montor;    $cant_fact=$cant_fact+1;     	
	$total_d=$total_d+$pmonto; $total_s=$total_s+$pmontos; $total_b=$total_b+$pmontob;  $total_ret=$pmontor+$total_ret;
}
if ($temp_totalr>$total_ret){$diferencia=$temp_totalr-$total_ret;}else{$diferencia=$total_ret-$temp_totalr;}
if (($diferencia>0.001)and($cant_fact>1)){ $busca_comp=1;}
...
class PDF extends FPDF{ 
	function Header(){global $nro_comp; global $fecha; global $nombre_emp; global $ced_rif_emp; global $ano_fiscal; global $mes_fiscal; global $direccion; global $nombre_planilla;
	   global $nombre_benef; global $ced_rif; global $unidad_sol; global $php_os;	global $fechae;	
		...
		$this->Cell(2,3,'',0,0,'L');
		$this->Cell(18,3,substr($fechae,6,4),'LR',1,'C');		
		$this->Cell(15);
		$this->SetFont('Arial','B',2);
		$this->Cell(100,1,'  ',0,0,'L');
		$this->Cell(35,1,'',0,0,'L');
  ...
  $pdf->SetAutoPageBreak(true, 45);  
	...
	while(($registro=pg_fetch_array($res))and($error==0)) {
	    $ref_compromiso=$registro["ref_compromiso"]; $tipo_compromiso=$registro["tipo_compromiso"];
		...
		$pmontor=$montor; $monto_r=formato_monto($montor);  $nro_cred=""; $nro_afecta=""; $mostrar=0;
		if($monto<0){$nro_cred=$nro_fact; $nro_fact=""; $nro_afecta=$registro["campo_str2"];}
		$morden=$orden; if(substr($morden,0,1)=="P"){$morden=""; $busca_comp=0;}
		if($busca_comp==1){ $tsql="select * from pag004 where nro_orden_ret='$orden' and tipo_retencion='$tipo_retencion' and ref_comp_ret='$ref_compromiso' and tipo_comp_ret='$tipo_compromiso'"; $tres=pg_query($tsql);		  
		  if ($treg=pg_fetch_array($tres)){  $tmonto_retencion=$treg["monto_retencion"]; $mostrar=0; } else{$mostrar=1;} }		  
		if($mostrar==0){
