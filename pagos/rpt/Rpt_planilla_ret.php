...
if($tipo_planilla=="06"){$nombre_planilla="ACTIVIDADES ECONOMICAS"; }
if($tipo_planilla=="05"){$nombre_planilla="COMPROBANTE DE RESPONSABILIDAD SOCIAL"; }
...
class PDF extends FPDF{ 
  ...
  $pdf->SetAutoPageBreak(true, 45);  
	...
	while(($registro=pg_fetch_array($res))and($error==0)) {
		...
		if(($tipo_planilla=="01")or($tipo_planilla=="05")){$montob=$montob;} else {$montob=$montoo;}
	...
	//$pdf->Cell(20,4,$registro["nro_operacion"],0,0,'C');
...
//	  } else { $total_d=$monto_o;  $total_s=0;  $total_b=$monto_o;  $total_iva=0;  $total_ret=$monto_r; $prev_ret=$total_ret;  $nro_op=1; }
	if($sustraendo>0){ 
	  $total_sut=formato_monto($sustraendo);  
	  $pdf->Cell(132,4,'',0,0,'C');
	  $pdf->Cell(28,4,'Menos Sustraendo ..',0,0,'C');
	  $pdf->Cell(12,4,'',0,0,'C');
	  $pdf->Cell(28,4,$total_sut,'T',1,'R');
	}
