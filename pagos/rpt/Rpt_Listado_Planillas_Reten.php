...
if ($registro=pg_fetch_array($res,0)){$criterio2=$registro["descripcion"];} }
        $sSQL = "SELECT ban012.Nro_Planilla, ban012.Fecha_Emision, ban012.Tipo_Planilla,ban012.Cod_Banco, ban012.Tipo_Mov, ban012.Referencia,
                ...
                ban012.Nro_Planilla>='".$numero_planilla_d."' AND ban012.Nro_Planilla<='".$numero_planilla_h."' AND
                ban012.Fecha_Emision>='".$fecha_desde."' AND ban012.Fecha_Emision<='".$fecha_hasta."' AND
                ban012.Tasa>='".$tasa_d."' AND ban012.Tasa<='".$tasa_h."' AND
   ...
   if($tipo_rpt=="PDF"){	
      ...
      class PDF extends FPDF{
		function Header(){global $criterio1; global $criterio2;  global $tam_logo;
	  ...
	  $pdf->AliasNbPages();
   	  $pdf->AddPage();
	  $pdf->SetFont('Arial','',7);
	  $i=0;  $totalp=0; $totalo=0; $totalr=0; $cant_ret=0;
	  $res=pg_query($sSQL);
