...
$conn = pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname.""); $php_os=PHP_OS;        
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
 ...
 function muestra_st_orden($mstatus_ord,$manu,$mfecha_anu,$mfecha_chq){global $status_orden; global $fecha_hasta;
   ...
   $sSQL = "SELECT pag001.ced_rif, pre099.nombre, pag001.fecha, pag001.nro_orden, pag001.concepto, pag001.tipo_documento, pag001.nro_documento,
         (pag001.total_causado-pag001.total_retencion-pag001.total_ajuste-pag001.monto_am_ant+pag001.total_pasivos) as monto_orden, pag001.status, pag001.anulado, pag001.fecha_anulado, pag001.fecha_cheque, to_char(fecha,'DD/MM/YYYY') as fechae, 
         pag001.cod_banco, pag001.nro_cheque, pag001.tipo_pago, to_char(pag001.fecha_cheque,'DD/MM/YYYY') as fechachq
	...
	$nom_rpt="Rpt_Orden_Pago_Beneficiario.xml"; 
   if (($status_orden=='I')and($detallado=="S")){ $nom_rpt="Rpt_Orden_Pago_Beneficiario_canc.xml"; }
   ...
   if($tipo_rpt=="HTML"){	include ("../../class/phpreports/PHPReportMaker.php");  
          ...
          $oRpt->setParameters(array("criterio1"=>$criterio1,"criterio2"=>$criterio2,"date"=>$date,"hora"=>$hora,"detallado"=>$detallado));
          $oRpt->run();
   }
   if($tipo_rpt=="PDF"){ $res=pg_query($sSQL); $ced_rif_grupo=""; $nombre_grupo="";	
      require('../../class/fpdf/fpdf.php');
      class PDF extends FPDF{ 	
		function Header(){ global $criterio1; global $ced_rif_grupo; global $nombre_grupo; global $registro; global $tam_logo;
	  ...
	  $pdf->AliasNbPages();
   	  $pdf->AddPage();
	  $pdf->SetFont('Arial','',7);
	  $i=0;  $total=0; $sub_total=""; $cantidad=0; $prev_ced_rif=""; $cb=0;
	  $res=pg_query($sSQL);
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $ced_rif=$registro["ced_rif"];  if($php_os=="WINNT"){$nombre=$registro["nombre"];}else{$nombre=utf8_decode($registro["nombre"]); }
		   ...
		   if($prev_ced_rif<>$ced_rif_grupo){ 
			 ...
			 $pdf->Cell(35,3,"CEDULA/RIF: ".$ced_rif_grupo,0,0,'L');
			 $pdf->Cell(155,3,"NOMBRE: ".$nombre_grupo,0,1,'L');
			 $pdf->SetFont('Arial','',7);	
			 $prev_ced_rif=$ced_rif_grupo; $sub_total=0; $cb=0;
		   }		   
		   $nro_orden=$registro["nro_orden"]; $fecha=$registro["fecha"]; $concepto=$registro["concepto"]; $tipo_documento=$registro["tipo_documento"]; $nro_documento=$registro["nro_documento"];
		   ...
		   $st_orden=muestra_st_orden($status,$anulado,$fecha_anulado,$fecha_cheque); $cod_banco=$registro["cod_banco"];  $nro_cheque=$registro["nro_cheque"]; $tipo_pago=$registro["tipo_pago"];
		   $total=$total+$monto_orden; $sub_total=$sub_total+$monto_orden; $cantidad=$cantidad+1; $cb=$cb+1;
		   $monto_orden=formato_monto($monto_orden); $fecha=formato_ddmmaaaa($fecha);  $fechachq=formato_ddmmaaaa($fecha_cheque);
		   ...
		   if($detallado=="S"){
		     $pdf->Cell(3,3,"",0,0);
			 $pdf->Cell(37,3,"Tipo Documento: ".$tipo_documento,0,0);
			 //$pdf->Cell(160,3,"Nro.".$nro_documento,0,1); 
			 $pdf->MultiCell(130,3,"Nro.".$nro_documento,0);
             if($status_orden=='I'){$pdf->Cell(3,3,"",0,0); $pdf->Cell(32,3,"CANCELADA CON : ".$tipo_pago." REFERENCIA: ".$nro_cheque." FECHA: ".$fechachq." BANCO: ".$cod_banco,0,1);}	
 		     $pdf->Ln(2);
		   }
