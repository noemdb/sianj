...
if (!(empty($fecha_h))){$ano1=substr($fecha_h,6,9);$mes1=substr($fecha_h,3,2);$dia1=substr($fecha_h,0,2);}else{$fecha_h='';} $fecha_hasta=$ano1.$mes1.$dia1;
   ...
   if($tipo_rpt=="PDF"){ $res=pg_query($sSQL); $ced_rif_grupo=""; $nombre_grupo=""; $tipo_planilla_grupo=""; $descripcion_grupo="";
      ...
      class PDF extends FPDF{ 	
		function Header(){ global $criterio1; global $tam_logo; global $ced_rif_grupo; global $nombre_grupo; global $tipo_planilla_grupo; global $descripcion_grupo; global $registro;
	  ...
	  $pdf->AliasNbPages();
   	  $pdf->AddPage();
	  $pdf->SetFont('Arial','',7);
	  $i=0;  $total=0; $sub_total=0; $totalo=0; $sub_totalo=0; $cantidad=0; $prev_ced_rif=""; $prev_tipo_planilla=""; $cant_ret=0;
	  $res=pg_query($sSQL);
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $ced_rif=$registro["ced_rif"];  $tipo_planilla=$registro["tipo_planilla"]; 
		   ...
		   if($prev_ced_rif<>$ced_rif_grupo){ 
			    ...
			    if($sub_total<>0){ $sub_total=formato_monto($sub_total); $sub_totalo=formato_monto($sub_totalo);					    
				    $pdf->Cell(150,2,'',0,0);
					$pdf->Cell(20,2,'-------------------',0,0,'R');
					$pdf->Cell(10,2,'',0,0,'R');   
					$pdf->Cell(20,2,'-------------------',0,1,'R');
					$pdf->Cell(150,5,"TOTAL BENEFICIARIO : ",0,0,'R'); 
					$pdf->Cell(20,5,$sub_totalo,0,0,'R'); 
					$pdf->Cell(10,5,'',0,0,'R'); 
					$pdf->Cell(20,5,$sub_total,0,1,'R'); 
					$pdf->Ln(10);}	
					$pdf->Cell(30,5,"Ced/Rif: ".$ced_rif_grupo,0,0,'L');
				    $pdf->Cell(170,5,"Nombre: ".$nombre_grupo,0,1,'L');
				$prev_ced_rif=$ced_rif_grupo; $sub_total=0; $sub_totalo=0; 
			}
		   ...
		   $totalo=$totalo+$monto_objeto; $sub_totalo=$sub_totalo+$monto_objeto;
		...
		$pdf->Cell(180,3,'',0,0);
		$pdf->Cell(20,3,'============',0,1,'R');
		$pdf->Cell(60,3,'Cantidad : '.$cant_ret,0,0,'L');
		$pdf->Cell(120,3,'TOTAL GENERAL : ',0,0,'R');
