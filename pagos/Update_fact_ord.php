...
$codigo_mov=$_POST["txtcodigo_mov"]; $ref_comp=$_POST["txtref_comp"]; $rif_fact=$_POST["txtrif_fact"];
...
$tasa_iva1=$_POST["txttasa_iva"]; $monto_factura=$_POST["txtmonto_factura"]; $campo_str1=$_POST["txtcampo_str1"];  $campo_str2=$_POST["txtcampo_str2"];
//$monto_factura=formato_numero($monto_factura);
...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('FACTURA NO EXISTE EN LA ORDEN');</script><? }
   else{ $tmonto_s=$monto_sin_iva; if($tmonto_s<0){ $tmonto_s=$tmonto_s*-1;}   $tmontof=$monto_factura; if($tmontof<0){ $tmontof=$tmontof*-1;}
	...
	if($error==0){$sfecha=formato_aaaammdd($fecha_factura); $monto_iva1=$monto_factura-$monto_sin_iva; $monto_iva1=cambia_coma_numero($monto_iva1);
      $resultado=pg_exec($conn,"SELECT ACTUALIZA_PAG029(2,'$codigo_mov','$nro_factura','$nro_con_factura','$ref_compromiso','$tipo_compromiso','$sfecha',$monto_sin_iva,$monto_iva1_so,$tasa_iva1,$monto_iva1,0,0,0,$monto_iva3_so,0,0,$monto_iva4_so,0,0,$monto_factura,'$rif_fact','N','N','$campo_str1','$campo_str2')");
