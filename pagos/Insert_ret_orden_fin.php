...
$monto_retencion=$_POST["txtmonto_retencion"];$ced_rif=$_POST["txtced_rif"];$nombre=$_POST["txtnombre"];$des_orden_ret=$_POST["txtdes_orden_ret"];
$tipo_compromiso="0000";$referencia_comp="";$fuente_financ="00";$cod_presup="";$fecha=asigna_fecha_hoy();
...
$url="Det_inc_ret_ord_fin.php?codigo_mov=".$codigo_mov;
...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('RETENCION YA EXISTE EN LA ORDEN');</script><? }
    ...
    if($error==0){$sSQL="Select * from PAG003 WHERE tipo_retencion='$tipo_retencion'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE RETENCION NO EXISTE');</script><? } else {$registro=pg_fetch_array($resultado); $cod_contable=$registro["cod_contable"];}
    }
