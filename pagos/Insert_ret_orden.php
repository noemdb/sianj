<?include ("../class/conect.php");  include ("../class/funciones.php"); $formato_presup="XX-XX-XX-XXX-XX-XX-XX"; $error=0;
...
$monto_retencion=$_POST["txtmonto_retencion"]; $cod_ret=$_POST["txtcod_ret"]; $ced_rif=$_POST["txtced_rif"];
$nombre=$_POST["txtnombre"]; $des_orden_ret=$_POST["txtdes_orden_ret"];
$tipo_compromiso=substr($cod_ret,0,4); $referencia_comp=substr($cod_ret,5,8); $fuente_financ=substr($cod_ret,14,2); 
$cod_presup=substr($cod_ret,17,32); $fecha=asigna_fecha_hoy();
...
if($error==0){ 
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('RETENCION YA EXISTE EN LA ORDEN');</script><? }
   else{ $cod_contable="";
    $sSQL="Select * from PRE026 WHERE codigo_mov='$codigo_mov' and referencia_comp='$referencia_comp' and tipo_compromiso='$tipo_compromiso' and cod_presup='$cod_presup' and fuente_financ='$fuente_financ' and monto>0 ";
    $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
    ...
    if($error==0){$sSQL="Select * from PRE099 WHERE ced_rif='$ced_rif'"; $resultado=pg_query($sSQL);   $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CEDULA/RIF DE BENEFICIARIO NO EXISTE');</script><? }else{$registro=pg_fetch_array($resultado); $nombre=$registro["nombre"];}
    }
    ...
    if($error==0){  $sfecha=formato_aaaammdd($fecha); $sSQL="SELECT ACTUALIZA_PAG028(1,'$codigo_mov','00000000','$tipo_retencion','$referencia_comp','$tipo_compromiso','$cod_presup','$fuente_financ','00000000','0000','$cod_contable',$tasa_retencion,$monto_objeto,$monto_retencion,0,'R','$ced_rif','$nombre','S','0000','0000','$des_orden_ret')";
...
pg_close(); if (($error==0)or($error==1)){?><script language="JavaScript"> document.location='<? echo $url; ?>'; </script> <?}
