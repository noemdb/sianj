...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
    ...
    if($error==0){
      ...
      if ($filas>0){$error=1; echo $sSQL; ?> <script language="JavaScript"> muestra('NÚMERO DE OPERACIÓN YA EXISTE');</script><? }
    }
    ...
    if($error==0){$sfechaf=formato_aaaammdd($fecha_factura);
      $sSQL="SELECT INCLUYE_BAN029('$codigo_mov','0000','O/P','$nro_orden','$tipo_retencion','$nro_planilla','$ced_rif','$sfechaf','$nro_orden','$nro_orden','C','$tipo_retencion','$tipo_documento','$nro_documento','$nro_con_factura','$nro_doc_afectado','$sfechaf','00000000','',$monto_pago,$monto_objeto,$tasa_retencion,$monto_retencion,$monto1,$monto2,$monto3)";
      echo $sSQL; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error="ERROR GRABANDO: ".substr($error,0,91);  if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? }
    }
