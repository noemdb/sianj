<?include ("../class/seguridad.inc"); include ("../class/conects.php");  include ("../class/funciones.php"); $cod_estructura=$_GET["txtcod_estructura"];$equipo = getenv("COMPUTERNAME");$error=0;
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
 ...
 if($error==0){$sSQL="Select * from PAG006 WHERE cod_estructura='$cod_estructura'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('CODIGO DE ESTRUCTURA NO EXISTE');  </script> <? }
   else{$registro=pg_fetch_array($resultado); $descripcion_est=$registro["descripcion_est"];
     if($error==0){$sSQL="SELECT ELIMINA_PAG006('$cod_estructura')";   $resultado=pg_exec($conn,$sSQL);$error=pg_errormessage($conn); $error=substr($error, 0, 61);
       if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?} else{?><script language="JavaScript">  muestra('ELIMINO EXITOSAMENTE'); </script><?}
         $desc_doc="ESTRUCTURA DE ORDEN, CODIGO:".$cod_estructura.", DESCRIPCION:".$descripcion_est; $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('01','$usuario_sia','$usuario_sia','$equipo','Elimino','$sfecha','$desc_doc')");
