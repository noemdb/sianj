<?include ("../class/funciones.php");
   ...
   if (!$conn) { echo "<p><b>Ocurrio un error conectando a la base de datos: .</b></p>"; exit; }
    $codigo_mov="00000002";  $cod_pre_ret="";
   ...
   $path="fijos.txt"; $fp = fopen($path,"r");
   while ($linea= fgets($fp,1024)) {
	  ...
	  if($unidad=="202551"){ $unidad="02-05-51";}
      ...
      if(($tipo=="1")and($error==0)and(substr($codigo,0,1)=="4")){ $monto=elimina_puntos($monto); $monto=cambia_coma_numero($monto);  $cod_partida=substr($codigo,0,3)."-".substr($codigo,3,2)."-".substr($codigo,5,2) ."-".substr($codigo,7,2);
      ...
      if($codigo=="211040100"){$cod_ret="001";} if($codigo=="211040300"){$cod_ret="002";} if($codigo=="211040400"){$cod_ret="003";}
      ...
      if($codigo=="112020200"){$cod_ret="020";} if($codigo=="211013539"){$cod_ret="022";}
      if($codigo=="211049902"){$cod_ret="000";   if(substr($des_concepto,0,11)=="RETENC.F.AH"){$cod_ret="007";}  
		...
		if(substr($des_concepto,0,12)=="CREDITO FERR"){$cod_ret="015";}		 
		...
		if(substr($des_concepto,0,12)=="RECREACIONAL"){$cod_ret="016";}		
		if(substr($des_concepto,0,15)=="CREDITO DE COMP"){$cod_ret="014";}		
		if(substr($des_concepto,0,15)=="PRESTAMO RECREA"){$cod_ret="016";}		
		if(substr($des_concepto,0,15)=="CORTO  ESPECIAL"){$cod_ret="010";}
      ...
      if($cod_ret=="000"){$error=1; if ($tipo=="2"){echo "no localizado ".$codigo,"<br>";} }
	  
      if((($tipo=="2")or(($tipo=="1")and($cod_ret<>"000")))and(($error==0)or($codigo=="401010100")or($codigo=="401070900")or($codigo=="401030400"))){ $monto=elimina_puntos($monto);
		...
		if(($codigo=="401010100")or($codigo=="401070900")or($codigo=="401030400")){  
		  $ssql="UPDATE pag009 SET monto_est=monto_est-$monto where cod_estructura='$codigo_mov' and ref_comp_est='$referencia_comp' and tipo_comp_est='$tipo_compromiso' and cod_presup_est='$cod_presup' and fuente_est='$fuente_financ'"; 
		  echo "Retencion al codigo ".$cod_presup,"<br>";
		}else{
