...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO EXISTE');</script><? }
	 ...
	 if ($registro["modulo"]=="C"){$error=0;}else{$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE NO PUEDE SER ELIMINADO');</script><?}
	 ...
	 if ($error==0){ $resultado=pg_exec($conn,"SELECT ELIMINA_CON002('$referencia','$sfecha','$tipo_Comp')");$error=pg_errormessage($conn); $error=substr($error,0,91);
