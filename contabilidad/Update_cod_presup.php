...
echo "ESPERE POR FAVOR MODIFICANDO...."; $MInf_Usuario=$equipo." ".date("d/m/y H:i a");
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script>  <?}
  ...
  if ($filas==0){?> <script language="JavaScript">  muestra('CODIGO PRESUPUESTARIO NO EXISTE'); </script> <? }
   else{  $resultado=pg_exec($conn,"SELECT ACTUALIZA_CON019(2,'$cod_presup','$cod_contab_asoc','','',0,0,'$MInf_Usuario')");     $error=pg_errormessage($conn);   $error=substr($error,0,91);
