...
<?php include ("../class/configura.inc"); error_reporting(E_ALL); $formato_presup="XX-XX-XX-XXX-XX-XX-XX";
$codigo_mov=$_POST["txtcodigo_mov"]; $referencia_caus="00000001"; $tipo_causado="0004";
$referencia_comp="00000001"; $tipo_compromiso="0000"; $fecha_causado=$_POST["txtFecha"]; $ced_rif="G-20009014-6";
$descripcion_caus="AMORTIZACIONES DEL ".$fecha_causado;$num_proyecto="0000000000"; $caus_directo="SI";
$func_inv="";if($func_inv==""){$func_inv="CORRIENTE";}$func_inv=substr($func_inv,0,1);
$genera_comprobante="SI";$genera_comprobante=substr($genera_comprobante,0,1);$unidad_sol="";
$equipo = getenv("COMPUTERNAME");$minf_usuario = $usuario_sia." ".$equipo." ".date("d/m/y H:i a");echo "ESPERE POR FAVOR INCLUYENDO....","<br>";
if (checkData($fecha_causado)=='1'){$error=0;}else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><? }
if ($error==0){  $sfecha=formato_aaaammdd($fecha_causado);  $rfecha=$sfecha;
  ...
  if($error==0){$sSQL="Select max(referencia_caus) as referencia from pre007 WHERE tipo_causado='$tipo_causado'";
        $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado); echo $sSQL;
    if($filas>0){$registro=pg_fetch_array($resultado); $ult_ref=$registro["referencia"]+1; $len=strlen($ult_ref); $ult_ref=substr("00000000",0,8-$len).$ult_ref; $referencia_caus=$ult_ref;}
  }
  if($error==0){
    if($caus_directo=="SI"){$referencia_comp=$referencia_caus;$tipo_compromiso="0000";}
     else{$sSQL="Select * from pre006 WHERE tipo_compromiso='$tipo_compromiso' and referencia_comp='$referencia_comp'";
  ...
  if($error==0){ $sSQL="Select * from pre007 WHERE referencia_caus='$referencia_caus' and tipo_causado='$tipo_causado' and tipo_compromiso='$tipo_compromiso' and referencia_comp='$referencia_comp'";   $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
  ...
  if($error==0){$sql="Select * from CODIGOS_PRE026 where codigo_mov='$codigo_mov' order by cod_presup";  $res=pg_query($sql);
    ...
    while(($registro=pg_fetch_array($res))and($error==0)){
      ...
      if($caus_directo=="SI"){
        if (verifica_disponibilidad($conn,$registro["cod_presup"],$registro["fuente_financ"],$sfecha,$monto_c)==0){$error=0;}
        ...
        if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('NO EXISTE EN EL COMPROMISO EL CODIGO PRESUPUESTARIO:<? echo $registro["cod_presup"]; ?> FUENTE:<? echo $registro["fuente_financ"]; ?> REF.CREDITO:<? echo $registro["ref_imput_presu"]; ?>');</script><?}
           ...
           if($monto_c>$compromiso){$error=1; ?> <script language="JavaScript"> muestra('MONTO A CAUSAR MAYOR AL MONTO DEL CÓDIGO POR COMPROMETER');</script><? }
         }
      }
    }
    if($total==0){$error=1;?><script language="JavaScript">muestra('MONTO TOTAL DEL CAUSADO INVALIDO');</script><?}
  }
  if(($error==0)and($genera_comprobante=='S')){$t_debe=0; $t_haber=0;$balance=0;
  ...
  if($error==0){ $sfecha=formato_aaaammdd($fecha_causado);
     $sSQL="SELECT INCLUYE_PRE007('$codigo_mov','$referencia_caus','$tipo_causado','$referencia_comp','$tipo_compromiso','$sfecha','$referencia_caus','$ced_rif','P','$func_inv','$num_proyecto','$genera_comprobante','','$minf_usuario','S','$unidad_sol','D','$descripcion_caus')";
...
if ($error==0){?><script language="JavaScript">document.location ='Gen_int_bienes.php';</script> <? }else {?>  <script language="JavaScript">history.back();</script> <? } ?>
