...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
 else{$sSQL="Select campo101 from SIA001 WHERE campo101='$login' and campo102='$claveA'"; $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
  ...
  if($error==0){ if($ClaveN<>$ClaveNR) {$error=1;?> <script language="JavaScript"> muestra('CLAVES NO SON IGUALES'); </script><?}
     else{$resultado=pg_exec($conn,"SELECT cambio_sia001('$login','$ClaveN')"); $error=pg_errormessage($conn); $error=substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?} else{?><script language="JavaScript">muestra('COMBIO CLAVE EXITOSAMENTE');</script><?}
  }}
