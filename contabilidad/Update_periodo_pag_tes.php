<?include ("../class/conect.php");  include ("../class/funciones.php"); $periodo=$_POST["txtperiodo"]; $periodotes=$_POST["txtperiodotes"];
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
   ...
   $resultado=pg_exec($conn,"UPDATE SIA005 set campo503='$periodotes' where campo501='02'"); $error=pg_errormessage($conn); $error=substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?} 
