<?include ("../class/conect.php");  include ("../class/funciones.php"); $periodo=$_POST["txtperiodo"]; $error=0;
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
 else{ $resultado=pg_exec($conn,"UPDATE SIA000 set campo033='$periodo'"); $error=pg_errormessage($conn); $error=substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?}   }
