...
$periodo=$_GET["periodo"]; $vimprimir=$_GET["vimprimir"]; $tipo_rpt=$_GET["tipo_rep"];
...
$criterio1="Desde ".$fecha_d." Al ".$fecha_h;  $Sql=""; $Cta_Resultado="7"; $date = date("d-m-Y"); $hora = date("H:i:s a");
...
$sql="Select * from SIA005 where campo501='06'"; $resultado=pg_query($sql);if ($registro=pg_fetch_array($resultado,0)){ $Cta_Resultado=$registro["campo509"]; }
$Sql="SELECT RPT_RES_DIARIO_G_CON013('".$usuario_sia."','6','".$periodo."','".$vimprimir."','".$Cta_Resultado."')";
$resultado=pg_exec($conn,$Sql);  $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error, 0, 61);
if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? }
   else{$Sql= "select * from RPT_RES_DIARIO_G WHERE nombre_usuario='".$usuario_sia."' AND tipo_registro='6' ORDER BY cod_cuenta"; $sSQL = $Sql;

   if($tipo_rpt=="HTML"){	 include ("../../class/phpreports/PHPReportMaker.php");
             $oRpt = new PHPReportMaker();
             $oRpt->setXML("Resumen_Diario_G.xml");
             $oRpt->setUser("$user");
    ...
    if($tipo_rpt=="PDF"){ 	
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; 
				...
				$this->Cell(50);
				$this->Cell(100,10,'RESUMEN DE DIARIO GENERAL',1,0,'C');
				$this->Ln(10);
    ...
    if($tipo_rpt=="EXCEL"){	
		  ...
		  header("Content-Disposition: attachment; filename=Resumen_Diario_General.xls");
		  ?>
		   <table border="0" cellspacing='0' cellpadding='0' align="left">
			 <tr height="20">
				<td width="100" align="left" ><strong></strong></td>
				<td width="400" align="center" > <font size="4" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong>RESUMEN DIARIO GENERAL</strong></font></td>
			 </tr>
			 <tr height="20">
				<td width="100" align="left" ><strong></strong></td>
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1; 
		       $cod_cuenta=$registro["cod_cuenta"]; $nombre_cuenta=$registro["nombre_cuenta"]; $columna1=$registro["columna1"]; $columna2=$registro["columna2"]; 	 
               $total_columna1=$total_columna1+$columna1; $total_columna2=$total_columna2+$columna2;$columna1=formato_monto($columna1); $columna2=formato_monto($columna2);   $nombre_cuenta=conv_cadenas($nombre_cuenta,0);
			   ?>	   
				<tr>
