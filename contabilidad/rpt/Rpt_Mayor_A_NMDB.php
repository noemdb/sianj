...
$ced_rif_d      = $_GET["ced_rif_d"];
$ced_rif_h      = $_GET["ced_rif_h"];
$cod_cuenta_d   = $_GET["cod_cuenta_d"];
$cod_cuenta_h   = $_GET["cod_cuenta_h"];
$tipo_asiento_d = $_GET["tipo_asiento_d"];
$tipo_asiento_h = $_GET["tipo_asiento_h"];
$ordenar        = $_GET["ordenar"];
$imprimir       = $_GET["imprimir"];
$tipo_rep       = $_GET["tipo_rep"];
$inc_benef      = $_GET["inc_benef"];
$criterio1      = "Desde " . $fecha_d . " Al " . $fecha_h;
if ($fecha_d == "") {
    $sfecha_d = "2010-01-01";
} else {
    $sfecha_d = formato_aaaammdd($fecha_d);
}
if ($fecha_h == "") {
    $sfecha_h = "9999-12-31";
} else {
    $sfecha_h = formato_aaaammdd($fecha_h);
}
...
$date        = date("d-m-Y");
$hora        = date("H:i:s a");
$nombre_rep  = "Mayor_Analitico.xml";
if ($tipo_rep == "HTMLL") {
    $nombre_rep = "Mayor_Analitico_Largo.xml";
    $tipo_rep   = "HTML";
}
if ($inc_benef == "SI") {
    $nombre_rep = "Mayor_Analitico_Benef.xml";
    if ($tipo_rep == "HTMLL") {
        $tipo_rep = "HTML";
    }
    if ($tipo_rep == "PDF") {
...
?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?
    ...
    $fecha_i = formato_ddmmaaaa($Fec_Ini_Ejer);
    if ($Cod_Emp == "70") {
        $col_ac_n = "S";
    }
    $Sql       = "SELECT ELIMINA_CON013('" . $usuario_sia . "','8')";
    $resultado = pg_exec($conn, $Sql);
    $error     = pg_errormessage($conn);
    $error     = "ERROR INICIALIZANDO: " . substr($error, 0, 91);
        ...
        if ($tipo_rep == "PDF") {
            ...
            while ($registro = pg_fetch_array($res)) {
                ...
                if ($php_os == "WINNT") {
                    ...
                    $nombre_cuenta = utf8_decode($nombre_cuenta);
                }
                if ($prev_cta <> $codigo_cuenta) {
                    $anterior = $registro["columna3"];
                    ...
                    if ($registro["columna10"] == 0) {
                        if (($registro["tsaldo"] == "Acreedor") and ($col_ac_n == "S")) {
                            $anterior = $registro["columna6"];
                        } else {
                            $anterior = $registro["columna3"];
                        }
                    } else {
                        if (($registro["tsaldo"] == "Acreedor") and ($col_ac_n == "S")) {
                            $anterior = $registro["columna5"];
                        } else {
                            $anterior = $registro["columna4"];
                        }
                    }
                    $pdf->SetFont('Arial', 'B', 7);
                    if (($sub_totald > 0) or ($sub_totalh > 0) or ($cant_mov > 0)) {
                        $sub_totald = formato_monto($sub_totald);
                        $sub_totalh = formato_monto($sub_totalh);
                        $pdf->Cell(140, 2, '', 0, 0);
                        $pdf->Cell(20, 2, '-------------------', 0, 0, 'R');
                        $pdf->Cell(20, 2, '-------------------', 0, 0, 'R');
                        $pdf->Cell(20, 2, '-------------------', 0, 1, 'R');
                        ...
                        $cta_enc      = $codigo_cuenta;
                        $nomb_cta_enc = $nombre_cuenta;
                        $pdf->AddPage();
                    } else {
                        $anterior     = formato_monto($anterior);
                        $cta_enc      = $codigo_cuenta;
                ...
                $debe         = $registro["columna1"];
                $haber        = $registro["columna2"];
                $saldo        = $registro["columna4"];
                //if($registro["tsaldo"]=="Acreedor"){ $saldo=$registro["columna5"]; }else{$saldo=$registro["columna4"]; }    
                if (($registro["tsaldo"] == "Acreedor") and ($col_ac_n == "S")) {
                    $saldo = $registro["columna5"];
                } else {
                    $saldo = $registro["columna4"];
                }
                if ($inc_benef == "SI") {
                    $descripcion = $registro["descripcion"] . "; BENEFICIARIO: " . $registro["nombre"];
                }
                $totald     = $totald + $debe;
                $totalh     = $totalh + $haber;
                $sub_totald = $sub_totald + $debe;
                $sub_totalh = $sub_totalh + $haber;
                $cant_mov   = $cant_mov + 1;
                $debe       = formato_monto($debe);
                $haber      = formato_monto($haber);
                ...
                if (($debe == 0) and ($haber == 0) and ($referencia == "00000000")) {
                    $referencia = "";
                    $fechaf     = "";
                }
                $pdf->Cell(15, 2, '', 0, 1);
