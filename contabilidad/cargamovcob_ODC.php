...
$res00 = odbc_exec($conex, "Select * from tabla where XAÑOAP and XMESAP=$nmes and XDIAAP=$ndia"); 

while (odbc_fetch_row($res00)){ 
  $ano=trim(odbc_result($res00,4)); $mes=trim(odbc_result($res00,5)); $mes=Rellenarcerosizq($mes,2); $dia=trim(odbc_result($res00,6)); $dia=Rellenarcerosizq($dia,2);
  if(($cdia==$dia)and($cmes==$mes)and($cano==$ano)){  $fecha_mov=$fecha;  
	$num_asiento=odbc_result($res00,7); $tipo_contab=odbc_result($res00,8); $num_linea=odbc_result($res00,9); $num_cuenta=odbc_result($res00,10); $tipo_mov=odbc_result($res00,11);
	$clave_con=odbc_result($res00,12); $des_referencia=odbc_result($res00,13); $referencia_doc=odbc_result($res00,14); $tipo_doc="DEP"; $status_conc=odbc_result($res00,15);
	$monto_mov=odbc_result($res00,16); $monto_mov=cambia_coma_numero($monto_mov); $monto_mov=$monto_mov*1; 
	$monto_mov_div=odbc_result($res00,17); $monto_mov_div=cambia_coma_numero($monto_mov_div); $monto_mov_div=$monto_mov_div*1; 
	$cod_div=odbc_result($res00,18); $monto_div=odbc_result($res00,19); $monto_div=cambia_coma_numero($monto_div); $monto_div=$monto_div*1; 
	$fecha_v=odbc_result($res00,20); $l=strlen($fecha_v); if($l==8){$fecha_vence=$fecha_v;}else{$fecha_vence=$fecha;}
	$cod_act=odbc_result($res00,21); $cod_destino=odbc_result($res00,22); $iva_mov =odbc_result($res00,23); $status_b=odbc_result($res00,24); $situacion=odbc_result($res00,25); 
	$usuario_graba=odbc_result($res00,26); $fecha_graba=odbc_result($res00,27); $hora_graba=odbc_result($res00,28); $fecha_gen_mov=$fecha;  
