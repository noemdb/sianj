...
$fecha=$_POST["txtfecha"];  $descripcion=$_POST["txtdescripcion"];  $monto=$_POST["txtmonto_mov_banco"]; $benef_mov_banco=$_POST["txtbenef_mov_banco"];  $tipodc="D";
$equipo = getenv("COMPUTERNAME"); $minf_usuario=$usuario_sia." ".$equipo." ".date("d/m/y H:i a"); echo "ESPERE POR FAVOR INCLUYENDO....","<br>"; $error=0;
$url="Act_Mov_Banco.php?Gcod_banco=C".$cod_banco.$referencia.$tipo_mov;   $monto=formato_numero($monto); if(is_numeric($monto)){$monto=$monto;} else{$monto=0;}
...
if($error==10){$nmes=substr($fecha,3, 2);  if($periodom<$SIA_Periodo){$periodom=$SIA_Periodo;}if($periodom>$nmes){$error=1;?><script language="JavaScript">muestra('FECHA DE MES MENOR A ULTIMO PERIODO CERRADO');</script><?}}
