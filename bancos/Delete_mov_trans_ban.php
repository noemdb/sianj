...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
 else{$Nom_Emp=busca_conf();  $error=0; $sSQL="SELECT * FROM BAN008 WHERE cod_banco='$cod_banco' and referencia='$referencia' and tipo_trans_banco='$tipo_mov'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
  if ($filas==0){$error=1; ?><script language="JavaScript">  muestra('MOVIMIENTO EN TRANSITO NO EXISTE');  </script><?}
   else{$registro=pg_fetch_array($resultado); $adescripcion=$registro["des_mov_trans_banco"]; $mes_conciliacion=$registro["mes_conciliacion"]; $fecha_mov=$registro["fecha_trans_banco"]; $anulado="N"; $fecha_anulado=$registro["fecha_trans_banco"]; $monto=$registro["monto_trans_banco"]; $beneficiario=$registro["bene_mov_trans_banco"]; $error=0; $tipo_chq="N";}
  ...
  if($error==0){$DOperacion="1";
      ...
      if($error==0){ $sfecha=$fecha_mov; $sfechaa=$fecha_anulado; $proceso=0;  echo $tipo_mova;
       $sSQL="SELECT ACTUALIZA_BAN008(3,'$cod_banco','$referencia','$tipo_mov','$sfecha',$monto,'00','$beneficiario','$minf_usuario','$adescripcion')";  $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $error=substr($error,0,91);
       if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?}else{?><script language="JavaScript">  muestra('ELIMINO EXITOSAMENTE'); </script><?
          $desc_doc="MOVIMIENTO TRANSITO EN BANCO, BANCO:".$cod_banco.", REFERENCIA:".$referencia.", TIPO MOVIMIENTO:".$tipo_mov.", DESCRIPCION:".$adescripcion;$resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('02','$usuario_sia','$usuario_sia','$equipo','Elimino','$sfecha','$desc_doc')");
