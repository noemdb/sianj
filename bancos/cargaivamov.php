...
$sql="SELECT nro_orden,ced_rif,fecha,total_causado,total_retencion,total_ajuste,total_pagado,tipo_documento,nro_documento FROM PAG001 Where (cod_banco='$cod_banco') and (nro_cheque='$referencia') and (tipo_pago='$tipo_mov')";
$resultado=pg_exec($conn,$sql);  $filas=pg_numrows($resultado); $tipo_r=0;
while($reg=pg_fetch_array($resultado)){
  $nro_orden=$reg["nro_orden"]; $ced_rif=$reg["ced_rif"]; $fecha_ord=$reg["fecha"]; $tipo_d=$reg["tipo_documento"]; $nro_fact=$reg["nro_documento"]; $neto_ord=$reg["total_causado"]-$reg["total_retencion"]-$reg["total_ajuste"];  $neto_ord=cambia_coma_numero($neto_ord);
  $sql="SELECT pag004.nro_orden_ret,pag004.tipo_retencion,pag004.aux_orden,pag004.tasa_retencion,sum(pag004.monto_objeto_ret) as monto_objeto,sum(pag004.monto_retencion) as monto_retencion From pag004,pag003 Where (pag003.ret_Grupo='A') and (pag003.tipo_retencion=pag004.tipo_retencion) and (nro_orden_ret='$nro_orden') group by pag004.nro_orden_ret,pag004.tipo_retencion,pag004.aux_orden,pag004.tasa_retencion order by pag004.nro_orden_ret,pag004.tipo_retencion";
  $res=pg_exec($conn,$sql);  $filas=pg_numrows($res);if ($filas>0){$registro=pg_fetch_array($res); $tasa_retencion=$registro["tasa_retencion"];} else{$tasa_retencion=0;}
  $sql="SELECT nro_factura,nro_con_factura,fecha_factura,monto_factura,monto_sin_iva,monto_iva1,tasa_iva1,monto_iva1_so FROM PAG016 Where (nro_orden='$nro_orden')";$res=pg_query($sql); 
  while($registro=pg_fetch_array($res))
  { $tipo_r=$tipo_r+1; $fecha_e=$fecha_ord; $nro_fact=$registro["nro_factura"]; $nro_d=elimina_ceros($nro_fact);  $nro_con_fac=$registro["nro_con_factura"]; $nro_con_fac=elimina_ceros($nro_con_fac); $fecha_f=$registro["fecha_factura"];
