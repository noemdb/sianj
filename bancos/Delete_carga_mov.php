...
$codigo_mov=$_GET["codigo_mov"]; $fechac=$_GET["fecha"]; $monto_d=$_GET["monto_d"]; $monto_h=$_GET["monto_h"]; $solop=$_GET["solop"];
  ...
  if($error==0){$DOperacion="1";
      ...
      if($error==0){ $sfecha=$fecha_mov; $sfechaa=$fecha_anulado; $proceso=0;  echo $tipo_mova;
       ...
       if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?}
          else{$sSQL="SELECT ACT_MES_BAN035('$codigo_mov','$cod_banco','$referencia','$tipo_mov','99','$fecha_mov')";  $resultado=pg_exec($conn,$sSQL);  $error=pg_errormessage($conn);  $error=substr($error, 0, 61);
