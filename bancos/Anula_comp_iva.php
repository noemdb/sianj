...
error_reporting(E_ALL);$critero=$_GET["criterio"]; $nro_comprobante=substr($criterio,6,8);  $ano_fiscal=substr($criterio,0,4);  $mes_fiscal=substr($criterio,4,2);
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if($error==0){
   ...
   if ($filas==0){$error=1; echo $sql; ?> <script language="JavaScript"> muestra('NÚMERO DE COMPROBANTE NO EXISTE');</script><? }
    else{$sSQL="SELECT ANULA_BAN027('$ano_fiscal','$mes_fiscal','$nro_comprobante')";  $resultado=pg_exec($conn,$sSQL);  $error=pg_errormessage($conn);  $error="ERROR ELIMINANDO: ".substr($error, 0, 61);
