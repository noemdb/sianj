...
$url="Emision_Ndb_ord_nomina.php?continua=N"; $furl="/sia/bancos/rpt/Rpt_formato_mov_libro.php?cod_banco=".$cod_banco."&referencia=".$num_nota."&tipo_mov=NDB";
...
if($error==0){ $l_cat=0; $sql="Select campo503,campo504,campo526 from SIA005 where campo501='05'"; $resultado=pg_query($sql); if($registro=pg_fetch_array($resultado,0)){$formato_presup=$registro["campo504"];$formato_cat=$registro["campo526"]; $l_cat=strlen($formato_cat); if($SIA_Periodo<$registro["campo503"]){$SIA_Periodo=$registro["campo503"];}}
    $campo502="NNNNNNNNNNNNNNNNNNNN"; $des_chq=""; $periodom=$SIA_Periodo; $sql="Select campo502,campo503,campo510 from SIA005 where campo501='02'"; $resultado=pg_query($sql); if($registro=pg_fetch_array($resultado,0)){$campo502=$registro["campo502"]; $periodom=$registro["campo503"]; $des_chq=$registro["campo510"];}
...
if($error==0){ $sSQL="SELECT cod_banco FROM BAN004 WHERE cod_banco='$cod_banco' and referencia='$num_nota' and tipo_mov_libro='NDB'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('NUMERO NOTA DEBITO YA EXISTE');</script><? }}
...
if($error==0){ $tipo_comp="B".$cod_banco; $sSql="Select * from con002 where text(fecha)='$sfecha' and referencia='$num_nota' and tipo_comp='$tipo_comp'";  $resultado=pg_query($sSql);  $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE YA EXISTE');</script><? }}
if($error==0){ $nmes=substr($fecha,3, 2); $codc_banco=""; $tasa_idb=0; $cod_cont_idb=""; $sSQL="SELECT cod_banco,nombre_banco,nro_cuenta,descripcion_banco,tipo_cuenta,cod_contable,tipo_bco,control_chequera,status_control,activa,fecha_activa,fecha_desactiva,s_inic_libro,deb_libro01,cre_libro01,deb_libro02,cre_libro02,deb_libro03,cre_libro03,deb_libro04,cre_libro04,deb_libro05,cre_libro05,deb_libro06,cre_libro06,deb_libro07,cre_libro07,deb_libro08,cre_libro08,deb_libro09,cre_libro09,deb_libro10,cre_libro10,deb_libro11,cre_libro11,deb_libro12,cre_libro12,campo_str1,campo_str2,campo_num1,campo_num2 FROM ban002 WHERE cod_banco='$cod_banco'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
 if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE BANCO NO EXISTE');</script><? }else {$registro=pg_fetch_array($resultado,0); $codc_banco=$registro["cod_contable"];  $activo=$registro["activa"]; $saldo_ant_libro=$registro["s_inic_libro"]; $tasa_idb=$registro["campo_num1"]; $cod_cont_idb=$registro["campo_str1"];  if($activo=="N"){$error=1; ?> <script language="JavaScript"> muestra('CÓDIGO DE BANCO NO ESTA ACTIVO');</script><? }
...
if($error==0){$sql="Select * from ORD_PAGO where nro_orden='$nro_orden' and anulado='N'"; $res=pg_query($sql); $filas=pg_num_rows($res); 
	if($filas>=1){ $registro=pg_fetch_array($res,0);   $tipo_caus=$registro["tipo_causado"];  $fecha_ord=$registro["fecha"]; $orden_permanen=$registro["orden_permanen"]; $nro_periodos=$registro["nro_periodos"];  $cod_contable_o=$registro["cod_contable_o"]; $tipo_orden=$registro["tipo_orden"];
	  $anulado=$registro["anulado"]; $mstatus_ord=$registro["status"]; $pago_ces=$registro["pago_ces"];  $ced_rif_ces=$registro["ced_rif_ces"];   $nombre_ces=$registro["nombre_ces"]; 	  $beneficiario=$registro["nombre"]; 
	  ...
	  $dispo_ord=$total_neto; $total_neto=0; if($total_abono==0){$total_retencion=$registro["total_retencion"]; $total_pasivos=$registro["total_pasivos"]; $nro_orden_ret=$nro_orden;}else{$total_retencion=0;$total_pasivos=0;$nro_orden_ret="";}	  	  
	  $resta_abono=$monto_nota+$total_retencion-$total_pasivos; $monto_abono=0;	 $monto_bruto=$monto_nota+$total_retencion-$total_pasivos;
	  ...
	  if(($tipo_orden=="0002")or($tipo_orden=="0005")or($tipo_orden=="0015")or($tipo_orden=="0016")){}else{ $error=1; ?><script language="JavaScript">muestra('TIPO DE ORDEN NO ES VALIDO');</script><? }    
	  if($error==0){ $resultado=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')"); $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,91); 
	    ...
	    while($reg=pg_fetch_array($res)){ $monto_c=$reg["monto"]-$reg["ajustado"]; $pagado=$reg["pagado"]; $cod_presup=$reg["cod_presup"]; $fuente_financ=$reg["fuente_financ"];  $referencia_comp=$reg["referencia_comp"]; $tipo_compromiso=$reg["tipo_compromiso"]; $tipo_imput_presu=$reg["tipo_imput_presu"];  $ref_imput_presu=$reg["ref_imput_presu"]; $monto_credito=$reg["monto_credito"]; $func_inv=$reg["func_inv"];  $fecha_aep=$reg["fecha_aep"]; $ref_aep=$reg["ref_aep"]; $num_proyecto=$reg["num_proyecto"];  $operacion="N";  if($reg["ref_compromiso"]="SI"){$operacion="C";}
		   $monto_cre=0; $monto_p=0;  $m_cod=$monto_c-$pagado;
		   if(($m_cod>0)and($resta_abono>0)){if($resta_abono>=$m_cod){$m_cod=$m_cod;}else{$m_cod=$resta_abono;}   $resta_abono=$resta_abono-$m_cod;  $monto_abono=$monto_abono+$m_cod;   $monto_p=$m_cod; 
			 if($tipo_imput_presu=="P"){$monto_cre=0;}else{if($m_cod>=$monto_credito){$monto_cre=$monto_credito;}else{$monto_cre=$m_cod;} }			 
		   } 
		   $monto_c=cambia_coma_numero($monto_c); $monto_p=cambia_coma_numero($monto_p); $monto_cre=cambia_coma_numero($monto_cre);
	  ...
	  if($error==0){ $monto_asiento=0; $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')"); $merror=pg_errormessage($conn); $merror=substr($merror, 0, 91); 
		$descripcion_a=$beneficiario." Orden:".$prev_ord_can;
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_contable_o','00000','',$monto_nota,'D','B','N','02','0','$descripcion_a')"); $merror=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($merror,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><? $error=1; }
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','C','$codc_banco','00000','',$monto_nota,'D','B','N','02','0','$descripcion_a')"); $merror=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($merror,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><? $error=1; }
	  }	 
...
if($error==0){$status_ord="A"; $monto_abono=$total_orden-$total_abono;  if($monto_nota>$monto_abono){$balance=$monto_nota-$monto_abono;}else{$balance=$monto_abono-$monto_nota;} $balance=round($balance,2); if(($monto_nota>=$monto_abono)or($balance==0)){ $status_ord="I";} } echo $monto_nota." ".$monto_abono." ".$balance." ".$status_ord;
if($error==0){ if($tasa_idb>0)  {$mstatus="I";$monto_idb=$total_Cheques*($tasa_idb/100); $monto_idb=Round($monto_idb, 2);} else{$cod_cont_idb=""; $mstatus="N"; $monto_idb=0;}    $sfecha=formato_aaaammdd($fecha);
   $sSQL="SELECT INCLUYE_NDB_ABONA_ORDEN('$codigo_mov','$cod_banco','$num_nota','$sfecha','$ced_rif',$monto_nota,'N','','$usuario_sia','$minf_usuario','$mstatus',$monto_idb,'$cod_cont_idb','$codc_banco','$tipo_pago','$statusc','$status_ord','$nro_orden','$tipo_caus','$descripcion')";  $resultado=pg_exec($conn,$sSQL);  $merror=pg_errormessage($conn); $merror=substr($merror, 0, 91); 
   if(!$resultado){?><script language="JavaScript"> muestra('<? echo $merror; ?>');</script><? $error=1;} else{$error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE');</script><?   
