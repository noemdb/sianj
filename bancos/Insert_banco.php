...
if (pg_ErrorMessage($conn)) { ?>  <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
 else{  $sSQL="Select nombre_banco from ban002 WHERE cod_banco='$cod_banco'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
  if ($filas>0){$error=1; ?>  <script language="JavaScript">  muestra('CODIGO DE BANCO YA EXISTE');  </script> <? }
   else{$error=0;
     ...
     if($error==0){$sSQL="SELECT ACTUALIZA_ban002(1,'$cod_banco','$nombre_banco','$nro_cuenta','$descripcion_banco','$tipo_cuenta','$cod_contable','$tipo_bco','00000000','00000000','N','','S','$formato_cheque','$sfecha','$sfecha',$s_inic_libro,$s_inic_banco,'$campo_str1','$campo_str2',$campo_num1,$campo_num2,'$minf_usuario')"; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);$error=substr($error, 0, 61);
