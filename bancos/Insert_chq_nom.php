...
$codigo_mov=$_POST["txtcodigo_mov"]; $tipo_pago=$_POST["txttipo_pago"]; $cod_banco=$_POST["txtcod_banco"]; $num_cheque=$_POST["txtnro_cheque"]; $nro_orden=$_POST["txtnro_orden"];  $monto=$_POST["txtmonto_cheque"];  $fecha=$_POST["txtfecha"]; $ced_rif=$_POST["txtced_rif"]; $descripcion=$_POST["txtconcepto"]; $multiple="N"; $fecha_cheque=$fecha;
$equipo = getenv("COMPUTERNAME"); $minf_usuario=$usuario_sia." ".$equipo." ".date("d/m/y H:i a"); echo "ESPERE POR FAVOR INCLUYENDO....","<br>"; $error=0;
$url="Emision_Cheque_orden_nom.php?continua=N"; $furl="../bancos/rpt/Rpt_formato_chq.php?cod_banco=".$cod_banco."&num_cheque=".$num_cheque;
...
if($error==0){ $nmes=substr($fecha,3, 2); $codc_banco=""; $tasa_idb=0; $cod_cont_idb="";   $total=$monto; $monto_cheque=$monto;  $monto_bruto=$monto;
...
if($error==0){$sql="Select * from ORD_PAGO where nro_orden='$nro_orden' and anulado='N'"; $res=pg_query($sql); $filas=pg_num_rows($res); 
	if($filas>=1){ $registro=pg_fetch_array($res,0);   $tipo_caus=$registro["tipo_causado"];  $fecha_ord=$registro["fecha"];
	  $orden_permanen=$registro["orden_permanen"]; $nro_periodos=$registro["nro_periodos"];  $cod_contable_o=$registro["cod_contable_o"];  $tipo_orden=$registro["tipo_orden"]; $beneficiario=$registro["nombre"];
	  ...
	  $resta_abono=$monto_cheque+$total_retencion; $monto_abono=0;		$monto_bruto=$monto_cheque+$total_retencion;  	  
	  ...
	  if($error==0){ $monto_asiento=0; $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')"); $error=pg_errormessage($conn); $error=substr($error,0,91); 
		$descripcion_a=$beneficiario." Orden:".$prev_ord_can;
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_contable_o','00000','',$monto_cheque,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','C','$codc_banco','00000','',$monto_cheque,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }
	  }	  
...
if($error==0){$status_ord="A"; $monto_abono=$total_orden-$total_abono; if($monto_cheque>=$monto_abono){ $status_ord="I";} }
if($error==0){ if($tasa_idb>0)  {$mstatus="I";$monto_idb=$total_Cheques*($tasa_idb/100); $monto_idb=Round($monto_idb, 2);} else{$cod_cont_idb=""; $mstatus="N"; $monto_idb=0;}    $sfecha=formato_aaaammdd($fecha);
   $sSQL="SELECT INCLUYE_CHQ_ABONA_ORDEN('$codigo_mov','$cod_banco','$num_cheque','$sfecha','$ced_rif','B','$nro_orden','$tipo_caus','$referencia_comp','$tipo_compromiso',$total,'$st_chq','$usuario_sia','N','','$minf_usuario','$mstatus',$monto_idb,'$cod_cont_idb','$codc_banco','$tipo_pago','$statusc','$status_ord','$descripcion')";  $resultado=pg_exec($conn,$sSQL);  $error=pg_errormessage($conn);  $error=substr($error,0,91);
