...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE PLANILLA NO EXISTE');</script><? }
    ...
    if($error==0){
      ...
      $sSQL="SELECT codigo_mov,ced_rif,fecha_emision,monto_pago,cod_banco,referencia,tipo_mov FROM BAN029 WHERE codigo_mov='$codigo_mov' and tipo_retencion='$tipo_retencion' and nro_orden='$nro_orden'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; echo $sSQL; ?> <script language="JavaScript"> muestra('ERROR LOCALIZANO DETALLE DE PLANILLA');</script><? }
      else{ $reg=pg_fetch_array($resultado); $ced_rif=$reg["ced_rif"]; $fecha_emision=$reg["fecha_emision"]; $monto_pago=$reg["monto_pago"];  $cod_banco=$reg["cod_banco"]; $referencia=$reg["referencia"];  $tipo_mov=$reg["tipo_mov"]; } }    
	...
	if($error==0){$sfechaf=formato_aaaammdd($fecha_factura);   echo $tipo_operacion;
      $sSQL="SELECT ACTUALIZA_BAN012($num_opcion,'$cod_banco','$tipo_mov','$referencia','$tipo_planilla','$nro_planilla','$ced_rif','$fecha_emision','$nro_orden','$aux_orden','$tipo_retencion','$tipo_documento','$nro_documento','$nro_con_factura','$sfechaf','','$tipo_en',$monto_pago,$monto_objeto,$tasa_retencion,$monto_retencion,$monto1,$monto2,$monto3,'$MInf_Usuario','$codigo_mov')";
