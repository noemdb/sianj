...
$cod_banco=$_POST["txtcod_banco"]; $referencia=$_POST["txtreferencia"]; $tipo_mov=$_POST["txttipo_movimiento"];  $cod_bancoA=$_POST["txtcod_bancoA"]; $referenciaA=$_POST["txtreferenciaA"];
$fecha=$_POST["txtfecha"];  $descripcion=$_POST["txtdescripcion"];  $monto=$_POST["txtmonto_mov_banco"]; $beneficiario=$_POST["txtbeneficiario"];  $tipodc="D";
$equipo = getenv("COMPUTERNAME"); $minf_usuario = $equipo." ".date("d/m/y H:i a"); echo "ESPERE POR FAVOR INCLUYENDO....","<br>"; $error=0;
$url="Act_Mov_Trans_Banco.php?Gcod_banco=C".$cod_banco.$referencia.$tipo_mov;   $monto=formato_numero($monto); if(is_numeric($monto)){$monto=$monto;} else{$monto=0;}
...
if($error==0){$sfecha=formato_aaaammdd($fecha); if($sfecha>=$Fec_Ini_Ejer){echo $sfecha; $error=1;?><script language="JavaScript">muestra('FECHA DE MOVIMIENTO INVALIDA');</script><?}}
...
if($error==0){$sSQL="SELECT cod_banco FROM BAN008 WHERE cod_banco='$cod_banco' and referencia='$referencia' and tipo_trans_banco='$tipo_mov'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('MOVIMIENTO EN TRANSITO YA EXISTE');</script><? }}
...
if($error==0){$sfecha=formato_aaaammdd($fecha); if($tipodc=="C"){$tipodca="D";}else{$tipodca="C";}  $DOperacion="0"; if($tipodc=="C"){$AOperacion="03";}else{$AOperacion="01";}
  $sSQL="SELECT ACTUALIZA_BAN008(1,'$cod_banco','$referencia','$tipo_mov','$sfecha',$monto,'00','$beneficiario','$minf_usuario','$descripcion')";  $resultado=pg_exec($conn,$sSQL);  $error=pg_errormessage($conn);  $error=substr($error, 0, 61);
