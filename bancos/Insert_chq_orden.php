...
$codigo_mov=$_POST["txtcodigo_mov"]; $tipo_pago=$_POST["txttipo_pago"]; $cod_banco=$_POST["txtcod_banco"]; $num_cheque=$_POST["txtnro_cheque"];  $monto=$_POST["txtmonto_chq"];  $fecha=$_POST["txtfecha"]; $multiple=$_POST["txtmultiple"]; $fecha_cheque=$fecha;  $concepto=$_POST["txtconcepto"];
$equipo = getenv("COMPUTERNAME"); $minf_usuario=$usuario_sia." ".$equipo." ".date("d/m/y H:i a"); echo "ESPERE POR FAVOR INCLUYENDO....","<br>"; $error=0;
$url="Emision_Cheque_orden.php?continua=S";   $furl="../bancos/rpt/Rpt_formato_chq.php?cod_banco=".$cod_banco."&num_cheque=";
...
if(checkData($fecha)=='1'){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('FECHA NO ES VALIDA');</script><? }
...
if($error==0){$nmes=substr($fecha,3, 2); $codc_banco=""; $tasa_idb=0; $cod_cont_idb="";
 ...
 } $furl="../bancos/rpt/".$formato_cheque."?cod_banco=".$cod_banco."&num_cheque="; }
}$ced_rif=""; $fecha1=$fecha; $num_cheque_d=$num_cheque; $num_cheque_h=$num_cheque;
if(($error==0)and($multiple=="S")){ $sql="SELECT * FROM pag027 where codigo_mov='$codigo_mov' and seleccionada='S' order by ced_rif"; $res=pg_query($sql); while($registro=pg_fetch_array($res)){if($ced_rif==""){$ced_rif=$registro["ced_rif"];} else { if($ced_rif<>$registro["ced_rif"]){$error=1; ?> <script language="JavaScript"> muestra('SELECCION DE DIFERENTE BENEFICIARIO');</script><? } }} }
if($error==0){ 
 if($multiple=="S"){ $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')");  $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<? echo $mvalor; ?>'); </script> <? }  $res=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");  $merror=pg_errormessage($conn);  $merror=substr($merror,0,90);if(!$res){ ?> <script language="JavaScript">  muestra('<? echo $merror; ?>'); </script> <? $error=1; }
  $sql="SELECT * FROM pag027 where codigo_mov='$codigo_mov' and seleccionada='S' order by nro_orden,tipo_causado"; $resp=pg_query($sql); $total=0; $cant=0; $total_Cheques=0;  $prev_ord_can = ""; $t_debe=0;  $t_haber=0; $balance=0; 
  while($registro=pg_fetch_array($resp)){  $nro_orden=$registro["nro_orden"]; $tipo_causado=$registro["tipo_causado"]; $monto=$registro["monto_orden"]; $t_ajuste=$registro["total_ajuste"]; IF($t_ajuste==0){$orden_aju="N";}else{$orden_aju="S";} $monto_orden_aju=0; $busca_orden_aju="S"; $cta_orden_aju="";  $total_Cheques=$total_Cheques+$monto; $orden_ret=$registro["orden_ret"];  
     ...
     if(((($status=="N") and (($orden_ret=="S") Or ($orden_ret=="N"))) Or  (($status_r=="N") and ($orden_ret=="R"))) and ($anulado=="N")){ $tipo_chq="O"; if($orden_ret=="R"){$tipo_chq="R";}  $fecha_orden=$pag001["fecha"]; $tipo_comp="O".$tipo_causado; $descripcion=$des_chq.$pag001["concepto"]; if(($doc_concepto=="S")and($nro_documento!=="")){$descripcion=$descripcion.", ".$tipo_documento.": ".$nro_documento;} $F=0; $cod_contable_o=$pag001["cod_contable_o"]; $afecta_presu=$pag001["afecta_presu"];  $ced_rif=$pag001["ced_rif"]; $beneficiario=$pag001["nombre"]; if($pag001["pago_ces"]=="S"){$ced_rif=$pag001["ced_rif_ces"];$beneficiario=$pag001["nombre_ces"];} $descripcion_a=$beneficiario." Orden:".$prev_ord_can;
       if((($g_comp_ret=="S") Or ($retencion=="N"))and($genera_comp=="N")) { $sql="SELECT * FROM pag020  where nro_orden='$nro_orden' order by cod_cuenta"; $res=pg_query($sql);
        while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_pasivo"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc=$registro["debito_credito"]; $F=$F+1; $t_debe=$t_debe+$monto_asiento;
          ...
          if($filas>0){ $reg=pg_fetch_array($resultado);   $monto_c=$monto_asiento+$reg["monto_asiento"]; $monto_c=cambia_coma_numero($monto_c);  if($monto_c>0){$resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta',$monto_c,'$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1;} } }
           else{$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_cuenta','00000','',$monto_asiento,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }}
        }
       ...
       if((($g_comp_ret=="S") Or ($retencion=="N"))and($genera_comp=="S")) { $sql="SELECT * FROM con003  where referencia='$nro_orden' and fecha='$fecha_orden' and tipo_comp='$tipo_comp' and debito_credito='C' order by cod_cuenta"; $res=pg_query($sql); $continua=1;
        ...
        while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_asiento"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc=$registro["debito_credito"]; $continua=1;
         if($genera_orden_r=="N"){  if($orden_ret=="R"){$resto=abs($total_retencion-($t_debe+$monto_asiento)); if(($total_retencion!=($t_debe+$monto_asiento))and($resto>0.001)){$continua=0;}}
          else{ if($g_comp_ret=="N"){$monto=$total_causado-$total_retencion;} else{ if($status_2=="C"){$monto=$total_causado-$total_retencion-$monto_am_ant+$monto_pasivo;} else{$monto=$total_causado-$total_retencion-$monto_am_ant+$monto_pasivo;} $resto=abs($monto-$monto_asiento); } if(($total_retencion!=0)and($resto>0.001)){$continua=0;} } }
         if($continua==1){
        ...
        if(($monto_pasivo>0) and ($status_2<>"C")){ $sql="SELECT * FROM pag020 where nro_orden='$nro_orden' and debito_credito='D' order by cod_cuenta"; $res=pg_query($sql);
         while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_pasivo"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc="D"; $F=$F+1;
          $sSQL="Select * from CON010 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cod_cuenta' and debito_credito='D'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
          if($filas>0){ $reg=pg_fetch_array($resultado);   $monto_c=$monto_asiento+$reg["monto_asiento"];   $monto_c=cambia_coma_numero($monto_c);  if($monto_c>0){$resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta',$monto_c,'$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1;} } }
           else{$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_cuenta','00000','',$monto_asiento,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }}  $t_debe=$t_debe+$monto_asiento;
         }
        }		
		if(($monto_pasivo==0) and ($status_2=="C")){ $sql="SELECT * FROM pag020 where nro_orden='$nro_orden' and debito_credito='C' order by cod_cuenta"; $res=pg_query($sql);
          while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_pasivo"];  $cod_cuenta=$registro["cod_cuenta"]; 
		    $sSQL="Select * from CON010 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cod_cuenta' and debito_credito='D'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
            if($filas>0){ $sSQL="SELECT ELIMINA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta')";    $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);  $error="ERROR ELIMINANDO: ".substr($error,0,90); 
        ...
        if($orden_aju=="S"){$sSQL="Select * from CON010 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cta_orden_aju' and debito_credito='D'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
          if($filas>0){ $reg=pg_fetch_array($resultado);  $monto_c=$reg["monto_asiento"]-$t_ajuste; if($monto_c<0){$monto_c=0;} $monto_c=cambia_coma_numero($monto_c);  $resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta',$monto_c,'$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?  $error=1; }  $t_debe=$t_debe-$t_ajuste;  }
        }
       }
       ...
       if($error==0){ $sql="select pre037.referencia_caus,pre037.tipo_causado,pre037.referencia_comp,pre037.tipo_compromiso,pre037.cod_presup,pre037.fuente_financ,pre037.monto,pre037.ajustado,pre037.tipo_imput_presu,pre037.ref_imput_presu,pre037.monto_credito,pre037.amort_anticipo,pre007.ref_aep,pre007.num_proyecto,pre007.fecha_aep,pre007.func_inv,pre003.ref_compromiso from pre037,pre007 Left Join pre003 On (pre003.tipo_causado=pre007.tipo_causado) where (pre037.referencia_caus=pre007.referencia_caus) and (pre037.tipo_causado=pre007.tipo_causado) and (pre037.referencia_comp=pre007.referencia_comp) and (pre037.tipo_compromiso=pre007.tipo_compromiso) and (pre007.referencia_caus='$nro_orden') and (pre007.tipo_causado='$tipo_causado') order by cod_presup,fuente_financ"; $res=pg_query($sql);
        while($reg=pg_fetch_array($res)){  $cod_presup=$reg["cod_presup"]; $fuente_financ=$reg["fuente_financ"];  $referencia_comp=$reg["referencia_comp"]; $tipo_compromiso=$reg["tipo_compromiso"]; $tipo_imput_presu=$reg["tipo_imput_presu"];  $ref_imput_presu=$reg["ref_imput_presu"];  $func_inv=$reg["func_inv"];  $fecha_aep=$reg["fecha_aep"]; $ref_aep=$reg["ref_aep"]; $num_proyecto=$reg["num_proyecto"];  $operacion="N";  if($reg["ref_compromiso"]="SI"){$operacion="C";}
          $monto_c=$reg["monto"]-$reg["ajustado"]; $monto_credito=$reg["monto_credito"]; if ($monto_credito>=$reg["ajustado"]){ $monto_credito=$reg["monto_credito"]-$reg["ajustado"];}
		   $monto_c=cambia_coma_numero($monto_c);$monto_credito=cambia_coma_numero($monto_credito);
		   $resultado=pg_exec($conn,"SELECT INCLUYE_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$nro_orden','$tipo_causado','','0000','','0000','$operacion','','','','$ref_aep','$num_proyecto','$fecha_aep','$func_inv','$tipo_imput_presu','$ref_imput_presu','$fecha_orden',$monto_c,0,$monto_credito,0)"); $merror=pg_errormessage($conn);   $merror="ERROR GRABANDO: ".substr($merror,0,90);  if(!$resultado){?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><? $error=1; }
        }
       }
	   ...
	   if(($total_retencion>0) and ($retencion=="N") and (($g_comp_ret=="N")Or($genera_orden_r=="N"))) { $prev_orden = ""; $sql="select nro_orden_ret,aux_orden,tipo_caus_ret,ref_comp_ret,tipo_comp_ret,cod_presup_ret,fuente_fin_ret,monto_retencion from pag004 where (nro_orden_ret='$nro_orden')"; $res=pg_query($sql);
         while($reg=pg_fetch_array($res)){ $aux_orden=$reg["aux_orden"]; $monto_r=$reg["monto_retencion"]; $monto_c=$reg["monto_retencion"]; $cod_presup=$reg["cod_presup_ret"]; $fuente_financ=$reg["fuente_fin_ret"]; $nro_orden_r=$reg["nro_orden_ret"]; $tipo_causado_r=$reg["tipo_caus_ret"]; $referencia_comp=$reg["ref_comp_ret"]; $tipo_compromiso=$reg["tipo_comp_ret"];
           if(($genera_orden_r=="N") and ($ret_presup=="S") and ($afecta_presu=="S")) {
			...
			$monto_c=cambia_coma_numero($monto_c); $monto_credito=cambia_coma_numero($monto_credito);
			$sqlg="SELECT MOD_MONTOA_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$nro_orden_r','$tipo_causado_r',$monto_c,0)";
			$sqlg="SELECT MOD_MONTOA_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$nro_orden_r','$tipo_causado_r','$ref_imput_presu',$monto_c,$monto_credito)";
			//echo $sqlg,"<br>";
			//$sqlg="SELECT MOD_MONTO_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$ref_imput_presu',$monto_c,$monto_credito)";
			//echo $sqlg,"<br>";
			$resultado=pg_exec($conn,$sqlg);  $error=pg_errormessage($conn);   $error="ERROR GRABANDO: ".substr($error,0,90);  if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1;} }
           }
           if(($prev_orden!==$aux_orden) and ($g_comp_ret=="N")) { $prev_orden=$aux_orden; $sSQL="select referencia,cod_cuenta,monto_asiento,debito_credito from pag005 Where (referencia='$nro_orden') and (fecha='$fecha_orden') and (debito_credito='c')"; $res=pg_query($sSQL);
            while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_asiento"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc=$registro["debito_credito"];
              ...
              if($filas>0){ $reg=pg_fetch_array($resultado);   $monto_c=$monto_asiento+$reg["monto_asiento"];  $monto_c=cambia_coma_numero($monto_c); if($monto_c>0){$resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','C','$cod_cuenta',$monto_c,'$descripcion_a')"); $F=$F+1; $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1;} } }
               else{$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','C','$cod_cuenta','00000','',$monto_asiento,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn); $F=$F+1; $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }}  $t_haber=$t_haber+$monto_asiento;
            }
           }
         }
       }
       if($F==0){$error=1; ?> <script language="JavaScript"> muestra('LOCALIZANDO COMPROBANTES MULTIPLE DE ORDEN :'+<? echo $nro_orden; ?>);</script><? }
	   if($error==0){  $fecha2=formato_ddmmaaaa($fecha_e); if(FDate($fecha1)<FDate($fecha2)){ echo "Orden .".$nro_orden." Fecha: ".$fecha2.", Fecha Cheque: ".$fecha1,"<br>"; $error=1; ?> <script language="JavaScript">  muestra('Fecha Cheque menor a Fecha Orden'); </script> <? } }
       if($error==0){ $resultado=pg_exec($conn,"SELECT SELECCIONA_PAG027('$codigo_mov','$nro_orden','$tipo_causado','I')"); $merror=pg_errormessage($conn);  $merror=substr($merror,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<? echo $merror; ?>'); </script> <?  $error=1;}}
     }
    }else{$error=1; ?> <script language="JavaScript"> muestra('ERROR LOCALIZANDO ORDEN :'+<? echo $nro_orden; ?>);</script><? }
  }
  if(($total_Cheques>0) and ($error==0)){  $descripcion_a=$beneficiario." Orden:".$prev_ord_can; $total_Cheques=cambia_coma_numero($total_Cheques);
     $resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$num_cheque','C','$codc_banco','00000','',$total_Cheques,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn); $F=$F+1; $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }  $t_haber=$t_haber+$total_Cheques;
     ...
     if($error==0){ $resultado=pg_exec($conn,"SELECT MODIFICA_DES_CON010('$codigo_mov','$descripcion_a')"); $merror=pg_errormessage($conn);  $merror=substr($merror,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<? echo $merror; ?>'); </script> <?  $error=1;}}
     if($error==0){ if($tasa_idb>0)  {$mstatus="I";$monto_idb=$total_Cheques*($tasa_idb/100); $monto_idb=Round($monto_idb, 2);} else{$cod_cont_idb=""; $mstatus="N"; $monto_idb=0;}    $sfecha=formato_aaaammdd($fecha_cheque);    $descripcion=$concepto;
        $sSQL="SELECT INCLUYE_CHQ_ORDEN('$codigo_mov','$cod_banco','$num_cheque','$sfecha','$ced_rif',$total_Cheques,'$st_chq','$usuario_sia','N','','$minf_usuario','$mstatus',$monto_idb,'$cod_cont_idb','$codc_banco','$tipo_pago','$statusc','$descripcion')";  $resultado=pg_exec($conn,$sSQL);  $merror=pg_errormessage($conn);  $merror=substr($merror,0,90);
        if(!$resultado){?><script language="JavaScript"> muestra('<? echo $error; ?>');</script><? $error=1;}
        else{ $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE CHEQUE: '+<? echo $num_cheque; ?>);</script><? $cant=$cant+1;  $num_cheque_h=$num_cheque; $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')");  $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<? echo $mvalor; ?>'); </script> <? }  $res=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");  $merror=pg_errormessage($conn);  $merror=substr($merror,0,90);if(!$res){ ?> <script language="JavaScript">  muestra('<? echo $error; ?>'); </script> <? $error=1; } }
       /* */
	 }
  } 
 }
  ...
  while(($registro=pg_fetch_array($resp))) {  if($cant>=1){$ult_ref=$num_cheque+1; $len=strlen($ult_ref); $ult_ref=substr("00000000",0,8-$len).$ult_ref;  $num_cheque=$ult_ref; }
    $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')");  $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<? echo $mvalor; ?>'); </script> <? }  $res=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");  $merror=pg_errormessage($conn);  $merror=substr($merror,0,90);if(!$res){ ?> <script language="JavaScript">  muestra('<? echo $error; ?>'); </script> <? $error=1; }
    $nro_orden=$registro["nro_orden"]; $tipo_causado=$registro["tipo_causado"]; $monto=$registro["monto_orden"]; $t_ajuste=$registro["total_ajuste"]; IF($t_ajuste==0){$orden_aju="N";}else{$orden_aju="S";} $monto_orden_aju=0; $busca_orden_aju="S"; $cta_orden_aju=""; $t_debe=0;  $t_haber=0;  $total_Cheques=$monto; $orden_ret=$registro["orden_ret"];
    $sSql="Select * from ORD_PAGO where nro_orden='$nro_orden' and tipo_causado='$tipo_causado' and status<>'I' and anulado='N'";  $resultado=pg_query($sSql);  $filas=pg_num_rows($resultado);
    if($filas==1){$pag001=pg_fetch_array($resultado,0); $fecha_e=$pag001["fecha"]; $total_causado=$pag001["total_causado"]; $total_retencion=$pag001["total_retencion"]; $monto_pasivo=$pag001["total_pasivos"]; $monto_am_ant=$pag001["monto_am_ant"]; $genera_comp=$pag001["genera_comp"]; $status=$pag001["status"];  $status_r=$pag001["status_r"];  $anulado=$pag001["anulado"];  $nro_documento=$pag001["nro_documento"]; $tipo_documento=$pag001["tipo_documento"]; $amort_ant=$pag001["status_1"]; $status_1=$pag001["status_1"]; $status_2=$pag001["status_2"]; $cta_anticipo=$pag001["campo_str1"]; $g_comp_ret=$pag001["genera_comp_ret"]; $retencion=$pag001["retencion"]; $genera_orden_r=$pag001["genera_orden_r"];
     if(((($status=="N") and (($orden_ret=="S") Or ($orden_ret=="N"))) Or  (($status_r=="N") and ($orden_ret=="R"))) and ($anulado=="N")){ $tipo_chq="O"; if($orden_ret=="R"){$tipo_chq="R";}  $fecha_orden=$pag001["fecha"]; $tipo_comp="O".$tipo_causado; 
	    
		if($des_chq<>""){ $descripcion=$des_chq." ".$pag001["concepto"];  if($Cod_Emp=="86"){$descripcion=$des_chq." ".substr($nro_orden,3,5)." ".$pag001["concepto"];} } else { $descripcion=$pag001["concepto"]; }
       ...
       if((($g_comp_ret=="S") Or ($retencion=="N"))and($genera_comp=="S")) { 
	    $sql="SELECT * FROM con003  where referencia='$nro_orden' and fecha='$fecha_orden' and tipo_comp='$tipo_comp' and debito_credito='C' order by cod_cuenta"; $res=pg_query($sql); $continua=1;
         while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_asiento"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc=$registro["debito_credito"]; $continua=1;
         if($genera_orden_r=="N"){ 
		...
		if($t_debe>$t_haber){$balance=$t_debe-$t_haber;}else{$balance=$t_haber-$t_debe;}
		//echo $t_debe.' '.$t_haber.' '.formato_monto($balance).' '.$busca_orden_aju.' '.$cta_anticipo.' '.$monto_orden_aju.' '.$cta_orden_aju,"<br>"; 
        if(($monto_pasivo>0) and ($status_2<>"C")) { $sql="SELECT * FROM pag020 where nro_orden='$nro_orden' and debito_credito='D' order by cod_cuenta"; $res=pg_query($sql);
         while($registro=pg_fetch_array($res)){ $monto_asiento=$registro["monto_pasivo"]; $cod_cuenta=$registro["cod_cuenta"];  $tipo_dc="D"; $F=$F+1;
          $sSQL="Select * from CON010 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cod_cuenta' and debito_credito='D'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
          if($filas>0){ $reg=pg_fetch_array($resultado);   $monto_c=$monto_asiento+$reg["monto_asiento"];  $monto_c=cambia_coma_numero($monto_c); if($monto_c>0){$resultado=pg_exec($conn,"SELECT MODIFICA_CUENTA_CON010('$codigo_mov','D','$cod_cuenta',$monto_c,'$descripcion_a')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1;} } }
        ...
        //echo $t_debe.' '.$t_haber.' '.formato_monto($balance).' '.$orden_aju,"<br>"; 		
        if($orden_aju=="S"){ $sSQL="Select * from CON010 WHERE codigo_mov='$codigo_mov' and cod_cuenta='$cta_orden_aju' and debito_credito='D'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
       ...
       if($error==0){ $sql="select pre037.referencia_caus,pre037.tipo_causado,pre037.referencia_comp,pre037.tipo_compromiso,pre037.cod_presup,pre037.fuente_financ,pre037.monto,pre037.ajustado,pre037.tipo_imput_presu,pre037.ref_imput_presu,pre037.monto_credito,pre037.amort_anticipo,pre007.ref_aep,pre007.num_proyecto,pre007.fecha_aep,pre007.func_inv,pre003.ref_compromiso from pre037,pre007 Left Join pre003 On (pre003.tipo_causado=pre007.tipo_causado) where (pre037.referencia_caus=pre007.referencia_caus) and (pre037.tipo_causado=pre007.tipo_causado) and (pre037.referencia_comp=pre007.referencia_comp) and (pre037.tipo_compromiso=pre007.tipo_compromiso) and (pre007.referencia_caus='$nro_orden') and (pre007.tipo_causado='$tipo_causado') order by cod_presup,fuente_financ"; $res=pg_query($sql);
        while($reg=pg_fetch_array($res)){ $cod_presup=$reg["cod_presup"]; $fuente_financ=$reg["fuente_financ"];  $referencia_comp=$reg["referencia_comp"]; $tipo_compromiso=$reg["tipo_compromiso"]; $tipo_imput_presu=$reg["tipo_imput_presu"];  $ref_imput_presu=$reg["ref_imput_presu"]; $monto_credito=$reg["monto_credito"]; $func_inv=$reg["func_inv"];  $fecha_aep=$reg["fecha_aep"]; $ref_aep=$reg["ref_aep"]; $num_proyecto=$reg["num_proyecto"];  $operacion="N";  if($reg["ref_compromiso"]="SI"){$operacion="C";}
          $monto_c=$reg["monto"]-$reg["ajustado"]; $monto_credito=$reg["monto_credito"]; if ($monto_credito>=$reg["ajustado"]){ $monto_credito=$reg["monto_credito"]-$reg["ajustado"];} $monto_c=cambia_coma_numero($monto_c);
		  $monto_c=cambia_coma_numero($monto_c);  $monto_credito=cambia_coma_numero($monto_credito);  $resultado=pg_exec($conn,"SELECT INCLUYE_PRE026('$codigo_mov','$cod_presup','$fuente_financ','$referencia_comp','$tipo_compromiso','$nro_orden','$tipo_causado','','0000','','0000','$operacion','','','','$ref_aep','$num_proyecto','$fecha_aep','$func_inv','$tipo_imput_presu','$ref_imput_presu','$fecha_orden',$monto_c,0,$monto_credito,0)"); $merror=pg_errormessage($conn);   $merror="ERROR GRABANDO: ".substr($merror,0,90);  if(!$resultado){?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><? $error=1; }
        }
       }
	  ...
	  // echo $total_retencion." ".$retencion." ".$g_comp_ret." ".$genera_orden_r,"<br>";       
       if(($total_retencion>0) and ($retencion=="N") and (($g_comp_ret=="N")Or($genera_orden_r=="N"))) { $prev_orden = ""; $sql="select nro_orden_ret,aux_orden,tipo_caus_ret,ref_comp_ret,tipo_comp_ret,cod_presup_ret,fuente_fin_ret,monto_retencion from pag004 where (nro_orden_ret='$nro_orden')"; $res=pg_query($sql);
         while($reg=pg_fetch_array($res)){ $aux_orden=$reg["aux_orden"]; $monto_r=$reg["monto_retencion"]; $cod_presup=$reg["cod_presup_ret"]; $fuente_financ=$reg["fuente_fin_ret"]; $nro_orden_r=$reg["nro_orden_ret"]; $tipo_causado_r=$reg["tipo_caus_ret"]; $referencia_comp=$reg["ref_comp_ret"]; $tipo_compromiso=$reg["tipo_comp_ret"];
           if(($genera_orden_r=="N") and ($ret_presup=="S") and ($afecta_presu=="S")) { if($tipo_compromiso=="0000"){ $referencia_comp=$nro_orden_r; }
            $sSQL="Select monto,monto_credito,ref_imput_presu from pre026 WHERE codigo_mov='$codigo_mov' and cod_presup='$cod_presup' and fuente_financ='$fuente_financ' and referencia_comp='$referencia_comp' and tipo_compromiso='$tipo_compromiso' and referencia_caus='$nro_orden_r' and tipo_causado='$tipo_causado_r'"; $resultado=pg_exec($conn,$sSQL); $filas=pg_numrows($resultado);
           // echo $sSQL." ".$filas,"<br>";
			if($filas>0){ $reg=pg_fetch_array($resultado); $monto_c=$reg["monto"]-$monto_r;  $monto_credito=$reg["monto_credito"]; if ($monto_credito>=$monto_r){ $monto_credito=$reg["monto_credito"]-$monto_r;}
	   ...
	   if($error==0){  $fecha2=formato_ddmmaaaa($fecha_e); if(FDate($fecha1)<FDate($fecha2)){ echo "Orden .".$nro_orden." Fecha: ".$fecha2.", Fecha Cheque: ".$fecha1,"<br>"; $error=1; ?> <script language="JavaScript">  muestra('Fecha Cheque menor a Fecha Orden'); </script> <? } }
       if($error==0){ $resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$num_cheque','C','$codc_banco','00000','',$total_Cheques,'D','B','N','02','0','$descripcion_a')"); $error=pg_errormessage($conn); $F=$F+1; $error="ERROR GRABANDO: ".substr($error,0,90); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }  $t_haber=$t_haber+$total_Cheques; }
       if($t_debe>$t_haber){$balance=$t_debe-$t_haber;}else{$balance=$t_haber-$t_debe;}
       if($balance>0.001){$error=1; echo ' Debe: '.$t_debe.' Haber: '.$t_haber.' Dif: '.formato_monto($balance).' Monto chq: '.formato_monto($total_Cheques),"<br>"; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE NO CUADRA');</script><? }
       if(($t_debe==0)or($t_haber==0)){$error=1; echo $t_debe.' '.$t_haber.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><? }
       if($error==0){ $resultado=pg_exec($conn,"SELECT SELECCIONA_PAG027('$codigo_mov','$nro_orden','$tipo_causado','I')"); $merror=pg_errormessage($conn);  $merror=substr($merror,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<? echo $merror; ?>'); </script> <?  $error=1;}}
       if($error==0){ if($tasa_idb>0)  {$mstatus="I";$monto_idb=$total_Cheques*($tasa_idb/100); $monto_idb=Round($monto_idb, 2);} else{$cod_cont_idb=""; $mstatus="N"; $monto_idb=0;}    $sfecha=formato_aaaammdd($fecha_cheque);
         $descripcion=str_replace("\r\n","",$descripcion); $descripcion=str_replace("\n","",$descripcion);  $descripcion=str_replace('"','',$descripcion); $descripcion=str_replace("'","",$descripcion); $total_Cheques=cambia_coma_numero($total_Cheques);
/**/	
		 $sSQL="SELECT INCLUYE_CHQ_ORDEN('$codigo_mov','$cod_banco','$num_cheque','$sfecha','$ced_rif',$total_Cheques,'$st_chq','$usuario_sia','N','','$minf_usuario','$mstatus',$monto_idb,'$cod_cont_idb','$codc_banco','$tipo_pago','$statusc','$descripcion')"; 
         $resultado=pg_exec($conn,$sSQL);  $merror=pg_errormessage($conn);  $merror=substr($merror,0,90); if(!$resultado){?><script language="JavaScript"> muestra('<? echo $merror; ?>');</script><? $error=1;}
         else{ $error=0;?><script language="JavaScript">  muestra('INCLUYO EXITOSAMENTE CHEQUE: '+'<? echo $num_cheque; ?>'+' DE ORDEN :'+'<? echo $nro_orden; ?>');</script><?  $cant=$cant+1;  $num_cheque_h=$num_cheque;  
		 $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')");  $mvalor=pg_errormessage($conn); $mvalor=substr($mvalor,0,90); if(!$resultado){ ?> <script language="JavaScript">  muestra('<? echo $mvalor; ?>'); </script> <? }
		 $res=pg_exec($conn,"SELECT BORRAR_PRE026('$codigo_mov')");$merror=pg_errormessage($conn);  $merror=substr($merror,0,90);if(!$res){ ?> <script language="JavaScript">  muestra('<? echo $merror; ?>'); </script> <? $error=1; } 
		 }
...
<table width="957">
  ...
  <tr><td><table width="923">
      <td width="10"></td>
      <td width="170">CHEQUE A IMPRIMIR:</td>  
	  <td width="120">
<? $num=0; $temp_chq=$num_cheque_d;
?> <select name="txtnum_chq_imp" id="txtnum_chq_imp" onFocus="encender(this)" onBlur="apagar(this)" ><?
while($num<$cant){ $ult_ref=$temp_chq+$num; $len=strlen($ult_ref); $ult_ref=substr("00000000",0,8-$len).$ult_ref;    $num=$num+1;
?><option value="<? echo $ult_ref;?>"><? echo $ult_ref;?></option>
<?}?></select> </td>
      </td>
	  <td width="200"></td>
	  <td width="250"></td>
...
<? if($error==0){	
	?> <td width="160"></td>
    <td width="130"><input name="txttipo_rpt" type="hidden" id="txttipo_rpt" value='TXT'></td>    
	<td width="200"><input name="btEmitir" type="button" id="btEmitir" value="Imprimir Cheque" title="Impresion Cheque Emitido" onclick="javascript:Ventana_chq('<? echo $furl; ?>'+document.form1.txtnum_chq_imp.value+'&tipo_rpt='+document.form1.txttipo_rpt.value);"></td>
<? }else{?>   
    <td width="160"></td>
    <td width="130"><input name="txttipo_rpt" type="hidden" id="txttipo_rpt" value='TXT'></td>
	<td width="200"><input name="txtnum_cheque" type="hidden" id="txtnum_cheque" value="<?echo $num_cheque?>"></td> 
<? } ?>
