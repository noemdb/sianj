...
if($error==0){$nmes=substr($fecha,3, 2); $codc_banco=""; $tasa_idb=0; $cod_cont_idb="";   $total=$monto;
 ...
 if($error==0){ $disponible=$saldo_ant_libro;  
 echo "Saldo Inicial: ".formato_monto($disponible)." ".$nmes,"<br>"; 
 $m=$nmes*1;
...
$sSQL="SELECT cod_banco FROM BAN004 WHERE cod_banco='$cod_banco' and referencia='$referencia' and (tipo_mov_libro='DEP' OR tipo_mov_libro='NCR')"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('DEPOSITO O NOTA CREDITO EXISTE EN MOVIMIENTO EN LIBROS');</script><? }
$sSQL="SELECT cod_banco FROM BAN005 WHERE cod_banco='$cod_banco' and referencia='$referencia' and (tipo_mov_banco='DEP' or tipo_mov_banco='NCR')"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('DEPOSITO O NOTA CREDITO EXISTE EN MOVIMIENTO EN BANCOS');</script><? }
$sSQL="SELECT cod_banco FROM BAN007 WHERE cod_banco='$cod_banco' and referencia='$referencia' and (tipo_trans_libro='DEP' or tipo_trans_libro='NCR')"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('DEPOSITO O NOTA CREDITO NO EXISTE EN TRANSITO LIBROS');</script><? } 
}
...
if($error==0){ $sSQL="SELECT cod_banco,cod_contable,activa,fecha_activa,fecha_desactiva FROM ban002 WHERE cod_banco='$cod_bancoA'";  
 $sSQL="SELECT cod_banco,nombre_banco,nro_cuenta,descripcion_banco,tipo_cuenta,cod_contable,tipo_bco,control_chequera,status_control,activa,fecha_activa,fecha_desactiva,s_inic_libro,deb_libro01,cre_libro01,deb_libro02,cre_libro02,deb_libro03,cre_libro03,deb_libro04,cre_libro04,deb_libro05,cre_libro05,deb_libro06,cre_libro06,deb_libro07,cre_libro07,deb_libro08,cre_libro08,deb_libro09,cre_libro09,deb_libro10,cre_libro10,deb_libro11,cre_libro11,deb_libro12,cre_libro12,campo_str1,campo_str2,campo_num1,campo_num2 FROM ban002 WHERE cod_banco='$cod_bancoA'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
 if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE BANCO A DEBITAR NO EXISTE');</script><? }
 else{$registro=pg_fetch_array($resultado,0); $codc_bancoA=$registro["cod_contable"]; $saldo_ant_libro=$registro["s_inic_libro"]; $activoA=$registro["activa"]; if($activoA=="N"){$error=1; ?> <script language="JavaScript"> muestra('BANCO A DEBITAR NO ESTA ACTIVO');</script><? }
   $disponible=$saldo_ant_libro;  echo "Saldo Inicial: ".formato_monto($disponible)." ".$nmes,"<br>"; $m=$nmes*1;
   for ($i=1;$i<=$m;$i++){$spos=$i; If($i<=9){$spos="0".$spos;} $disponible=$disponible+$registro["deb_libro".$spos] - $registro["cre_libro".$spos]; }   
   if($disponible<$monto){if($sobreg_saldo=="N"){$error=1;} echo "Disponible: ".formato_monto($disponible).'  Requerido: '.formato_monto($monto),"<br>"; ?> <script language="JavaScript"> muestra('SOBREGIRA SALDO DEL BANCO A DEBITAR');</script><? }
 } }
if($error==0){if($cod_bancoA==$cod_banco){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE BANCO A DEBITAR NO PUEDE SER EL MISMO');</script><? } }
...
} else{$cod_bancoA=$cod_banco; $referenciaA=$referencia;}   $existe_cta="N";
if($error==0){  $sql="SELECT * FROM CON010  where codigo_mov='$codigo_mov' order by debito_credito desc,cod_cuenta";  $res=pg_query($sql);  $t_debe=0; $t_haber=0; $balance=0;
  while($registro=pg_fetch_array($res)){ $balance=0;
    if($registro["debito_credito"]=="D"){$t_debe=$t_debe+$registro["monto_asiento"];}	
	if($registro["debito_credito"]=="C"){$t_haber=$t_haber+$registro["monto_asiento"];} 
	if($codc_banco==$registro["cod_cuenta"]){$existe_cta="S";}  
  }
  ...
  if(($t_debe==0)or($t_haber==0)){$error=1; echo $t_debe.' '.$t_haber,"<br>";  ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><? }
  if($t_debe>$total){$balance=$t_debe-$total;}else{$balance=$total-$t_debe;}	
  /*if($error==0){if(($total<>$t_debe)){$error=1; ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><? } }*/  
  /*if($error==0){if($balance>0.001){$error=1; echo $t_debe.' '.$total.' '.formato_monto($balance),"<br>";  ?> <script language="JavaScript"> muestra('MONTO DEL COMPROBANTE CONTABLE NO VALIDO');</script><? } }*/ 
