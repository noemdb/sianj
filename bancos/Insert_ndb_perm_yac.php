...
$url="Emision_Ndb_ord_nomina_yac.php?continua=N"; $furl="/sia/bancos/rpt/Rpt_formato_mov_libro.php?cod_banco=".$cod_banco."&referencia=".$num_nota."&tipo_mov=NDB";
...
if($error==0){$sql="Select * from ORD_PAGO where nro_orden='$nro_orden' and anulado='N'"; $res=pg_query($sql); $filas=pg_num_rows($res); 
	if($filas>=1){ $registro=pg_fetch_array($res,0);   $tipo_caus=$registro["tipo_causado"];  $fecha_ord=$registro["fecha"]; $orden_permanen=$registro["orden_permanen"]; $nro_periodos=$registro["nro_periodos"];  $cod_contable_o=$registro["cod_contable_o"]; $tipo_orden=$registro["tipo_orden"];
	  ...
	  if(($tipo_orden=="0002")or($tipo_orden=="0002")or($tipo_orden=="0002")){}else{ $error=1; ?><script language="JavaScript">muestra('TIPO DE ORDEN NO ES VALIDO');</script><? }    
	  ...
	  if($error==0){ $monto_asiento=0; $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')"); $merror=pg_errormessage($conn); $merror=substr($merror, 0, 91); 
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_contable_o','00000','',$monto_nota,'D','B','N','02','0','$descripcion')"); $merror=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($merror,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><? $error=1; }
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','C','$codc_banco','00000','',$monto_nota,'D','B','N','02','0','$descripcion')"); $merror=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($merror,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><? $error=1; }
	  }	 
