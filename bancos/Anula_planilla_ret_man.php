...
$nro_orden=$_GET["orden"]; $nro_planilla=$_GET["nro_planilla"];$tipo_planilla=$_GET["tipo_planilla"];
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  }$posicion=strpos($Mcamino,'S');if(is_numeric($posicion)){$Mcamino=$Mcamino;}else{$error=1;} if($Mcamino{7}=="S"){$error=0;}else{$error=1;}  
  ...
  if($error==0){  $sfecha=$fecha_emision;
     $sSQL="SELECT ACTUALIZA_BAN012(9,'$cod_banco','$tipo_mov','$referencia','$tipo_planilla','$nro_planilla','$ced_rif','$fecha_emision','$nro_orden','$aux_orden','$tipo_retencion','','','','$fecha_emision','','',0,0,0,0,0,0,0,'$minf_usuario','')";
     $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);  $error=substr($error, 0, 61);    if (!$resultado){?> <script language="JavaScript"> muestra('<? echo $error; ?>'); </script> <? } else{?><script language="JavaScript">  muestra('ANULO EXITOSAMENTE'); </script><?
	     $desc_doc="PLANILLA DE RETENCION, BANCO:".$cod_banco.", REFERENCIA:".$referencia.", TIPO MOVIMIENTO:".$tipo_mov.", TIPO PLANILLA:".$tipo_planilla.", NRO. PLANILLA:".$nro_planilla.", MONTO:".$monto_retencion;$resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('02','$usuario_sia','$usuario_sia','$equipo','Anulo','$sfecha','$desc_doc')");
         $error=pg_errormessage($conn); $error=substr($error, 0, 61);  if (!$resultado){?><script language="JavaScript">muestra('<?echo $error;?>');</script><? }
	 }
  }
}
pg_close(); 
error_reporting(E_ALL ^ E_WARNING);?><script language="JavaScript"> window.close(); window.opener.location.reload(); </script>
?>