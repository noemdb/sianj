...
$aptd_postal=$_POST["txtaptd_postal"]; $tipo_orden=$_POST["txttipo_orden"]; $observaciones=""; $status=""; if($pais==""){$pais="VENEZUELA";}$nombre=str_replace("\r\n","",$nombre); $nombre=str_replace("\n","",$nombre);$nombre=str_replace('"','',$nombre); $nombre=str_replace("'","",$nombre);
if($ced_rif_aut==""){$ced_rif_aut=$ced_rif;} if($nombre_autorizado==""){$nombre_autorizado=$nombre;} $campo_str1=$_POST["txtgrupo_banco"]; $campo_str2=$_POST["txtnro_cuenta"];
$equipo = getenv("COMPUTERNAME");$minf_usuario = $usuario_sia." ".$equipo." ".date("d/m/y H:i a"); $fecha=asigna_fecha_hoy();
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CÉDULA/RIF DE BENEFICIARIO YA EXISTE'); </script> <? }
