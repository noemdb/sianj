...
while($registro=pg_fetch_array($res)){ $monto=$registro["monto"]; $monto_presup=$registro["monto_presup"];
  $monto_credito=$registro["monto_credito"];   $amort_anticipo=$registro["amort_anticipo"]; 
  ...
  $tipo_compromiso=$registro["tipo_compromiso"]; $tipo_imput_presu=$registro["tipo_imput_presu"];  $ref_imput_presu=$registro["ref_imput_presu"];  
  $m_cod=$monto_presup-$amort_anticipo;
  if(($m_cod>0)and($resta_abono>0)){
    if($resta_abono>=$m_cod){$m_cod=$m_cod;}else{$m_cod=$resta_abono;}  
