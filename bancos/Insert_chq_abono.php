...
$url="Emision_Cheque_abono_orden.php?continua=N"; $furl="../bancos/rpt/Rpt_formato_chq.php?cod_banco=".$cod_banco."&num_cheque=".$num_cheque;
...
if($error==0){$sql="Select * from ORD_PAGO where nro_orden='$nro_orden' and anulado='N'"; $res=pg_query($sql); $filas=pg_num_rows($res); 
	if($filas>=1){ $registro=pg_fetch_array($res,0);   $tipo_caus=$registro["tipo_causado"];  $fecha_ord=$registro["fecha"]; $clasif_orden=$registro["clasif_orden"];
      ...
      */
	  ...
	  if($error==0){ $monto_asiento=0; $resultado=pg_exec($conn,"SELECT ELIMINA_CON010('$codigo_mov')"); $error=pg_errormessage($conn); $error=substr($error,0,91); 
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','D','$cod_contable_o','00000','',$monto_cheque,'D','B','N','02','0','$descripcion')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }
		$resultado=pg_exec($conn,"SELECT INCLUYE_CON010('$codigo_mov','$nro_orden','C','$codc_banco','00000','',$monto_cheque,'D','B','N','02','0','$descripcion')"); $error=pg_errormessage($conn);  $error="ERROR GRABANDO: ".substr($error,0,91); if(!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><? $error=1; }
	  }	  
	...
	if($clasif_orden=="F"){  $referencia_comp=""; $tipo_compromiso="0000";}
	else{ if(($monto_bruto<>$monto_abono)and($balance>0.001)){$error=1; echo $monto_bruto.' '.$monto_abono.' '.formato_monto($balance),"<br>"; ?> <script language="JavaScript"> muestra('MONTO DEL PAGADO PRESUPUESTARIO NO VALIDO');</script><? } } }
}
