...
$cod_banco=$_POST["txtcod_banco"]; $tipo_mov=$_POST["txttipo_mov"]; $referencia=$_POST["txtreferencia"];
...
$fecha=asigna_fecha_hoy(); $ced_rif =""; $fecha_emision=formato_aaaammdd($fecha);  $nro_comprobante=""; $tipo_planilla="";   $tipo_en="";
...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
    ...
    if($error==0){$sSQL="SELECT codigo_mov,ced_rif,fecha_emision,monto_pago FROM BAN029 WHERE codigo_mov='$codigo_mov' and tipo_retencion='$tipo_retencion' and cod_banco='$cod_banco' and tipo_mov='$tipo_mov' and referencia='$referencia'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
    ...
    if($error==0){$sfechaf=formato_aaaammdd($fecha_factura);
      $sSQL="SELECT MODIFICA_BAN029('$codigo_mov','$cod_banco','$tipo_mov','$referencia','$tipo_retencion','$nro_planilla','$ced_rif','$sfechaf','$nro_orden','$nro_orden','A','$tipo_retencion','$tipo_documento','$nro_documento','$nro_con_factura','$nro_doc_afectado','$sfechaf','00000000','',$monto_pago,$monto_objeto,$tasa_retencion,$monto_retencion,$monto1,$monto2,$monto3)";
