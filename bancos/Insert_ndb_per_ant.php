...
$codigo_mov=$_POST["txtcodigo_mov"]; $tipo_pago=$_POST["txttipo_pago"]; $nro_orden=$_POST["txtnro_orden"]; $cod_banco=$_POST["txtcod_banco"]; $nro_ndb=$_POST["txtnro_ndb"];  $monto=$_POST["txtmonto_nota"];  $fecha=$_POST["txtfecha"]; $ced_rif=$_POST["txtced_rif"]; $descripcion=$_POST["txtconcepto"]; $multiple="N"; $fecha_cheque=$fecha;
$equipo = getenv("COMPUTERNAME"); $minf_usuario=$usuario_sia." ".$equipo." ".date("d/m/y H:i a"); echo "ESPERE POR FAVOR INCLUYENDO....","<br>"; $error=0;
$url="Emision_Ndb_orden_ant.php?continua=S"; $furl="../bancos/rpt/Rpt_formato_mov_libro.php?cod_banco=".$cod_banco."&referencia=".$nro_ndb."&tipo_mov=NDB";
...
if($error==0){if(strlen($nro_ndb)==8){$error=0;} else {$error=1; ?> <script language="JavaScript"> muestra('LONGITUD NUMERO DE CHEQUE INVALIDA');</script><? } }
if($error==0){	
	$sSQL="SELECT cod_banco FROM BAN004 WHERE (cod_banco='$cod_banco') and (referencia='$nro_ndb') and (tipo_mov_libro='NDB')"; 	
	$resultado=pg_query($sSQL); $filas=pg_num_rows($resultado);  if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('NUMERO NOTA DEBITO YA EXISTE EN MOVIMIENTO EN LIBROS');</script><? }
}
...
if($error==0){$tipo_comp="B".$cod_banco;$sSql="Select * from con002 where text(fecha)='$sfecha' and referencia='$nro_ndb' and tipo_comp='$tipo_comp'"; $resultado=pg_query($sSql); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('COMPROBANTE CONTABLE YA EXISTE');</script><? }}
...
if($error==0){ if($tasa_idb>0)  {$mstatus="I";$monto_idb=$total_Cheques*($tasa_idb/100); $monto_idb=Round($monto_idb, 2);} else{$cod_cont_idb=""; $mstatus="N"; $monto_idb=0;}    $sfecha=formato_aaaammdd($fecha);
   $sSQL="SELECT INCLUYE_NDB_PER_ANT('$codigo_mov','$cod_banco','$nro_ndb','$sfecha','$ced_rif',$total,'$st_chq','$usuario_sia','N','','$minf_usuario','$mstatus',$monto_idb,'$cod_cont_idb','$codc_banco','$tipo_pago','$statusc','$nro_orden','$descripcion')"; 
