...
$url="Emision_Cheque_orden_per.php?continua=N"; $furl="../bancos/rpt/Rpt_formato_chq.php?cod_banco=".$cod_banco."&num_cheque=".$num_cheque;
...
if($error==0){$sql="Select * from ORD_PAGO where nro_orden='$nro_orden' and anulado='N'"; $res=pg_query($sql); $filas=pg_num_rows($res); 
	if($filas>=1){ $registro=pg_fetch_array($res,0);   $tipo_caus=$registro["tipo_causado"]; 
	  ...
	  $orden_permanen=$registro["orden_permanen"]; $nro_periodos=$registro["nro_periodos"];  $cod_contable_o=$registro["cod_contable_o"]; 
	  ...
	  $dispo_ord=$total_neto; $total_neto=0;	$resta_abono=$monto_cheque; $monto_abono=0;		  	  
