...
class PDF extends FPDF{ 
	function Header(){global $monto_cheque; global $nombre_benef; global $monto_letras; global $lugar; global $ano; global $nombre_banco; 
	   global $concepto; global $nro_cuenta; global $num_cheque; global $fecha; global $nro_orden;  
		...
		$this->SetFont('lucon','',10);
		$long_line=68; $part1=$monto_letras; $part2=' '; $l=strlen($part1); if($l>$long_line){$part1=substr($monto_letras,0,$long_line); }
		...
		$this->Cell(50, 4, $num_cheque, 0, 0, 'L');
        ...
        $this->Cell(150, 4, $nombre_benef, 0, 1, 'L');
		$this->Cell(200, 3, '', 0, 1, 'L');
		
		$this->Cell(20, 4, '', 0, 0, 'L');
		$this->MultiCell(150,4,$concepto,0); 
		
		$y=$this->GetY();
		if($y<160){$t=160-$y; $this->ln($t);		}
