    ...
    if($tipo_rpt=="1"){  
		  ...
		  class PDF extends FPDF{
			function Header(){ global $Nom_Emp;  global $cod_banco; global $referencia; global $tipo_mov; global $des_tipo_mov; global $monto_mov_libro;
				...
				$this->Image('../../imagenes/logo escudo.png',12,8,23);
				...
				$this->Cell(200,5,'CONTABILIDAD FISCAL',1,1,'C',true);
			...
			function Footer(){ global $total_columna1; global $total_columna2; $ffechar=date("d-m-Y");$fhorar=date("H:i:s a"); 
				...
				$this->Cell(100,3,'ADMINISTRADOR: ',1,1,'C',true);
	...
	if($tipo_rpt=="3"){  
		  ...
		  class PDF extends FPDF{
			function Header(){ global $Nom_Emp;  global $cod_banco; global $referencia; global $tipo_mov; global $des_tipo_mov; global $monto_mov_libro;
				...
				$this->SetFont('Arial','B',13);
				$this->Cell(40);
				$this->Cell(130,10,$des_tipo_mov,0,1,'C');
				$this->Ln(4);
				$this->SetFont('Arial','B',10);
				$this->Cell(25,7,'REFERENCIA:','TB',0,'L');
				$this->SetFont('Arial','',10);
				$this->Cell(33,7,$referencia,'TB',0,'L');
				$this->SetFont('Arial','B',10);
				$this->Cell(32,7,'FECHA EMISION :','TB',0,'L');
				$this->SetFont('Arial','',10);
				$this->Cell(70,7,$fecha,'TB',0,'L');
				$this->SetFont('Arial','B',10);
				$this->Cell(20,7,'MONTO :','TB',0,'L');
				$this->SetFont('Arial','',10);
				$this->Cell(20,7,$monto_mov_libro,'TB',1,'R');
				
				$this->SetFont('Arial','B',10);
				$this->Cell(18,7,'BANCO :','TB',0,'L');
				$this->SetFont('Arial','',10);
				$this->Cell(107,7,$cod_banco." ".$nombre_banco,'TB',0,'L');
				$this->SetFont('Arial','B',10);
				$this->Cell(30,7,'CUENTA NRO. :','TB',0,'L');
				$this->SetFont('Arial','',10);
				$this->Cell(45,7,$nro_cuenta,'TB',1,'L');
				
				$this->SetFont('Arial','B',10);
				$this->Cell(27,7,'BENEFICIARIO: ','TB',0,'L');
				$this->SetFont('Arial','',10);
				$this->Cell(123,7,$nombre_benef,'TB',0,'L');
				$this->SetFont('Arial','B',10);
				$this->Cell(25,7,'CEDULA/RIF : ','TB',0,'L');
				$this->SetFont('Arial','',10);
				$this->Cell(25,7,$ced_rif,'TB',1,'L');
		
				$this->SetFont('Arial','B',10);
				$this->Cell(40,5,"POR CONCEPTO DE : ",0,1);
				$this->SetFont('Arial','',10);
				$this->MultiCell(200,4,$descripcion,0);
				$this->Cell(200,2,' ',0,1,'C');
				$this->SetFont('Arial','B',10);
				$this->Cell(200,5,'CONTABILIDAD FISCAL','TB',1,'C');
				$this->SetFont('Arial','B',9);
				$this->Cell(30,5,'CODIGO CUENTA','TB',0,'C');
				$this->Cell(130,5,'NOMBRE CUENTA','TB',0,'C');	
				$this->Cell(20,5,'DEBE','TB',0,'C');
				$this->Cell(20,5,'HABER','TB',1,'C');
			}
		...
		while($registro=pg_fetch_array($res)){ $i=$i+1; 
             ...
             if($php_os=="WINNT"){$nombre_cuenta=$registro["nombre_cuenta"];}else{$nombre_cuenta=utf8_decode($registro["nombre_cuenta"]); } 		     
			 $pdf->SetFont('Arial','',9);	
		   	 $pdf->Cell(30,5,$codigo_cuenta,0,0);
			 $x=$pdf->GetX();   $y=$pdf->GetY(); $n=130; 			   
			 $pdf->SetXY($x+$n,$y);
			 $pdf->Cell(20,5,$debe,0,0,'R');
             $pdf->Cell(20,5,$haber,0,1,'R'); 				
			 $pdf->SetXY($x,$y);
			 $pdf->MultiCell($n,5,$nombre_cuenta,0); 
		} 
		$pdf->Output();     
	}
	...
	if($tipo_rpt=="2"){  
		  ...
		  class PDF extends FPDF{
			function Header(){ global $Nom_Emp;  global $cod_banco; global $referencia; global $tipo_mov; global $des_tipo_mov; global $monto_mov_libro;
				...
				$this->Cell(200,3,' ','B',1,'C');
				$this->SetFillColor(192,192,192);
				$this->Cell(200,5,'CONTABILIDAD PRESUPUESTARIA',1,1,'C',true);
       ...
       $pdf->Cell(200,5,'CONTABILIDAD FISCAL',1,1,'C',true);
