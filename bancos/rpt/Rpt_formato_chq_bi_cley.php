...
class PDF extends FPDF{ 
	function Header(){global $monto_cheque; global $nombre_benef; global $monto_letras; global $lugar; global $ano; global $nombre_banco; 
	   global $concepto; global $nro_cuenta; global $num_cheque; global $fecha; global $nro_orden; global $ced_rif; 
		...
		//$this->ln(02);
          $this->Cell(190,12,'',0,1,'L');
		  $this->Cell(88,4,'',0,0,'L');
          //$this->Cell(12, 4, $num_cheque, 0, 0, 'L');	
		...
		//$this->Cell(70,4,'CADUCA A LOS 45 DIAS',0,1,'L');
		//$this->ln(3);
		...
		$this->Cell(70,4,'NO ENDOSABLE',0,1,'L');
        $this->Cell(65,4,'',0,0,'L');
		$this->Cell(70,4,'CADUCA A LOS 45 DIAS',0,1,'L');
		//$this->ln(3);
		...
		$this->Cell(200,20,'',0,1,'L');   //CAMBIAR PARA SUBIR LE COLOCA MENOS de 20
		...
		$y=$this->GetY();
		if($y<140){$t=140-$y; $this->ln($t);}  //CAMBIAR PARA SUBIR LE COLOCA MENOS de 140
		
		//$this->Cell(30, 4, $nro_orden, 0, 0, 'L');
		$this->Cell(30, 4," ", 0, 1, 'L');
		$this->Cell(30, 4, $num_cheque, 0, 0, 'L');
		$this->Cell(60, 4, $nombre_banco, 0, 1, 'L');
		//$this->Cell(60, 4, $nro_cuenta, 0, 0, 'L');
		
		
		
        //$this->Cell(100, 4, $nombre_benef, 0, 0, 'L');
...
if ($cant_cont > $max_cpre) {
	...
	for ($k = 0; $k < $max_cpre; $k++) {
		$pdf->Cell(55, 3, $cod_p[$k], 0, 0, 'L');
		$pdf->Cell(80, 3, $den_p[$k], 0, 0, 'L');
...
if ($cant_cont > $max_ccont) {
	...
	$m = ($l / 2);
	$pdf->Ln($m);
	$pdf->Cell(190, 3, 'VER RELACION ANEXA', 0, 1, 'C');
	$pdf->Ln($m);
} else {
	for ($k = 0; $k < $max_ccont; $k++) {
		$pdf->Cell(44, 3, $cod_c[$k], 0, 0, 'L');
		$pdf->Cell(80, 3, $den_c[$k], 0, 0, 'L');
		$pdf->Cell(25, 3, $debe_c[$k], 0, 0, 'R');
		$pdf->Cell(10, 3, $haber_c[$k], 0, 1, 'R');
	}
}
