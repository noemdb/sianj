...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
    ...
    if($tipo_rep=="HTML"){	include ("../../class/phpreports/PHPReportMaker.php");  
          ...
          $oRpt->setXML("Rpt_Disponibilidad_Bancaria_Diaria.xml");
          $oRpt->setUser("$user");
   ...
   if($tipo_rep=="PDF"){	
      ...
      class PDF extends FPDF{ 
		function Header(){ global $criterio1; global $tam_logo; 
			...
			$this->Cell(30);
			$this->Cell(150,10,'REPORTE DISPONIBILIDAD BANCARIA DIARIA',1,0,'C');
			$this->Ln(18);
			$this->SetFont('Arial','B',8);
			$this->Cell(100,5,'FECHA: '.$criterio1,0,1,'L');
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1;
		   $cod_banco=$registro["cod_banco"]; $nombre_banco=$registro["nombre_banco"]; $nro_cuenta=$registro["nro_cuenta"]; $cod_contable=$registro["cod_contable"]; 
            $saldo_ini=$registro["saldo_ini"]; $monto4=$registro["monto4"]; $monto5=$registro["monto5"]; $saldo_act=$registro["saldo_act"]; 
		   $total=$total+$saldo_act; 
    ...
    if($tipo_rep=="EXCEL"){	$criterio1='FECHA: '.$criterio1;
	  header("Content-type: application/vnd.ms-excel");
      header("Content-Disposition: attachment; filename=Rpt_Disponibilidad_Bancaria_Diaria.xls");	
