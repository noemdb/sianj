...
$periodoh=$_GET["periodoh"];$fecha_d=$_GET["fecha_d"];$fecha_h=$_GET["fecha_h"];$subtotales=$_GET["subtotales"];$ordenarcheque=$_GET["ordenarcheque"];$imprimirbene=$_GET["imprimirbene"];$imprimircuen=$_GET["imprimircuen"];$imprimirorden=$_GET["imprimirorden"];$imp_sinmov=$_GET["imp_sinmov"]; $tipo_rep=$_GET["tipo_rep"];
$criterio1="";$Sql="";
...
if($fecha_d==""){$sfecha_d="2014-01-01";}else{$sfecha_d=formato_aaaammdd($fecha_d);}  
...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
   ...
   $Sql="SELECT RPT_MOV_LIBRO_BAN035('".$codigo_mov."','0','".$sfecha_d."','".$sfecha_h."','".$cod_banco_d."','".$cod_banco_h."','".$tipo_mov_d."','".$tipo_mov_h."','".$imprimirorden."','".$imp_sinmov."')";
   ...
   //$Sql="SELECT RPT_MOV_LIBRO_ban035('".$codigo_mov."','0','".$sfecha_d."','".$sfecha_h."','".$cod_banco_d."','".$cod_banco_h."','".$tipo_mov_d."','".$tipo_mov_h."','".$imprimirorden."','".$imp_sinmov."')";
	...
	$sSQL = "SELECT ban002.cod_banco, ban002.nombre_banco, ban002.nro_cuenta,ban035.nro_linea, ban035.tipo_mov_libro, ban035.referencia,ban035.descrip_mov_libro, ban035.fecha_mov_libro,ban035.monto_mov_libro,
				ban035.columna1, ban035.columna2, ban035.columna3, ban035.columna4, ban035.columna5, ban035.ced_rif, ban035.anulado, ban035.aoperacion, ban035.doperacion, ban035.por_emision, ban003.tipo, ban003.operacion, pre099.nombre, 
				extract(month from fecha_mov_libro) as mes_mov, to_char( ban035.fecha_mov_libro,'DD/MM/YYYY') as fecham ".$des_mov."
                FROM ban002 LEFT JOIN ban035 ON ban002.cod_banco=ban035.cod_banco LEFT JOIN ban003 ON ban003.Tipo_Movimiento = ban035.tipo_mov_libro
                LEFT JOIN pre099 ON ban035.ced_rif = pre099.ced_rif
                WHERE ban035.codigo_mov='".$codigo_mov."' AND ban002.cod_banco>='".$cod_banco_d."' AND ban002.cod_banco<='".$cod_banco_h."' AND ban035.ced_rif>='".$cedula_d."' AND ban035.ced_rif<='".$cedula_h."' AND
                ...
                ban035.fecha_mov_libro>='".$sfecha_d."' AND ban035.fecha_mov_libro<='".$sfecha_h."'  ORDER BY ban002.cod_banco,ban035.nro_linea";
   ...
   if($tipo_rep=="PDF"){  $res=pg_query($sSQL); $filas=pg_num_rows($res); $cod_banco_grupo=""; $nombre_banco_grupo=""; $nro_cuenta_grupo=""; $prev_col3=0; $prev_col3=0; $prev_col5=0; $saldo_anterior=0;
		  ...
		  class PDF extends FPDF{
			function Header(){global $tam_logo;  global $criterio1; global $cod_banco_grupo; global $nombre_banco_grupo; global $nro_cuenta_grupo; global $saldo_anterior; 
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1;  
		       ...
		       $mes_mov=$registro["mes_mov"]; $des_mov=$registro["descrip_mov_libro"]; $monto_mov_libro=$registro["monto_mov_libro"];
			   $columna1=$registro["columna1"]; $columna2=$registro["columna2"]; $columna4=$registro["columna4"];  $por_emision=$registro["por_emision"];
			   if($imp_benef=="S"){ $des_mov=$nombre.', '.$descrip_mov_libro; }
			   if($imp_benef=="B"){ $des_mov=$nombre; }
			   if(($Cod_Emp=="A2")and(($tipo_mov_libro=="CHQ")or($tipo_mov_libro=="NDB"))and($por_emision=="S")){ $morden="";			   
				   $sqlbc="SELECT nro_orden, tipo_causado, fecha_cheque, cod_banco, nro_cheque,tipo_pago From pag007 Where (cod_banco='$cod_banco') And (nro_cheque='$referencia') and (tipo_pago='$tipo_mov_libro')"; $resbc=pg_exec($conn,$sqlbc);$filasbc=pg_numrows($resbc);
                   if($filasbc>=1){$regbc=pg_fetch_array($resbc); $morden=$regbc["nro_orden"]; }
		           if($morden<>""){$des_mov=$des_mov.', Orden: '.$morden; }	
			   }
    ...
    $Sql="Delete from ban035 Where (codigo_mov='".$codigo_mov."')"; $resultado=pg_exec($conn,$Sql);  $error=pg_errormessage($conn); 	
}
?>
