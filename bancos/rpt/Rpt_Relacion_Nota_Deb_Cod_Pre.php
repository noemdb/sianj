...
$cod_banco_d=$_GET["cod_banco_d"];$cod_banco_h=$_GET["cod_banco_h"];$referencia_d=$_GET["referencia_d"];$referencia_h=$_GET["referencia_h"];$cedula_d=$_GET["cedula_d"];$cedula_h=$_GET["cedula_h"];$fecha_d=$_GET["fecha_d"];$fecha_h=$_GET["fecha_h"];$tipo_rep=$_GET["tipo_rep"];$Sql="";
...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
   ...
   if(($en_d==1)or($en_h==1)){$codigo_d=str_replace("?","0",$cod_presup_d); $long_d=strlen($codigo_d); $codigo_h=str_replace("?","9",$cod_presup_h); $long_h=strlen($codigo_h);
    ...
    $sSQL = "SELECT BAN004.Cod_Banco, BAN002.Nombre_Banco, BAN002.Nro_Cuenta, BAN004.Referencia, BAN004.Tipo_Mov_Libro, BAN004.Fecha_Mov_Libro, BAN004.Descrip_Mov_Libro, BAN004.Anulado, BAN004.Monto_Mov_Libro*-1 as monto_mov_anu,
                BAN004.Fecha_Anulado, BAN004.Monto_Mov_Libro, PRE008.Tipo_Pago, PRE038.Cod_Presup, PRE038.fuente_financ, PRE022.Denominacion_P, PRE038.Monto, PRE008.Ced_Rif, PRE099.Nombre, to_char(BAN004.Fecha_Mov_Libro,'DD/MM/YYYY') as fecham, text(BAN004.Referencia)||text(BAN004.Tipo_Mov_Libro) as ref_grupo
                FROM BAN002 BAN002, BAN004 BAN004, PRE022 PRE022, PRE008 PRE008, PRE038 PRE038, PRE099 PRE099 WHERE (BAN004.Cod_Banco = BAN002.Cod_Banco) AND (PRE008.Ced_Rif = PRE099.Ced_Rif) AND
                (PRE022.Cod_Presup_P = PRE038.Cod_Presup) AND (PRE022.Cod_Fuente_P = PRE038.Fuente_Financ)  AND (BAN004.Referencia = PRE008.Referencia_Pago) AND (PRE008.Referencia_Pago = PRE038.Referencia_Pago)   AND (pre038.tipo_pago=pre008.tipo_pago) AND
                ...
                (pre038.Cod_Banco = PRE008.Cod_Banco) AND (BAN004.Fecha_Mov_Libro = PRE008.Fecha_Pago) AND
                (BAN004.Cod_Banco = PRE008.Cod_Banco) AND ((BAN004.Tipo_Mov_Libro='NDB') OR (BAN004.Tipo_Mov_Libro='AND')) AND ((SUBSTRING(BAN004.Cod_BancoA,1,1)='A') or (SUBSTRING(BAN004.Cod_BancoA,1,1)='D') or (SUBSTRING(BAN004.Cod_BancoA,1,1)='P') )  AND  
				BAN004.Cod_Banco>='".$cod_banco_d."' AND  BAN004.Cod_Banco<='".$cod_banco_h."'  AND  BAN004.referencia>='".$referencia_d."' AND BAN004.referencia<='".$referencia_h."' AND PRE008.Ced_Rif>='".$cedula_d."' AND PRE008.Ced_Rif<='".$cedula_h."'  AND  
    ...
    if($tipo_rep=="HTML"){	include ("../../class/phpreports/PHPReportMaker.php");  //echo $sSQL;		
             ...
             $oRpt->setXML("Rpt_Relacion_Nota_Deb_Cod_Presup.xml");
             $oRpt->setUser("$user");
	...
	if($tipo_rep=="PDF"){  $res=pg_query($sSQL); $filas=pg_num_rows($res); $cod_banco_grupo="0000"; $nombre_banco_grupo=""; $nro_cuenta_grupo="00000000"; 
		  ...
		  class PDF extends FPDF{
			function Header(){ global $tam_logo;  global $criterio1; global $cod_banco_grupo; global $nombre_banco_grupo; global $nro_cuenta_grupo; global $criterio1; 
				...
				$this->Cell(30);
				$this->Cell(150,10,'RELACION NOTAS DEBITO / CODIGOS PRESUPUESTARIO',1,0,'C');
				$this->Ln(18);
		  ...
		  $pdf->AddPage();
		  $pdf->SetFont('Arial','',7);
		  $i=0;  $total1=0; $total2=0; $sub_total1=0;  $sub_total2=0;  $prev_cod_banco="";  $prev_tipo_mov_libro=""; $prev_nombre_banco=""; $prev_nro_cuenta="";
		  while($registro=pg_fetch_array($res)){ $i=$i+1;  $cod_banco=$registro["cod_banco"];  $nombre_banco=$registro["nombre_banco"]; $nro_cuenta=$registro["nro_cuenta"];
		       $tipo_mov_libro=$registro["tipo_mov_libro"]; $referencia=$registro["referencia"]; $fecham=$registro["fecham"]; $ref_grupo=$registro["ref_grupo"];
		       $descrip_mov_libro=$registro["descrip_mov_libro"]; $monto_mov_libro=$registro["monto_mov_libro"]; $ced_rif=$registro["ced_rif"]; $nombre=$registro["nombre"];
		       ...
		       $cod_banco_grupo=$cod_banco; $nombre_banco_grupo=$nombre_banco; $nro_cuenta_grupo=$nro_cuenta; $tipo_mov_libro_grupo=$tipo_mov_libro; 
		       $referencia_grupo=$referencia; 	$fecham_grupo=$fecham; $descrip_mov_libro_grupo=$descrip_mov_libro; $monto_mov_libro_grupo=$monto_mov_libro; 
               $nombre_grupo=$nombre; $ced_rif_grupo=$ced_rif; $monto_mov_libro_grupo=$monto_mov_libro; 
			   ...
			   if($prev_tipo_mov_libro<>$ref_grupo){ $monto_mov_libro_grupo=$registro["monto_mov_libro"]; if($registro["tipo_mov_libro"]=="NDB"){$monto_mov_libro_grupo=$monto_mov_libro_grupo;}else{$monto_mov_libro_grupo=$monto_mov_libro_grupo*-1;} 
			      $sub_total2=$sub_total2+$monto_mov_libro_grupo; $total2=$total2+$monto_mov_libro_grupo; $monto_mov_libro_grupo=formato_monto($monto_mov_libro_grupo);
				  if($prev_tipo_mov_libro<>''){$pdf->Ln(2);}
		       ...
		       $fecham=$registro["fecham"]; $tipo_mov_libro=$registro["tipo_mov_libro"]; $referencia=$registro["referencia"]; $cod_presup=$registro["cod_presup"]; $fuente_financ=$registro["fuente_financ"]; 
			   $denominacion_p=$registro["denominacion_p"]; $monto_mov_libro=$registro["monto_mov_libro"]; $monto=$registro["monto"]; 
			   $sub_total1=$sub_total1+$monto;  $total1=$total1+$monto; 
			...
			} $total1=formato_monto($total1); $total2=formato_monto($total2);
			$pdf->SetFont('Arial','B',7);
			if(($sub_total1>0)or($sub_total2>0)){ $sub_total1=formato_monto($sub_total1); $sub_total2=formato_monto($sub_total2);				    
				$pdf->Cell(160,2,'',0,0);
				$pdf->Cell(20,2,'--------------------',0,0,'R');
			...
			$pdf->Cell(20,5,$total2,0,1,'R');
			$pdf->Output();  
		}
    if($tipo_rep=="EXCEL"){	
		  header("Content-type: application/vnd.ms-excel");
		  header("Content-Disposition: attachment; filename=Rpt_Relacion_Nota_Deb_Cod_Presup.xls");
		  ?>
		   <table border="0" cellspacing='0' cellpadding='0' align="left">
			 <tr height="20">
				<td width="100" align="left" ><strong></strong></td>
				<td width="200" align="left" ><strong></strong></td>
				<td width="400" align="center" > <font size="4" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong>RELACION NOTAS DEBITO / CODIGOS PRESUPUESTARIO</strong></font></td>
			 </tr>
			 <tr height="20">
				<td width="100" align="left" ><strong></strong></td>
				<td width="200" align="left" ><strong></strong></td>
			 ...
			 <tr height="20">
			   ...
			   <td width="200" align="left" bgcolor="#99CCFF"><strong>Fecha Emision</strong></td>
		  ...
		  <?  $i=0;  $total1=0; $total2=0; $sub_total1=0;  $sub_total2=0;  $prev_cod_banco="";  $prev_nombre_banco=""; $prev_nro_cuenta=""; $prev_tipo_mov_libro=""; $res=pg_query($sSQL);
		  while($registro=pg_fetch_array($res)){ $i=$i+1;  $cod_banco=$registro["cod_banco"];  $nombre_banco=$registro["nombre_banco"]; $nro_cuenta=$registro["nro_cuenta"];
		       $tipo_mov_libro=$registro["tipo_mov_libro"]; $referencia=$registro["referencia"]; $fecham=$registro["fecham"]; $ref_grupo=$registro["ref_grupo"];
		       $descrip_mov_libro=$registro["descrip_mov_libro"]; $monto_mov_libro=$registro["monto_mov_libro"];
			   $nombre_banco=conv_cadenas($nombre_banco,0); $descrip_mov_libro=conv_cadenas($descrip_mov_libro,0); 
		       $cod_banco_grupo=$cod_banco; $nombre_banco_grupo=$nombre_banco; $nro_cuenta_grupo=$nro_cuenta; $tipo_mov_libro_grupo=$tipo_mov_libro; 
		       $referencia_grupo=$referencia; 	$fecham_grupo=$fecham; $descrip_mov_libro_grupo=$descrip_mov_libro; $monto_mov_libro_grupo=$monto_mov_libro; 
			    ...
			    if(($sub_total1>0)or($sub_total2>0)){ $sub_total1=formato_monto($sub_total1); $sub_total2=formato_monto($sub_total2);	
			     ?>	 				 
			      ...
			      <tr>
			          <td width="100" align="left"></td>
				      <td width="200" align="left"></td>
			          <td width="400" align="right"><strong><? echo "Total Banco: ".$prev_cod_banco."    ".$prev_nombre_banco."    ".$prev_nro_cuenta; ?></strong></td>
			   ...
			   if($prev_tipo_mov_libro<>$ref_grupo){ $monto_mov_libro_grupo=$registro["monto_mov_libro"]; if($registro["tipo_mov_libro"]=="NDB"){$monto_mov_libro_grupo=$monto_mov_libro_grupo;}else{$monto_mov_libro_grupo=$monto_mov_libro_grupo*-1;} 
			     $sub_total2=$sub_total2+$monto_mov_libro; $total2=$total2+$monto_mov_libro; $monto_mov_libro_grupo=formato_monto($monto_mov_libro_grupo);
			      ?>	   
			      <tr>
				  <td width="100" align="left">'<font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><? echo $referencia; ?></td>
				  <td width="200" align="center"><? echo $fecham; ?></td>
		       ...
		       $descrip_mov_libro=$registro["descrip_mov_libro"]; $num_cheque=$registro["referencia"];  
			   $nombre=$registro["nombre"]; $cod_banco=$registro["cod_banco"];  $nombre_banco=$registro["nombre_banco"]; $nro_cuenta=$registro["nro_cuenta"];
		       $fecham=$registro["fecham"]; $tipo_mov_libro=$registro["tipo_mov_libro"]; $referencia=$registro["referencia"]; $cod_presup=$registro["cod_presup"]; 
			   $denominacion_p=$registro["denominacion_p"]; $monto_mov_libro=$registro["monto_mov_libro"]; $monto=$registro["monto"]; 
			   $sub_total1=$sub_total1+$monto;  $total1=$total1+$monto; $monto=formato_monto($monto);  $monto_mov_libro=formato_monto($monto_mov_libro);	$denominacion_p=conv_cadenas($denominacion_p,0); 
			   ?>	   
				<tr>
