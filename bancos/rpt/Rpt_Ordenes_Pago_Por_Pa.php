...
$nro_orden_d=$_GET["nro_orden_d"];$nro_orden_h=$_GET["nro_orden_h"];$fecha_d=$_GET["fecha_d"];$fecha_h=$_GET["fecha_h"];$cedula_d=$_GET["cedula_d"];$cedula_h=$_GET["cedula_h"];$tipo_orden_d=$_GET["tipo_orden_d"];$tipo_orden_h=$_GET["tipo_orden_h"];$ordenes=$_GET["ordenes"];$tipo_rpt=$_GET["tipo_rpt"];
...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   $sSQL = "SELECT pag001.FECHA, pag001.nro_orden, PRE099.nombre, pag001.concepto, to_char(fecha,'DD/MM/YYYY') as fechae,
            ...
            FROM pag001, PRE099 WHERE pag001.Ced_Rif = PRE099.Ced_Rif and
            pag001.nro_orden>='".$nro_orden_d."' and pag001.nro_orden<='".$nro_orden_h."' and
            pag001.FECHA>='".$fecha_desde."' and pag001.FECHA<='".$fecha_hasta."'and
            pag001.Ced_Rif>='".$cedula_d."' and pag001.Ced_Rif<='".$cedula_h."' and
            pag001.TIPO_CAUSADO>='".$tipo_orden_d."' and pag001.TIPO_CAUSADO<='".$tipo_orden_h."' and ".$status_pag."  ORDER BY pag001.nro_orden";
   ...
   if($tipo_rpt=="PDF"){	
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1;
		   ...
		   $x=$pdf->GetX();   $y=$pdf->GetY(); $n=50; $w=100;		   
		   $pdf->SetXY($x+$w+$n,$y);		   
