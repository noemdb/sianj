...
$equipo = getenv("COMPUTERNAME"); $minf_usuario = $usuario_sia." ".$equipo." ".date("d/m/y H:i a"); echo "ESPERE POR FAVOR MODIFICANDO....","<br>";$error=0;
...
if (pg_ErrorMessage($conn)) { ?>  <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('CODIGO DE MOVIMIENTO NO EXISTE');  </script> <? }
   else{$error=0;  if($error==0){$sSQL="SELECT ACTUALIZA_ban015(2,'$cod_movimiento','$linea','$descripcion','$cod_grupo','$operacion','$tipo_operacion','$activo','$modulo','$signo','$cod_contab','$cod_contable','$tipo_mov',$monto,$acumulado,'$cod_titulo','$cargable','$minf_usuario')";  echo $sSQL,"<br>";  $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);$error=substr($error, 0, 61);
