...
if(($error==0)and($tipo_mov=="CAN")){$sSQL="SELECT cod_banco FROM BAN004 WHERE cod_banco='$cod_banco' and referencia='$referencia' and tipo_mov_libro='CHQ'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CHEQUE EXISTE EN MOVIMIENTO EN LIBROS');</script><? }
$sSQL="SELECT cod_banco FROM BAN005 WHERE cod_banco='$cod_banco' and referencia='$referencia' and tipo_mov_banco='CHQ'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CHEQUE EXISTE EN MOVIMIENTO EN BANCOS');</script><? }
$sSQL="SELECT cod_banco FROM BAN007 WHERE cod_banco='$cod_banco' and referencia='$referencia' and tipo_mov_banco='CHQ'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CHEQUE NO EXISTE EN TRANSITO LIBROS');</script><? } }
