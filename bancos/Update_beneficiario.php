...
$aptd_postal=$_POST["txtaptd_postal"];$tipo_orden=$_POST["txttipo_orden"];$observaciones="";$status="";if($pais==""){$pais="VENEZUELA";}if($ced_rif_aut==""){$ced_rif_aut=$ced_rif;}
$nombre=str_replace("\r\n","",$nombre); $nombre=str_replace("\n","",$nombre);$nombre=str_replace('"','',$nombre); $nombre=str_replace("'","",$nombre);  $campo_str1=$_POST["txtgrupo_banco"]; $campo_str2=$_POST["txtnro_cuenta"];
if($nombre_autorizado==""){$nombre_autorizado=$nombre;}
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CÉDULA/RIF DE BENEFICIARIO NO EXISTE'); </script> <? }
   else{ $registro=pg_fetch_array($resultado,0); $nom_ant=$registro["nombre"]; $error=1;
     ...
     $sSQL="SELECT ACTUALIZA_PRE099(2,'$ced_rif','$nombre','$cedula','$rif','$nit','$direccion','$tipo_benef','$ced_rif_aut','$nombre_autorizado','$ciudad','$municipio','$region','$estado','$pais','$telefono','$fax','$tlf_movil','$pasaporte','$nacionalidad','$residente','$observaciones','$clasificacion','$rep_legal','$cod_postal','$aptd_postal','$sfecha','$tipo_orden','$status','','','$campo_str1','$campo_str2',0,0,'$minf_usuario')";
     $resultado=pg_exec($conn,$sSQL);$error=pg_errormessage($conn);$error=substr($error, 0, 91);
     if (!$resultado){ ?> <script language="JavaScript"> muestra('<? echo $error; ?>'); </script> <? }
      else{?><script language="JavaScript">muestra('MODIFICO EXITOSAMENTE');</script><? $error=0;         $desc_doc="CEDULA/RIF:".$ced_rif.",  NOMBRE BENEFICIARIO:".$nom_ant;
...
pg_close();
if ($error==0){?><script language="JavaScript">document.location ='Mod_beneficiario.php?Gced_rif=<?echo $ced_rif?>';</script> <? }
