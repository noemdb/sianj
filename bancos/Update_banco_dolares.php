...
if (pg_ErrorMessage($conn)) { ?>  <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
 else{  $sSQL="Select * from ban042 WHERE cod_banco='$cod_banco'"; $resultado=pg_exec($conn,$sSQL);$filas=pg_numrows($resultado);
  if ($filas==0){$error=1; ?>  <script language="JavaScript">  muestra('CODIGO DE BANCO NO EXISTE');  </script> <? }
     ...
     if($error==0){$sSQL="SELECT ACTUALIZA_ban042(2,'$cod_banco','$nombre_banco','$nro_cuenta','$descripcion_banco','$tipo_cuenta','$cod_contable','$tipo_bco','00000000','00000000','N','','S','$formato_cheque','$sfecha','$sfecha',$s_inic_libro,$s_inic_banco,'$campo_str1','$campo_str2',$campo_num1,$campo_num2,'$minf_usuario')"; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);$error=substr($error, 0, 61);
       echo $sSQL;
	   if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?}else{$error=0;?><script language="JavaScript">muestra('MODIFICO EXITOSAMENTE');</script><?
		...
		$desc_doc="DEFINICION CUENTA BANCO DOLARES, CODIGO:".$cod_banco.", NOMBRE:".$anombre.", CODIGO CONTABLE:".$c_contable.", SALDO ANT. LIBRO:".$saldo_ant_libro.", SALDO ANT. LIBRO:".$saldo_ant_banco;$resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('02','$usuario_sia','$usuario_sia','$equipo','Modifico','$sfecha','$desc_doc')");
