...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
 ...
 if($error==0){
  $sSQL="Select * from NOM070 WHERE codigo_mov='$codigo_mov' and cod_empleado='$cod_empleado' and fecha_desde='$fechad'";
  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('EXPERIENCIA LABORAL YA EXISTE');</script><? }
   else{$sfechad=formato_aaaammdd($fechad);  $sfechah=formato_aaaammdd($fechah);
      $sSQL="SELECT ACTUALIZA_NOM070(1,'$codigo_mov','$cod_empleado','$sfechad','$sfechah','$empresa','$departamento','$cargo',$sueldo)"; echo $sSQL;
