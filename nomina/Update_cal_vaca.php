...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if($error==0){ $afechac=$fechacd;
     ...
     $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn); $merror="ERROR ELIMINANDO: ".substr($error, 0, 61); if (!$resultado){$error=1;?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><?}else{$error=0;}
  } 
