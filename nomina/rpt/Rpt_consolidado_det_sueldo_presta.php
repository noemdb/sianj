...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
   ...
   $criterio=$criterio." and (cod_concepto>='209' and cod_concepto<='217')";
   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Cell(50);
			$this->Cell(160,7,"DETALLE SUELDO PRESTACIONES MES ACTUAL",1,0,'C');
			$this->Ln(17);
			...
			$this->Cell(18,4,'SUELDO','T',0,'C',true);			
			$this->Cell(30,4,'SALARIO PROMEDIO','T',0,'C',true);
			$this->Cell(22,4,'BONIF. FIN DE AÑO','T',0,'C',true);
			$this->Cell(22,4,'BONO VACACIONAL','T',0,'C',true);
			$this->Cell(22,4,'BONO 12 MESES','T',0,'C',true);
			$this->Cell(20,4,'5 DIAS','T',0,'C',true);
			$this->Cell(20,4,'SALARIO','RT',1,'C',true);			
           
            $this->Cell(14,4,'CODIGO','LB',0,'L',true);
			$this->Cell(78,4,'NOMBRE TRABAJADOR','B',0,'L',true);
			$this->Cell(14,4,'INGRESO','B',0,'L',true);		
			
			$this->Cell(18,4,'BASE','B',0,'C',true);
			$this->Cell(30,4,'P/PRESTACIONES','B',0,'C',true);			
			$this->Cell(22,4,'P/PRESTACIONES','B',0,'C',true);
			$this->Cell(22,4,'P/PRESTACIONES','B',0,'C',true);
			$this->Cell(22,4,'P/PRESTACIONES','B',0,'C',true);
			$this->Cell(20,4,'PRESTACIONES','B',0,'C',true);
			$this->Cell(20,4,'PRESTACIONES','BR',1,'C',true);					   
		}
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		...
		if($prev_cod_empleado<>$cod_emp){ $cant_emp=$cant_emp+1;
			...
			$pdf->Cell(18,4,$sueldoc,0,0,'R');
			$pdf->Cell(16,4,$conc1,0,0,'R');
			$pdf->Cell(14,4,$conc2,0,0,'R');
			$pdf->Cell(22,4,$conc3,0,0,'R');
			$pdf->Cell(22,4,$conc4,0,0,'R');
			$pdf->Cell(22,4,$conc5,0,0,'R');
			...
			if($prev_dep<>$cod_dep){ $sub_tot6=formato_monto($sub_tot6);			
			  $pdf->Cell(30,4,'',0,0);
			  $pdf->Cell(190,4,$prev_dep.'    '.$prev_den_dep,0,0);			
			  $pdf->Cell(20,4,$sub_tot6,'T',1,'R');
			  $pdf->Ln(3);
		...
		if($cod_concepto=="209") { if(($valoru==0)and($concepto_vac=="S")){ $valoru=$monto*30; } $conc1=$conc1+$valoru; }
		if($cod_concepto=="209") { $conc2=$conc2+$monto; }
		if($cod_concepto=="211") { $conc3=$conc3+$monto; }
	    if($cod_concepto=="213") { $conc4=$conc4+$monto; }
	    if($cod_concepto=="215") { $conc5=$conc5+$monto; }
	    if($cod_concepto=="217") { $conc6=$conc6+$tmonto; }		
	    if($cod_concepto=="217") { $conc7=$conc7+$monto; }			
	  }		
	  ...
	  $sub_tot1=$sub_tot1+$conc1; $sub_tot2=$sub_tot2+$conc2; $sub_tot3=$sub_tot3+$conc3; $sub_tot4=$sub_tot4+$conc4; $sub_tot5=$sub_tot5+$conc5; $sub_tot6=$sub_tot6+$conc6;  $sub_tot7=$sub_tot7+$conc7; 
		...
		$pdf->Cell(20,4,$conc6,0,0,'R');
		$pdf->Cell(20,4,$conc7,0,1,'R');     
		$sub_tot6=formato_monto($sub_tot6);			
	    $pdf->Cell(30,4,'',0,0);
	    $pdf->Cell(190,4,$prev_dep.'    '.$prev_den_dep,0,0);			
	    $pdf->Cell(20,4,$sub_tot6,'T',1,'R');	
		$pdf->Ln(2);
