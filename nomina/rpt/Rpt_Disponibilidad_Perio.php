...
if ($_GET){$cod_presup_d=$_GET["cod_presupd"];$cod_presup_h=$_GET["cod_presuph"];$cod_fuente_d=$_GET["cod_fuented"];$cod_fuente_h=$_GET["cod_fuenteh"];$mes_desde=$_GET["mes_desde"];$mes_hasta=$_GET["mes_hasta"];$asig_global=$_GET["asig_global"]; $c_cat=$_GET["csubtotal"]; $tipo_rpt=$_GET["tipo_rpt"];} 
else{$codigod="";$codigoh="";$fuented="";$fuenteh="";$fecha=""; $cant_cat=1; $tipo_rpt="HTML";}   $equipo=getenv("COMPUTERNAME"); $cod_mov="PRE020".$usuario_sia; 
//$asig_global="N"; 
...
if (pg_ErrorMessage($conn)){$error=1;?> <script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script> <? } else { $php_os=PHP_OS; $Nom_Emp=busca_conf(); if($utf_rpt=="SI"){if($php_os=="WINNT"){$php_os="LINUX";}else{$php_os="WINNT";}} }
   ...
   $criterio1="Desde: ".$mesd." Hasta: ".$mesh; $criterio2="USUARIO: ".$usuario_sia; 
  ...
  while($registro=pg_fetch_array($res)){ $cod_presup=$registro["cod_presup"]; $denominacion=$registro["denominacion"]; 
    ...
    if($tipo_rpt=="HTML"){ include ("../../class/phpreports/PHPReportMaker.php");
		...
		if($c_cat==0){$oRpt->setXML("Rpt_Disponibilidad_Periodo.xml");}else{$oRpt->setXML("Rpt_Disponibilidad_Periodo_Subt.xml");}
        $oRpt->setUser("$user");
	...
	if($tipo_rpt=="PDF"){	$res=pg_query($sSQL); $filas=pg_num_rows($res); if($filas>=1){ $registro=pg_fetch_array($res,0); }
      ...
      class PDF extends FPDF{ 
		function Header(){global $criterio1; global $criterio2; global $registro; global $c_cat; global $criterio_s; global $mostrar;
