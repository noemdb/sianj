...
$sueldo_adic=0;  $valor_formula=36000; $fecha_ley="19/06/1997"; $fecha_dep_ley="19/07/1997"; $fecha_c=$fecha_hasta;  $anos_cumplidos=0; 
function Verifica_Antiguedad($fecha_i){global $fecha_ingreso,$fecha_ley,$fecha_c,$anos_cumplidos,$prev_cod_empleado;   $fecha_l=formato_aaaammdd($fecha_ley); $va=0;
   $fechaa=$fecha_c;  $f1=FDate($fecha_i); $f2=FDate($fecha_ley); $fechai=formato_ddmmaaaa($fecha_ingreso); $anos_cumplidos=0;
   //if($fecha_ingreso>$fecha_l){$fechai=formato_ddmmaaaa($fecha_ingreso);}else{$fechai=$fecha_i;} 
   if(substr($fechaa,3,2)==substr($fechai,3,2)){  $anos_cumplidos=diferencia_años($fechai,$fechaa); $va=1;  }   
   //if($prev_cod_empleado=="06860182"){
   //echo $fecha_i." ".$fechaa." ".$va,"<br>";	 
  // }
return $va;}
...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      if($filas>=1){ $registro=pg_fetch_array($res,0); $cod_empleado=$registro["cod_empleado"];  $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"];	   
		...
		$cod_concepto=$registro["cod_concepto"]; $denominacion=$registro["denominacion"];  	$cedula=$registro["cedula"];	$cod_categ=$registro["cod_categ"]; $fecha_ingreso=$registro["fecha_ingreso"];	$fecha_i=formato_ddmmaaaa($fecha_ingreso);	
       ...
       header("Content-Disposition: attachment; filename=arch_sueldo_dias_adic.csv");
	   $salto="\r\n";  
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		...
		if($prev_cod_empleado<>$cod_emp){ 
		  if($conc8>0){ $cant_emp=$cant_emp+1; $num_dias_adic=0;
		    if(Verifica_Antiguedad($fecha_i)==1){  
	          $ma_cumplido=$anos_cumplidos-1;  $mopa=$ma_cumplido*2;  if($mopa<0){ $mopa=0; }
              if($mopa<30){  $num_dias_adic=$mopa; }     else{  $num_dias_adic=30;}
            }
				
			
			$cant_dias=$num_dias_adic; $conc13=$cant_dias;
		  
		    $conc8=formato_monto($conc8); $conc13=round($conc13,0);		   
			$monoto1=$conc13; $monoto2=$conc8; $monoto2=elimina_puntos($monoto2);  
			echo $prev_cod_empleado.';'.$monoto1.';'.$monoto2.';'.$fecha_i.$salto;			
          }			
		  $prev_cod_empleado=$cod_emp;
		  $conc1=0; $conc2=0; $conc3=0; $conc4=0; $conc5=0; $conc6=0; $conc7=0; $conc8=0; $conc9=0; $conc10=0; $conc11=0; $conc12=0; $conc13=0; $conc14=0; $conc15=0;
		}
		$tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"];
	    $fechad=$registro["fechapd"]; $fechah=$registro["fechaph"]; $cod_departam=$registro["cod_departam"]; $des_departam=$registro["des_departam"]; $fecha_ingreso=$registro["fecha_ingreso"];
		...
		$des_cargo=$registro["des_cargo"]; $sueldoc=$registro["sueldo_cargo"]; $cantidad=$registro["cantidad"]; $monto=$registro["monto"];  $fecha_i=formato_ddmmaaaa($fecha_ingreso);
