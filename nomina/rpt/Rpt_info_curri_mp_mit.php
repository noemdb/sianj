...
$nomb_rpt="Rpt_info_curri_mp_mit_re.xml";
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
else{  $php_os=PHP_OS; $Nom_Emp=busca_conf(); if($utf_rpt=="SI"){if($php_os=="WINNT"){$php_os="LINUX";}else{$php_os="WINNT";}}

       $sSQL = "SELECT nom006.cod_empleado, nom006.Cedula, nom006.Nombre, nom006.Status, nom014.Fecha, nom014.Titulo, nom014.Instituto, nom014.Descripcion, to_char(nom007.Fecha_Nacimiento,'DD/MM/YYYY') as fechan
                  FROM nom006, nom007,  nom014 WHERE nom006.cod_empleado=nom007.cod_empleado and nom006.cod_empleado = nom014.cod_empleado and
                  nom006.Tipo_Nomina>='".$tipo_nominad."' and nom006.Tipo_Nomina<='".$tipo_nominah."' and  nom006.cod_empleado>='".$cod_empleado_d."' and nom006.cod_empleado<='".$cod_empleado_h."' and
		          ...
		          nom006.cod_departam>='".$codigo_departamentod."' and nom006.cod_departam<='".$codigo_departamentoh."' and  nom007.Fecha_Nacimiento>='".$fecha_desde."' and nom007.Fecha_Nacimiento<='".$fecha_hasta."'and
                  nom007.Edad>='".$edad_d."' and nom007.Edad<='".$edad_h."' and  nom006.Fecha_Ingreso>='".$fecha_desde_ing."' and nom006.Fecha_Ingreso<='".$fecha_hasta_ing."' ".$crit_st." ORDER BY nom006.cod_empleado, nom014.Fecha";
	...
	if($tipo_rpt=="PDF"){$res=pg_query($sSQL);  $cod_empleado_grupo=""; 
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $cod_empleado_grupo;  global $registro;
			...
			$this->Cell(50);
			$this->Cell(100,10,'INFORMACION CURRICULAR',1,0,'C');
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_empleado=$registro["cod_empleado"];  $nombre=$registro["nombre"]; 
          ...
          $fechai=formato_ddmmaaaa($fechai); $fechan=formato_ddmmaaaa($fechan); $monto_declaracion=formato_monto($monto_declaracion);
		   ...
		   if($prev_cod_empleado<>$cod_empleado_grupo){ 
				...
				$pdf->Cell(72,4,'Titulo ',0,0,'L');  
				$pdf->Cell(65,4,'Instituto ',0,0,'L');
				$pdf->Cell(50,4,'Descripcion ',0,1,'L'); 
            ...
            $cedula=$registro["cedula"]; $status=$registro["status"];  $fechan=$registro["fechan"]; $titulo=$registro["titulo"]; $instituto=$registro["instituto"];
	        $descripcion=$registro["descripcion"];  if($php_os=="WINNT"){$titulo=$titulo; }else{$titulo=utf8_decode($titulo);  $instituto=utf8_decode($instituto); $descripcion=utf8_decode($descripcion);}
            $pdf->SetFont('Arial','',6);
			$pdf->Cell(13,3,$fechan,0,0,'L');  
			$pdf->Cell(72,3,substr($titulo,0,55),0,0,'L');
			$pdf->Cell(65,3,$instituto,0,0,'L');
			$pdf->Cell(50,3,$descripcion,0,1,'L');            
        }
