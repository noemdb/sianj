...
FROM ban019, trabajadores a WHERE ban019.ced_rif = a.cod_empleado And ban019.ced_rif>='$cedula_d' And ban019.ced_rif<='$cedula_h' and nombre_usuario='$usuario_sia' ORDER BY  ban019.ced_rif,ban019.Fecha_Abono";
...
require('../../class/fpdf/fpdf.php'); 
      class PDF extends FPDF{ 	    
		function Header(){  global $nombre_agente; global $ced_rif_emp; global $rif; global $direccion_ag; global $telefono_ag; global $fecha_ini; global $fecha_fin; global $nombre; global $cedula; global $ced_rif; global $nacionalidad; global $direccion; global $ciudad; global $cod_postal; global $estado; global $telefono; 
			...
			$this->Image('../../imagenes/Logo_emp.png',7,10,20);
			$this->Ln(3);
			$this->SetFont('Arial','B',11);
			$this->Cell(50);
			$this->Cell(150,4,'AR-CV','0',1,'C');			
			$this->Cell(50);
			$this->Cell(150,4,'COMPROBANTE DE RETENCIONES VARIAS',0,1,'C');
			$this->Cell(50);
			$this->Cell(150,4,'DEL IMPUESTO SOBRE LA RENTA','0',1,'C');
			$this->SetFont('Arial','',6);		
			$this->Cell(260,3,'(  SUELDOS, SALARIOS Y DEMAS REMUNERACIONES SIMILARES A PERSONAS NATURALES RESIDENTES )','0',1,'C');
			...
			$this->Cell(5,4,'',0,0,'L');
			$this->rect(140,37,130,53);
			$this->Cell(5,4,'',0,0,'L');
		    $this->Cell(90,4,'Apellido(s) Y Nombre(s): ','RL',0,'L');	
            $this->Cell(40,4,'Tipo de Empresa','RL',1,'L');
            ...
            $this->Cell(40,4,$rif_empleado,'RL',1,'L');	
			...
			$this->Cell(40,4,'Periodo a que corresponde','RLT',1,'L');
			...
			$this->Cell(30,2.5,'TOTAL CANTIDAD SUELDOS Y','LRT',0,'C',true);
			...
			$this->Cell(30,3,'SALARIOS ABONADA A CUENTA','LRB',0,'C',true);
	  ...
	  $prev_pag=0;
 	  while($registro=pg_fetch_array($res)){ $ced_rif=$registro["ced_rif"]; $paso_enc=0; $pag=$pdf->PageNo();
		  ...
		  if($prev_ced<>$ced_rif){ $total_monto_abonado=formato_monto($total_monto_abonado);  $total_monto_objeto=formato_monto($total_monto_objeto);  $total_monto_retencion=formato_monto($total_monto_retencion);
			...
			$pdf->Cell(80,5,'SUJETO RETENIDO ( FECHA Y FIRMA): ',0,0,'R');			
			$pdf->AddPage();
