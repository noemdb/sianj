   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Cell(50);
			$this->Cell(160,6,"CONTRALORIA ESTADO BOLIVARIANO DE MIRANDA",0,1,'C');
			$this->Cell(50);
			$this->Cell(160,6,"RELACION BONO ALIMENTACION",0,1,'C');
			$this->SetFont('Arial','B',10);
			$this->Cell(50);
			$this->Cell(160,5,$criterio1,0,1,'C');
			
			$this->Cell(50);
			if($tipo_nomina_d==$tipo_nomina_h){	$this->Cell(160,5,"NOMINA : ".$tipo_nomina." ".$des_nomina,0,1,'C');}
			...
			$this->Cell(108,4,'','T',0,'L',true);
			$this->Cell(20,4,'MONTO','T',0,'C',true);
			$this->Cell(10,4,'DIAS','T',0,'C',true);
			$this->Cell(18,4,'MONTO','T',0,'C',true);
			$this->Cell(20,4,'TOTAL','T',0,'C',true);
			$this->Cell(69,4,'OBSERVACIONES ','RT',1,'C',true);
			
			
			
            $this->Cell(15,4,'CEDULA','LB',0,'L',true);
			$this->Cell(108,4,'NOMBRE TRABAJADOR','B',0,'L',true);
			$this->Cell(20,4,'REAL','B',0,'C',true);
			$this->Cell(10,4,'DESC.','B',0,'C',true);
			$this->Cell(18,4,'DESCONTADO','B',0,'C',true);
			$this->Cell(20,4,'CESTA','B',0,'C',true);			
			$this->Cell(69,4,'','BR',1,'C',true);					   
		}
	 ...
	 // $pdf->MultiCell(200,3,$sSQL,0); 
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if($prev_cod_empleado<>$cod_emp){ $cant_emp=$cant_emp+1;
		    ...
		    //$pdf->SetXY($x+$w,$y);
			$pdf->Cell(108,4,$nombre,0,0);     
			$pdf->Cell(20,4,$conc1,0,0,'R');
			$pdf->Cell(10,4,$conc2,0,0,'R');
			$pdf->Cell(18,4,$conc3,0,0,'R');
			$pdf->Cell(20,4,$conc4,0,0,'R');
            $x=$pdf->GetX();   $y=$pdf->GetY(); $w=69;		   
		    //$pdf->SetXY($x+$w,$y);			
            // $pdf->SetXY($x+$n,$y);
		    $pdf->MultiCell($w,4,$observacion,0);			
			$pdf->Ln(2);
		...
		if($php_os=="WINNT"){$des_nomina=$des_nomina; }else{$des_nomina=utf8_decode($des_nomina);  $des_departam=utf8_decode($des_departam); $denominacion=utf8_decode($denominacion);}
		$mdia=substr($fechah,0,2); $tcantidad=$cantidad;
	  ...
	  $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $tot4=$tot4+$conc4; $tot5=$tot5+$conc5; $tot6=$tot6+$conc6; $tot7=$tot7+$conc7; $tot8=$tot8+$conc8; $tot9=$tot9+$conc9; 
		...
		$pdf->Cell(123,1,' ',0,0,'R');			
		$pdf->Cell(20,1,'','T',0,'R');
		$pdf->Cell(10,1,'',0,0,'R');
		$pdf->Cell(18,1,'','T',0,'R');
		$pdf->Cell(20,1,'','T',0,'R');
		$pdf->Cell(69,1,'',0,1,'R');
		$pdf->Cell(50,5,'NRO.TRABAJADORES : '.$cant_emp,0,0,'L');
		$pdf->Cell(73,5,'TOTALES : ',0,0,'R');			
		$pdf->Cell(20,5,$tot1,'T',0,'R');
		$pdf->Cell(10,5,'',0,0,'R');
		$pdf->Cell(18,5,$tot3,'T',0,'R');
		$pdf->Cell(20,5,$tot4,'T',1,'R');
		
		$pdf->Ln(2);
		$pdf->Cell(50,5,'',0,0,'L');
		$pdf->Cell(73,5,'MONTO A PAGAR : ',0,0,'R');			
		$pdf->Cell(20,5,'',0,0,'R');
		$pdf->Cell(10,5,'',0,0,'R');
		$pdf->Cell(18,5,'',0,0,'R');
		$pdf->Cell(20,5,$tot4,'T',1,'R');
        ...
        $y=$pdf->GetY();  $t=10;
        if($y>170){$t=10; $pdf->Cell(5,4,'',0,1);  } 
        $pdf->ln($t); $y=$pdf->GetY();
        ...
        $pdf->Cell(10,4,'',0,0);
		$pdf->Cell(55,4,'Conformado por ','T',0,'C');
        $pdf->Cell(10,4,'',0,0);
        $pdf->Cell(55,4,'Aprobado por Contralor(a)','T',1,'C');
		
        $pdf->Cell(70,3,' ',0,0,'C');
		$pdf->Cell(60,3,'Dir. Recursos Humanos y Capacitacion',0,0,'C');
		$pdf->Cell(10,3,'',0,0,'C');
        $pdf->Cell(55,3,'Dir. Administracion y Presupuesto',0,0,'C'); 
		$pdf->Cell(10,3,'',0,0,'C');
        $pdf->Cell(55,3,'Interventora de la CEBM',0,1,'C'); 		
