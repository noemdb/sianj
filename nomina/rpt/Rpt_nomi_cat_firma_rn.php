...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
  ...
  $Sql = "SELECT count(distinct cod_empleado) as cant_trab  FROM ".$criterio ;
  $res=pg_query($Sql); $filas=pg_num_rows($res); if($filas>0){ $registro=pg_fetch_array($res,0); $criterio2=$registro["cant_trab"];  }	
  $sSQL = "SELECT *  FROM ".$criterio."  order by tipo_nomina, cod_categ, cod_cargo, cod_empleado, cod_concepto";	
  if($tipo_rpt=="PDF"){	 $res=pg_query($sSQL); $prev_tipo=""; $prev_den_nom=""; $prev_dep=""; $prev_den_dep=""; $prev_emp=""; $filas=pg_num_rows($res);
      ...
      if($filas>=1){ $registro=pg_fetch_array($res,0); $cod_empleado=$registro["cod_empleado"];  $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"];	   
        $fechad=$registro["fechapd"]; $fechah=$registro["fechaph"]; $cod_categ=$registro["cod_categ"]; $des_departam=$registro["des_departam"]; 
		if($php_os=="WINNT"){$des_nomina=$des_nomina; }else{$des_nomina=utf8_decode($des_nomina);  $des_departam=utf8_decode($des_departam);}
        $prev_tipo=$tipo_nomina; $prev_den_nom=$des_nomina; $prev_dep=$cod_categ;  $prev_den_dep=$des_departam; $prev_emp="";
	  }	  
      require('../../class/fpdf/fpdf.php');
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_categ; global $des_departam;
			...
			$this->Cell(140,5,"CODIGO CATEGORIA : ".$cod_categ,0,1,'L');
		}
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"];
		...
		if(($prev_emp<>$cod_empleado)or($prev_dep<>$cod_categ)or($prev_tipo<>$tipo_nomina)){
		  $pdf->SetFont('Arial','',7);
		  ...
		  if(($prev_dep<>$cod_categ)or($prev_tipo<>$tipo_nomina)){$neto=$totala_dep-$totald_dep; $neto=formato_monto($neto);
		    $totala_dep=formato_monto($totala_dep); $totald_dep=formato_monto($totald_dep);
		    $pdf->Cell(140,2,'',0,0);
			$pdf->Cell(20,2,'============',0,0,'R');
			$pdf->Cell(20,2,'============',0,0,'R');
			$pdf->Cell(20,2,'============',0,1,'R');			
			$pdf->Cell(130,3,'Total : '.$prev_dep,0,0);
			$pdf->Cell(10,3,$cant_dep,0,0,'C');			
			$pdf->Cell(20,3,$totala_dep,0,0,'R');
			$pdf->Cell(20,3,$totald_dep,0,0,'R');
			$pdf->Cell(20,3,$neto,0,1,'R');
			$prev_dep=$cod_categ;  $prev_den_dep=$des_departam; $cant_dep=0; $totala_dep=0; $totald_dep=0;
			...
			if($salto_dep=="SI"){$pdf->AddPage();}else{$pdf->Ln(5); $pdf->Cell(140,5,"CODIGO CATEGORIA : ".$cod_categ,0,1,'L');}
		...
		$totala_g=$totala_g+$monto_asignacion; $totald_g=$totald_g+$monto_deduccion; 
			...
			$pdf->Cell(20,2,'============',0,0,'R');
			$pdf->Cell(20,2,'============',0,1,'R');			
			$pdf->Cell(130,2,'Total : '.$prev_dep,0,0);
