   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Ln(4);
			$this->Cell(35,4,'CODIGO','LRT',0,'C');
			$this->Cell(35,4,'APORTE','RT',0,'C');
			$this->Cell(35,4,'APORTE','RT',0,'C');
			$this->Cell(35,4,'PRESTAMOS','RT',0,'C');
            $this->Cell(35,4,'TOTAL','RT',1,'C');	

            $this->Cell(35,3,'PRESUPUESTARIO','LR',0,'C');
			$this->Cell(35,3,'EMPLEADO','R',0,'C');
			$this->Cell(35,3,'EMPRESA','R',0,'C');
			$this->Cell(35,3,'','R',0,'C');
			$this->Cell(35,3,'','R',1,'C');

            $this->Cell(35,4,'UNIDAD','LRB',0,'C');
			$this->Cell(35,4,'','RB',0,'C');
			$this->Cell(35,4,'401-07-07-00','RB',0,'C');
			$this->Cell(35,4,'','RB',0,'C');
			$this->Cell(35,4,'','RB',1,'C');		
		}		
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if($prev_cod_cat<>$cod_cat){ $cant_emp=$cant_emp+1;
			...
			$pdf->Cell(35,4,$prev_cod_cat,1,0);			
			$pdf->Cell(35,4,$sub_tot1,1,0,'R');
			$pdf->Cell(35,4,$sub_tot2,1,0,'R');
			$pdf->Cell(35,4,$sub_tot3,1,0,'R');
			$pdf->Cell(35,4,$sub_tot4,1,1,'R');  
		...
		if( (($cod_concepto>= '542') and ($cod_concepto<= '554')) or ($cod_concepto=='564') or ($cod_concepto=='582') or ($cod_concepto=='584')) { $conc3=$conc3+$monto; }
	  ...
	  $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $tot4=$tot4+$conc4; $tot5=$tot5+$conc5; 
		...
		$pdf->Cell(35,5,$tot1,1,0,'R');
		$pdf->Cell(35,5,$tot2,1,0,'R');
		$pdf->Cell(35,5,$tot3,1,0,'R');
		$pdf->Cell(35,5,$tot4,1,1,'R');
	$pdf->Output(); 
}
?>
