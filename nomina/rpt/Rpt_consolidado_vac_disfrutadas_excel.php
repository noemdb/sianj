...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   $sql="select fecha_p_hasta from nom017 where (fecha_p_hasta='".$cfechan."') and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') ".$cri_tp.$ordenar;
   ...
   $criterio=$criterio." and (cod_concepto='005' or cod_concepto='011' or cod_concepto='182')";
   ...
   $criterio1="Mes : ".$mesh." Año: ".$mano; $criterio1="Fecha Desde : ".$fecha_desde." Hasta: ".$fecha_hasta;  
  ...
  //echo $sSQL,"<br>";
      ...
      $cod_empleado=""; $tipo_nomina=""; $des_nomina=""; $prev_conc=""; $den_conc=""; $prev_emp=""; $prev_nombre=""; $prev_cedula=""; $prev_cod_empleado=""; $prev_cod_cat=""; $prev_fecha="";
      if($filas>=1){ $registro=pg_fetch_array($res,0); $cod_empleado=$registro["cod_empleado"];  $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"]; $fecha_p_desde=$registro["fecha_p_desde"]; 	   
        $fechad=$registro["fechapd"]; $fechah=$registro["fechaph"]; $cod_departam=$registro["cod_departam"]; $des_departam=$registro["des_departam"]; $des_cargo=$registro["des_cargo"]; $nombre=$registro["nombre"];
        ...
        if($php_os=="WINNT"){$des_nomina=$des_nomina; }else{$des_nomina=utf8_decode($des_nomina);  $des_departam=utf8_decode($des_departam); $des_cargo=utf8_decode($des_cargo); $denominacion=utf8_decode($denominacion);}
		...
		$prev_cod_empleado=$cod_empleado; $prev_cod_cat=$cod_categ; $prev_fecha=$fecha_p_desde;
	  }	  
	  
	  
	  header("Content-type: application/vnd.ms-excel");
      header("Content-Disposition: attachment; filename=Reporte_Cantidad_Vacaciones_Disfrutadas.xls"); 	
	  ?>
	   <table border="0" cellspacing='0' cellpadding='0' align="left">
		 <tr height="20">
		    <td width="100" align="left" ><strong></strong></td>
			<td width="400" align="center" > <font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong>REPORTE VACACIONES DISFRUTADAS</strong></font></td>
		 </tr>
		 <tr height="20">
		 </tr>
		 <?if($tipo_nomina_d==$tipo_nomina_h){ ?>
		 ...
		 <tr height="20">
		   ...
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>FECHA REINCORP.</strong></td>
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>DIAS HABILES VACACIONES</strong></td>
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>DIAS TRABAJADOS CALCULO DE VACACIONES</strong></td>
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>FERIADO EN VACACIONES</strong></td>
		 </tr>
	  <? 
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"]; $fecha_p_desde=$registro["fecha_p_desde"]; 
	    if($prev_cod_empleado<>$cod_emp){ $cant_emp=$cant_emp+1; $fecha_reincorp="";
			...
			$sql25="select fecha_reincorp from nom025 where cod_empleado='$cod_empleado' and fecha_calculo_d>='$prev_fecha'"; $res25=pg_query($sql25); $filas25=pg_num_rows($res25);
			if($filas25>=1){ $reg25=pg_fetch_array($res25,0); $fecha_reincorp=$reg25["fecha_reincorp"]; $fecha_reincorp=formato_ddmmaaaa($fecha_reincorp); }
			//else{ echo $sql25." ".$filas25,"<br>"; }
			?>
		      <tr>
                  ...
                  <td width="100" align="left"><? echo $fecha_reincorp; ?></td>	                  
		    ...
		    <? 
			$prev_cod_empleado=$cod_emp; $prev_fecha=$fecha_p_desde; $conc1=0; $conc2=0; $conc3=0; $conc4=0; $conc5=0; 			
		}
		$tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"]; $fecha_p_desde=$registro["fecha_p_desde"]; 
		...
		if($cod_concepto=="005") { $conc1=$conc1+$cantidad; }
		if($cod_concepto=="011") { $conc2=$conc2+$cantidad; }
		if($cod_concepto=="182") { $conc3=$conc3+$cantidad; }
		if($cod_concepto=="184") { $conc4=$conc4+$cantidad; }
		if($cod_concepto=="014") { $conc5=$conc5+$cantidad; }
	  }		
	  $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $tot4=$tot4+$conc4; $tot5=$tot5+$conc5; $cant_emp=$cant_emp+1;
			$sub_tot1=$sub_tot1+$conc1; $sub_tot2=$sub_tot2+$conc2; $sub_tot3=$sub_tot3+$conc3; $sub_tot4=$sub_tot4+$conc4; $sub_tot5=$sub_tot5+$conc5; 
			$conc1=formato_monto($conc1); $conc2=formato_monto($conc2); $conc3=formato_monto($conc3); $conc4=formato_monto($conc4); $conc5=formato_monto($conc5);
			$sql25="select fecha_reincorp from nom025 where cod_empleado='$cod_empleado' and fecha_calculo_d>='$fecha_p_desde'"; $res25=pg_query($sql25); $filas25=pg_num_rows($res25);
			if($filas25>=1){ $reg25=pg_fetch_array($res25,0); 	$fecha_reincorp=$reg25["fecha_reincorp"]; $fecha_reincorp=formato_ddmmaaaa($fecha_reincorp); }			
			?>
		      <tr>
				  <td width="100" align="left" style="mso-number-format:'@';" ><? echo $cod_empleado; ?></td>
