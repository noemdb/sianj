...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   $criterio=$criterio." and (cod_concepto>='510' and cod_concepto<='550')";
   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Cell(95,4,'','T',0,'L',true);
			$this->Cell(15,4,'PRESTAMO','T',0,'C',true);
			$this->Cell(15,4,'PRESTAMO','T',0,'C',true);
			$this->Cell(15,4,'PRESTAMO','T',0,'C',true);
			$this->Cell(15,4,'PRESTAMO','T',0,'C',true);
			$this->Cell(15,4,'SERVICIO','T',0,'C',true);
			$this->Cell(15,4,'SERVICIO','T',0,'C',true);
			$this->Cell(15,4,'PRESTAMO','T',0,'C',true);
			$this->Cell(15,4,'IBERO','T',0,'C',true);
			$this->Cell(15,4,'SEGUROS','T',0,'C',true);
			$this->Cell(15,4,'TOTAL ','RT',1,'C',true);

			
           
            $this->Cell(15,4,'CEDULA','LB',0,'L',true);
			$this->Cell(95,4,'NOMBRE TRABAJADOR','B',0,'L',true);
					
				
			$this->Cell(15,4,'(MP)','B',0,'C',true);
			$this->Cell(15,4,'(CP)','B',0,'C',true);
			$this->Cell(15,4,'(PE)','B',0,'C',true);
			$this->Cell(15,4,'(ALBNF)','B',0,'C',true);
			$this->Cell(15,4,'FUNERA','B',0,'C',true);
			$this->Cell(15,4,'TELEFONO.','B',0,'C',true);
			$this->Cell(15,4,'COMPUT.','B',0,'C',true);
			$this->Cell(15,4,'SEGUROS','B',0,'C',true);
			$this->Cell(15,4,'UNIVERSITA','B',0,'C',true);
			
			$this->Cell(15,4,'','BR',1,'C',true);					   
		}
	 ...
	 // $pdf->MultiCell(200,3,$sSQL,0); 
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if($prev_cod_empleado<>$cod_emp){ $cant_emp=$cant_emp+1;
		    ...
		    //$pdf->SetXY($x+$w,$y);
			$pdf->Cell(95,4,$nombre,0,0);
            
			
			$pdf->Cell(15,4,$conc1,0,0,'R');
			$pdf->Cell(15,4,$conc2,0,0,'R');
			...
			$pdf->Cell(15,4,$conc8,0,0,'R');
			$pdf->Cell(15,4,$conc9,0,0,'R');
			$pdf->Cell(15,4,$conc13,0,1,'R');
		...
		if($cod_concepto=="510") { $conc1=$conc1+$monto; }
		if($cod_concepto=="511") { $conc2=$conc2+$monto; }
		if($cod_concepto=="512") { $conc3=$conc3+$monto; }
	    if($cod_concepto=="513") { $conc4=$conc4+$monto; }
	    if($cod_concepto=="515") { $conc5=$conc5+$monto; }
	    if($cod_concepto=="531") { $conc6=$conc6+$monto; }
	    if($cod_concepto=="530") { $conc7=$conc7+$monto; }		   
	    if($cod_concepto=="518") { $conc8=$conc8+$monto; }
	    if($cod_concepto=="541") { $conc9=$conc9+$monto; }	
	  ...
	  $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $tot4=$tot4+$conc4; $tot5=$tot5+$conc5; $tot6=$tot6+$conc6; $tot7=$tot7+$conc7; $tot8=$tot8+$conc8; $tot9=$tot9+$conc9; 
		...
		$pdf->Cell(110,0.5,' ',0,0,'R');			
		$pdf->Cell(15,0.5,'','T',0,'R');
		$pdf->Cell(15,0.5,'','T',0,'R');
		$pdf->Cell(15,0.5,'','T',0,'R');
		$pdf->Cell(15,0.5,'','T',0,'R');
		$pdf->Cell(15,0.5,'','T',0,'R');
		...
		$pdf->Cell(15,0.5,'','T',0,'R');
		$pdf->Cell(15,0.5,'','T',0,'R');
		$pdf->Cell(15,0.5,'','T',1,'R');
		$pdf->Cell(40,5,'NRO.TRABAJADORES : '.$cant_emp,0,0,'L');
		$pdf->Cell(70,5,'TOTAL GENERAL : ',0,0,'R');			
		$pdf->Cell(15,5,$tot1,'T',0,'R');
		$pdf->Cell(15,5,$tot2,'T',0,'R');
		$pdf->Cell(15,5,$tot3,'T',0,'R');
		$pdf->Cell(15,5,$tot4,'T',0,'R');
		$pdf->Cell(15,5,$tot5,'T',0,'R');
        $pdf->Cell(15,5,$tot6,'T',0,'R');
		$pdf->Cell(15,5,$tot7,'T',0,'R');
		$pdf->Cell(15,5,$tot8,'T',0,'R');
		$pdf->Cell(15,5,$tot9,'T',0,'R');
		$pdf->Cell(15,5,$tot13,'T',0,'R');		
	$pdf->Output(); 
}
?>
