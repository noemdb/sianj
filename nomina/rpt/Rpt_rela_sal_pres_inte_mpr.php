<? include ("../../class/conect.php");  require ("../../class/fun_fechas.php"); require ("../../class/fun_numeros.php");  include ("../../class/configura.inc"); $php_os=PHP_OS;   error_reporting(E_ALL ^ E_NOTICE);
   $tipo_nominad=$_GET["tipo_nominad"]; $tipo_nominah=$_GET["tipo_nominah"];    $fecha_h=$_GET["fecha_h"];   
   $codigo_departamento_d=$_GET["codigo_departamento_d"];  $codigo_departamento_h=$_GET["codigo_departamento_h"]; $tipo_rpt=$_GET["tipo_rpt"];  
...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
    ...
    $criterio=" (nom006.cod_empleado=nom030.cod_empleado) and (nom030.tipo_calculo='P') and (nom006.tipo_nomina>='".$tipo_nominad."' and nom006.tipo_nomina<='".$tipo_nominah."') 
	   and (nom030.fecha_calculo<='".$fecha_hasta."')  and (nom006.cod_departam>='".$codigo_departamento_d."' and nom006.cod_departam<='".$codigo_departamento_h."' ) ";
	...
	if($tipo_rpt=="HTML"){ include ("../../class/phpreports/PHPReportMaker.php");
          ...
          $oRpt->setXML("Rpt_rela_sal_pres_inte_mpr_re.xml");
          $oRpt->setUser("$user");
          ...
          $oRpt->setParameters(array("criterio1"=>$criterio1,"date"=>$date,"hora"=>$hora,"fecha_h"=>$fecha_h));
          $oRpt->run();
   ...
   if($tipo_rpt=="PDF"){ $res=pg_query($sSQL); $filas=pg_num_rows($res); $mes=""; $periodof=""; $cod_empleado_grupo=""; 	$nombre_grupo="";  $cedula_grupo=""; $fechai_grupo=""; $prev_cod_empleado=""; 
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1;  global $fecha_h; 
			...
			$this->Cell(50);
			$this->Cell(100,7,'RELACION  SALDO  DE  PRESTACIONES  E  INTERESES ',1,0,'C');
			$this->Ln(20);	
            $this->SetFont('Arial','B',9); 	
            $this->Cell(50,5,"Fecha Corte : ".$fecha_h,0,1,'L');			
			$this->SetFont('Arial','B',7);
			
			$this->Cell(15,4,'Codigo ','RTL',0,'C');
			$this->Cell(85,4,'','RTL',0,'C');
	  ...
	  $pdf->Cell(20,5,$Gtotal_acumulado,0,1,'R');	
	  $pdf->Output(); 
    }
