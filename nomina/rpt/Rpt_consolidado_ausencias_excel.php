...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   $criterio=$criterio." and (cod_concepto='002' or cod_concepto='008' or cod_concepto='009' or cod_concepto='039' or cod_concepto='040')";
   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
	  ...
	  ?>
	   <table border="0" cellspacing='0' cellpadding='0' align="left">
		 <tr height="20">
			...
			<td width="400" align="center" > <font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong>REPORTE DE AUSENCIAS</strong></font></td>
		 </tr>
		 <tr height="20">
		 </tr>
		 <?if($tipo_nomina_d==$tipo_nomina_h){ ?>
		 ...
		 <tr height="20">
		   ...
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>DIAS NO TRABAJADOS</strong></td>
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>ASIGN. x PERMISO REMUNERADO</strong></td>
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>PERMISO DE PATERNIDAD REMUNER.</strong></td>
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>REPOSO MEDICO CORTO</strong></td>
		   <td width="100" align="center" bgcolor="#99CCFF"><strong>REPOSO MEDICO</strong></td>
		 </tr>
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if($prev_cod_empleado<>$cod_emp){ $cant_emp=$cant_emp+1;
		    ...
		    <? 
			$prev_cod_empleado=$cod_emp; $conc1=0; $conc2=0; $conc3=0; $conc4=0; $conc5=0; 			
		}
		...
		if($cod_concepto=="002") { $conc1=$conc1+$cantidad; }
		if($cod_concepto=="008") { $conc2=$conc2+$cantidad; }
		if($cod_concepto=="009") { $conc3=$conc3+$cantidad; }
		if($cod_concepto=="039") { $conc4=$conc4+$cantidad; }
		if($cod_concepto=="040") { $conc5=$conc5+$cantidad; }
	  }		
