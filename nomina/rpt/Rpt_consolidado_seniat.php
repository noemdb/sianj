   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Cell(50);
			$this->Cell(160,7,"DECLARACION DE INGRESOS Y RETENCIONES (SENIAT)",1,0,'C');
			$this->Ln(17);
			...
			$this->Cell(15,4,'','LT',0,'L');
			$this->Cell(110,4,'','T',0,'L');
			$this->Cell(15,4,'FECHA','T',0,'L');				
			$this->Cell(20,4,'NUMERO','T',0,'C');			
			$this->Cell(20,4,'NUMERO','T',0,'C');
			$this->Cell(20,4,'CODIGO','T',0,'C');
			$this->Cell(20,4,'MONTO','T',0,'C');
			$this->Cell(20,4,'MONTO','T',0,'C');
			$this->Cell(20,4,'PORCENTAJE A','RT',1,'C');			
           
            $this->Cell(15,4,'CEDULA','LB',0,'L');
			$this->Cell(110,4,'NOMBRE TRABAJADOR','B',0,'L');
			$this->Cell(15,4,'INGRESO','B',0,'L');	
			$this->Cell(20,4,'FACTURA','B',0,'C');
			$this->Cell(20,4,'CONTROL','B',0,'C');			
			$this->Cell(20,4,'CONCEPTO','B',0,'C');
			$this->Cell(20,4,'OPERACION','B',0,'C');
			$this->Cell(20,4,'RETENIDO','B',0,'C');
			$this->Cell(20,4,'RETENCION','BR',1,'C');					   
		}
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if($prev_cod_empleado<>$cod_emp){ $cant_emp=$cant_emp+1;
		    $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3;  
			$conc1=formato_monto($conc1); $conc2=formato_monto($conc2); $conc3=formato_monto($conc3); 
		    $pdf->SetFont('Arial','',7);
			$pdf->Cell(15,4,$cedula,0,0);
			//$x=$pdf->GetX();   $y=$pdf->GetY(); $w=110;		   
		    //$pdf->SetXY($x+$w,$y);
			$pdf->Cell(110,4,$nombre,0,0);
            $pdf->Cell(15,4,$fechai,0,0);			
			$pdf->Cell(20,4,'0',0,0,'C');
			$pdf->Cell(20,4,'NA',0,0,'C');
			$pdf->Cell(20,4,'01',0,0,'C');
			$pdf->Cell(20,4,$conc1,0,0,'R');
			$pdf->Cell(20,4,$conc2,0,0,'R');
			$pdf->Cell(20,4,$conc3,0,1,'R');
	  ...
	  $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $cant_emp=$cant_emp+1;
		$conc1=formato_monto($conc1); $conc2=formato_monto($conc2); $conc3=formato_monto($conc3); 
		$pdf->SetFont('Arial','',7);
		$pdf->Cell(15,4,$cedula,0,0);
		//$x=$pdf->GetX();   $y=$pdf->GetY(); $w=110;		   
		...
		$pdf->Cell(200,0.5,' ',0,0,'R');	
		$pdf->Cell(20,0.5,'','T',0,'R');
		$pdf->Cell(20,0.5,'','T',1,'R');
		$pdf->Cell(100,5,'NRO.TRABAJDORES : '.$cant_emp,0,0,'L');
		$pdf->Cell(100,5,'TOTAL GENERAL : ',0,0,'R');	
        		
		$pdf->Cell(20,5,$tot1,'T',0,'R');
		$pdf->Cell(20,5,$tot2,'T',1,'R');
	$pdf->Output(); 
}
?>
