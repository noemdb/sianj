...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   //$criterio=$criterio." and (cod_concepto='506' or cod_concepto='906')";
   $criterio=$criterio." and (cod_concepto>='506' and cod_concepto<='906') and (cod_categ>='".$cod_presup_catd."' and cod_categ<='".$cod_presup_cath."')";
   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Cell(50);
			$this->Cell(160,7,"CONSOLIDADO DE RETENCION Y APORTE PATRONAL L.P.H",1,0,'C');
			$this->Ln(17);
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		...
		if(($cod_concepto=="506")and($mdia=="15")) { $conc1=$conc1+$monto; }
		if(($cod_concepto=="506")and($mdia<>"15"))  { $conc2=$conc2+$monto; }
		if($cod_concepto=="506") { $conc3=$conc3+$monto; }
		if($cod_concepto=="906") { $conc4=$conc4+$monto; }
		$conc5=$conc3+$conc4; 
	  }		
