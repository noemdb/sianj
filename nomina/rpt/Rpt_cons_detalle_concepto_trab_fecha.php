...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
  ...
  $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      $res=pg_query($sSQL); $prev_tipo=""; $prev_den_nom=""; $prev_dep=""; $prev_den_dep=""; $prev_fecha=""; $filas=pg_num_rows($res);
      ...
      if($filas>=1){ $registro=pg_fetch_array($res,0); $cod_empleado=$registro["cod_empleado"];  $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"];	   
		...
		$prev_fecha=$fechah; $prev_conc=$cod_concepto; $den_conc=$denominacion; $prev_tipo=$tipo_nomina; $prev_den_nom=$des_nomina; $prev_dep=$cod_departam;  $prev_den_dep=$des_departam; $prev_emp=$cod_empleado;  $prev_nombre=$nombre; 
	  }	  
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; 
						   ...
						   global $cod_empleado; global $nombre; 
			...
			$this->Cell(120,5,'Denominacion del Concepto',1,0,'L');
			$this->Cell(15,5,'Fecha',1,0);
			$this->Cell(13,5,'Cantidad',1,0,'C');
			$this->Cell(20,5,'Asignaciones',1,0);
			$this->Cell(20,5,'Deduccion',1,1);
			$this->Cell(140,5,$cod_empleado." ".$nombre,0,1,'L');
		}
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"];
        ...
        if($prev_emp<>$cod_empleado){		
			...
			$pdf->Cell(121,3,$den_conc,0,0);
            $pdf->Cell(15,3,$prev_fecha,0,0);			
		...
		if(($prev_conc<>$cod_concepto)or($prev_fecha<>$fechah)){		
		   if($totala_emp==0){$totala_emp="";}else{$totala_emp=formato_monto($totala_emp);} 
			if($totald_emp==0){$totald_emp="";}else{$totald_emp=formato_monto($totald_emp);}			
			$pdf->SetFont('Arial','',8);
			$pdf->Cell(12,3,$prev_conc,0,0);
			...
			$pdf->Cell(20,3,$totala_emp,0,0,'R');
			$pdf->Cell(20,3,$totald_emp,0,1,'R');			
		    $can_conc=0;  $totala_emp=0; $totald_emp=0;   $prev_conc=$cod_concepto; $den_conc=$denominacion; $prev_fecha=$fechah;
		}	
