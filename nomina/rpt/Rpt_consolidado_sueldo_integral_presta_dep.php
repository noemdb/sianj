...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   $criterio="rpt_nom_cal WHERE (oculto<>'N') ";   
   ...
   if($act_hist=="NMM"){ $criterio2=$criterio; $criterio2=str_replace( 'rpt_nom_cal','rpt_nom_hist',$criterio2); 
    $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
	  (cod_departam>='".$cod_departd."' and cod_departam<='".$cod_departh."') and (cod_concepto>='".$cod_conceptod."' AND cod_concepto<='".$cod_conceptoh."')  and
     ...
     $tsSQL=$sSQL;
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Cell(50);
			$this->Cell(160,7,"DETALLE SUELDO PARA CALCULO PRESTACIONES",1,0,'C');
			$this->Ln(17);
			...
			$this->Cell(5,4,'','LT',0,'L',true);
			$this->Cell(58,4,'','T',0,'L',true);
			$this->Cell(10,4,'','T',0,'L',true);
			$this->Cell(10,4,'FECHA','T',0,'C',true);
			$this->Cell(8,4,'','T',0,'L',true);
			$this->Cell(7,4,'','T',0,'L',true);
			$this->Cell(13,4,'SUELDO','T',0,'C',true);
            $this->Cell(13,4,'DIFERENCIA','T',0,'C',true);
			$this->Cell(13,4,'','T',0,'L',true);
			$this->Cell(13,4,'PRIMA','T',0,'C',true);
			$this->Cell(13,4,'PRIMA','T',0,'C',true);
			$this->Cell(13,4,'PRIMA','T',0,'C',true);
			$this->Cell(13,4,'PRIMA','T',0,'C',true);
			$this->Cell(13,4,'TOTAL','T',0,'C',true);
			$this->Cell(10,4,'SUELDO','T',0,'C',true);	
            $this->Cell(10,4,'ALIC.','T',0,'C',true);	
            $this->Cell(10,4,'ALIC.','T',0,'C',true);	
            $this->Cell(10,4,'TOTAL','T',0,'C',true);				
			$this->Cell(5,4,'DIAS','T',0,'C',true);
			$this->Cell(15,4,'TOTAL','RT',1,'C',true);	
            $this->Cell(5,4,'No.','LB',0,'L',true);
			$this->Cell(58,4,'NOMBRE TRABAJADOR','B',0,'L',true);
			$this->Cell(10,4,'CEDULA','B',0,'C',true);
			$this->Cell(10,4,'INGRESO','B',0,'C',true);
			$this->Cell(8,4,'GRADO','B',0,'C',true);
			$this->Cell(7,4,'PASO','B',0,'C',true);
			$this->Cell(13,4,'BASICO','B',0,'C',true);
			$this->Cell(13,4,'SUELDO','B',0,'C',true);
			$this->Cell(13,4,'COMPENSACION','B',0,'C',true);
			$this->Cell(13,4,'POR HOGAR','B',0,'C',true);
			$this->Cell(13,4,'PROFESIONAL','B',0,'C',true);
			$this->Cell(13,4,'POR HIJO','B',0,'C',true);
			$this->Cell(13,4,'ANTIG.','B',0,'C',true);
			$this->Cell(13,4,'SUELDO','B',0,'C',true);	
			$this->Cell(10,4,'INT.DIA','B',0,'C',true);	
			$this->Cell(10,4,'VAC.','B',0,'C',true);	
            $this->Cell(10,4,'PREST.','B',0,'C',true);	
			$this->Cell(10,4,'DIA','B',0,'C',true);	
			$this->Cell(5,4,'ANTIG.','B',0,'C',true);
			$this->Cell(15,4,'ANTIGUEDAD','BR',1,'C',true);					   
		}
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		...
		if($prev_cod_empleado<>$cod_emp){ 
		  if($conc8>0){
			...
			$pdf->Cell(5,4,$cant_emp_dep,0,0);			
			//$x=$pdf->GetX();   $y=$pdf->GetY(); $w=58;		   
		    //$pdf->SetXY($x+$w,$y);
			$pdf->Cell(58,4,$nombre,0,0);
			$pdf->Cell(10,4,$cedula,0,0);
            $pdf->Cell(10,4,$fechai,0,0);
            $pdf->Cell(8,4,$paso,0,0,'C');
            $pdf->Cell(7,4,$grado,0,0,'C');			
			...
			$pdf->Cell(10,4,$conc9,0,0,'R');			
			$pdf->Cell(10,4,$conc10,0,0,'R');
			$pdf->Cell(10,4,$conc11,0,0,'R');
			$pdf->Cell(10,4,$conc12,0,0,'R');
            $pdf->Cell(5,4,$conc13,0,0,'R');			
			$pdf->Cell(15,4,$conc14,0,1,'R');                
            // $pdf->SetXY($x+$n,$y);	
		  ...
		  if($prev_dep<>$cod_dep){ 
			...
			$pdf->Cell(5,4,'    ',0,0);
			$pdf->Cell(93,4,$prev_den_dep,0,0);			
			$pdf->Cell(13,4,$sub_tot1,'T',0,'R');
			$pdf->Cell(13,4,$sub_tot2,'T',0,'R');
			$pdf->Cell(13,4,$sub_tot3,'T',0,'R');
			$pdf->Cell(13,4,$sub_tot4,'T',0,'R');
			$pdf->Cell(13,4,$sub_tot5,'T',0,'R');
			$pdf->Cell(13,4,$sub_tot6,'T',0,'R');
			$pdf->Cell(13,4,$sub_tot7,'T',0,'R');
			$pdf->Cell(13,4,$sub_tot8,'T',0,'R');
			$pdf->Cell(10,4,$sub_tot9,'T',0,'R');			
			$pdf->Cell(10,4,$sub_tot10,'T',0,'R');
			$pdf->Cell(10,4,$sub_tot11,'T',0,'R');
			$pdf->Cell(10,4,$sub_tot12,'T',0,'R');
            $pdf->Cell(5,4,'    ','T',0);		
			$pdf->Cell(15,4,$sub_tot14,'T',1,'R');  
			$pdf->Ln(3);
			$prev_den_dep=$des_departam; $prev_dep=$cod_dep; $cant_emp_dep=0;  $sub_tot1=0; $sub_tot2=0; $sub_tot3=0; $sub_tot4=0; $sub_tot5=0;  $sub_tot6=0; $sub_tot7=0; $sub_tot8=0; $sub_tot9=0; $sub_tot10=0; $sub_tot11=0; $sub_tot12=0; $sub_tot13=0; $sub_tot14=0; $sub_tot15=0;
		  }	
		}
		...
		$mdia=substr($fechah,0,2); 	
            ...
            $conc9=$conc8/30;    $conc10=(($conc9/12)*40)/30; if($tipo_nomina=="01"){$conc11=(($conc9/12)*90)/30;}else{$conc11=(($conc9/12)*120)/30;}
	  ...
	  $sub_tot1=formato_monto($sub_tot1); $sub_tot2=formato_monto($sub_tot2); $sub_tot3=formato_monto($sub_tot3); $sub_tot4=formato_monto($sub_tot4); $sub_tot5=formato_monto($sub_tot5);
	    ...
	    $pdf->Cell(45,5,'NRO.TRABAJADORES : '.$cant_emp,0,0,'L');
	    $pdf->Cell(53,5,'TOTAL GENERAL : ',0,0,'R');
	  
     		
		$pdf->Cell(13,4,$tot1,'T',0,'R');
		$pdf->Cell(13,4,$tot2,'T',0,'R');
		$pdf->Cell(13,4,$tot3,'T',0,'R');
		$pdf->Cell(13,4,$tot4,'T',0,'R');
		$pdf->Cell(13,4,$tot5,'T',0,'R');
		$pdf->Cell(13,4,$tot6,'T',0,'R');
		$pdf->Cell(13,4,$tot7,'T',0,'R');
		$pdf->Cell(13,4,$tot8,'T',0,'R');
		$pdf->Cell(10,4,$tot9,'T',0,'R');			
		$pdf->Cell(10,4,$tot10,'T',0,'R');
		$pdf->Cell(10,4,$tot11,'T',0,'R');
		$pdf->Cell(11,4,$tot12,'T',0,'R');
        $pdf->Cell(4,4,'    ','T',0);				
		$pdf->Cell(15,4,$tot14,'T',1,'R');
	    $pdf->Ln(5);
