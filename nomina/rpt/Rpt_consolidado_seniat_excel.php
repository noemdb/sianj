...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   $res=pg_query($sql); $filas=pg_num_rows($res); if($filas==0){$act_hist="S";  }   $act_hist="S"; 
   ...
   $criterio=$criterio." and (cod_concepto>='499' and cod_concepto<='508')";
   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      if($filas>=1){ $registro=pg_fetch_array($res,0); $cod_empleado=$registro["cod_empleado"];  $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"];	   
        $fechad=$registro["fechapd"]; $fechah=$registro["fechaph"]; $cod_departam=$registro["cod_departam"]; $des_departam=$registro["des_departam"];  $nombre=$registro["nombre"]; $rif_empleado=$registro["rif_empleado"];
	    $cod_concepto=$registro["cod_concepto"]; $denominacion=$registro["denominacion"]; $cedula=$registro["cedula"]; $fechai=$registro["fechai"]; $cod_categ=$registro["cod_categ"];		
	  ...
	  ?>
	   <table border="0" cellspacing='0' cellpadding='0' align="left">
		 <tr height="20">
			...
			<td width="100" align="left" ><strong></strong></td>
			<td width="400" align="center" > <font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><strong>DECLARACION DE INGRESOS Y RETENCIONES (SENIAT)</strong></font></td>
		 </tr>
		 <tr height="20">
		 </tr>
		 <?if($tipo_nomina_d==$tipo_nomina_h){ ?>
		 ...
		 <tr height="20" >
		   ...
		   <td width="100" align="left" bgcolor="#A4A4A4"><strong>RIF TRABAJADOR</strong></td>
		   ...
		   <td width="100" align="center" bgcolor="#A4A4A4"><strong>MONTO OPERACION</strong></td>
		   <td width="100" align="center" bgcolor="#A4A4A4"><strong>MONTO RETENIDO</strong></td>
		   <td width="100" align="center" bgcolor="#A4A4A4"><strong>PORCENTAJE A RETENCION</strong></td>
		 </tr>      
	  <? 
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if($prev_cod_empleado<>$cod_emp){ $cant_emp=$cant_emp+1;
			...
			?>
		      <tr>
				  ...
				  <td width="100" align="left"><? echo $rif_empleado; ?></td>	
				  <td width="400" align="left"><? echo $nombre; ?></td>	
                             ...
                             <td width="100" align="center" style="mso-number-format:'@';"><? echo 'NA'; ?></td>	
        ...
        $cod_empleado=$registro["cod_empleado"]; $nombre=$registro["nombre"]; $cedula=$registro["cedula"]; $rif_empleado=$registro["rif_empleado"]; $fechai=$registro["fechai"]; 
		...
		if($cod_concepto=="499") { $conc1=$conc1+$monto; }
		if($cod_concepto=="508") { $conc2=$conc2+$monto; }
		if($cod_concepto=="508") { $conc3=$cantidad; }
	  }		
	  $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $tot4=$tot4+$conc4; $tot5=$tot5+$conc5; $cant_emp=$cant_emp+1;
		...
		?>
		  ...
		  <tr>
		      <td width="100" align="left"></td>	
              <td width="100" align="left"></td>			  
			  <td width="400" align="left">NRO.TRABAJADORES: </td>
			  <td width="100" align="left"><? echo $cant_emp; ?></td>              
			  <td width="100" align="left"></td>
			  <td width="100" align="left"></td>
