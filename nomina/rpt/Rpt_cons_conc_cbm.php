...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
   ...
   $criterio="rpt_nom_cal WHERE (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') ";
  ...
  $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and (cod_concepto>='".$cod_conceptod."' AND cod_concepto<='".$cod_conceptoh."') and
	  (cod_departam>='".$cod_departd."' and cod_departam<='".$cod_departh."') and (cod_empleado>='".$cod_empleado_d."' and cod_empleado<='".$cod_empleado_h."') ORDER BY tipo_nomina, cod_concepto, cod_empleado";
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $criterio4;  global $mes_comp; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $denominacion; global $criterio3;
			...
			$this->Cell(40);
			$this->Cell(130,7,"RELACION DE CONCEPTOS DETALLE",1,0,'C');
			$this->Ln(17);
			...
			$this->Cell(20,4,'Codigo',1,0,'L');
			$this->Cell(140,4,'Nombre Trabajador',1,0,'L');
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"];
		...
		if(($prev_tipo<>$tipo_nomina)and($mes_comp=='N')){		$total_emp=$totala_emp+$totald_emp;	$total_emp=formato_monto($total_emp);	
			...
			$pdf->Cell(140,4,$prev_nombre,0,0);	
			$pdf->Cell(20,4,$can_conc,0,0,'R');
		...
		$pdf->Cell(80,4,'Nro. Trabjadores : '.$cant_emp,0,0);			
