...
$temp_monto_art142=$monto_art142;  $temp_monto_ant_depositada=$monto_ant_depositada; $temp_dias=$tiempo_servicio*30; $temp_diario=$monto_art142/$temp_dias;
...
$monto_letras=monto_letras($total_neto); $monto_letras_ban=monto_letras($monto_banco);
...
class PDF extends FPDF{ 
	function Header(){ global $nombre; global $cedula; global $fecha_ingreso; global $des_cargo;  global $sueldo_basico; global $sueldo_liquidacion; global $fecha_liquidacion;
	    global $ant_ano; global $ant_mes; global $ant_dia; global $tipo_liquidacion; global $referencia; global $sueldo_vacaciones; global $letra;		
		...
		$this->Cell(145,4,'CALCULO LIQUIDACION POR ',0,0,'C');
		$this->SetFont('Arial','',9);
		...
		$this->Cell(25);
		$this->SetFont('Arial','B',14);		
		$this->Cell(145,4,'RETIRO DE TRABAJADOR('.$letra.')',0,0,'C');
		$this->SetFont('Arial','',9);	   
		...
		$this->SetFont('Arial','',9);
		$this->Cell(65,5," ".$ant_ano." Años  ".$ant_mes." Meses  ".$ant_dia."  Dias",1,0,'C');
		...
		$this->Cell(11,3,'','RL',0,'C',true);
		$this->Cell(14,3,'Salario','RL',0,'R',true);
		...
		$this->Cell(11,4,'Dias','RLB',0,'C',true);
		$this->Cell(14,4,'Diario','RLB',0,'R',true);
	...
	function Footer(){ global $monto_letras;  global $monto_letras_ban;
	  ...
	  $texto1='Monto neto a pagar el fideicomiso segun soporte de la entidad bancaria';
      $this->Cell(200,6,$texto1,'T',1,'L');
      $this->SetFont('Arial','B',9);
      $this->Cell(200,6,$monto_letras_ban,0,1,'C');
      $this->SetFont('Arial','B',8);
	  $this->Cell(200,4,'CONSTANCIA DE LIQUIDACION',1,1,'C',true);		
	  $this->SetFont('Arial','',10);
      $texto1='El suscrito Trabajador declara haber recibido a su entera satisfacccion la cantidad de bolivares';
      $this->Cell(200,6,$texto1,0,1,'L');
      $this->SetFont('Arial','B',9);
      $this->Cell(200,6,$monto_letras,0,1,'C');
      $texto2='por concepto de pago completo de los salarios e indemnizaciones hasta la fecha de la presente liquidacion';
      $this->MultiCell(200,4,$texto2,0);
      $this->Ln(15);
	  ...
	  $this->SetFont('Arial','B',8);
		...
		$this->SetFont('Arial','B',8);
		$this->Cell(100,4,'Analista de Gestion Humana',1,0,'C',true);
		$this->Cell(100,4,' Director(a)/Coordinador(a) Gestion Humana',1,1,'C',true);		
        $this->SetFont('Arial','',7);		
		$this->Cell(100,13,'','LR',0,'C');
		$this->Cell(100,13,'','R',1,'C');		
		$this->Cell(80,3,'Firma:',0,0,'L');
		$this->Cell(20,3,'Sello','R',0,'C');
		$this->Cell(80,3,'Firma:',0,0,'L');	
		$this->Cell(20,3,'Sello','R',1,'C');
		$this->Cell(100,3,'Nombre:','R',0,'L');
		$this->Cell(100,3,'Nombre:','R',1,'L');	
		$this->Cell(100,4,'Fecha: ','R',0,'L');
		$this->Cell(100,4,'Fecha:','R',1,'L');	  
	} 
  }  
  ...
  while($registro=pg_fetch_array($res)){     $tmonto=$registro["monto"];
	...
	if(($letra=="A")and($registro["cod_concepto"]=="L01")){
	    if($tmonto==$temp_monto_ant_depositada){ $denominacion="Garantia de Prestaciones Sociales enviadas a Fidecomiso"; $cantidad=""; $sueldo_dia="";}
	}
	$monto=formato_monto($tmonto);   $denominacion=utf8_decode($denominacion);
	...
	if($asig_ded_apo=="A"){ $total=$total+$tmonto; $totala=$totala+$tmonto; $asignacion=$monto; $sueldo_dia=formato_monto($sueldo_dia); $na=$na+1; 
	  ...
	  $pdf->Cell(11,5,$cantidad,'RL',0,'C'); 
	  $pdf->Cell(14,5,$sueldo_dia,'RL',0,'R'); 
