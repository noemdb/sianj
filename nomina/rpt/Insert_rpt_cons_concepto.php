...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if($error==0){if ($des_repote==''){$error=1; ?> <script language="JavaScript">muestra('DESCRIPCION NO ES VALIDO');</script><? } }  
  if($error==0){if ($den_arch_rpt==''){$error=1; ?> <script language="JavaScript">muestra('NOMBRE REPORTES NO ES VALIDO');</script><? } }  
  
  if($error==0){$sSQL="SELECT ACTUALIZA_NOM047(1,'$cod_reporte','$des_repote','$den_arch_rpt','$status')";
