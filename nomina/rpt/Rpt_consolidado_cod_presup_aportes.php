   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Cell(140,5,'NOMBRE TRABAJADOR',1,0,'L');
			$this->Cell(20,5,'SSO',1,0,'C');
			$this->Cell(20,5,'SPF',1,0,'C');
			$this->Cell(20,5,'UTILIDADES',1,0,'C');
			$this->Cell(20,5,'VACACIONES',1,0,'C');
			$this->Cell(20,5,'BONO VAC.',1,1,'C');				
		}
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if(($prev_cod_empleado<>$cod_emp)or($prev_cod_cat<>$cod_cat)){ $cant_emp=$cant_emp+1;
			...
			if( $prev_cod_cat<>$cod_cat){
			  ...
			  $pdf->Cell(10,4,'',0,0);
			  $pdf->Cell(150,4,$prev_cod_cat.'    '.$nomb_cat,0,0);			
			  $pdf->Cell(20,4,$sub_tot1,'T',0,'R');
			  $pdf->Cell(20,4,$sub_tot2,'T',0,'R');
			  $pdf->Cell(20,4,$sub_tot3,'T',0,'R');
			  $pdf->Cell(20,4,$sub_tot4,'T',0,'R');
			  $pdf->Cell(20,4,$sub_tot5,'T',1,'R');  
