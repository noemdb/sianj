   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			...
			$this->Cell(30);
			$this->Cell(160,7,"RELACION MENSUAL DE APORTE PATRONAL POR CODIGOS PRESUPUESTARIOS",1,0,'C');
			$this->Ln(17);
			...
			$this->Ln(4);
			$this->Cell(28,4,'CODIGO','LRT',0,'C');
			$this->Cell(24,4,'SSO','RT',0,'C');
			$this->Cell(24,4,'SPF','RT',0,'C');
			$this->Cell(24,4,'UTILIDADES','RT',0,'C');
			$this->Cell(25,4,'VACACIONES','RT',0,'C');
			$this->Cell(25,4,'BONO','RT',0,'C');	
            $this->Cell(25,4,'TOTAL','RT',1,'C');	

            $this->Cell(28,3,'PRESUPUESTARIO','LR',0,'C');
			$this->Cell(24,3,'401-06-01-00','R',0,'C');
			$this->Cell(24,3,'401-06-04-00','R',0,'C');
			$this->Cell(24,3,'401-05-01-00','R',0,'C');
			$this->Cell(25,3,'401-05-03-00','R',0,'C');
			$this->Cell(25,3,'VACACIONAL','R',0,'C');	
            $this->Cell(25,3,'VACACIONAL','R',1,'C');

            $this->Cell(28,4,'UNIDAD','LRB',0,'C');
			$this->Cell(24,4,'','RB',0,'C');
			$this->Cell(24,4,'','RB',0,'C');
			$this->Cell(24,4,'','RB',0,'C');
			$this->Cell(25,4,'','RB',0,'C');
			$this->Cell(25,4,'401-05-03-00','RB',0,'C');	
            $this->Cell(25,4,'401-05-03-00','RB',1,'C');			
		}		
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if($prev_cod_cat<>$cod_cat){ $cant_emp=$cant_emp+1;
			...
			$pdf->Cell(28,4,$prev_cod_cat,1,0);			
			$pdf->Cell(24,4,$sub_tot1,1,0,'R');
			$pdf->Cell(24,4,$sub_tot2,1,0,'R');
			$pdf->Cell(24,4,$sub_tot3,1,0,'R');
			$pdf->Cell(25,4,$sub_tot4,1,0,'R');
			$pdf->Cell(25,4,$sub_tot5,1,0,'R');
			$pdf->Cell(25,4,$sub_tot6,1,1,'R');  
	  ...
	  $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $tot4=$tot4+$conc4; $tot5=$tot5+$conc5; 
		...
		$pdf->Cell(28,5,'TOTAL : ',1,0,'R');			
		$pdf->Cell(24,5,$tot1,1,0,'R');
		$pdf->Cell(24,5,$tot2,1,0,'R');
		$pdf->Cell(24,5,$tot3,1,0,'R');
		$pdf->Cell(25,5,$tot4,1,0,'R');
		$pdf->Cell(25,5,$tot5,1,0,'R');  
		$pdf->Cell(25,5,$tot6,1,1,'R'); 
	$pdf->Output(); 
}
?>
