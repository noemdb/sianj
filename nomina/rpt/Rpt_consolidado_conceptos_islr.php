...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTandO LA BASE DE DATOS'); </script> <?}
   ...
   //$criterio=$criterio." and (cod_concepto='502' or cod_concepto='802')";
   $criterio=$criterio." and (cod_concepto>='509') and (cod_categ>='".$cod_presup_catd."' and cod_categ<='".$cod_presup_cath."')";
   ...
   $sSQL = "SELECT *  FROM ".$criterio."  and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and
      ...
      class PDF extends FPDF{
		function Header(){ global $criterio1; global $tipo_nomina; global $des_nomina; global $fechad; global $fechah; global $cod_concepto; global $tipo_nomina_d; global $tipo_nomina_h;  global $criterio4; global $criterio3;
			$this->Image('../../imagenes/Logo_emp.png',10,7,20);
			$this->SetFont('Arial','B',11);
			$this->Cell(50);
			$this->Cell(160,7,"LISTADO DE IMPUESTO SOBRE LA RENTA",1,0,'C');
			$this->Ln(17);
			...
			$this->Cell(16,4,'','LRT',0,'L',true);
			$this->Cell(90,4,'','TR',0,'L',true);
			$this->Cell(30,4,'','TR',0,'L',true);
			$this->Cell(30,4,'PRIMERA','TR',0,'R',true);
			$this->Cell(30,4,'SEGUNDA','TR',0,'R',true);
			$this->Cell(30,4,'TOTAL','TR',1,'R',true);
			//$this->Cell(20,4,'PRIMERA','TR',0,'C',true);
			//$this->Cell(20,4,'SEGUNDA','TR',0,'C',true);
			//$this->Cell(20,4,'TOTAL','TR',0,'C',true);			
            //$this->Cell(20,4,'TOTAL','TR',1,'C',true);
			
            $this->Cell(16,4,'CODIGO','LRB',0,'L',true);
			$this->Cell(90,4,'NOMBRE TRABAJADOR','BR',0,'L',true);
			$this->Cell(30,4,'CEDULA','BR',0,'C',true);			
			$this->Cell(30,4,'QUINCENA','BR',0,'R',true);
			$this->Cell(30,4,'QUINCENA','BR',0,'R',true);
			$this->Cell(30,4,'RETENCION','BR',1,'R',true);
			//$this->Cell(20,4,'QUINCENA','BR',0,'C',true);
			//$this->Cell(20,4,'QUINCENA','BR',0,'C',true);
			//$this->Cell(20,4,'APORTE','BR',0,'C',true);
			//$this->Cell(20,4,'GENERAL','BR',1,'C',true);					   
		}
	  ...
	  $pdf->AliasNbPages();
   	  $pdf->AddPage();
	  $pdf->SetFont('Arial','',8);
	  $i=0; $can_conc=0; $cant_emp=0;  $tot1=0; $tot2=0; $tot3=0; $tot4=0; $tot5=0; $tot6=0; $totgen=0; $conc1=0; $conc2=0; $conc3=0; $conc4=0; $conc5=0; $conc6=0;
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_emp=$registro["cod_empleado"]; $tipo_nom=$registro["tipo_nomina"];	$cod_cat=$registro["cod_categ"];	
		if($prev_cod_empleado<>$cod_emp){ $cant_emp=$cant_emp+1;
		    $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $tot4=$tot4+$conc4; $tot5=$tot5+$conc5; $tot6=$tot6+$conc6; $totgen=$conc3+$conc6;
			...
			$totgen=formato_monto($totgen);
			$pdf->SetFont('Arial','',8);
			$pdf->Cell(16,4,$cod_empleado,0,0);
			$pdf->Cell(90,4,$nombre,0,0);
            $pdf->Cell(30,4,$cedula,0,0,'R');			
			$pdf->Cell(30,4,$conc1,0,0,'R');
			$pdf->Cell(30,4,$conc2,0,0,'R');
			$pdf->Cell(30,4,$conc3,0,1,'R');
			//$pdf->Cell(20,4,$conc4,0,0,'R');
			//$pdf->Cell(20,4,$conc5,0,0,'R');  
			//$pdf->Cell(20,4,$conc6,0,0,'R');  
			//$pdf->Cell(20,4,$totgen,0,1,'R'); 
		...
		if(($cod_concepto=="650")and($mdia=="15")) { $conc1=$conc1+$monto; }
		if(($cod_concepto=="650")and($mdia<>"15"))  { $conc2=$conc2+$monto; }
		//if(($cod_concepto=="802")and($mdia=="15")) { $conc4=$conc4+$monto; }
		//if(($cod_concepto=="802")and($mdia<>"15"))  { $conc5=$conc5+$monto; }
		$conc3=$conc1+$conc2;
		$conc6=$conc4+$conc5;
		$totgen=$conc6+$conc3;
		//if($cod_concepto=="502") { $conc3=$conc3+$monto; }
		//if($cod_concepto=="802") { $conc4=$conc4+$monto; }
		//$conc5=$conc3+$conc4; 
	  }		
	  $tot1=$tot1+$conc1; $tot2=$tot2+$conc2; $tot3=$tot3+$conc3; $tot4=$tot4+$conc4; $tot5=$tot5+$conc5; $tot6=$tot6+$conc6; $cant_emp=$cant_emp+1; $totgen=$conc3+$conc6;
			$sub_tot1=$sub_tot1+$conc1; $sub_tot2=$sub_tot2+$conc2; $sub_tot3=$sub_tot3+$conc3; $sub_tot4=$sub_tot4+$conc4; $sub_tot5=$sub_tot5+$conc5; $sub_tot6=$sub_tot6+$conc6;
			...
			//$pdf->Cell(20,4,$conc6,0,0,'R');  
			//$pdf->Cell(20,4,$totgen,0,1,'R');
		$pdf->Ln(2); $totgen=$conc3+$conc6;
		$tot1=formato_monto($tot1); $tot2=formato_monto($tot2); $tot3=formato_monto($tot3); $tot4=formato_monto($tot4); $tot5=formato_monto($tot5); $tot6=formato_monto($tot6);  $totgen=formato_monto($totgen);
		$pdf->Cell(90,0.5,' ',0,0,'R');			
		...
		$pdf->Cell(56,5,$tot1,'T',0,'R');
		$pdf->Cell(30,5,$tot2,'T',0,'R');
		$pdf->Cell(30,5,$tot3,'T',1,'R');
		//$pdf->Cell(20,5,$tot4,'T',0,'R');
		//$pdf->Cell(20,5,$tot5,'T',0,'R'); 
        //$pdf->Cell(20,5,$tot6,'T',0,'R'); 		
		//$pdf->Cell(20,5,$totgen,'T',1,'R'); 
	$pdf->Output(); 
}
?>
