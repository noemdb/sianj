<? include ("../../class/seguridad.inc"); include ("../../class/conects.php"); include("../../class/fun_fechas.php"); include("../../class/fun_numeros.php");   include ("../../class/configura.inc"); error_reporting(E_ALL ^ E_NOTICE); 
   $tipo_nomina_d=$_GET["tipo_nomina_d"];   $tipo_nomina_h=$_GET["tipo_nomina_h"]; $act_hist=$_GET["act_hist"]; $fecha_nom=$_GET["fecha_nom"]; $tipo_concepto=$_GET["tipo_concepto"]; $cod_concepto_d=$_GET["cod_concepto_d"]; $cod_concepto_h=$_GET["cod_concepto_h"]; 
   ...
   if($rango_f=='S'){ $act_hist='S';  $mes_comp='S'; $criterio="rpt_nom_hist WHERE (fecha_p_hasta>='".$cfechad."') and (fecha_p_hasta<='".$cfechah."') and (oculto='NO')  ";   
                      $criterio1="Fecha Desde: ".$fecha_desde." al ".$fecha_hasta;	} 
...
if(($tipo_rpt=="PDF")and(($Cod_Emp=="99"))){ $tipo_rpt="PDF4";}	
if(($tipo_rpt=="PDF")and(($Cod_Emp=="02"))){ $tipo_rpt="PDF5";}	
if(($tipo_rpt=="PDF")and(($Cod_Emp=="38"))){ $tipo_rpt="PDF6";}	
if(($tipo_rpt=="ARC")and(($Cod_Emp=="38"))){ $tipo_rpt="ARC6";}

if(($tipo_rpt=="TXT")and(($Cod_Emp=="02"))){ $tipo_rpt="TXT5";}	
if(($tipo_rpt=="WORD")and(($Cod_Emp=="02"))){ $tipo_rpt="WORD5";}	
    $sSQL = "SELECT *  FROM ".$criterio." and (tp_calculo='".$tipo_calculo."') and (tipo_nomina>='".$tipo_nomina_d."' and tipo_nomina<='".$tipo_nomina_h."') and (cod_empleado>='".$cod_empleado_d."' and cod_empleado<='".$cod_empleado_h."') and (cod_concepto>='".$cod_concepto_d."' AND cod_concepto<='".$cod_concepto_h."') and
	...
	//if($tipo_rpt=="PDF"){ 
    if(($tipo_rpt=="PDF")or($tipo_rpt=="ARC")){	$res=pg_query($sSQL);  $cod_empleado_grupo=""; $cod_concepto_grupo="";  	  
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_empleado=$registro["cod_empleado"]; $cod_concepto=$registro["cod_concepto"]; $nombre=$registro["nombre"]; 
	      ...
	      $paso_grupo=$registro["paso"];$grado_grupo=$registro["grado"]; $compensa_cargo=$registro["compensa_cargo"];
		  $sueldo_cargo_grupo=$sueldo_cargo; $nombre_banco_grupo=$nombre_banco; $compensa_cargo_grupo=$compensa_cargo; 
          ...
          if(($prev_cod_concepto<>$cod_concepto_grupo)or($prev_cod_empleado<>$cod_empleado_grupo)){ 
		   ...
		   if($prev_cod_empleado<>$cod_empleado_grupo){ 
			 if(($sub_total_monto_asignacion>0)or($sub_total_monto_deduccion>0)or($sub_total_monto>0)){ $sub_total_monto_asignacion=formato_monto($sub_total_monto_asignacion);
				...
				$pdf->Cell(80,3,'',0,1,'R');
                $pdf->AddPage();
			   }	$temp_sueldo=formato_monto($sueldo_cargo_grupo); $temp_compensa=formato_monto($compensa_cargo_grupo);
				...
				if($rango_f=='S'){$pdf->Cell(100,4,$criterio1,0,1,'L');}else{ $pdf->Cell(100,4,'Fecha Desde: '.$fechad_grupo."   "."hasta"."   ".$fechah_grupo,0,0,'L'); } 
				$pdf->Cell(60,4,'Sueldo Mensual: '.$temp_sueldo,0,0,'L');
				$pdf->Cell(40,4,'Compensacion: '.$temp_compensa,0,1,'L');
		  ...
		  $cod_categ=$registro["cod_categ"]; $fechad=$registro["fechad"]; $fechah=$registro["fechah"]; $sueldo_cargo=$registro["sueldo_cargo"]; $saldo=$registro["saldo"]; $compensa_cargo=$registro["compensa_cargo"];
        ...
        if($tipo_rpt=="PDF"){ $pdf->Output(); }else{ $pdf->Output('recibo.pdf','D'); } 
    }

    if(($tipo_rpt=="PDF6")or($tipo_rpt=="ARC6")){  $res=pg_query($sSQL);  $cod_empleado_grupo=""; $cod_concepto_grupo="";  	  
      require('../../class/fpdf/fpdf.php');
	    class PDF extends FPDF{
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_empleado=$registro["cod_empleado"]; $cod_concepto=$registro["cod_concepto"]; $nombre=$registro["nombre"];   
          ...
          if(($prev_cod_concepto<>$cod_concepto_grupo)or($prev_cod_empleado<>$cod_empleado_grupo)){ 
		   ...
		   if($prev_cod_empleado<>$cod_empleado_grupo){ 
			 if(($sub_total_monto_asignacion>0)or($sub_total_monto_deduccion>0)or($sub_total_monto>0)){ $sub_total_monto_asignacion=formato_monto($sub_total_monto_asignacion);
			    ...
			    $pdf->SetFont('Arial','B',11);
			    $pdf->Cell(20);
				$pdf->Cell(100,5,$Nom_Emp,0,1,'L');
				$pdf->Cell(20);
				$pdf->Cell(100,5,'RIF: G-20007147-8',0,0,'L');
				$pdf->SetFont('Arial','B',13);
			    $pdf->Cell(70,5,'RECIBO DE PAGO',0,1,'C');
		   	    $pdf->Ln(4);			
			    $pdf->SetFont('Arial','',9);
				$pdf->Cell(160,4,$tipo_nomina.' '.$des_nomina,0,0,'L');  
				$pdf->Cell(40,4,'Codigo : '.$cod_empleado_grupo,0,1,'L');
                ...
                //$pdf->Cell(40,4,'Paso: '.$paso_grupo.' Grado:'.$grado_grupo,0,0,'L'); 				
				$pdf->Cell(40,4,'Fecha Ingreso: '.$fechai_grupo,0,1,'L');
				$pdf->Cell(160,4,'Departamento: '.$des_departam_grupo,0,1,'L');  
				if($rango_f=='S'){$pdf->Cell(140,4,$criterio1,0,1,'L');}else{ $pdf->Cell(140,4,'Fecha Desde: '.$fechad_grupo."   "."hasta"."   ".$fechah_grupo,0,0,'L'); } 
				$pdf->Cell(60,4,'Sueldo Mensual: '.$temp_sueldo,0,1,'L');
				$pdf->Ln(2);
				$pdf->Cell(15,4,'Codigo',1,0,'L');
				$pdf->Cell(130,4,'Descripcion Concepto',1,0,'L');
				$pdf->Cell(15,4,'Cantidad',1,0,'C');
		...
		if ($tipo_rpt=="PDF6"){$pdf->Output();} else { $pdf->Output($nomb_arch,'D'); }
    ...
    if($tipo_rpt=="PDF4"){  $res=pg_query($sSQL);  $cod_empleado_grupo=""; $cod_concepto_grupo="";  	  
      require('../../class/fpdf/fpdf.php');
      class PDF extends FPDF{
		function Header(){ $y=$this->GetY(); }
		function Footer(){ $ffechar=date("d-m-Y");$fhorar=date("H:i:s a");  $y=$this->GetY(); }
	...
	if($tipo_rpt=="PDF5"){  $res=pg_query($sSQL);  $cod_empleado_grupo=""; $cod_concepto_grupo="";  	  
      require('../../class/fpdf/fpdf.php');
      class PDF extends FPDF{
		function Header(){ $y=$this->GetY(); }
		function Footer(){ $ffechar=date("d-m-Y");$fhorar=date("H:i:s a");  $y=$this->GetY(); }
	  ...
	  $pdf->SetAutoPageBreak(true, 7);
   	  $pdf->AddPage(); $tipo_letra='Arial'; //$tipo_letra='Times'; 
	  $pdf->SetFont($tipo_letra,'',9); $l=0; $num_lin_pag=15; $temp_num_rec=0; $num_conc=0; $prev_prestamo=""; $prev_valore=0; $prev_valorq=0; $prev_valoru=0;
	  $i=0; $total_monto=0; $sub_total_monto_asignacion=0;  $sub_total_monto_deduccion=0; $sub_total_monto=0; $total_cantidad=0; $resultado1=0; $resultado2=0;   $resultado3=0;		 $prev_cod_empleado=""; $prev_cod_concepto=""; $prev_nombre="";   
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_empleado=$registro["cod_empleado"]; $cod_concepto=$registro["cod_concepto"]; $nombre=$registro["nombre"];  $num_recibo=$registro["num_recibo"]; 
          $denominacion=$registro["denominacion"];$cedula=$registro["cedula"]; $des_cargo=$registro["des_cargo"]; $fechai=$registro["fechai"]; $des_departam=$registro["des_departam"];  
	      $cod_categ=$registro["cod_categ"]; $fechad=$registro["fechad"]; $fechah=$registro["fechah"]; $sueldo_cargo=$registro["sueldo_cargo"]; $tipo_pago=$registro["tipo_pago"];
	      $nombre_banco=$registro["nombre_banco"];      $cta_empleado=$registro["cta_empleado"]; $tipo_nomina=$registro["tipo_nomina"]; $des_nomina=$registro["des_nomina"]; $descripcion_ubi=$registro["descripcion_ubi"];
		  $prestamo=$registro["prestamo"]; $valoru=$registro["valoru"]; $valorq=$registro["valorq"];
		  if($php_os=="WINNT"){$des_nomina=$des_nomina; }else{$des_nomina=utf8_decode($des_nomina);  $des_cargo=utf8_decode($des_cargo); $nombre_banco=utf8_decode($nombre_banco);  $des_departam=utf8_decode($des_departam);  $nombre=utf8_decode($nombre); $denominacion=utf8_decode($denominacion); $descripcion_ubi=utf8_decode($descripcion_ubi);}
		  $cod_empleado_grupo=$cod_empleado; $cod_concepto_grupo=$cod_concepto; $nombre_grupo=$nombre; $denominacion_grupo=$denominacion; $cedula_grupo=$cedula; 
          $des_cargo_grupo=$des_cargo;  $fechai_grupo=$fechai;$des_departam_grupo=$des_departam; $cod_categ_grupo=$cod_categ; $fechad_grupo=$fechad; $fechah_grupo=$fechah; 
	      $sueldo_cargo_grupo=$sueldo_cargo; $nombre_banco_grupo=$nombre_banco; 
		  ...
		  if(($prev_cod_concepto<>$cod_concepto_grupo)or($prev_cod_empleado<>$cod_empleado_grupo)){ 
			 if(($total_cantidad>0)or($resultado1>0)or($resultado2>0)or($resultado3>0)){  $num_conc=$num_conc+1;
				...
				if($resultado2==0){$resultado2="";}else{$resultado2=formato_monto($resultado2); } 
				if($resultado3==0){$resultado3="";}else{$resultado3=formato_monto($resultado3);	}
				if(($prev_prestamo=="S")and($prev_valoru>0)){ $total_cantidad=round($prev_valoru,0).'/'.round($prev_valorq,0); }   
				$temp1=substr($prev_denominacion,0,100); $pdf->SetFont($tipo_letra,'',9);
				if($num_conc<=$num_lin_pag){ 
				$pdf->Cell(150,4,$temp1,0,0,'L');
				$pdf->Cell(10,4,$total_cantidad,0,0,'R');
		   		...
		   		$pdf->Cell(20,4,$resultado2,0,1,'R'); }
					...
					$tsub_total_monto_asignacion=formato_monto($sub_total_monto_asignacion); $tsub_total_monto_deduccion=formato_monto($sub_total_monto_deduccion); 
					$pdf->SetFont($tipo_letra,'',9);				
					$pdf->Cell(156,5,'SUB-TOTALES VAN ... ',0,0,'R');
					$pdf->SetFont($tipo_letra,'',9);
					$pdf->Cell(22,5,$tsub_total_monto_asignacion,0,0,'R');
					$pdf->Cell(22,5,$tsub_total_monto_deduccion,0,1,'R');
					$pdf->Ln(2);
					$y=$pdf->GetY();
					if($y<140){ $l=145-$y; $pdf->Ln($l);} else{$pdf->AddPage();}
					$l=0;
					...
					$pdf->SetFont($tipo_letra,'',9);				
					$pdf->Cell(156,5,'VIENEN ... ',0,0,'R');
					$pdf->Cell(22,5,$tsub_total_monto_asignacion,0,0,'R');
					$pdf->Cell(22,5,$tsub_total_monto_deduccion,0,1,'R');
					
                    $pdf->Cell(150,4,$Ptemp1,0,0,'L');
					$pdf->Cell(10,4,$total_cantidad,0,0,'R');
					$pdf->Cell(20,4,$resultado1,0,0,'R'); 
					$pdf->Cell(20,4,$resultado2,0,1,'R');
                    $num_conc=1;				
			 ...
			 $prev_cod_concepto=$cod_concepto_grupo; $prev_denominacion=$denominacion_grupo; $prev_prestamo=$prestamo; $prev_valore=$valore; $prev_valorq=$valorq; $prev_valoru=$valoru; $total_cantidad=0; $resultado1=0; $resultado2=0; $resultado3=0;}
		   if($prev_cod_empleado<>$cod_empleado_grupo){ 
			  if(($sub_total_monto_asignacion>0)or($sub_total_monto_deduccion>0)or($sub_total_monto>0)){ $sub_total_monto_asignacion=formato_monto($sub_total_monto_asignacion);
                $sub_total_monto_deduccion=formato_monto($sub_total_monto_deduccion); $sub_total_monto=formato_monto($sub_total_monto);	
                $pdf->SetFont($tipo_letra,'',9);				
    ...
    if(($tipo_rpt=="TXT") or ($tipo_rpt=="WORD")){  $res=pg_query($sSQL);  $cod_empleado_grupo=""; $cod_concepto_grupo=""; 	   
	   if($tipo_rpt=="TXT"){ $num_lin_pag=32; $salto="\n";
	   header('Content-type: application/txt');
       header("Content-Disposition: attachment; filename=recibo.txt");	
	   }else{ $num_lin_pag=32; $num_lin_pag2=31; $num_lin_pag2=32; $salto="\n";	   
           // $salto="\par";
	  ...
	  function encabezado(){ global $sueldo_cargo_grupo; global $salto; global $criterio1;  global $temp_num_rec; global $des_cargo_grupo; global $num_concepto;
	    global $fechad_grupo; global $fechah_grupo; global $des_nomina; global $rango_f; global $cedula_grupo; global $nombre_grupo; global $Nom_Emp; global $l; global $tipo_pago;
	    global $fechai_grupo; global $des_departam_grupo; global $cod_categ_grupo; global $descripcion_ubi; global $nombre_banco_grupo; global $cta_empleado_grupo; global $oporden;
	    $temp_sueldo=formato_monto($sueldo_cargo_grupo);
	    $temp_num_rec=$temp_num_rec+1; $temp_rec=$temp_num_rec; $len=strlen($temp_rec); $temp_rec=substr("00000",0,5-$len).$temp_rec;
	    if($oporden<>1){ $num_recibo=$temp_num_rec; }                			   
	    //echo $Nom_Emp."   RECIBO DE PAGO Nro. ".$num_recibo.$salto; $l=$l+1;
	    echo $Nom_Emp."       "."     RECIBO DE PAGO  ".$salto; $l=$l+1;
	    $temp2="Periodo: " .$fechad_grupo. " al " .$fechah_grupo; if($rango_f=='S'){$temp2=$criterio1;}	   
	    $temp1=substr($des_nomina,0,46);	   $temp1=Rellenarespder($temp1,46);
	    echo $temp1."   ".$temp2.$salto; $l=$l+1;
	    $temp1="Cedula: ".$cedula_grupo; $temp2="Nombre: ".$nombre_grupo;
	    $temp1=Rellenarespder($temp1,18); $temp2=Rellenarespder($temp2,65);
	    echo $temp1." ".$temp2.$salto; $l=$l+1;
	    $temp1="Cargo: ".substr($des_cargo_grupo,0,49); $temp2="Fecha Ingreso: ".$fechai_grupo;
	    $temp1=Rellenarespder($temp1,57); $temp2=Rellenarespder($temp2,28);
	    echo $temp1." ".$temp2.$salto; $l=$l+1;
	    $temp1="Departamento: ".substr($des_departam_grupo,0,52); 
	    if($oporden==3){ $temp1="Codigo Categoria: ".$cod_categ_grupo;  }
	    if($oporden==4){ $temp1="Ubicacion: ".substr($descripcion_ubi,0,52);  }
	    $temp1=Rellenarespder($temp1,65);	$temp2="Sueldo: ".$temp_sueldo;
	    echo $temp1." ".$temp2.$salto; $l=$l+1;
	    $temp1=Rellenarespder($temp1,55); $temp2=Rellenarespder($temp2,28);
	    if($tipo_pago=="DEPOSITO"){
		  $temp1="Deposito Banco: ".substr($nombre_banco_grupo,0,35);  $temp2="Cta.Nro: ".$cta_empleado_grupo;
		  $temp1=Rellenarespder($temp1,53); $temp2=Rellenarespder($temp2,30);
		  echo $temp1." ".$temp2.$salto; $l=$l+1;
	    }
	    $temp1="-----------------------------------------------------------------------------------";
	    echo $temp1.$salto; $l=$l+1;
	    $temp1="NOMBRE DEL CONCEPTO                              CANT.   ASIGNACIONES   DEDUCCIONES";
	    echo $temp1.$salto; $l=$l+1;
	    $temp1="-----------------------------------------------------------------------------------";
	    echo $temp1.$salto; $l=$l+1;
	    $num_concepto=0;
      }	  
      $l=0;  $temp_num_rec=0; $num_concepto=0;
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_empleado=$registro["cod_empleado"]; $cod_concepto=$registro["cod_concepto"]; $nombre=$registro["nombre"]; $num_recibo=$registro["num_recibo"]; 
          ...
          $cta_empleado_grupo=$cta_empleado; $fechai=formato_ddmmaaaa($fechai); $fechad=formato_ddmmaaaa($fechad); $fechah=formato_ddmmaaaa($fechah);
		  */
          if(($prev_cod_concepto<>$cod_concepto)or($prev_cod_empleado<>$cod_empleado)){ 
			if(($total_cantidad>0)or($resultado1>0)or($resultado2>0)or($resultado3>0)){ 
			    if($total_cantidad==0){$total_cantidad="";}else{$total_cantidad=formato_monto($total_cantidad);} 
				if($resultado1==0){$resultado1="";}else{$resultado1=formato_monto($resultado1); }
				if($resultado2==0){$resultado2="";}else{$resultado2=formato_monto($resultado2); } 
				if($resultado3==0){$resultado3="";}else{$resultado3=formato_monto($resultado3);	}	
                if(($prev_prestamo=="S")and($prev_valoru>0)){ $total_cantidad=round($prev_valoru,0).'/'.round($prev_valorq,0); }				
				$temp1=Rellenarespizq($resultado1,13); $temp2=Rellenarespizq($resultado2,13); $temp4=Rellenarespizq($total_cantidad,5);
				$temp3=substr($prev_denominacion,0,49); $temp3=Rellenarespder($temp3,49);
				$num_concepto=$num_concepto+1;				
				/* CONTROLA CANTIDAD DE CONCEPTOS POR LINEA*/
				if($num_concepto>14){ $rn=$num_lin_pag-5;
				  $sub_total_monto_asignacion=$sub_total_monto_asignacion-$monto_asignacion; 
                  $sub_total_monto_deduccion=$sub_total_monto_deduccion-$monto_deduccion;  $sub_total_monto=$sub_total_monto-($monto_asignacion-$monto_deduccion);
		          echo " ".$salto; $l=$l+1;
				  $esub_total_monto_asignacion=formato_monto($sub_total_monto_asignacion); $esub_total_monto_deduccion=formato_monto($sub_total_monto_deduccion); $esub_total_monto=formato_monto($sub_total_monto);	
				  $etemp1=Rellenarespizq($esub_total_monto_asignacion,13); $etemp2=Rellenarespizq($esub_total_monto_deduccion,13);
				  $etemp3="                   SUB-TOTALES VAN ... "; $etemp3=Rellenarespder($etemp3,55);
				  echo $etemp3." " .$etemp1." ".$etemp2.$salto; $l=$l+1;
                  $rn=$num_lin_pag; $tnum=$temp_num_rec/2;  $rnum=round($tnum,0);
				  if($tnum==$rnum){$rn=$num_lin_pag-5;}else{$rn=$num_lin_pag+1;} 				  
				  for ($i=$l; $i < $rn; $i++){echo " ".$salto;}
				  $l=0;
				  encabezado();
				  $sub_total_monto_asignacion=$sub_total_monto_asignacion+$monto_asignacion; 
                  $sub_total_monto_deduccion=$sub_total_monto_deduccion+$monto_deduccion;  $sub_total_monto=$sub_total_monto+$monto_asignacion-$monto_deduccion;
				  $etemp3="                        VIENEN ... "; $etemp3=Rellenarespder($etemp3,55);
				  echo $etemp3." " .$etemp1." ".$etemp2.$salto; $l=$l+1;	
				}
				echo $temp3." ".$temp4." ".$temp1." ".$temp2.$salto; $l=$l+1;
			}	
			
			if(($prestamo=="S")and($valoru>0)){ $txt_prestamo=round($valoru,0).'/'.round($valorq,0);  }else{$txt_prestamo="";}
			
			$prev_cod_concepto=$cod_concepto; $prev_denominacion=$denominacion; $prev_prestamo=$prestamo; $prev_valore=$valore; $prev_valorq=$valorq; $prev_valoru=$valoru;   $total_cantidad=0; $resultado1=0; $resultado2=0; $resultado3=0;}
		    ...
		    if($prev_cod_empleado<>$cod_empleado){ 
			 if(($sub_total_monto_asignacion>0)or($sub_total_monto_deduccion>0)or($sub_total_monto>0)){ 
				...
				$temp1="-----------------------------------------------------------------------------------";
			    echo $temp1.$salto;
				$temp1=Rellenarespizq($sub_total_monto_asignacion,13); $temp2=Rellenarespizq($sub_total_monto_deduccion,13); 
				$temp3="                                  TOTAL "; $temp3=Rellenarespder($temp3,55);
				echo $temp3." ".$temp1." ".$temp2.$salto; $l=$l+1;
				//echo " ".$salto; $l=$l+1;
				//$temp3="-------------------               NETO A COBRAR Bs. ==> ";
				$temp3="                                  NETO A COBRAR Bs. ==> ";
                                 $temp3=Rellenarespder($temp3,66);
				$temp2=Rellenarespizq($sub_total_monto,15);
				echo $temp3."  ".$temp2.$salto; $l=$l+1; 
                                $rn=$num_lin_pag; $tnum=$temp_num_rec/2;  $rnum=round($tnum,0);
				if($tnum==$rnum){$rn=$num_lin_pag-6;}else{$rn=$num_lin_pag;} 
                                
               // $temp3="  Recibe Conforme ";  
               // echo $temp3.$salto; $l=$l+1;
	...
	if(($tipo_rpt=="TXT5") or ($tipo_rpt=="WORD5")){  $res=pg_query($sSQL);  $cod_empleado_grupo=""; $cod_concepto_grupo="";     
	   if($tipo_rpt=="TXT5"){ $num_lin_pag=37;  $num_lin_pag2=37; $salto="\n";
	   header('Content-type: application/txt');
       header("Content-Disposition: attachment; filename=recibo.txt");	
	   }else{ $num_lin_pag=32;  $num_lin_pag2=32; $salto="\n";	   
           // $salto="\par";
       header('Content-type: application/vnd.ms-word');  
       header('Content-Disposition: attachment; filename=recibo.rtf');	
             //echo="\fs9"; 
	  ...
	  function encabezado5(){ global $sueldo_cargo_grupo; global $salto; global $criterio1;  global $temp_num_rec; global $des_cargo_grupo; global $num_concepto;
	    global $fechad_grupo; global $fechah_grupo; global $des_nomina; global $rango_f; global $cedula_grupo; global $nombre_grupo; global $Nom_Emp; global $l; global $tipo_pago;
	    global $fechai_grupo; global $des_departam_grupo; global $cod_categ_grupo; global $descripcion_ubi; global $nombre_banco_grupo; global $cta_empleado_grupo; global $oporden;
       ...
       $l=0;  $temp_num_rec=0; $num_concepto=0;  $tpag=0;
	  $i=0; $total_monto=0; $sub_total_monto_asignacion=0;  $sub_total_monto_deduccion=0; $sub_total_monto=0; $total_cantidad=0; $resultado1=0; $resultado2=0;   $resultado3=0;	$prev_num_recibo="";	 $prev_cod_empleado=""; $prev_cod_concepto=""; $prev_nombre="";   
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_empleado=$registro["cod_empleado"]; $cod_concepto=$registro["cod_concepto"]; $nombre=$registro["nombre"]; $num_recibo=$registro["num_recibo"]; 
          $denominacion=$registro["denominacion"];$cedula=$registro["cedula"]; $des_cargo=$registro["des_cargo"]; $fechai=$registro["fechai"]; $des_departam=$registro["des_departam"];  
	      $cod_categ=$registro["cod_categ"]; $fechad=$registro["fechad"]; $fechah=$registro["fechah"]; $sueldo_cargo=$registro["sueldo_cargo"]; $tipo_pago=$registro["tipo_pago"];
		  ...
		  */
          if(($prev_cod_concepto<>$cod_concepto)or($prev_cod_empleado<>$cod_empleado)){ 
			if(($total_cantidad>0)or($resultado1>0)or($resultado2>0)or($resultado3>0)){ 
				...
				$num_concepto=$num_concepto+1;				
				/* CONTROLA CANTIDAD DE CONCEPTOS POR LINEA*/
				if($num_concepto>15){ $rn=$num_lin_pag-5;
				  $sub_total_monto_asignacion=$sub_total_monto_asignacion-$monto_asignacion; 
                  $sub_total_monto_deduccion=$sub_total_monto_deduccion-$monto_deduccion;  $sub_total_monto=$sub_total_monto-($monto_asignacion-$monto_deduccion);
				  ...
				  $rn=$num_lin_pag2; $tnum=$temp_num_rec/2;  $rnum=round($tnum,0);
				  if($tnum==$rnum){$rn=$num_lin_pag2-6;}else{$rn=$num_lin_pag2;} $rn=$num_lin_pag2;
				  $tpag=$tpag+1; if($tpag==1){$rn=$rn+1;}else{$tpag=0; $rn=$rn;}
				  for ($i=$l; $i < $rn; $i++){echo " ".$salto;}
				  $l=0;
				  encabezado5();
				  $sub_total_monto_asignacion=$sub_total_monto_asignacion+$monto_asignacion; 
		    ...
		    if($prev_cod_empleado<>$cod_empleado){ 
			 if(($sub_total_monto_asignacion>0)or($sub_total_monto_deduccion>0)or($sub_total_monto>0)){ 
               ...
               // $temp3="  Recibe Conforme ";  
               // echo $temp3.$salto; $l=$l+1;
                $tpag=$tpag+1; if($tpag==1){$rn=$rn+1;}else{$tpag=0; $rn=$rn-1;}
				for ($i=$l; $i < $rn; $i++){echo " ".$salto;}
				$l=0;
			   }	
