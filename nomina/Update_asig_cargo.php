...
$codigo_mov=$_POST["txtcodigo_mov"]; $fecha=$_POST["txtfecha_asigna"]; $cod_cargo=$_POST["txtcodigo_cargo"]; $des_cargo=$_POST["txtdenominacion"]; $cod_empleado=$_POST["txtcod_empleado"];
...
$paso=$_POST["txtpaso"];  $grado=$_POST["txtgrado"];$sueldo=$_POST["txtsueldo"]; $prima=$_POST["txtprima"]; $compensacion=$_POST["txtcompensacion"];  $observacion="";
...
$url="Det_inc_asig_cargo.php?codigo_mov=".$codigo_mov;
...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
 ...
 /*
 if($error==0){if($sueldo==0){$error=1; ?> <script language="JavaScript">muestra('MONTO SUELDO NO ES VALID0');</script><?} } */ $sfecha=formato_aaaammdd($fecha);
 if($error==0){ $sSQL="Select * from NOM068 WHERE codigo_mov='$codigo_mov' and cod_empleado='$cod_empleado' and fecha_asigna='$sfecha'";    $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas==0){$error=1;  ?> <script language="JavaScript"> muestra('ASIGNACION DE CARGO NO EXISTE');</script><? }
   else{ $registro=pg_fetch_array($resultado);  $otros=$registro["otros"]; $otros=formato_numero($otros); $sueldo_integral=$registro["sueldo_integral"]; $sueldo_integral=formato_numero($sueldo_integral); $sfecha=formato_aaaammdd($fecha);
      $sSQL="SELECT ACTUALIZA_NOM068(2,'$codigo_mov','$cod_empleado','$sfecha','$cod_cargo','$cod_departamento','$des_cargo','$des_departamento','$cod_tipo_personal','$paso','$grado','$observacion',$sueldo,$prima,$compensacion,$otros,$sueldo_integral)";
      //ECHO $sSQL;
