...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('FORMULA DE CONCEPTO NO EXISTE');</script><? }
   ...
   if($error==0){ $sSQL="SELECT ACTUALIZA_NOM048(3,'$tipo_nomina','$cod_concepto','$consecutivo','$aaccion',$mrago1,$mrago2,'','','','$minf_usuario')";
      ...
      $desc_doc="FORMULA DE CONCEPTO EXTRAORDINARIA, TIPO NOMINA:".$tipo_nomina.", CODIGO CONCEPTO:".$cod_concepto.", CONSECUTIVO:".$consecutivo.", ACCION:".$aaccion.", RANGO INICIAL:".$mrago1.", RANGO FINAL:".$mrago2.", RESULTADO FINAL:".$mresult; $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('04','$usuario_sia','$usuario_sia','$equipo','Elimino','$sfecha','$desc_doc')");
