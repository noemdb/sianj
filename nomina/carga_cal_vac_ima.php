...
while(($registro=pg_fetch_array($res))and($dias_vac==0)) { $desde=$registro["desde"]; $hasta=$registro["hasta"];
  if(($f >=$desde)and($f<=$hasta)) { $dias_vac=$registro["vacaciones"]+$registro["vac_adicional"];  $dias_bono_vac=$registro["bono_vacacional"]; $dias_nohabiles=6; $dias_bono_vac_a=$registro["auxiliar1"]; 
  echo $f." ".$desde." ".$hasta." ".$registro["consecutivo"],"<br>";}
}  
}
if($tipo_nomina=="02"){ $dias_nohabiles=0; }
...
$num_dias=$dias_vac+$dias_nohabiles; 
if($dias_nohabiles==0){ $num_dias=$dias_vac+$dias_nohabiles-1;} else{ $num_dias=$dias_vac+$dias_nohabiles-1;} 
$fecha_hasta=nextDate($fecha_desde,$num_dias); $num_dias=$num_dias+1;
$fecha_rein=nextDate($fecha_desde,$num_dias);$fecha_hist=$temp_fecha; $monto_base=0;

echo $ultima_fecha." ".$temp_fecha." ".$sfecha." Desde:".$fecha_desde." Hasta:".$fecha_hasta,"<br>";
...
$monto_bono_vac=($monto_base/30) * $dias_bono_vac; $monto_bono_vac_a=($monto_base/30) * $dias_bono_vac_a;
$total_bono_vac=$monto_bono_vac+$monto_bono_vac_a; $total_bono_vac=formato_monto($total_bono_vac);
$monto_base=formato_monto($monto_base); $monto_bono_vac=formato_monto($monto_bono_vac); $dias_bono_vac=formato_monto($dias_bono_vac); $monto_bono_vac_a=formato_monto($monto_bono_vac_a); $dias_bono_vac_a=formato_monto($dias_bono_vac_a);
$sSQL="SELECT ELIMINA_NOM076('$codigo_mov')"; $resultado=pg_exec($conn,$sSQL);
?>
<form name="form2" method="post" action="Inc_cal_vacaciones_ima.php">
<table width="10">
  <tr>
     ...
     <td width="5"><input name="txtmonto_bono_vac_a" type="hidden" id="txtmonto_bono_vac_a" value="<?echo $monto_bono_vac_a?>"></td>
     <td width="5"><input name="txttotal_bono_vac" type="hidden" id="txttotal_bono_vac" value="<?echo $total_bono_vac?>"></td>	 
