...
$cod_estructura=$_POST["txtcod_estructura"];  $cod_concepto=$_POST["txtcod_concepto"]; $tp_calculo=$_POST["txttp_calculo"];  $codigo_mov=$_POST["txtcodigo_mov"];
...
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");  $url="Gen_orden_aporte.php?criterio=".$criterio;
...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if($error==0){ $dfecha=formato_aaaammdd($fecha_hoy); $tipo_documento="APORTE";  $nro_documento=$cod_concepto;  
    $sSQL="SELECT EST_APORTE_PAG006('$codigo_mov','$cod_estructura','$fecha_d','$fecha_h','$tipo_documento','$nro_documento')";$resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);$error=substr($error, 0, 61);
