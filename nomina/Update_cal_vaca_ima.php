...
$cod_empleado=$_POST["txtcod_empleado"];  $codigo_mov=$_POST["txtcodigo_mov"]; $cod_bon_vac="000"; $tipo_nomina ="00"; $cod_bon_vac_a='000'; $sfecha=formato_aaaammdd($fecha_hoy);
$fecha_causa_desde=$_POST["txtfecha_caus_desde"]; $fecha_causa_hasta=$_POST["txtfecha_caus_hasta"]; $fecha_d_desde=$_POST["txtfecha_d_desde"]; $fecha_d_hasta=$_POST["txtfecha_d_hasta"]; $fecha_reincorp=$_POST["txtfecha_reincorp"]; 
...
$dias_bono_vac_a=$_POST["txtdias_bono_vac_a"]; $dias_bono_vac_a=formato_numero($dias_bono_vac_a); if(is_numeric($dias_bono_vac_a)){$dias_bono_vac_a=$dias_bono_vac_a;}else{$dias_bono_vac_a=0;}
$monto_bono_vac_a=$_POST["txtmonto_bono_vac_a"]; $monto_bono_vac_a=formato_numero($monto_bono_vac_a); if(is_numeric($monto_bono_vac_a)){$monto_bono_vac_a=$monto_bono_vac_a;}else{$monto_bono_vac_a=0;}
$dias_disfrutados=0;
...
$url="Act_calculo_vacaciones_ima.php?Gcriterio=C".$cod_empleado;
...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  if($error==0){$sSQL="Select tipo_nomina,descripcion,con_sue_bas,con_compen,g_orden_pago,con_cal_vac,con_bon_vac,con_liqui2 from NOM001 WHERE tipo_nomina='$tipo_nomina'"; $resultado=pg_query($sSQL); $filas=pg_num_rows($resultado); if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('TIPO DE NOMINA NO EXISTE');</script><?} else{$registro=pg_fetch_array($resultado); $con_cal_vac=$registro["con_cal_vac"]; $cod_bon_vac=$registro["con_bon_vac"];  $cod_bon_vac_a=$registro["con_liqui2"];  $g_orden_pago=$registro["g_orden_pago"]; $cod_conc_s=$registro["con_sue_bas"]; $cod_conc_c=$registro["con_compen"];}}
  ...
  if(($error==0)and($calcula_nomina=="NO")){   
	...
	if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CONCEPTO BONO VACACIONAL NO ASIGNADO AL TRABAJADOR');</script><?} 
	  ...
	  $sSQL="SELECT ACTUALIZA_NOM076(1,'$codigo_mov','$cod_concepto','$den_concepto','$calculable','$asignacion','$acumula','$oculto','$tipo_a','$asig_ded_apo','$frec_valida','$prestamo','$concepto_vac','$int_cal_vac',$acumulado,$saldo,$valore,$valorq,$valoru,$valorv,$valorw,$valorx,$valory,$valorz,$cantidad,$valor,$monto_orig,'$cod_presup','$cod_contable','$afecta_presup','$cod_retencion','$fecha_ini','$fecha_exp','$fechadh',$frecuenciaa,'$cod_orden')"; $resultado=pg_exec($conn,$sSQL);$merror=pg_errormessage($conn); $merror=substr($merror,0,70);if(!$resultado){?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><? $error=11;}
    }
	$sqla="SELECT * FROM CONCEPTOS_ASIGNADOS WHERE (tipo_nomina='$tipo_nomina') And (cod_empleado='$cod_empleado') and (cod_concepto='$cod_bon_vac_a')"; $resa=pg_query($sqla); $filas=pg_num_rows($resa);
	if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE CONCEPTO BONO VACACIONAL ADICIONAL NO ASIGNADO AL TRABAJADOR');</script><?} 
	 else{$rega=pg_fetch_array($resa); $cod_concepto=$rega["cod_concepto"]; $den_concepto=$rega["denominacion"]; $cod_orden=$rega["cod_orden"]; $fecha_exp=$rega["fecha_exp"]; $fecha_ini=$rega["fecha_ini"]; $frecuenciaa=$rega["frecuenciaa"]; $frecuencia=$rega["frecuencia"]; $frec_valida="S";  $calculable=$rega["calculable"]; $statusa=$rega["status"]; $concepto_vac="S";
	  $calculable="NO"; $asignacion=$rega["asignacion"]; $oculto=$rega["oculto"]; $acumula=$rega["acumula"]; $tipo_a=$rega["tipo_asigna"]; $asig_ded_apo=$rega["asig_ded_apo"]; $prestamo=$rega["prestamo"]; $int_cal_vac=substr($statusa,0,1); $cantidad=$rega["cantidad"]; $monto_orig=$rega["monto"]; $acumulado=$rega["acumulado"]; $saldo=$rega["saldo"]; $cod_contable=$rega["cod_contable"];$cod_presup=$rega["cod_presup"]; $afecta_presup=$rega["afecta_presup"]; $cod_retencion=$rega["cod_retencion"];
      $valor=$monto_bono_vac_a; $cantidad=$dias_bono_vac_a; $valore=0; $valorq=0; $valoru=0; $valorv=0; $valorw=0; $valorx=0; $valory=0; $valorz=0; 
	  $cantidad=cambia_coma_numero($cantidad); $valor=cambia_coma_numero($valor);
	  $sSQL="SELECT ACTUALIZA_NOM076(1,'$codigo_mov','$cod_concepto','$den_concepto','$calculable','$asignacion','$acumula','$oculto','$tipo_a','$asig_ded_apo','$frec_valida','$prestamo','$concepto_vac','$int_cal_vac',$acumulado,$saldo,$valore,$valorq,$valoru,$valorv,$valorw,$valorx,$valory,$valorz,$cantidad,$valor,$monto_orig,'$cod_presup','$cod_contable','$afecta_presup','$cod_retencion','$fecha_ini','$fecha_exp','$fechadh',$frecuenciaa,'$cod_orden')"; $resultado=pg_exec($conn,$sSQL);$merror=pg_errormessage($conn); $merror=substr($merror,0,70);if(!$resultado){?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><? $error=11;}
    }
  }
  if($error==0){ $afechac=$fechacd;
     $sSQL="SELECT ACTUALIZA_NOM022(3,'','$cod_empleado','$afechac','$afechac','$afechac','$afechac','$afechac','000','000',0,0,0,0,0,0,'NO','$afechac','$afechac',0,'N','N','n','','','','$afechac','','','$afechac','','$usuario_sia','$minf_usuario','00')";
     $resultado=pg_exec($conn,$sSQL); $merror=pg_errormessage($conn); $merror="ERROR ELIMINANDO: ".substr($merror, 0, 91); if (!$resultado){$error=1;?><script language="JavaScript">muestra('<? echo $merror; ?>');</script><?}else{$error=0;}
  } 
  if($error==0){    $tipo_pago=substr($tipo_pago,0,3);    
     ...
     $resultado=pg_exec($conn,$sSQL); $merror=pg_errormessage($conn); $merror="ERROR GRABANDO: ".substr($merror, 0, 91); if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?}else{$error=0;?><script language="JavaScript">muestra('INCLUYO EXITOSAMENTE');</script><?  }  
    echo $sSQL;  
  } 
}
pg_close();
