...
$codigo_mov=$_POST["txtcodigo_mov"]; $cedula=$_POST["txtcedula"]; $nombre=$_POST["txtnombre"]; $parentesco=$_POST["txtparentesco"]; $sexo=$_POST["txtsexo"];
$fecha_nac=$_POST["txtfecha_nac"]; $edad=$_POST["txtedad"]; $edad=formato_numero($edad); if(is_numeric($edad)){ $edad=$edad;} else{$edad=0;} $status=$_POST["txtstatus"];
...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
 ...
 if($error==0){
  $sSQL="Select * from NOM069 WHERE codigo_mov='$codigo_mov' and cod_empleado='$cod_empleado' and ci_partida='$cedula'";
  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('INFORMACION FAMILIAR YA EXISTE');</script><? }
   else{$sfecha=formato_aaaammdd($fecha_nac);
      $sSQL="SELECT ACTUALIZA_NOM069(1,'$codigo_mov','$cod_empleado','$cedula','$nombre','$sexo','$sfecha',$edad,'$parentesco','$status','$campo_str1',$campo_num1)";
