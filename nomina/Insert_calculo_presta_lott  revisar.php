...
$sueldo_adic=0;  $valor_formula=36000; $fecha_ley="19/06/1997"; $fecha_dep_ley="19/07/1997"; $fecha_c=$tfechah;  $anos_cumplidos=0; $fecha_tope="30/04/2012"; $fecha_i_tope="01/01/2012"; $dias_dep_mes=6;
...
global $fecha_mes,$mprestaciones,$tprestaciones,$tadelanto,$sprestaciones,$interes_noacum,$tinteres,$interes_acum,$minteres,$mtasa,$mtotal,$monto_sueldo,$mtiempo;
  ...
  if($mcontinua==0){ if($fecha_ingreso>$fecha_l){$fecha_ing=formato_ddmmaaaa($fecha_ingreso);} else{$sdia=substr($fecha_ing,0,2); if($sdia=="31"){$sdia="30";} $fecha_ing=$sdia.substr($fecha_ley,2,8);} }
	...
	if($m1<$m2){ $mcontinua=1; $error=1;?><script language="JavaScript">muestra('FECHA CALCULO DE PRESTACIONES INVALIDA PARA EL TRABJADOR <? echo $cod_empleado; ?>');</script><?}
...
global $fecha_mes,$mprestaciones,$tprestaciones,$tadelanto,$sprestaciones,$interes_noacum,$tinteres,$interes_acum,$minteres,$mtasa,$mtotal,$monto_sueldo,$mtiempo,$dias_dep_mes;
  $dep_dias_adic="N";
  $p=0; $anos_cumplidos=0; $mcontinua=0; $fecha_max=$tfechah; $fecha_max=formato_aaaammdd($fecha_max); $fecha_l=formato_aaaammdd($fecha_ley);  $fecha_ing=formato_ddmmaaaa($fecha_ingreso);  $m1=FDate($fecha_ing); $m2=FDate($fecha_ley); $mcontinua=1;
  $StrSQL= "SELECT fecha_calculo,Sueldo_calculo,total_prestaciones,total_adelanto,saldo_prestaciones,acumulado_total,total_interes,interes_acum,interes_noacum from NOM030 Where (cod_empleado='$cod_empleado') Order by fecha_calculo desc,num_calculo desc"; $resultado=pg_query($StrSQL);
  if($registro=pg_fetch_array($resultado,0)){ $primera_fecha=$fecha_ingreso;  $fecha_umov=$registro["fecha_calculo"]; $fecha_c=formato_ddmmaaaa($registro["fecha_calculo"]); $monto_sueldo=$registro["sueldo_calculo"];
  $tprestaciones=$registro["total_prestaciones"]; $tadelanto=$registro["total_adelanto"]; $sprestaciones=$registro["saldo_prestaciones"]; $mtotal=$registro["acumulado_total"];
  $tinteres=$registro["total_interes"]; $interes_acum=$registro["interes_acum"]; $interes_noacum=$registro["interes_noacum"];
...
if($error==0){ $sql= "SELECT nom006.cod_empleado,nom006.nombre,nom006.cedula,nom006.fecha_ingreso,nom006.status,nom001.tipo_nomina,nom001.status_tipo from NOM006,NOM001 Where (nom001.tipo_nomina=nom006.tipo_nomina) and (nom006.Status='ACTIVO' Or nom006.Status='VACACIONES' Or nom006.Status='PERMISO') And (nom006.Status<>'JUBILADO') And (nom006.Status<>'PENSIONADO') And (nom001.tipo_nomina>='$tipo_nomina_d') And (nom001.tipo_nomina<='$tipo_nomina_h') And (nom006.cod_empleado>='$cod_empleado_d') And (nom006.cod_empleado<='$cod_empleado_h') order by nom006.cod_empleado"; $res=pg_query($sql);
while($reg=pg_fetch_array($res)){ $cod_empleado=$reg["cod_empleado"]; $fecha_ingreso=$reg["fecha_ingreso"];  $status_trab=$reg["status"];  $status_tipo=$reg["status_tipo"];
 ...
 if($dep_prest_mes=="N"){  if(substr($status_tipo,1,1)=="S"){$dep_prest_mes="S"; $prom_sueldo="N";} } 
 //echo $status_trab." ".$fecha_ingreso." ".$fecha_m." ".$cal_intereses." ".$dep_prest_mes." ".$prom_sueldo,"<br>"; 
 If((($status_trab=="ACTIVO") Or ($status_trab=="VACACIONES") Or ($status_trab=="PERMISO"))and($fecha_ingreso<$fecha_m)){ $mCal_intereses=$cal_intereses; if(substr($status_tipo,0,1)=="S"){$mCal_intereses="S";} $p=0;
   if($mCal_intereses=="S"){ if($dep_prest_mes=="N"){ $p=procesar_prestaciones(); } else { $p=procesar_prestaciones(); } } else{ if($dep_prest_mes=="S"){ $p=procesar_prestaciones_mes2(); } else {$p=procesar_prestaciones2(); } }
