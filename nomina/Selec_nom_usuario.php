...
$criterio=$_GET["criterio"];  $tipo_nomina=$_GET["tipo_nomina"]; $gnomina=$tipo_nomina;
$equipo = getenv("COMPUTERNAME"); $minf_usuario = $equipo." ".date("d/m/y H:i a");  echo "ESPERE POR FAVOR ELIMINANDO....","<br>";$error=0;
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");  $url="Det_sel_nom_usuarios.php?criterio=".$criterio;
if (pg_ErrorMessage($conn)) { ?>  <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
  ...
  if ($filas==0){ $error=1; ?>  <script language="JavaScript">  muestra('TIPO DE NOMINA NO EXISTE EN LA DEFINCION DE ARCHIVO');  </script> <? }
   else{$sSQL="SELECT ACTUALIZA_NOM059(2,'$criterio','$tipo_nomina','SI')"; $resultado=pg_exec($conn,$sSQL); $error=pg_errormessage($conn);$error=substr($error, 0, 61);
       if (!$resultado){?><script language="JavaScript">muestra('<? echo $error; ?>');</script><?}
	   else{$error=0; $_SESSION["gnom"]=$gnomina; ?><script language="JavaScript">muestra('ACTUALIZO EXITOSAMENTE');</script><?}
  }
