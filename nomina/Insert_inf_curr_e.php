...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
 ...
 if($error==0){
  $sSQL="Select * from NOM067 WHERE codigo_mov='$codigo_mov' and cod_empleado='$cod_empleado' and fecha='$fecha'";
  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas>=1){$error=1; ?> <script language="JavaScript"> muestra('INFORMACION CURRICULAR YA EXISTE');</script><? }
   else{$sfecha=formato_aaaammdd($fecha);
      $sSQL="SELECT ACTUALIZA_NOM067(1,'$codigo_mov','$cod_empleado','$sfecha','$titulo','$instituto','$descripcion')";
