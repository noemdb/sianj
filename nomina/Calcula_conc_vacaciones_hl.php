...
$codigo_mov=$_GET["codigo_mov"]; $cod_empleado=$_GET["cod_empleado"];$fecha_desde=$_GET["fdesde"];$fecha_hasta=$_GET["fhasta"]; $mbono=$_GET["mbono"]; $dbono=$_GET["dbono"]; $cant_cal=$_GET["cant_cal"]; $mbase=$_GET["mbase"];
$fecha_nacimiento=""; $tipo_nomina=""; $mbono=formato_numero($mbono); $dbono=formato_numero($dbono); $mbase=formato_numero($mbase);
...
if($filas>=1){ $registro=pg_fetch_array($res); $error=0; 
  ...
  $tipo_nomina=$registro["tipo_nomina"]; $sueldo=$registro["sueldo"]; $fecha_nacimiento=$registro["fecha_nacimiento"];
}
$nro_semanas=Asigna_Nro_Semanas($fecha_desde,$fecha_hasta); $num_semanas=$nro_semanas;
//echo "Semanas: ".$nro_semanas,"<br>";
$fecha_pago_vac=$fecha_hoy; $pago_vacaciones="N"; $num_recibo=0; $redondear="N"; $bloqueada="N"; $u_semana="S"; 
...
return $H;}
function Valor_Concepto($codigo){global $cual,$codigo_mov,$cod_concepto,$valorx,$valorw,$valory,$valorz,$fechah;
...
$cant_trab=0; $hora1=time();  $campo502="NNNNNNNNNNNNNNNNNNN"; $error=0;  $Monto_Sueldo_SSO=0;
...
if($filas==0){$error=1;?><script language="JavaScript">muestra('TIPO DE NOMINA NO EXISTE');</script><?}else{$registro=pg_fetch_array($resultado); $g_orden_pago=$registro["g_orden_pago"]; $frec_nom=$registro["frecuencia"]; $des_nomina=$registro["descripcion"]; $desc_grupo=$registro["desc_grupo"]; $redondear=$registro["redondear"]; $bloqueada=$registro["bloqueada"];$con_cal_vac=$registro["con_cal_vac"];$con_bon_vac=$registro["con_bon_vac"];$con_cal_vac=$registro["con_cal_vac"]; $con_bon_vac_ant=$registro["con_bon_vac"]; }}
...
if($error==0){ $fecha_proc_h=$fecha_hasta; $f_proc_h=$fechah; $cant_cal=0; $frec=$frec_nom;
$mf=FDate($fecha_hasta);  $md=FDate($fecha_desde); $fin_ciclo=0; $mdia=$mbase/30;
while(($mf>=$md)and($fin_ciclo==0)){ $cambio_periodo="N";
   ...
   If(($status_trab=="ACTIVO") Or ($status_trab=="PERMISO RE")  Or ($status_trab=="REPOSO")  Or (($status_trab=="VACACIONES") And ($reg["tipo_vacaciones"]=="S") And ($proc_vac_nom=="S"))){  
	 ...
	 while(($rega=pg_fetch_array($resa))and($error==0)){ $cod_concepto=$rega["cod_concepto"]; $den_concepto=$rega["denominacion"]; $cod_orden=$rega["cod_orden"]; $fecha_exp=$rega["fecha_exp"]; $fecha_ini=$rega["fecha_ini"]; $frecuenciaa=$rega["frecuenciaa"]; $frecuencia=$rega["frecuencia"]; $frec_valida="S";  $calculable=$rega["calculable"]; $status=$rega["status"]; $concepto_vac="N"; if($camb_fecha_exp=='S'){ $fecha_ini=$fecha_aus_hasta; }
      if($fechad<=$fecha_exp){ $continua=0;  if(frecuencia_valida($frec_nom,$cal_frecuencia,$frecuencia,$u_semana)==false){$continua=1;} if(frecuencia_valida($frec_nom,$cal_frecuencia,$frecuenciaa,$u_semana)==false){$continua=1;} 
       if($continua==0){$calculable=$rega["calculable"]; $asignacion=$rega["asignacion"]; $oculto=$rega["oculto"]; $acumula=$rega["acumula"]; $tipo_a=$rega["tipo_asigna"]; $asig_ded_apo=$rega["asig_ded_apo"]; $prestamo=$rega["prestamo"]; $int_cal_vac=substr($status,0,1); $cantidad=$rega["cantidad"]; $monto_orig=$rega["monto"]; $acumulado=$rega["acumulado"]; $saldo=$rega["saldo"]; $cod_contable=$rega["cod_contable"];$cod_presup=$rega["cod_presup"]; $afecta_presup=$rega["afecta_presup"]; $cod_retencion=$rega["cod_retencion"];
		...
		if(($cod_concepto=="105")or($cod_concepto=="300")or($cod_concepto=="305")){ $ndif=15; if($frec_nom=="S"){$ndif=7;} if($frec_nom=="M"){$ndif=30;} $cantidad=$ndif;
		...
		if(($cod_concepto=="001")and($cantidad>0)){ $oculto="SI";  $valor=0; $monto_sueldo=$monto_orig; }		
		if($con_bon_vac==$cod_concepto){ $calculable="NO";  if($cant_cal==1){$valor=$mbono; $cantidad=$dbono;}	$concepto_vac="S"; }
		if(($cod_concepto=="300")or($cod_concepto=="301")or($cod_concepto=="305")){ $oculto="SI"; if($cod_concepto=="301"){$valor=$mdia*30;} $calculable="NO"; }		
		if(($asig_ded_apo=="A")and($cambio_periodo=="S")){
		 if(($cod_concepto<>"300")and($cod_concepto<>"301")and($cod_concepto<>"305")and($cod_concepto<>"001")and($cod_concepto<>"303")and($cod_concepto<>"350")and($cod_concepto<>"105")){ $calculable="NO"; $valor=0; $cantidad=0;}
		 if($cod_concepto=="303"){$calculable='SI';}
		...
		if($cod_concepto=="105"){ $monto_303=$valor; $cant_105=$cantidad; if($fecha_hasta=="31/12/2010"){$cantidad=16; $valor=($valor/15)*16;} }

        if(($cod_concepto=="001") and ($fecha_hasta=="31/12/2010")){ $cantidad=16; } 		
		if($cod_concepto=="300" or $cod_concepto=="305"){ if($fecha_hasta=="31/12/2010"){$cantidad=16; $valor=($valor/15)*16;} }

		if(($asig_ded_apo=="D")and($cambio_periodo=="S")and($cant_105<10)){		
		  if(($cod_concepto<>'500')and($cod_concepto<>'502')and($cod_concepto<>'505')and($cod_concepto<>'506')and($cod_concepto<>'512')){ $calculable='NO'; $cantidad=0; $monto_orig=0; $valor=0;
		  }
		} 
		
		 //if($cod_concepto=="500"){  echo $cod_concepto," ".$calculable." ".$valor,"<br>"; }		 
	 ...
	 while(($rega=pg_fetch_array($resa))and($error==0)){ $cod_concepto=$rega["cod_concepto"]; $valor=$rega["valor"]; $cantidad=$rega["cantidad"]; $acumulado=$rega["acumulado"]; $saldo=$rega["saldo"]; $monto_orig=$rega["monto_orig"]; $valore=$rega["valore"]; $valoru=$rega["valoru"]; $valorq=$rega["valorq"]; $valorw=$rega["valorw"]; $valorx=$rega["valorx"]; $valory=$rega["valory"]; $valorz=$rega["valorz"]; $cod_orden=$rega["cod_orden"];
	  ...
	  $entra=10; 	   
	   ...
	   if(($cod_concepto=="303")and($valor==0)){ $valor=$monto_303;
		 //echo $cod_concepto," ".$calculable." ".$valor,"<br>"; 
		 }		 
	   //if($cod_concepto=="500"){  echo $cod_concepto," ".$calculable." ".$valor,"<br>"; }	 
 ...
 if($g_orden_pago=="S"){ $sSQL="SELECT cod_presup,valor,cod_concepto,cod_empleado  FROM nom076 where (afecta_presup='SI') and (asignacion='SI') and (codigo_mov='$codigo_mov') and (cod_presup not in (select cod_presup from pre001)) order by cod_presup"; $res=pg_query($sSQL);
 ...
 $sSQL="SELECT cod_presup,sum(valor) as valor FROM nom076 where (afecta_presup='SI') and (asignacion='SI') and (codigo_mov='$codigo_mov') group by cod_presup order by cod_presup"; $res=pg_query($sSQL);
 while(($reg=pg_fetch_array($res))and($error==0)){ $cod_presup=$reg["cod_presup"]; $valor=$reg["valor"]; $cod_fuente="00";
...
pg_close();?>
<script language="JavaScript">alert('FINALIZO CALCULO'); document.location ='<? echo $url; ?>'; </script>
