...
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  else{ $registro=pg_fetch_array($resultado); $adescrip=$registro["denominacion"]; $asueldo=$registro["sueldo_cargo"]; $acant=$registro["nro_cargos"]; $agrado=$registro["grado"]; $apaso=$registro["paso"]; }
  ...
  if($error==0){$sfecha=formato_aaaammdd($fecha_hoy);
    $sSQL="SELECT ACTUALIZA_NOM004(2,'$codigo_cargo','$denominacion','$grado','$paso',$nro_cargos,$asignados,$sueldo_cargo)";
    ...
    $desc_doc="CARGO, CODIGO:".$codigo_cargo.", DESCRIPCION:".$adescrip.", SUELDO:".$asueldo.", CANTIDAD:".$acant.", PASO:".$apaso.", GRADO:".$agrado;$resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('04','$usuario_sia','$usuario_sia','$equipo','Modifico','$sfecha','$desc_doc')");
