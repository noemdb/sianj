...
$url="Act_formula_ex.php?Gcodigo=C".$tipo_nomina.$cod_concepto.$consecutivo;
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname.""); $error=0;
if (pg_ErrorMessage($conn)){$error=1; ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
 else{ $sSQL="Select * from NOM048 WHERE tipo_nomina='$tipo_nomina' and cod_concepto='$cod_concepto' and consecutivo='$consecutivo'"; $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('FORMULA DE CONCEPTO NO EXISTE');</script><? }
   ...
   if($error==0){$sfecha=formato_aaaammdd($fecha_hoy);
    $sSQL="SELECT ACTUALIZA_NOM048(2,'$tipo_nomina','$cod_concepto','$consecutivo','$accion',$rango_inicial,$rango_final,'$calculo1','$calculo2','$calculofinal','$minf_usuario')";
    ...
    $desc_doc="FROMULA DE CONCEPTO EXTRAORDINARIA, TIPO NOMINA:".$tipo_nomina.", CODIGO CONCEPTO:".$cod_concepto.", CONSECUTIVO:".$consecutivo.", ACCION:".$aaccion.", RANGO INICIAL:".$mrago1.", RANGO FINAL:".$mrago2.", RESULTADO FINAL:".$mresult; $resultado=pg_exec($conn,"SELECT INCLUYE_SIA004('04','$usuario_sia','$usuario_sia','$equipo','Modifico','$sfecha','$desc_doc')");
