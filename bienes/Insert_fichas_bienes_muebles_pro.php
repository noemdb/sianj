<?include ("../class/conect.php");  include ("../class/funciones.php"); include ("../class/configura.inc"); error_reporting(E_ALL); $equipo = getenv("COMPUTERNAME");  $error=0; $cod_modulo="13";
  ...
  $denominacion=$_POST["txtdenominacion"]; $cod_dependencia=$_POST["txtcod_dependencia"];   $cod_empresa=$_POST["txtcod_empresa"];  $long_num_bien=$_POST["txtlong_num_bien"];
  ...
  $bien_en_salida="N";   $status_bien_mue="";   $inf_usuario=$usuario_sia." ".$equipo." ".date("d/m/y H:i a");
  $fecha_desincorporado=$fecha_incorporacion;  $des_desincorporado="";  $desincorporado=substr($desincorporado,0,1);  
  $nom_imp_presup=substr($nom_imp_presup,0,199);  $des_imp_nopresup=substr($des_imp_nopresup,0,200);   $sfecha=$fecha_incorporacion; $fecha_in=$fecha_incorporacion;
...
echo "ESPERE POR FAVOR INCLUYENDO....","<br>"; $url="Act_fichas_bienes_muebles_pro.php?Gcod_bien_mue=".$cod_bien_mue;
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
   ...
   $sSQL="Select codigo_c from BIEN008 WHERE codigo_c='$cod_clasificacion'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);   
   if($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CLASIFICACION NO EXISTE');</script> <? }  
    if(($error==0)and($num_bien_unico=="S")){ $sSQL="Select num_bien from BIEN015 WHERE num_bien='$num_bien'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
	  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('NUMERO DE BIEN YA EXISTE'); </script> <? }
    }
    if (strlen($num_bien)==$long_num_bien){$error=$error;} else{$error=1; ?> <script language="JavaScript">muestra('LONGITUD NUMERO DE BIEN INVALIDA');</script><? }	
