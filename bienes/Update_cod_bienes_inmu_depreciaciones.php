...
$url="Det_inc_bienes_mue_depreciacion.php?codigo_mov=".$codigo_mov;
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");;$error=0;
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
 else{
      $sSQL="Select * from BIEN014 WHERE cod_bien_inm='$cod_bien'";
      $resultado=pg_query($sSQL);
      $filas=pg_num_rows($resultado);
      if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL BIEN NO EXISTE');</script><? }
  if($error==0)
    {
      $resultado=pg_exec($conn,"SELECT MODIFICA_MOVIMIENTOS_BIEN050('$codigo_mov','','$cod_bien','','','',0.00,'$monto_c')");
