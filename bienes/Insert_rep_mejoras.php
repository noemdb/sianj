...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
  ...
  $sSQL="Select * from BIEN058 WHERE cod_bien_aso='$cod_bien_aso' and cod_bien_mue='$cod_bien_mue'";  $resultado=pg_query($sSQL);  $filas=pg_num_rows($resultado);
  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DEL  COMPONENTE YA EXISTE EN EL BIEN');</script><? }
  ...
  if($error==0){ $sfecha=formato_aaaammdd($fecha); $sql="SELECT ACTUALIZA_BIEN058(1,'$cod_bien_mue','$cod_bien_aso','$fecha_rep',$valor_rep,'$denominacion','$campo_str2',$monto1,$monto2)";
