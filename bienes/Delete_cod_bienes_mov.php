...
$cod_bien=$_GET["codigo"]; $codigo_mov=$_GET["codigo_mov"];$url="Det_inc_bienes_mue_movimientos.php?codigo_mov=".$codigo_mov; echo "ESPERE POR FAVOR ELIMINANDO....","<br>";$error=0;
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO BIEN NO EXISTE');</script> <? }
   else{ $sfecha=formato_aaaammdd($fecha);  $resultado=pg_exec($conn,"SELECT ACTUALIZA_BIEN050(3,'$codigo_mov','','$sfecha','$cod_bien','','','',1,0,'','',0,0)");
