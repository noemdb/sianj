...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('EL TIPO DE MOVIMIENTO NO EXISTE');</script> <? }
    else{ $error=1; $resultado=pg_exec($conn,"SELECT ACTUALIZA_BIEN003(3,'$codigo','','','','')"); $error=pg_errormessage($conn);  $error=substr($error,0,91);
