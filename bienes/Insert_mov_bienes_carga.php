...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
	...
	else{$codigo="066"; $tipo="D"; $sSQL="Select * from BIEN055 where cod_dependencia='$cod_dependencia' and fecha_desincorporado>='$sfecha' and fecha_desincorporado<='$sfechah'";}
	$res=pg_query($sSQL); echo $sSQL;
	while($registro=pg_fetch_array($res)){ $cod_bien_mue=$registro["cod_bien_mue"]; if($tipo=="I"){$codigo=$registro["codigo_tipo_incorp"];}
