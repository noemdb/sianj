...
$url="Det_inc_bienes_inmu_depreciacion.php?codigo_mov=".$codigo_mov;
$conn = pg_connect("host=".$host." port=5432 password=".$password." user=".$user." dbname=".$dbname."");$error=0;
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
 else
  {
  $sSQL="Select * from BIEN050 WHERE codigo_mov='$codigo_mov' and cod_bien='$cod_bien_inm'";
  ...
  if($error==0)
    {
      $sSQL="Select * from BIEN014 WHERE cod_bien_inm='$cod_bien_inm'";
      $resultado=pg_query($sSQL);
      $filas=pg_num_rows($resultado);
  ...
  if($error==0)
    {
      ...
      $resultado=pg_exec($conn,"SELECT INCLUYE_BIEN050('$codigo_mov','','$cod_bien_inm','$sfecha','','','',0,'$monto_c')");
