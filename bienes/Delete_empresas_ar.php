...
$cod_empresa=$_GET["Gcod_empresa"];  $equipo = getenv("COMPUTERNAME"); $minf_usuario = $equipo." ".date("d/m/y H:i a");
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
  ...
  if ($filas==0){$error=1; ?> <script language="JavaScript"> muestra('LA EMPRESA NO EXISTE');</script> <? }
    else{ $error=1; $resultado=pg_exec($conn,"SELECT ACTUALIZA_BIEN007(3,'$cod_empresa','')"); 
