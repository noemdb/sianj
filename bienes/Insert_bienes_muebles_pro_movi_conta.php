...
$url="Act_bienes_muebles_pro_movi_conta.php?Greferencia=".$referencia;
if ($error==0){ $conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
  ...
  if($error==0){  $sSQL="Select * from bien025 WHERE referencia='$referencia'";
  ...
  if($error==0){$sfecha=formato_aaaammdd($fecha); if($ced_rif==""){ $ced_rif=$Rif_Emp; }
     ...
     $sqlg="SELECT actualiza_bien025(1,'$codigo_mov','$referencia','$sfecha','$cod_dependencia','$gen_comp','N','N','$sfecha',0,'$minf_usuario','$descripcion','$statusc','$ced_rif')"; 
