...
$cod_bien_inm=$_POST["txtcod_bien_inm"]; $cod_clasificacion=$_POST["txtcod_clasificacion"];  $num_bien=$_POST["txtnum_bien"];  $long_num_bien=$_POST["txtlong_num_bien"];
$cod_bien_inm=$cod_clasificacion."-".$num_bien; $denominacion=$_POST["txtdenominacion"]; $cod_dependencia=$_POST["txtcod_dependencia"];   $cod_empresa="000"; 
...
echo "ESPERE POR FAVOR INCLUYENDO....","<br>"; $url="Act_ficha_bienes_inmuebles_pro.php?Gcod_bien_inm=".$cod_bien_inm;
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript">   muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');  </script> <?}
   ...
   $sSQL="Select codigo_c from BIEN008 WHERE codigo_c='$cod_clasificacion'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
    ...
    if(($error==0)and($num_bien_unico=="S")){ $sSQL="Select num_bien from BIEN014 WHERE num_bien='$num_bien'";  $resultado=pg_exec($conn,$sSQL);  $filas=pg_numrows($resultado);
   ...
   if($error==0){
	  ...
	  if ($filas>0){$error=1; ?> <script language="JavaScript"> muestra('CODIGO DE BIEN YA EXISTE'); </script> <? }
	     ...
	     $sSQL="SELECT ACTUALIZA_BIEN014(1,'$cod_bien_inm','$cod_clasificacion','$num_bien','','','$denominacion','$descripcion','$cod_dependencia','$cod_empresa','$cod_direccion','$cod_departamento',$area_inmueble,$area_terreno,$area_construccion,'$caracteristicas','$ced_responsable','$fecha_actualizacion','$direccion','$cod_region','$cod_entidad','$cod_municipio','$cod_ciudad','$cod_parroquia','$cod_postal','$tiene_planos',
