...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
	...
	if($tipo_rep=="PDF"){  $res=pg_query($sSQL); $cod_dependencia=""; $denominacion_dep="";  $subtotal=0; $fin=0; $prev_cod_dep=""; $direccion_dep=""; $denom_departamento=""; $cod_departamento=""; $denominacion_dir=""; $cod_direccion="";
		  ...
		  $marca=$registro["marca"]; $modelo=$registro["modelo"]; $color=$registro["color"]; $matricula=$registro["matricula"]; $serial1=$registro["serial1"]; $material=$registro["material"];
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $Nom_Emp; global $cod_dependencia; global $denominacion_dep; global $denom_departamento; global $cod_departamento; global $direccion_dep; global $agrup_dep; global $subtotal; global $denominacion_dir; global $cod_direccion;
				...
				$this->Cell(220,10,'INVENTARIO DE BIENES ESTADALES MUEBLES',0,0,'C');
			...
			function Footer(){ global $observacion; global $subtotal; global $fin;
				...
				$this->Cell(160,2,'',0,'C');
				$this->Cell(50,2,'ESTADALES',0,0,'C');
	...
	if($tipo_rep=="EXCEL"){	
		  ...
		  //$prev_cod_dep=$cod_dependencia; IF($agrup_dep=="SI"){$prev_cod_dep=$cod_dependencia.$cod_direccion.$cod_departamento; } 
		  $marca=$registro["marca"]; $modelo=$registro["modelo"]; $color=$registro["color"]; $matricula=$registro["matricula"]; $serial1=$registro["serial1"]; $material=$registro["material"];
		  if($marca==''){ $desc_marca=''; }else{ $denominacion=$denominacion.', MARCA: '.$marca; }	
		  if($modelo==''){ $desc_modelo=''; }else{ $denominacion=$denominacion.',MODELO: '.$modelo; }
		  if($serial==''){ $desc_serial=''; }else{ $denominacion=$denominacion.',SERIAL: '.$serial; }
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1;
            $cod_clasificacion=$registro["cod_clasificacion"]; 	$cod_bien_mue=$registro["cod_bien_mue"];  $num_bien=$registro["num_bien"];
		    $grupo=$registro["grupo"]; $subgrupo=$registro["subgrupo"]; $seccion=$registro["seccion"];  $direccion_dep=$registro["direccion_dep"]; $cod_direccion=$registro["cod_direccion"];  $denominacion_dir=$registro["denominacion_dir"];
			$cod_bien_mue=$registro["cod_bien_mue"]; $denominacion=$registro["denominacion"]; $edo_bien=$registro["edo_bien"]; $denom_departamento=$registro["denom_departamento"]; $cod_departamento=$registro["cod_departamento"]; 
			$valor_incorporacion=$registro["valor_incorporacion"]; $cod_dependencia=$registro["cod_dependencia"]; $denominacion_dep=$registro["denominacion_dep"];
			$marca=$registro["marca"]; $modelo=$registro["modelo"]; $color=$registro["color"]; $matricula=$registro["matricula"]; $serial1=$registro["serial1"]; $material=$registro["material"];
