...
$cod_bien_mued=$_GET["cod_bien_mued"];$cod_bien_mueh=$_GET["cod_bien_mueh"];$cod_empresad=$_GET["cod_empresad"];$cod_empresah=$_GET["cod_empresah"]; $agrup_dep=$_GET["agrup_dep"];  //$observacion=$_GET["observacion"];
...
$mordenado=" bien015.cod_dependencia,bien015.cod_bien_mue";
IF($agrup_dep=="SI"){ $mordenado=" bien015.cod_dependencia,bien015.cod_direccion,bien015.cod_departamento,bien015.cod_bien_mue";}
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
	...
	if($tipo_rep=="HTML"){	include ("../../class/phpreports/PHPReportMaker.php");
            ...
            $oRpt->setXML("Rpt_ficha_bie_mue_mo3_repor_bie_mue.xml");
            $oRpt->setUser("$user");
	...
	if($tipo_rep=="PDF"){  $res=pg_query($sSQL); $cod_dependencia=""; $denominacion_dep="";  $subtotal=0; $fin=0; $prev_cod_dep=""; $direccion_dep=""; $denom_departamento=""; $cod_departamento=""; $cod_direccion="";
		  ...
		  $denom_departamento=$registro["denom_departamento"]; $cod_departamento=$registro["cod_departamento"];  $prev_cod_dep=$cod_dependencia; $cod_direccion=$registro["cod_direccion"]; IF($agrup_dep=="SI"){$prev_cod_dep=$cod_dependencia.$cod_direccion.$cod_departamento; } 
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $Nom_Emp; global $cod_dependencia; global $denominacion_dep; global $denom_departamento; global $direccion_dep; global $agrup_dep; global $subtotal;
				...
				$this->Cell(200,10,'FICHA DE BIENES MUEBLES',0,0,'C');
                ...
                $this->SetFont('Arial','B',6); 				
				$this->Cell(195,4,'DESCRIPCION','TL',0,'L');
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1;
            $cod_clasificacion=$registro["cod_clasificacion"];$cod_bien_mue=$registro["cod_bien_mue"];  $num_bien=$registro["num_bien"]; $cod_direccion=$registro["cod_direccion"];
