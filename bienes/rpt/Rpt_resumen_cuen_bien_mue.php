...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
   ...
   if($tipo_rep=="PDF"){
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $Nom_Emp; global $mes_proceso; global $des_mes; global $ano;
				...
				$this->Cell(260,10,'RESUMEN DE LA CUENTA DE BIENES MUEBLES',0,1,'C');
				$this->Ln(4);
				$this->SetFont('Arial','',9);
				$this->Cell(200,5,'UNIDAD INVENTARIADA: '.$Nom_Emp,0,1,'L');
				$this->Cell(130,5,'ESTADO: '.$estado,0,0,'L');
				$this->Cell(130,5,'MUNICIPIO: '.$municipio,0,1,'L');
		  ...
		  //$pdf->rect($x,$y,260,185);
		  $pdf->SetFont('Arial','B',9);
		  $pdf->Cell(200,7,"Existencia Anterior",1,0,'L');
          $pdf->Cell(30,7,$saldo_anterior,1,0,'R');
		  $pdf->Cell(30,7,"",1,1,'R');
		  //$pdf->Ln(1);
		  $pdf->SetFont('Arial','',9);
		  $pdf->Cell(200,7,"Incorporaciones en el mes de la cuenta",1,0,'L');
          $pdf->Cell(30,7,$incorp,1,0,'R');
		  $pdf->Cell(30,7,"",1,1,'R');
		  //$pdf->Ln(3);

		  $pdf->Cell(200,5,"Desincorporaciones en el mes de la cuenta por todos los conceptos, con",'LRT',0,'L');
          $pdf->Cell(30,5,"",'LRT',0,'R');
		  $pdf->Cell(30,5,"",'LRT',1,'R');
		  $pdf->Cell(200,5,'excepcion del 60,"Faltantes de Bienes por Investigar"','LRB',0,'L');
          $pdf->Cell(30,5,"",'LRB',0,'R');
		  $pdf->Cell(30,5,$desincorp,'LRB',1,'R');
		  //$pdf->Ln(3);

		  $pdf->Cell(200,5,'Desincorporaciones en el mes de la cuenta por concepto 60 "Faltantes','LRT',0,'L');
          $pdf->Cell(30,5,"",'LRT',0,'R');
		  $pdf->Cell(30,5,"",'LRT',1,'R');
		  $pdf->Cell(200,5,' de Bienes por Investigar"','LRB',0,'L');
          $pdf->Cell(30,5,"",'LRB',0,'R');
		  $pdf->Cell(30,5,$desincorp60,'LRB',1,'R');

		  //$pdf->Ln(3);
		  $pdf->SetFont('Arial','B',9);
		  $pdf->Cell(200,7,"Existencia Final",1,0,'L');
          $pdf->Cell(30,7,"",1,0,'R');
		  $pdf->Cell(30,7,$saldo_final,1,1,'R');
		  $pdf->SetFont('Arial','',9);



		  //$pdf->Cell(200,5,"",0,0,'L');
          //$pdf->Cell(30,5,$totali,0,0,'R');
		  //$pdf->Cell(30,5,$totald,0,1,'R');



		  $pdf->Ln(20);
		  $pdf->SetFont('Arial','',8);
		  $pdf->Cell(260,5,'Firma del Jefe Responsable de la Unidad de Trabajo o Departamento : _______________________________',0,1,'C');
		  $pdf->Cell(260,5,'Nombre : _______________________________',0,1,'C');
		  $pdf->Cell(260,5,'Cedula : _______________________________',0,1,'C');
		$pdf->Output();
   ...
   if($tipo_rep=="PDFA"){
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $Nom_Emp; global $mes_proceso; global $des_mes; global $ano;
                $ffechar=date("d-m-Y");$fhorar=date("H:i:s a");  
                $estado="LARA"; $distrito="";  $municipio="Iribarren"; 	
			    $y=$this->GetY();$x=$this->GetX();
				$this->SetFont('Arial','BU',8);
				$this->Cell(260,5,'FORMULARIO BM-4',0,1,'R');
				$this->SetFont('Arial','B',12);
		  ...
		  $pdf->Cell(200,5,"Desincorporaciones en el mes de la cuenta por todos los conceptos, con",0,0,'L');
          $pdf->Cell(30,5,"",0,0,'R');	
		  $pdf->Cell(30,5,"",0,1,'R');
		  ...
		  $pdf->Cell(200,5,'Desincorporaciones en el mes de la cuenta por concepto 60 "Faltantes',0,0,'L');
          $pdf->Cell(30,5,"",0,0,'R');	
		  $pdf->Cell(30,5,"",0,1,'R');
