...
$criterio ="(bien014.cod_bien_inm>='$cod_bien_inmd' AND bien014.cod_bien_inm<='$cod_bien_inmh') AND (bien024.cod_dependencia>='$cod_dependenciad' AND bien024.cod_dependencia<='$cod_dependenciah') and (bien014.cod_direccion>='$cod_direcciond' and bien014.cod_direccion<='$cod_direccionh') AND
...
if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
         ...
         // LLAMAR A PHP_REPORT
          $sSQL = "SELECT bien024.referencia, bien024.fecha, bien014.cod_bien_inm, bien014.cod_clasificacion, substr(bien014.Cod_Clasificacion,1,1) as grupo, substr(bien014.Cod_Clasificacion,3,2) as subgrupo, substr(bien014.Cod_Clasificacion,6,1) as seccion, bien014.Num_Bien, bien014.Denominacion, bien014.cod_direccion,bien014.cod_departamento,
bien024.Cod_Dependencia, bien001.Denominacion_Dep, bien001.Direccion_Dep, bien041.Tipo_Movimiento, bien003.Denomina_Tipo, bien041.Tipo_ID, bien024.Descripcion, bien024.Anulado, bien041.Cantidad, bien041.Monto, bien014.Direccion, bien014.Valor_Incorporacion, bien024.Saldo_Anterior, bien006.denominacion_dep as denom_departamento  
FROM bien001, bien003, (bien014 LEFT JOIN bien006 ON (bien006.cod_departamento=bien014.cod_departamento and bien006.cod_dependencia=bien014.cod_dependencia and bien006.cod_direccion=bien014.cod_direccion)), bien024, bien041  WHERE bien001.Cod_Dependencia = bien024.Cod_Dependencia AND (bien024.Fecha=bien041.Fecha) AND
bien014.cod_bien_inm=bien041.cod_bien_inm AND  bien024.referencia=bien041.referencia AND bien041.Tipo_Movimiento = bien003.Codigo AND ((bien024.Anulado='N')) AND ".$criterio." order by ".$mordenado;
    ...
    function buca_saldo_ant_dep($mcod_dep){global $sfechad; global $host; global $password; global $user; global $dbname;	
	   ...
	   $fsql="select cod_bien_inm from bien041,bien024 where (bien024.referencia=bien041.referencia) and (bien024.fecha=bien041.fecha) and (bien041.fecha<'$sfechad') and (bien041.tipo_id='I') and (bien024.cod_dependencia='$mcod_dep') ";
	   $fres=pg_exec($conn,$fsql);$filas=pg_numrows($fres); if($filas>0){
	   $fsql="select sum(monto) as monto_mov from bien041,bien024 where (bien024.referencia=bien041.referencia) and (bien024.fecha=bien041.fecha) and (bien041.fecha<'$sfechad') and (bien041.tipo_id='I') and (bien024.cod_dependencia='$mcod_dep') ";
	   $fres=pg_exec($conn,$fsql);$filas=pg_numrows($fres); if($filas>0){$freg=pg_fetch_array($fres); $msaldo_ant=$msaldo_ant+$freg["monto_mov"];} 
	   }
	   $fsql="select cod_bien_inm from bien041,bien024 where (bien024.referencia=bien041.referencia) and (bien024.fecha=bien041.fecha) and (bien041.fecha<'$sfechad') and (bien041.tipo_id='D') and (bien024.cod_dependencia='$mcod_dep') ";
	   $fres=pg_exec($conn,$fsql);$filas=pg_numrows($fres); if($filas>0){
	   $fsql="select sum(monto) as monto_mov from bien041,bien024 where (bien024.referencia=bien041.referencia) and (bien024.fecha=bien041.fecha) and (bien041.fecha<'$sfechad') and (bien041.tipo_id='D') and (bien024.cod_dependencia='$mcod_dep') ";
	   $fres=pg_exec($conn,$fsql);$filas=pg_numrows($fres); if($filas>0){$freg=pg_fetch_array($fres); $msaldo_ant=$msaldo_ant-$freg["monto_mov"];} 
	   }	  
	   return $msaldo_ant;
	}
    ...
    if($tipo_rep=="HTML"){	include ("../../class/phpreports/PHPReportMaker.php");
			...
			//Rpt_movi_bie_inmu_repor_bie_inmu
            $oRpt->setXML("Rpt_movi_bie_mue_mov_repor_bie_mue.xml");
            $oRpt->setUser("$user");
	     ...
	     $filas=pg_num_rows($res); if($filas>=1){ $registro=pg_fetch_array($res,0); $cod_dependencia=$registro["cod_dependencia"];  $prev_cod_dep=$registro["cod_dependencia"];
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $Nom_Emp; global $cod_dependencia; global $denominacion_dep;  global $denom_departamento; global $direccion_dep; global $agrup_dep;  global $mes_proceso;
				...
				$this->rect(9.8,10,260.2,179);
			    $y=$this->GetY();$x=$this->GetX();
				$this->SetFont('Arial','B',8);
				$this->Cell(150,5,$Nom_Emp,0,1);
				$this->SetFont('Arial','B',12);
				$this->Cell(200,10,'MOVIMIENTOS DE BIENES INMUEBLES',0,0,'C');
			...
			function Footer(){  global $subtotali; global $subtotald; global $fin; global $saldo_anterior; global $subtotal;
				...
				if($fin==1){
				    $this->Cell(223,4,'TOTALES ...  ','RT',0,'R');
					$this->Cell(18,4,$smontoi,'RT',0,'R');
					$this->Cell(19,4,$smontod,'T',1,'R');					
					$this->Cell(260,3,'RESUMEN',1,1,'C');					
					$this->Cell(65,3,'EXISTENCIA ANTERIOR',1,0,'C');
					$this->Cell(65,3,'MAS INCORPORACIONES',1,0,'C');
					$this->Cell(65,3,'MENOS DESINCORPORACIONES',1,0,'C');
					$this->Cell(65,3,'EXISTENCIA ACTUAL',1,1,'R');	
					$this->Cell(65,4,$smontoa,1,0,'R');
					$this->Cell(65,4,$smontoi,1,0,'R');
					$this->Cell(65,4,$smontod,1,0,'R');
					$this->Cell(65,4,$smontoc,1,1,'R');					
					$this->SetFont('Arial','',6);				   
				    $this->Cell(130,3,'OBSERVACIONES','RL',0,'L');
				    $this->Cell(50,3,'PRESPARDO POR','RL',0,'L');
				    $this->Cell(50,3,'JEFE DE LA UNIDAD','RL',0,'L');
				    $this->Cell(30,3,'SELLO','RL',1,'L');					
					$this->Cell(130,10,'','RLB',0,'C');
					$this->Cell(50,10,'','RLB',0,'C');
					$this->Cell(50,10,'','RLB',0,'C');
					$this->Cell(30,10,'','RLB',0,'C');
				}
				else{
				    $this->Cell(223,4,'SUBTOTALES ...  ','RT',0,'R');
					$this->Cell(18,4,$smontoi,'RT',0,'R');
					$this->Cell(19,4,$smontod,'T',1,'R');
				}
			}
		  ...
		  $pdf->SetFont('Arial','',7);
		  $i=0;  $totali=0; $totald=0;   $subtotali=0;  $subtotald=0; $c=0; $totalg=0; $subtotal=0;
		  while($registro=pg_fetch_array($res)){ $i=$i+1;
		    ...
		    $grupo=$registro["grupo"]; $subgrupo=$registro["subgrupo"]; $seccion=$registro["seccion"];  $direccion_dep=$registro["direccion_dep"];	$cod_bien_inm=$registro["cod_bien_inm"]; $denominacion=$registro["denominacion"]; $tipo_id=$registro["tipo_id"];
			...
			if($php_os=="WINNT"){$denominacion=$denominacion; }else{$denominacion=utf8_decode($denominacion); $denom_departamento=utf8_decode($denom_departamento); $denominacion_dep=utf8_decode($denominacion_dep); $direccion_dep=utf8_decode($direccion_dep); }
