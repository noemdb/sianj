...
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname.""); if (pg_ErrorMessage($conn)){ ?> <script language="JavaScript">  muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?}
    ...
    $sqle="Select * from SIA000 order by campo001";  $resultado=pg_query($sqle);if ($registro=pg_fetch_array($resultado,0)){$cod_emp=$registro["campo001"]; $direccion=$registro["campo006"]; $nombre_emp=$registro["campo004"]; 
    $nom_completo=$registro["campo005"]; $region=$registro["campo041"]; $estado=$registro["campo010"];$municipio=$registro["campo011"];$ciudad=$registro["campo009"];
    $rif_emp=$registro["campo007"]; $nit_emp=$registro["campo008"]; $fecha_ini=$registro["campo031"];$fecha_fin=$registro["campo032"]; $presup_ano=substr($fecha_ini,0,4);}
...
(BIEN015.Fecha_Incorporacion>='$fecha_desde' AND BIEN015.Fecha_Incorporacion<='$fecha_hasta') and (BIEN040.Tipo_Movimiento='060')";
	...
	if($tipo_rep=="PDF"){  $res=pg_query($sSQL); $referencia=""; $fecha=""; $cod_dependencia=""; $denominacion_dep="";  $subtotal=0; $fin=0; $prev_referencia=""; $prev_fecha=""; $direccion_dep=""; $saldo_anterior=0;
	     $filas=pg_num_rows($res); if($filas>=1){ 
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $estado; global $municipio; global $Nom_Emp; global $referencia; global $fecha; global $cod_dependencia; global $denominacion_dep;  global $direccion_dep; global $mes_proceso;
                $ffechar=date("d-m-Y");$fhorar=date("H:i:s a");  
                $distrito="";  $den_enc=$denominacion_dep;				
			    $y=$this->GetY();$x=$this->GetX();
				$this->Image('../../imagenes/Logo_emp.png',8,8,20);
			...
			function Footer(){  global $subtotali; global $subtotald; global $fin; global $saldo_anterior; global $subtotal;  global $descripcion;
				...
				if($fin==1){
					...
					$this->SetXY($x,$y);
		            $this->MultiCell($n,5,$descripcion,0);
				}
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1;
			...
			$cod_bien_mue=$registro["cod_bien_mue"];  $num_bien=$registro["num_bien"];  $descripcion=$registro["descripcion"];
			...
			if($php_os=="WINNT"){$denominacion=$denominacion; }else{$denominacion=utf8_decode($denominacion); $descripcion=utf8_decode($descripcion); $denominacion_dep=utf8_decode($denominacion_dep); $direccion_dep=utf8_decode($direccion_dep); }
