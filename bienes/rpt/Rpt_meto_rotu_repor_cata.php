...
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname.""); if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
	...
	if($tipo_u=="A"){$Mcamino="SSSSSSSSSSSSSSSSSSSS";}  else{$modulo="13"; $opcion="03-0000055"; $sql="select * from sia006 where campo601='$usuario_sia' and campo602='$modulo' and campo603='$opcion'";$res=pg_exec($conn,$sql);$filas=pg_numrows($res);
	 if ($filas>0){$reg=pg_fetch_array($res); $Mcamino=$reg["campo607"].$reg["campo608"].$reg["campo609"].$reg["campo610"].$reg["campo611"].$reg["campo612"].$reg["campo613"].$reg["campo614"].$reg["campo615"].$reg["campo616"].$reg["campo617"].$reg["campo618"].$reg["campo619"].$reg["campo620"].$reg["campo621"].$reg["campo622"].$reg["campo623"].$reg["campo624"].$reg["campo625"].$reg["campo626"]; }
	}$posicion=strpos($Mcamino,'S'); if(is_numeric($posicion)){$Mcamino=$Mcamino;}else{?><script language="JavaScript"> document.location='../menu.php';</script><?}

    $sSQL = "SELECT codigo, metodo_rotula, empleo FROM bien012 order by codigo";
	if($tipo_rpt=="HTML"){ include ("../../class/phpreports/PHPReportMaker.php"); 
            $oRpt = new PHPReportMaker();
            $oRpt->setXML("Rpt_meto_rotu_repor_cata.xml");
            $oRpt->setUser("$user");
    ...
    if($tipo_rpt=="PDF"){	 
	  ...
	  class PDF extends FPDF{
		function Header(){ global $criterio1; 
			...
			$this->Cell(40);
			$this->Cell(130,10,'CATALOGO DE METODOS DE ROTULACION',1,1,'C');
			$this->Ln(10);			
			$this->SetFont('Arial','B',8);	
			$this->Cell(13,4,'CODIGO',1,0,'L');
			$this->Cell(52,4,'METODO',1,0,'C');
			$this->Cell(135,4,'EMPLEO',1,1,'L');
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; 
		$codigo=$registro["codigo"]; $descripcion=$registro["empleo"]; $metodo_rotula=$registro["metodo_rotula"]; 
		if($php_os=="WINNT"){$descripcion=$registro["empleo"]; }else{$descripcion=utf8_decode($descripcion);}	    
		$pdf->Cell(13,4,$codigo,0,0,'L');
		$pdf->Cell(52,4,$metodo_rotula,0,0,'L');
