...
if (checkData($fecha_d)=='1'){$error=0; $sfecha=formato_aaaammdd($fecha_d);} else {$fecha_d="01/01/2014";} $fecha_h = colocar_udiames($fecha_d);$sfechad=formato_aaaammdd($fecha_d); $sfechah=formato_aaaammdd($fecha_h);
	     ...
	     $filas=pg_num_rows($res); if($filas>=1){ $registro=pg_fetch_array($res,0); $cod_dependencia=$registro["cod_dependencia"];  $prev_cod_dep=$registro["cod_dependencia"];
		  ...
		  class PDF extends FPDF{
			function Header(){ global $criterio1; global $Nom_Emp; global $cod_dependencia; global $denominacion_dep;  global $denom_departamento; global $direccion_dep; global $agrup_dep;  global $mes_proceso;
				...
				$this->Cell(200,10,'MOVIMIENTOS DE BIENES MUEBLES',0,0,'C');
			...
			function Footer(){  global $subtotali; global $subtotald; global $fin; global $saldo_anterior;
			    $ffechar=date("d-m-Y");$fhorar=date("H:i:s a");  $tmonto=formato_monto($saldo_anterior); 
				$this->SetY(-31);	
				...
				if($fin==1){
				    ...
				    $this->Cell(100,3,'OBSERVACIONES','RL',0,'L');
				    $this->Cell(50,3,'ELABORAD0 POR','RL',0,'L');
					 $this->Cell(50,3,'UNIDAD DE BIENES','RL',0,'L');
				    $this->Cell(50,3,'JEFE DE LA UNIDAD','RL',1,'L');		
					$this->Cell(100,10,'','RLB',0,'C');
		  ...
		  while($registro=pg_fetch_array($res)){ $i=$i+1;
		    ...
		    $pdf->SetXY($x,$y);
		    $pdf->MultiCell($n,4,utf8_decode($denominacion),0);
