...
$filas=pg_num_rows($resultado); if($filas>=1){  $registro=pg_fetch_array($resultado,0); $cod_emp=$registro["campo001"];
...
//Situacion Contable
$Ssql="SELECT * FROM bien010 where codigo='".$sit_contable."'"; $resultado=pg_query($Ssql);  $filas=pg_num_rows($resultado); if($filas>=1){ $registro=pg_fetch_array($resultado,0);   $tipo_situacion_cont=$registro["tipo_situacion"]; } else { $tipo_situacion_cont=$Ssql; }
...
class PDF extends FPDF{ 
	function Header(){global $cod_bien_mue; global $denominacion;
		...
		$this->Cell(200,3,'REGISTRO DE BIENES MUEBLES',0,1,'C');
		$this->Ln(5);	
        $x=$this->GetX();   $y=$this->GetY();
		$this->SetFont('Arial','',7);
		$this->Cell(200,5,'CODIGO DEL BIEN: '.$cod_bien_mue." - ".$denominacion,'T',1,'L');
	}
  ...
  $pdf->Cell(200,5,"RESPONSABLE DE USO: ".$ced_responsable_uso." ".$nombre_res_uso,'B',1,'L');
