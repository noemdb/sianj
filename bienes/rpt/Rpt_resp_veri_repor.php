...
if (pg_ErrorMessage($conn)){ ?><script language="JavaScript">muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS');</script><?}
         ...
         $sSQL = "SELECT BIEN030.ced_res_verificador, BIEN030.nombre_res_ver, BIEN030.observaciones_ver  FROM BIEN030 WHERE ced_res_verificador>='".$ced_res_verificadord."' AND 
                  ced_res_verificador<='".$ced_res_verificadorh."'ORDER BY $ordenado";
    if($tipo_rpt=="HTML"){ include ("../../class/phpreports/PHPReportMaker.php");
            $oRpt = new PHPReportMaker();
            $oRpt->setXML("Rpt_resp_veri_repor_cata.xml");
            $oRpt->setUser("$user");
   ...
   if($tipo_rpt=="PDF"){	 
	  ...
	  class PDF extends FPDF{
		function Header(){ global $criterio1; 
			...
			$this->Cell(40);
			$this->Cell(130,10,'CATALOGO DE RESPONSABLES VERIFICADOR',1,1,'C');
			$this->Ln(10);			
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; 
		$ced_responsable=$registro["ced_res_verificador"]; $nombre_res=$registro["nombre_res_ver"]; $observaciones=$registro["observaciones_ver"]; 
		if($php_os=="WINNT"){$nombre_res=$registro["nombre_res_ver"]; }else{$nombre_res=utf8_decode($nombre_res);$observaciones=utf8_decode($observaciones);}		
