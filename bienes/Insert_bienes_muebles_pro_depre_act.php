...
$met_calculo=$_POST["txtmet_calculo"];
//print_r($tipo_desin);
...
if ($error==0){
  ...
  if($error==0){  $sSQL="Select * from bien028 WHERE referencia_dep='$referencia_dep'";
  ...
  if($error==0){ $sql="SELECT * FROM CODIGOS_MUEBLES_BIEN050 where codigo_mov='$codigo_mov' order by referencia";  $res=pg_query($sql);
    $total=0;
 ...
 {
     $sfecha=formato_aaaammdd($fecha_dep);
     $resultado=pg_exec($conn,"SELECT incluye_bien028(1,'$codigo_mov','$referencia_dep','$sfecha','$met_calculo','S','','N','$sfecha','S','','','','D','','$descripcion')");
...
if ($error==0){?><script language="JavaScript">document.location ='Act_bienes_muebles_pro_depre_act.php';</script> <? }
